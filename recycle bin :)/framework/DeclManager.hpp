...
template< class type >
BUD_INLINE budDecl* budDeclAllocator()
{
...
class budDeclManager
{
public:
	virtual					~budDeclManager() {}
	...
	virtual void			RegisterDeclType( const char* typeName, declType_t type, budDecl * ( *allocator )() ) = 0;
	...
	virtual const budDecl* 	FindType( declType_t type, const char* name, bool makeDefault = true ) = 0;
	
	virtual const budDecl*	FindDeclWithoutParsing( declType_t type, const char* name, bool makeDefault = true ) = 0;
	...
	virtual const budDecl* 	DeclByIndex( declType_t type, int index, bool forceParse = true ) = 0;
	
	// List and print decls.
	virtual void			ListType( const budCmdArgs& args, declType_t type ) = 0;
	virtual void			PrintType( const budCmdArgs& args, declType_t type ) = 0;
	
	// Creates a new default decl of the given type with the given name in
	// the given file used by editors to create a new decls.
	virtual budDecl* 		CreateNewDecl( declType_t type, const char* name, const char* fileName ) = 0;
	...
	virtual void			WritePrecacheCommands( budFile* f ) = 0;
	
	// Convenience functions for specific types.
	virtual	const budMaterial* 		FindMaterial( const char* name, bool makeDefault = true ) = 0;
	virtual const budDeclSkin* 		FindSkin( const char* name, bool makeDefault = true ) = 0;
	virtual const idSoundShader* 	FindSound( const char* name, bool makeDefault = true ) = 0;
	
	virtual const budMaterial* 		MaterialByIndex( int index, bool forceParse = true ) = 0;
	virtual const budDeclSkin* 		SkinByIndex( int index, bool forceParse = true ) = 0;
	virtual const idSoundShader* 	SoundByIndex( int index, bool forceParse = true ) = 0;
	
	virtual void					Touch( const budDecl* decl ) = 0;
};

extern budDeclManager* 		declManager;


template< declType_t type >
BUD_INLINE void budListDecls_f( const budCmdArgs& args )
{
	declManager->ListType( args, type );
}

template< declType_t type >
BUD_INLINE void idPrintDecls_f( const budCmdArgs& args )
{
