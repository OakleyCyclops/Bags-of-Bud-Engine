...
/*
========================
budDebugGraph::budDebugGraph
========================
*/
budDebugGraph::budDebugGraph( int numItems ) :
...
========================
*/
void budDebugGraph::Init( int numBars )
{
	bars.SetNum( numBars );
	labels.Clear();
...
/*
========================
budDebugGraph::AddGridLine
========================
*/
void budDebugGraph::AddGridLine( float value, const budVec4& color )
{
...
/*
========================
budDebugGraph::SetValue
========================
*/
void budDebugGraph::SetValue( int b, float value, const budVec4& color )
{
	if( !enable )
	{
		return;
...
/*
========================
budDebugGraph::SetLabel
========================
*/
void budDebugGraph::SetLabel( int b, const char* text )
{
...
/*
========================
budDebugGraph::Render
========================
*/
void budDebugGraph::Render( budRenderSystem* gui )
{
	if( !enable )
	{
		return;
	...
	if( sideways )
	{
		...
		if( labels.Num() > 0 )
		{
			...
			budVec4 rect( position );
			rect.x -= SMALLCHAR_WIDTH * maxLen;
			rect.z = SMALLCHAR_WIDTH * maxLen;
			gui->DrawFilled( bgColor, rect.x, rect.y, rect.z, rect.w );
			for( int i = 0; i < labels.Num(); i++ )
			{
				budVec2 pos( barLeft - SMALLCHAR_WIDTH * maxLen, barTop + i * barHeight );
				gui->DrawSmallStringExt( budMath::Ftoi( pos.x ), budMath::Ftoi( pos.y ), labels[i], fontColor, true );
			}
		}
	}
	else
	{
		...
		{
			budVec4 rect( position.x, barBottom - barHeight * grid[i].value, position.z, 1.0f );
		...
		if( labels.Num() > 0 )
		{
			...
			for( int i = 0; i < labels.Num(); i++ )
			{
				budVec2 pos( barLeft + i * barWidth, barBottom + border );
				gui->DrawSmallStringExt( budMath::Ftoi( pos.x ), budMath::Ftoi( pos.y ), labels[i], fontColor, true );
			}
		}
	}
