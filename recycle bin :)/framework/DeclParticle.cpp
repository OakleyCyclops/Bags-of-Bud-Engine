...
budCVar binaryLoadParticles( "binaryLoadParticles", "1", 0, "enable binary load/write of particle decls" );
...
/*
=================
budDeclParticle::Size
=================
*/
size_t budDeclParticle::Size() const
{
	return sizeof( budDeclParticle );
}

/*
=====================
budDeclParticle::GetStageBounds
=====================
*/
void budDeclParticle::GetStageBounds( idParticleStage* stage )
{
...
/*
================
budDeclParticle::ParseParms

Parses a variable length list of parms on one line
================
*/
void budDeclParticle::ParseParms( budLexer& src, float* parms, int maxParms )
{
	budToken token;
...
/*
================
budDeclParticle::ParseParametric
================
*/
void budDeclParticle::ParseParametric( budLexer& src, idParticleParm* parm )
{
	budToken token;
	...
	{
		// table
		parm->table = static_cast<const budDeclTable*>( declManager->FindType( DECL_TABLE, token, false ) );
	}
...
/*
================
budDeclParticle::ParseParticleStage
================
*/
idParticleStage* budDeclParticle::ParseParticleStage( budLexer& src )
{
	budToken token;
...
/*
================
budDeclParticle::Parse
================
*/
bool budDeclParticle::Parse( const char* text, const int textLength, bool allowBinaryVersion )
{
	...
	{
		bounds = budBounds( vec3_origin ).Expand( 8.0f );
		return true;
	}
	//
...
/*
========================
budDeclParticle::LoadBinary
========================
*/
bool budDeclParticle::LoadBinary( budFile* file, unsigned int checksum )
{
	...
	struct local
	{
		static void LoadParticleParm( budFile* file, idParticleParm& parm )
		{
			budStr name;
			...
			{
				parm.table = ( budDeclTable* )declManager->FindType( DECL_TABLE, name, false );
			}
...
/*
========================
budDeclParticle::WriteBinary
========================
*/
void budDeclParticle::WriteBinary( budFile* file, unsigned int checksum )
{
	...
	struct local
	{
		static void WriteParticleParm( budFile* file, idParticleParm& parm )
		{
...
/*
================
budDeclParticle::FreeData
================
*/
void budDeclParticle::FreeData()
{
...
/*
================
budDeclParticle::DefaultDefinition
================
*/
const char* budDeclParticle::DefaultDefinition() const
{
	return
		"{\n"
		"\t"	"{\n"
...
/*
================
budDeclParticle::WriteParticleParm
================
*/
void budDeclParticle::WriteParticleParm( budFile* f, idParticleParm* parm, const char* name )
{
...
/*
================
budDeclParticle::WriteStage
================
*/
void budDeclParticle::WriteStage( budFile* f, idParticleStage* stage )
{
...
/*
================
budDeclParticle::RebuildTextSource
================
*/
bool budDeclParticle::RebuildTextSource()
{
	budFile_Memory f;
...
/*
================
budDeclParticle::Save
================
*/
bool budDeclParticle::Save( const char* fileName )
{
	RebuildTextSource();
	if( fileName )
	{
...
int idParticleStage::NumQuadsPerParticle() const
{
	...
	{
		int	trails = budMath::Ftoi( orientationParms[0] );
...
===============
*/
void idParticleStage::ParticleOrigin( particleGen_t* g, budVec3& origin ) const
{
	if( customPathType == PPATH_STANDARD )
	{
		...
		switch( distributionType )
		{
			...
			{
				angle1 = ( ( randomDistribution ) ? g->random.CRandomFloat() : 1.0f ) * budMath::TWO_PI;
				
				budMath::SinCos16( angle1, origin[0], origin[1] );
		...
		switch( directionType )
		{
			case PDIR_CONE:
			{
				...
				angle1 = g->random.CRandomFloat() * directionParms[0] * budMath::M_DEG2RAD;
				angle2 = g->random.CRandomFloat() * budMath::PI;
				
				float s1, c1, s2, c2;
				budMath::SinCos16( angle1, s1, c1 );
				budMath::SinCos16( angle2, s2, c2 );
	...
	else
	{
		...
		switch( customPathType )
		{
			case PPATH_HELIX:  		// ( sizeX sizeY sizeZ radialSpeed axialSpeed )
			{
				...
				angle1 = g->random.RandomFloat() * budMath::TWO_PI + customPathParms[3] * speed1 * g->age;
			...
			{
				speed1 = budMath::ClampFloat( 0.4f, 1.0f, g->random.CRandomFloat() );
				speed2 = budMath::ClampFloat( 0.4f, 1.0f, g->random.CRandomFloat() );
				angle1 = g->random.RandomFloat() * budMath::PI * 2 + customPathParms[0] * speed1 * g->age;
				angle2 = g->random.RandomFloat() * budMath::PI * 2 + customPathParms[1] * speed1 * g->age;
			...
			{
				angle1 = g->random.RandomFloat() * budMath::TWO_PI + customPathParms[1] * g->age;
...
==================
*/
int	idParticleStage::ParticleVerts( particleGen_t* g, budVec3 origin, budDrawVert* verts ) const
{
	...
	if( orientation == POR_AIMED )
	{
		...
		budDrawVert* verts_p = verts;
		budVec3		stepOrigin = origin;
		budVec3		stepLeft;
		int			numTrails = budMath::Ftoi( orientationParms[0] );
	...
	angle = angle / 180 * budMath::PI;
	float c = budMath::Cos16( angle );
	float s = budMath::Sin16( angle );
...
==================
*/
void idParticleStage::ParticleTexCoords( particleGen_t* g, budDrawVert* verts ) const
{
	float	s, width;
	float	t, height;
...
==================
*/
void idParticleStage::ParticleColors( particleGen_t* g, budDrawVert* verts ) const
{
	...
	for( int i = 0 ; i < 4 ; i++ )
	{
		...
		int		icolor = budMath::Ftoi( fcolor * 255.0f );
		if( icolor < 0 )
		{
			icolor = 0;
		}
...
================
*/
int idParticleStage::CreateParticle( particleGen_t* g, budDrawVert* verts ) const
{
	budVec3	origin;
...
void idParticleStage::SetCustomPathType( const char* p )
{
	...
	{
		if( budStr::Icmp( p, ParticleCustomDesc[i].name ) == 0 )
		{
