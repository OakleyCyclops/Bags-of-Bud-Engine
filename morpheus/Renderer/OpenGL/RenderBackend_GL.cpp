...
{
	// it probably isn't safe to do an libBud::Printf at this point
...
static void R_CheckPortableExtensions()
{
	...
	const char* badVideoCard = budLocalization::GetString( "#str_06780" );
	...
	{
		libBud::Error( "GL_ARB_multitexture not available" );
	}
	// GL_ARB_texture_compression + GL_EXT_texture_compression_s3tc
	if( !glConfig.textureCompressionAvailable )
	{
		libBud::Error( "GL_ARB_texture_compression or GL_EXT_texture_compression_s3tc not available" );
	}
	// GL_ARB_vertex_buffer_object
	if( !glConfig.vertexBufferObjectAvailable )
	{
		libBud::Error( "GL_ARB_vertex_buffer_object not available" );
	}
	// GL_ARB_map_buffer_range
	if( !glConfig.mapBufferRangeAvailable )
	{
		libBud::Error( "GL_ARB_map_buffer_range not available" );
	}
	// GL_ARB_vertex_array_object
	if( !glConfig.vertexArrayObjectAvailable )
	{
		libBud::Error( "GL_ARB_vertex_array_object not available" );
	}
	// GL_ARB_draw_elements_base_vertex
	if( !glConfig.drawElementsBaseVertexAvailable )
	{
		libBud::Error( "GL_ARB_draw_elements_base_vertex not available" );
	}
	// GL_ARB_vertex_program / GL_ARB_fragment_program
	//if( !glConfig.fragmentProgramAvailable )
	//{
	//	libBud::Warning( "GL_ARB_fragment_program not available" );
	//}
	// GLSL
	if( !glConfig.glslAvailable )
	{
		libBud::Error( "GLSL not available" );
	}
	// GL_ARB_uniform_buffer_object
	if( !glConfig.uniformBufferAvailable )
	{
		libBud::Error( "GL_ARB_uniform_buffer_object not available" );
	}
	// GL_EXT_stencil_two_side
	if( !glConfig.twoSidedStencilAvailable )
	{
		libBud::Error( "GL_ATI_separate_stencil not available" );
	}
...
void budRenderBackend::Init()
{
	...
	libBud::Printf( "OpenGL Version   : %3.1f\n", glVersion );
	libBud::Printf( "OpenGL Vendor    : %s\n", glConfig.vendor_string );
	libBud::Printf( "OpenGL Renderer  : %s\n", glConfig.renderer_string );
	libBud::Printf( "OpenGL GLSL      : %3.1f\n", glslVersion );
	libBud::Printf( "OpenGL Extensions: %s\n", glConfig.extensions_string );
...
void budRenderBackend::Shutdown()
{
	GLimp_Shutdown();
}
...
/*
=============
budRenderBackend::DrawElementsWithCounters
=============
*/
void budRenderBackend::DrawElementsWithCounters( const drawSurf_t* surf )
{
	// get vertex buffer
	...
	else
	{
		...
		{
			libBud::Warning( "RB_DrawElementsWithCounters, vertexBuffer == NULL" );
			return;
		}
	...
	else
	{
		...
		if( frameNum != ( ( vertexCache.currentFrame - 1 ) & VERTCACHE_FRAME_MASK ) )
		{
			libBud::Warning( "RB_DrawElementsWithCounters, indexBuffer == NULL" );
			return;
		}
	...
	if( surf->jointCache )
	{
		...
		{
			libBud::Warning( "RB_DrawElementsWithCounters, jointBuffer == NULL" );
			return;
		}
	...
	if( ( vertexLayout != LAYOUT_DRAW_VERT ) || ( currentVertexBuffer != ( GLintptr )vertexBuffer->GetAPIObject() ) || !r_useStateCaching.GetBool() )
	{
		...
		glVertexAttribPointer( PC_ATTRIB_INDEX_VERTEX, 3, GL_FLOAT, GL_FALSE, sizeof( budDrawVert ), ( void* )( DRAWVERT_XYZ_OFFSET ) );
		glVertexAttribPointer( PC_ATTRIB_INDEX_NORMAL, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof( budDrawVert ), ( void* )( DRAWVERT_NORMAL_OFFSET ) );
		glVertexAttribPointer( PC_ATTRIB_INDEX_COLOR, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof( budDrawVert ), ( void* )( DRAWVERT_COLOR_OFFSET ) );
		glVertexAttribPointer( PC_ATTRIB_INDEX_COLOR2, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof( budDrawVert ), ( void* )( DRAWVERT_COLOR2_OFFSET ) );
		glVertexAttribPointer( PC_ATTRIB_INDEX_ST, 2, GL_HALF_FLOAT, GL_TRUE, sizeof( budDrawVert ), ( void* )( DRAWVERT_ST_OFFSET ) );
		glVertexAttribPointer( PC_ATTRIB_INDEX_TANGENT, 4, GL_UNSIGNED_BYTE, GL_TRUE, sizeof( budDrawVert ), ( void* )( DRAWVERT_TANGENT_OFFSET ) );
...
/*
==================
budRenderBackend::GL_StartFrame
==================
*/
void budRenderBackend::GL_StartFrame()
{
...
/*
==================
budRenderBackend::GL_EndFrame
==================
*/
void budRenderBackend::GL_EndFrame()
{
...
========================
*/
void budRenderBackend::GL_SetDefaultState()
{
...
/*
====================
budRenderBackend::GL_State

This routine is responsible for setting the most commonly changed state
====================
*/
void budRenderBackend::GL_State( uint64 stateBits, bool forceGlState )
{
...
/*
====================
budRenderBackend::SelectTexture
====================
*/
void budRenderBackend::GL_SelectTexture( int unit )
{
	if( currenttmu == unit )
	{
		return;
...
/*
====================
budRenderBackend::GL_Scissor
====================
*/
void budRenderBackend::GL_Scissor( int x /* left*/, int y /* bottom */, int w, int h )
{
	glScissor( x, y, w, h );
}
...
/*
====================
budRenderBackend::GL_Viewport
====================
*/
void budRenderBackend::GL_Viewport( int x /* left */, int y /* bottom */, int w, int h )
{
	glViewport( x, y, w, h );
}
...
/*
====================
budRenderBackend::GL_PolygonOffset
====================
*/
void budRenderBackend::GL_PolygonOffset( float scale, float bias )
{
	polyOfsScale = scale;
	polyOfsBias = bias;
...
/*
========================
budRenderBackend::GL_DepthBoundsTest
========================
*/
void budRenderBackend::GL_DepthBoundsTest( const float zmin, const float zmax )
{
...
/*
========================
budRenderBackend::GL_Clear
========================
*/
void budRenderBackend::GL_Clear( bool color, bool depth, bool stencil, byte stencilValue, float r, float g, float b, float a, bool clearHDR )
{
	int clearFlags = 0;
	if( color )
	{
...
/*
=================
budRenderBackend::GL_GetCurrentState
=================
*/
uint64 budRenderBackend::GL_GetCurrentState() const
{
	return glStateBits;
}
...
/*
========================
budRenderBackend::GL_GetCurrentStateMinusStencil
========================
*/
uint64 budRenderBackend::GL_GetCurrentStateMinusStencil() const
{
...
/*
=============
budRenderBackend::CheckCVars

See if some cvars that we watch have changed
=============
*/
void budRenderBackend::CheckCVars()
{
	// gamma stuff
	...
	{
		libBud::Printf( "Updating texture filter parameters.\n" );
...
/*
==================
budRenderBackend::DrawStencilShadowPass
==================
*/
void budRenderBackend::DrawStencilShadowPass( const drawSurf_t* drawSurf, const bool renderZPass )
{
	// get vertex buffer
	...
	else
	{
		...
		if( frameNum != ( ( vertexCache.currentFrame - 1 ) & VERTCACHE_FRAME_MASK ) )
		{
			libBud::Warning( "DrawStencilShadowPass, vertexBuffer == NULL" );
			return;
		}
		vertexBuffer = &vertexCache.frameData[vertexCache.drawListNum].vertexBuffer;
	}
	...
	else
	{
		...
		if( frameNum != ( ( vertexCache.currentFrame - 1 ) & VERTCACHE_FRAME_MASK ) )
		{
			libBud::Warning( "DrawStencilShadowPass, indexBuffer == NULL" );
			return;
		}
		indexBuffer = &vertexCache.frameData[vertexCache.drawListNum].indexBuffer;
	}
	...
	if( drawSurf->jointCache )
	{
		...
		{
			libBud::Warning( "DrawStencilShadowPass, jointBuffer == NULL" );
			return;
		}
		assert( ( jointBuffer.GetOffset() & ( glConfig.uniformBufferOffsetAlignment - 1 ) ) == 0 );
...
/*
=============
budRenderBackend::DrawFlickerBox
=============
*/
void budRenderBackend::DrawFlickerBox()
{
...
/*
=============
budRenderBackend::SetBuffer
=============
*/
void budRenderBackend::SetBuffer( const void* data )
{
...
=============
*/
void budRenderBackend::GL_BlockingSwapBuffers()
{
...
/*
=============
budRenderBackend::budRenderBackend
=============
*/
budRenderBackend::budRenderBackend()
{
...
====================
*/
static void R_MakeStereoRenderImage( budImage* image )
{
	budImageOpts	opts;
...
/*
====================
budRenderBackend::StereoRenderExecuteBackEndCommands

Renders the draw list twice, with slight modifications for left eye / right eye
====================
*/
void budRenderBackend::StereoRenderExecuteBackEndCommands( const emptyCommand_t* const allCmds )
{
	...
	static budImage* stereoRenderImages[2];
	for( int i = 0; i < 2; i++ )
	{
