...
bool idVertexBuffer::AllocBufferObject( const void* data, int allocSize, bufferUsageType_t _usage )
{
	...
	if( allocSize <= 0 )
	{
		libBud::Error( "idVertexBuffer::AllocBufferObject: allocSize = %i", allocSize );
	}
	...
	if( apiObject == 0xFFFF )
	{
		libBud::FatalError( "idVertexBuffer::AllocBufferObject: failed" );
	}
	...
	{
		libBud::Warning( "idVertexBuffer::AllocBufferObject: allocation failed" );
		allocationFailed = true;
	}
	
	if( r_showBuffers.GetBool() )
	{
		libBud::Printf( "vertex buffer alloc %p, api %p (%i bytes)\n", this, ( GLuint* )&apiObject, GetSize() );
	}
...
void idVertexBuffer::FreeBufferObject()
{
	...
	if( r_showBuffers.GetBool() )
	{
		libBud::Printf( "vertex buffer free %p, api %p (%i bytes)\n", this, ( GLuint* )&apiObject, GetSize() );
	}
...
void idVertexBuffer::Update( const void* data, int updateSize, int offset ) const
{
	...
	{
		libBud::FatalError( "idVertexBuffer::Update: size overrun, %i > %i\n", updateSize, GetSize() );
	}
...
void* idVertexBuffer::MapBuffer( bufferMapType_t mapType )
{
	...
	if( buffer == NULL )
	{
		libBud::FatalError( "idVertexBuffer::MapBuffer: failed" );
	}
	return buffer;
}
...
void idVertexBuffer::UnmapBuffer()
{
	...
	{
		libBud::Printf( "idVertexBuffer::UnmapBuffer failed\n" );
	}
...
bool idIndexBuffer::AllocBufferObject( const void* data, int allocSize, bufferUsageType_t _usage )
{
	...
	if( allocSize <= 0 )
	{
		libBud::Error( "idIndexBuffer::AllocBufferObject: allocSize = %i", allocSize );
	}
	...
	if( apiObject == 0xFFFF )
	{
		...
		libBud::FatalError( "idIndexBuffer::AllocBufferObject: failed - GL_Error %d", error );
	}
	...
	if( err == GL_OUT_OF_MEMORY )
	{
		libBud::Warning( "idIndexBuffer:AllocBufferObject: allocation failed" );
		allocationFailed = true;
	}
	
	if( r_showBuffers.GetBool() )
	{
		libBud::Printf( "index buffer alloc %p, api %p (%i bytes)\n", this, ( GLuint* )&apiObject, GetSize() );
	}
...
void idIndexBuffer::FreeBufferObject()
{
	...
	if( r_showBuffers.GetBool() )
	{
		libBud::Printf( "index buffer free %p, api %p (%i bytes)\n", this, ( GLuint* )&apiObject, GetSize() );
	}
...
void idIndexBuffer::Update( const void* data, int updateSize, int offset ) const
{
	...
	if( updateSize > GetSize() )
	{
		libBud::FatalError( "idIndexBuffer::Update: size overrun, %i > %i\n", updateSize, GetSize() );
	}
...
void* idIndexBuffer::MapBuffer( bufferMapType_t mapType )
{
	...
	if( buffer == NULL )
	{
		libBud::FatalError( "idIndexBuffer::MapBuffer: failed" );
	}
	return buffer;
}
...
void idIndexBuffer::UnmapBuffer()
{
	...
	{
		libBud::Printf( "idIndexBuffer::UnmapBuffer failed\n" );
	}
...
bool idUniformBuffer::AllocBufferObject( const void* data, int allocSize, bufferUsageType_t _usage )
{
	...
	if( allocSize <= 0 )
	{
		libBud::Error( "idUniformBuffer::AllocBufferObject: allocSize = %i", allocSize );
	}
	...
	if( r_showBuffers.GetBool() )
	{
		libBud::Printf( "joint buffer alloc %p, api %p (%i joints)\n", this, ( GLuint* )&apiObject, GetSize() );
	}
...
void idUniformBuffer::FreeBufferObject()
{
	...
	if( r_showBuffers.GetBool() )
	{
		libBud::Printf( "joint buffer free %p, api %p (%i size)\n", this, ( GLuint* )&apiObject, GetSize() );
	}
...
void idUniformBuffer::Update( const void* data, int updateSize, int offset ) const
{
	...
	if( updateSize > GetSize() )
	{
		libBud::FatalError( "idUniformBuffer::Update: size overrun, %i > %i\n", updateSize, GetSize() );
	}
...
void* idUniformBuffer::MapBuffer( bufferMapType_t mapType )
{
	...
	if( buffer == NULL )
	{
		libBud::FatalError( "idUniformBuffer::MapBuffer: failed" );
	}
	return ( float* ) buffer;
}
...
void idUniformBuffer::UnmapBuffer()
{
	...
	{
		libBud::Printf( "idUniformBuffer::UnmapBuffer failed\n" );
	}
