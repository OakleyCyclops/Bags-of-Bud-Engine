...
/*
================
budRenderWorldLocal::FreeWorld
================
*/
void budRenderWorldLocal::FreeWorld()
{
	...
	// free all the inline budRenderModels
...
/*
================
budRenderWorldLocal::TouchWorldModels
================
*/
void budRenderWorldLocal::TouchWorldModels()
{
	for( int i = 0; i < localModels.Num(); i++ )
	{
...
/*
================
budRenderWorldLocal::ReadBinaryShadowModel
================
*/
...
/*
================
budRenderWorldLocal::ParseModel
================
*/
budRenderModel* budRenderWorldLocal::ParseModel( budLexer* src, const char* mapName, ID_TIME_T mapTimeStamp, budFile* fileOut )
{
	budToken token;
	...
	for( int i = 0; i < numSurfaces; i++ )
	{
		...
		( ( budMaterial* )surf.shader )->AddReference();
...
/*
================
budRenderWorldLocal::ParseShadowModel
================
*/
budRenderModel* budRenderWorldLocal::ParseShadowModel( budLexer* src, budFile* fileOut )
{
	budToken token;
...
/*
================
budRenderWorldLocal::SetupAreaRefs
================
*/
void budRenderWorldLocal::SetupAreaRefs()
{
	connectedAreaNum = 0;
	for( int i = 0; i < numPortalAreas; i++ )
	{
...
/*
================
budRenderWorldLocal::ParseInterAreaPortals
================
*/
void budRenderWorldLocal::ParseInterAreaPortals( budLexer* src, budFile* fileOut )
{
	src->ExpectTokenString( "{" );
	...
	areaScreenRect = ( budScreenRect* ) R_ClearedStaticAlloc( numPortalAreas * sizeof( budScreenRect ) );
...
================
*/
void budRenderWorldLocal::ReadBinaryAreaPortals( budFile* file )
{
...
/*
================
budRenderWorldLocal::ParseNodes
================
*/
void budRenderWorldLocal::ParseNodes( budLexer* src, budFile* fileOut )
{
	src->ExpectTokenString( "{" );
...
/*
================
budRenderWorldLocal::ReadBinaryNodes
================
*/
void budRenderWorldLocal::ReadBinaryNodes( budFile* file )
{
...
/*
================
budRenderWorldLocal::CommonChildrenArea_r
================
*/
int budRenderWorldLocal::CommonChildrenArea_r( areaNode_t* node )
{
	int	nums[2];
...
/*
=================
budRenderWorldLocal::ClearWorld

Sets up for a single area world
=================
*/
void budRenderWorldLocal::ClearWorld()
{
	numPortalAreas = 1;
	portalAreas = ( portalArea_t* )R_ClearedStaticAlloc( sizeof( portalAreas[0] ) );
	areaScreenRect = ( budScreenRect* ) R_ClearedStaticAlloc( sizeof( budScreenRect ) );
...
/*
=================
budRenderWorldLocal::FreeDefs

dump all the interactions
=================
*/
void budRenderWorldLocal::FreeDefs()
{
	...
	{
		budRenderLightLocal* light = lightDefs[i];
	...
	{
		budRenderEntityLocal*	 mod = entityDefs[i];
...
/*
=================
budRenderWorldLocal::InitFromMap

A NULL or empty name will make a world without a map model, which
is still useful for displaying a bare model
=================
*/
bool budRenderWorldLocal::InitFromMap( const char* name )
{
	budLexer* 		src;
	budToken			token;
	budRenderModel* 	lastModel;
	...
	if( name == mapName )
	{
		...
		{
			common->Printf( "budRenderWorldLocal::InitFromMap: retaining existing map\n" );
			FreeDefs();
			TouchWorldModels();
		...
		common->Printf( "budRenderWorldLocal::InitFromMap: timestamp has changed, reloading.\n" );
	}
	...
	if( file != NULL )
	{
		...
		if( magic == BPROC_MAGIC )
		{
			...
			for( int i = 0; i < numEntries; i++ )
			{
				...
				if( type == "model" )
				{
					budRenderModel* lastModel = ReadBinaryModel( file );
					if( lastModel == NULL )
					{
						loaded = false;
						break;
				...
				{
					libBud::Error( "Binary proc file failed, unexpected type %s\n", type.c_str() );
				}
			}
		}
	}
	...
	if( !loaded )
	{
		...
		src = new( TAG_RENDER ) budLexer( filename, LEXFL_NOSTRINGCONCAT | LEXFL_NODOLLARPRECOMPILE );
		if( !src->IsLoaded() )
		{
			common->Printf( "budRenderWorldLocal::InitFromMap: %s not found\n", filename.c_str() );
			ClearWorld();
			return false;
		}
		...
		{
			common->Printf( "budRenderWorldLocal::InitFromMap: bad id '%s' instead of '%s'\n", token.c_str(), PROC_FILE_ID );
			delete src;
			return false;
		}
		...
		while( 1 )
		{
			...
			src->Error( "budRenderWorldLocal::InitFromMap: bad token \"%s\"", token.c_str() );
		}
...
/*
=====================
budRenderWorldLocal::ClearPortalStates
=====================
*/
void budRenderWorldLocal::ClearPortalStates()
{
...
/*
=====================
budRenderWorldLocal::AddWorldModelEntities
=====================
*/
void budRenderWorldLocal::AddWorldModelEntities()
{
	...
	for( int i = 0; i < numPortalAreas; i++ )
	{
		...
		budRenderEntityLocal*	 def = new( TAG_RENDER_ENTITY ) budRenderEntityLocal;
		...
		{
			common->Error( "budRenderWorldLocal::InitFromMap: bad area model lookup" );
		}
		
		budRenderModel* hModel = def->parms.hModel;
...
=====================
*/
bool budRenderWorldLocal::CheckAreaForPortalSky( int areaNum )
{
...
=====================
*/
void budRenderWorldLocal::ResetLocalRenderModels()
{
