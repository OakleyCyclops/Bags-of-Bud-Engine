...
/*
====================
budMD5Mesh::ParseMesh
====================
*/
void budMD5Mesh::ParseMesh( budLexer& parser, int numJoints, const budJointMat* joints )
{
	budToken		token;
	budToken		name;
	...
	for( int i = 0; i < texCoords.Num(); i++ )
	{
		...
		for( int j = 0; j < numWeightsForVertex[i]; j++, num++, count++ )
		{
			...
			weightIndex[count * 2 + 0] = tempWeights[num].joint * sizeof( budJointMat );
		}
	...
	for( int j = 0, i = 0; i < texCoords.Num(); i++ )
	{
		...
		{
			j++;
			v += ( *( budJointMat* )( ( byte* )joints + weightIndex[j * 2 + 0] ) ) * scaledWeights[j];
		}
		j++;
	...
	for( int i = 0; i < texCoords.Num(); i++ )
	{
		...
		for( int j = 0; j < usedWeights; j++ )
		{
			...
			const budJointMat& joint = joints[finalJointIndecies[j]];
	...
	for( int i = 0; i < deformInfo->numOutputVerts; i++ )
	{
		for( int j = 0; j < 4; j++ )
		{
			...
			{
				libBud::FatalError( "Bad joint index" );
			}
		}
	}
...
============
*/
void TransformVertsAndTangents( budDrawVert* targetVerts, const int numVerts, const budDrawVert* baseVerts, const budJointMat* joints )
{
	for( int i = 0; i < numVerts; i++ )
	{
		const budDrawVert& base = baseVerts[i];
		
		const budJointMat& j0 = joints[base.color[0]];
		const budJointMat& j1 = joints[base.color[1]];
		const budJointMat& j2 = joints[base.color[2]];
		const budJointMat& j3 = joints[base.color[3]];
...
/*
====================
budMD5Mesh::UpdateSurface
====================
*/
void budMD5Mesh::UpdateSurface( const struct renderEntity_s* ent, const budJointMat* entJoints,
...
/*
====================
budMD5Mesh::CalculateBounds
====================
*/
void budMD5Mesh::CalculateBounds( const budJointMat* entJoints, budBounds& bounds ) const
{
#if defined(USE_INTRINSICS)
	...
	{
		const budJointMat& joint = entJoints[meshJoints[i]];
...
/*
====================
budMD5Mesh::NearestJoint
====================
*/
int budMD5Mesh::NearestJoint( int a, int b, int c ) const
{
	...
	const budDrawVert& v = deformInfo->verts[vertNum];
...
/*
====================
budRenderModelMD5::ParseJoint
====================
*/
void budRenderModelMD5::ParseJoint( budLexer& parser, budMD5Joint* joint, budJointQuat* defaultPose )
{
	//
	// parse name
	//
...
/*
====================
budRenderModelMD5::InitFromFile
====================
*/
void budRenderModelMD5::InitFromFile( const char* fileName )
{
	name = fileName;
	LoadModel();
}
...
/*
========================
budRenderModelMD5::LoadBinaryModel
========================
*/
bool budRenderModelMD5::LoadBinaryModel( budFile* file, const ID_TIME_T sourceTimeStamp )
{

	if( !budRenderModelStatic::LoadBinaryModel( file, sourceTimeStamp ) )
	{
		return false;
	}
	...
	for( int i = 0; i < meshes.Num(); i++ )
	{
		...
		deform.staticAmbientCache = vertexCache.AllocStaticVertex( deform.verts, ALIGN( deform.numOutputVerts * sizeof( budDrawVert ), VERTEX_CACHE_ALIGN ) );
...
/*
========================
budRenderModelMD5::WriteBinaryModel
========================
*/
void budRenderModelMD5::WriteBinaryModel( budFile* file, ID_TIME_T* _timeStamp ) const
{

	budRenderModelStatic::WriteBinaryModel( file );
...
/*
====================
budRenderModelMD5::LoadModel

used for initial loads, reloadModel, and reloading the data of purged models
Upon exit, the model will absolutely be valid, but possibly as a default model
====================
*/
void budRenderModelMD5::LoadModel()
{
	...
	int			parentNum;
	budToken		token;
	budLexer		parser( LEXFL_ALLOWPATHNAMES | LEXFL_NOSTRINGESCAPECHARS );
	...
	parser.ExpectTokenString( "{" );
	budJointMat* poseMat = ( budJointMat* )_alloca16( joints.Num() * sizeof( poseMat[0] ) );
	for( int i = 0; i < joints.Num(); i++ )
	{
		budMD5Joint* joint = &joints[i];
		budJointQuat*	 pose = &defaultPose[i];
...
/*
==============
budRenderModelMD5::Print
==============
*/
void budRenderModelMD5::Print() const
{
	common->Printf( "%s\n", name.c_str() );
	...
	int	totalVerts = 0;
	int	totalTris = 0;
	const budMD5Mesh* mesh = meshes.Ptr();
...
/*
==============
budRenderModelMD5::List
==============
*/
void budRenderModelMD5::List() const
{
	int			i;
	const budMD5Mesh*	mesh;
	int			totalTris = 0;
	int			totalVerts = 0;
...
/*
====================
budRenderModelMD5::DrawJoints
====================
*/
void budRenderModelMD5::DrawJoints( const renderEntity_t* ent, const viewDef_t* view ) const
{
	int					i;
	int					num;
	budVec3				pos;
...
====================
*/
static void TransformJoints( budJointMat* __restrict outJoints, const int numJoints, const budJointMat* __restrict inJoints1, const budJointMat* __restrict inJoints2 )
{
	...
	for( int i = 0; i < numJoints; i++ )
	{
		budJointMat::Multiply( outJoints[i], inJoints1[i], inJoints2[i] );
	}
...
/*
====================
budRenderModelMD5::InstantiateDynamicModel
====================
*/
budRenderModel* budRenderModelMD5::InstantiateDynamicModel( const struct renderEntity_s* ent, const viewDef_t* view, budRenderModel* cachedModel )
{
	...
	if( !ent->joints )
	{
		common->Printf( "budRenderModelMD5::InstantiateDynamicModel: NULL joints on renderEntity for '%s'\n", Name() );
		delete cachedModel;
		return NULL;
	}
	else if( ent->numJoints != joints.Num() )
	{
		common->Printf( "budRenderModelMD5::InstantiateDynamicModel: renderEntity has different number of joints than model for '%s'\n", Name() );
		delete cachedModel;
		return NULL;
	}
	...
	if( staticModel->jointsInverted == NULL )
	{
		...
		staticModel->jointsInverted = ( budJointMat* )Mem_ClearedAlloc( numInvertedJoints * sizeof( budJointMat ), TAG_JOINTMAT );
...
/*
====================
budRenderModelMD5::IsDynamicModel
====================
*/
dynamicModel_t budRenderModelMD5::IsDynamicModel() const
{
	return DM_CACHED;
}
...
/*
====================
budRenderModelMD5::NumJoints
====================
*/
int budRenderModelMD5::NumJoints() const
{
	return joints.Num();
}
...
/*
====================
budRenderModelMD5::GetJoints
====================
*/
const budMD5Joint* budRenderModelMD5::GetJoints() const
{
	return joints.Ptr();
}
...
/*
====================
budRenderModelMD5::GetDefaultPose
====================
*/
const budJointQuat* budRenderModelMD5::GetDefaultPose() const
{
...
/*
====================
budRenderModelMD5::GetJointHandle
====================
*/
jointHandle_t budRenderModelMD5::GetJointHandle( const char* name ) const
{
...
/*
=====================
budRenderModelMD5::GetJointName
=====================
*/
const char* budRenderModelMD5::GetJointName( jointHandle_t handle ) const
{
...
/*
====================
budRenderModelMD5::NearestJoint
====================
*/
int budRenderModelMD5::NearestJoint( int surfaceNum, int a, int b, int c ) const
{
	if( surfaceNum > meshes.Num() )
	{
		common->Error( "budRenderModelMD5::NearestJoint: surfaceNum > meshes.Num()" );
	}
...
====================
*/
void budRenderModelMD5::TouchData()
{
	for( int i = 0; i < meshes.Num(); i++ )
	{
...
/*
===================
budRenderModelMD5::PurgeModel

frees all the data, but leaves the class around for dangling references,
which can regenerate the data with LoadModel()
===================
*/
void budRenderModelMD5::PurgeModel()
{
	purged = true;
	joints.Clear();
	defaultPose.Clear();
...
/*
===================
budRenderModelMD5::Memory
===================
*/
int	budRenderModelMD5::Memory() const
{
	...
	for( int i = 0; i < meshes.Num(); i++ )
	{
		const budMD5Mesh* mesh = &meshes[i];
