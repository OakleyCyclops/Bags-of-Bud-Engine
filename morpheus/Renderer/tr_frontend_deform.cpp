...
=================
*/
static drawSurf_t* R_FinishDeform( drawSurf_t* surf, srfTriangles_t* newTri, const budDrawVert* newVerts, const triIndex_t* newIndexes )
{
...
static drawSurf_t* R_AutospriteDeform( drawSurf_t* surf )
{
	...
	budDrawVert* newVerts = ( budDrawVert* )_alloca16( ALIGN( srcTri->numVerts * sizeof( budDrawVert ), 16 ) );
	triIndex_t* newIndexes = ( triIndex_t* )_alloca16( ALIGN( srcTri->numIndexes * sizeof( triIndex_t ), 16 ) );
	
	for( int i = 0; i < srcTri->numVerts; i += 4 )
	{
		// find the midpoint
		newVerts[i + 0] = budDrawVert::GetSkinnedDrawVert( srcTri->verts[i + 0], joints );
		newVerts[i + 1] = budDrawVert::GetSkinnedDrawVert( srcTri->verts[i + 1], joints );
...
static drawSurf_t* R_TubeDeform( drawSurf_t* surf )
{
	...
	for( int i = 0, indexes = 0; i < srcTri->numVerts; i += 4, indexes += 6 )
	{
		...
		for( int j = 0; j < 2; j++ )
		{
			...
			newVerts[i1] = budDrawVert::GetSkinnedDrawVert( srcTri->verts[i1], joints );
			newVerts[i2] = budDrawVert::GetSkinnedDrawVert( srcTri->verts[i2], joints );
...
static drawSurf_t* R_ExpandDeform( drawSurf_t* surf )
{
	...
	budDrawVert* newVerts = ( budDrawVert* )_alloca16( ALIGN( newTri->numVerts * sizeof( budDrawVert ), 16 ) );
...
static drawSurf_t* R_TurbulentDeform( drawSurf_t* surf )
{
	...
	const budDeclTable* table = ( const budDeclTable* )surf->material->GetDeformDecl();
...
static drawSurf_t* R_ParticleDeform( drawSurf_t* surf, bool useArea )
{
	...
	const budDeclParticle* particleSystem = ( const budDeclParticle* )surf->material->GetDeformDecl();
	const srfTriangles_t* srcTri = surf->frontEndGeo;
	...
	if( useArea )
	{
		...
		{
			float area = idWinding::TriangleArea(	budDrawVert::GetSkinnedDrawVertPosition( srcTri->verts[ srcTri->indexes[ i + 0 ] ], joints ),
													budDrawVert::GetSkinnedDrawVertPosition( srcTri->verts[ srcTri->indexes[ i + 1 ] ], joints ),
													budDrawVert::GetSkinnedDrawVertPosition( srcTri->verts[ srcTri->indexes[ i + 2 ] ], joints ) );
	...
	//
	// create the particles almost exactly the way budRenderModelPrt does
	//
	particleGen_t g;
	...
	for( int stageNum = 0; stageNum < particleSystem->stages.Num(); stageNum++ )
	{
		...
		for( int currentTri = 0; currentTri < ( ( useArea ) ? 1 : numSourceTris ); currentTri++ )
		{
			...
			for( int index = 0; index < maxStageParticles[stageNum]; index++ )
			{
				...
				const budDrawVert v1 = budDrawVert::GetSkinnedDrawVert( srcTri->verts[ srcTri->indexes[ pointTri * 3 + 0 ] ], joints );
				const budDrawVert v2 = budDrawVert::GetSkinnedDrawVert( srcTri->verts[ srcTri->indexes[ pointTri * 3 + 1 ] ], joints );
				const budDrawVert v3 = budDrawVert::GetSkinnedDrawVert( srcTri->verts[ srcTri->indexes[ pointTri * 3 + 2 ] ], joints );
