...
/*
=============
budMaterial::CommonInit
=============
*/
void budMaterial::CommonInit()
{
	desc = "<none>";
	renderBump = "";
...
/*
===============
budMaterial::FreeData
===============
*/
void budMaterial::FreeData()
{
	int i;
	
	if( stages )
	{
		// delete any budCinematic textures
...
/*
==============
budMaterial::GetEditorImage
==============
*/
budImage* budMaterial::GetEditorImage() const
{
	if( editorImage )
	{
		return editorImage;
...
/*
===============
budMaterial::CheckSurfaceParm

See if the current token matches one of the surface parm bit flags
===============
*/
bool budMaterial::CheckSurfaceParm( budToken* token )
{
...
/*
===============
budMaterial::MatchToken

Sets defaultShader and returns false if the next token doesn't match
===============
*/
bool budMaterial::MatchToken( budLexer& src, const char* match )
{
...
/*
=================
budMaterial::ParseSort
=================
*/
void budMaterial::ParseSort( budLexer& src )
{
	budToken token;
...
/*
=================
budMaterial::ParseStereoEye
=================
*/
void budMaterial::ParseStereoEye( budLexer& src )
{
	budToken token;
...
/*
=================
budMaterial::ParseDecalInfo
=================
*/
void budMaterial::ParseDecalInfo( budLexer& src )
{
	budToken token;
...
/*
=============
budMaterial::GetExpressionConstant
=============
*/
int budMaterial::GetExpressionConstant( float f )
{
	int		i;
...
/*
=============
budMaterial::GetExpressionTemporary
=============
*/
int budMaterial::GetExpressionTemporary()
{
	if( numRegisters >= MAX_EXPRESSION_REGISTERS )
	{
...
/*
=============
budMaterial::GetExpressionOp
=============
*/
expOp_t*	budMaterial::GetExpressionOp()
{
...
=================
*/
int budMaterial::EmitOp( int a, int b, expOpType_t opType )
{
	expOp_t*	op;
...
/*
=================
budMaterial::ParseEmitOp
=================
*/
int budMaterial::ParseEmitOp( budLexer& src, int a, expOpType_t opType, int priority )
{
	int		b;
...
/*
=================
budMaterial::ParseTerm

Returns a register index
=================
*/
int budMaterial::ParseTerm( budLexer& src )
{
	budToken token;
	int		a, b;
	...
	const budDeclTable* table = static_cast<const budDeclTable*>( declManager->FindType( DECL_TABLE, token.c_str(), false ) );
	if( !table )
	{
...
/*
=================
budMaterial::ParseExpressionPriority

Returns a register index
=================
*/
#define	TOP_PRIORITY 4
int budMaterial::ParseExpressionPriority( budLexer& src, int priority )
{
	budToken token;
	int		a;
...
/*
=================
budMaterial::ParseExpression

Returns a register index
=================
*/
int budMaterial::ParseExpression( budLexer& src )
{
...
/*
===============
budMaterial::ClearStage
===============
*/
void budMaterial::ClearStage( shaderStage_t* ss )
{
	ss->drawStateBits = 0;
...
/*
================
budMaterial::ParseBlend
================
*/
void budMaterial::ParseBlend( budLexer& src, shaderStage_t* stage )
{
	budToken token;
...
================
*/
void budMaterial::ParseVertexParm( budLexer& src, newShaderStage_t* newStage )
{
	budToken				token;
...
/*
================
budMaterial::ParseVertexParm2
================
*/
void budMaterial::ParseVertexParm2( budLexer& src, newShaderStage_t* newStage )
{
	budToken	token;
	src.ReadTokenOnLine( &token );
	int	parm = token.GetIntValue();
...
/*
================
budMaterial::ParseFragmentMap
================
*/
void budMaterial::ParseFragmentMap( budLexer& src, newShaderStage_t* newStage )
{
	const char*			str;
	textureFilter_t		tf;
	textureRepeat_t		trp;
...
/*
===============
budMaterial::MultiplyTextureMatrix
===============
*/
void budMaterial::MultiplyTextureMatrix( textureStage_t* ts, int registers[2][3] )
{
	int		old[2][3];
...
=================
*/
void budMaterial::ParseStage( budLexer& src, const textureRepeat_t trpDefault )
{
	budToken				token;
	const char*			str;
	shaderStage_t*		ss;
	...
	while( 1 )
	{
		...
		if( !token.Icmp( "videomap" ) )
		{
			...
			ts->cinematic = budCinematic::Alloc();
		...
		if( !token.Icmp( "rotate" ) )
		{
			...
			table = static_cast<const budDeclTable*>( declManager->FindType( DECL_TABLE, "sinTable", false ) );
			if( !table )
			{
			...
			table = static_cast<const budDeclTable*>( declManager->FindType( DECL_TABLE, "cosTable", false ) );
			if( !table )
			{
...
/*
===============
budMaterial::ParseDeform
===============
*/
void budMaterial::ParseDeform( budLexer& src )
{
	budToken token;
...
==============
*/
void budMaterial::AddImplicitStages( const textureRepeat_t trpDefault /* = TR_REPEAT  */ )
{
	char	buffer[1024];
	budLexer		newSrc;
...
/*
===============
budMaterial::SortInteractionStages
...
===============
*/
void budMaterial::SortInteractionStages()
{
	int		j;
...
=================
*/
void budMaterial::ParseMaterial( budLexer& src )
{
	budToken		token;
	int			s;
	char		buffer[1024];
	...
	while( 1 )
	{
		...
		// an entity guisurf must have an budUserInterface
...
=========================
*/
void budMaterial::SetGui( const char* _gui ) const
{
...
=========================
*/
bool budMaterial::Parse( const char* text, const int textLength, bool allowBinaryVersion )
{
	budLexer	src;
	budToken	token;
...
===============
*/
bool budMaterial::Save( const char* fileName )
{
...
/*
===============
budMaterial::AddReference
===============
*/
void budMaterial::AddReference()
{
	refCount++;
...
===============
*/
void budMaterial::EvaluateRegisters(
	float* 			registers,
	...
	idSoundEmitter* soundEmitter ) const
{
	...
	for( i = 0 ; i < numOps ; i++, op++ )
	{
		switch( op->opType )
		{
			...
			case OP_TYPE_TABLE:
			{
				const budDeclTable* table = static_cast<const budDeclTable*>( declManager->DeclByIndex( DECL_TABLE, op->a ) );
...
=============
*/
texgen_t budMaterial::Texgen() const
{
	if( stages )
	{
		for( int i = 0; i < numStages; i++ )
...
/*
=============
budMaterial::GetImageWidth
=============
*/
int budMaterial::GetImageWidth() const
{
...
/*
=============
budMaterial::GetImageHeight
=============
*/
int budMaterial::GetImageHeight() const
{
	assert( GetStage( 0 ) && GetStage( 0 )->texture.image );
...
/*
=============
budMaterial::CinematicLength
=============
*/
int	budMaterial::CinematicLength() const
{
...
/*
=============
budMaterial::UpdateCinematic
=============
*/
void budMaterial::UpdateCinematic( int time ) const
{
}

/*
=============
budMaterial::CloseCinematic
=============
*/
void budMaterial::CloseCinematic() const
{
	for( int i = 0; i < numStages; i++ )
	{
...
/*
=============
budMaterial::ResetCinematicTime
=============
*/
void budMaterial::ResetCinematicTime( int time ) const
{
	for( int i = 0; i < numStages; i++ )
	{
		if( stages[i].texture.cinematic )
...
/*
=============
budMaterial::GetCinematicStartTime
=============
*/
int budMaterial::GetCinematicStartTime() const
{
	for( int i = 0; i < numStages; i++ )
	{
		if( stages[i].texture.cinematic )
...
bool budMaterial::CinematicIsPlaying() const
{
	if( !stages || !stages[0].texture.cinematic )
	{
		return 0;
...
/*
===================
budMaterial::ImageName
===================
*/
const char* budMaterial::ImageName() const
{
	if( numStages == 0 )
	{
		return "_scratch";
	}
	budImage*	image = stages[0].texture.image;
	if( image )
	{
...
=================
*/
size_t budMaterial::Size() const
{
	return sizeof( budMaterial );
}

/*
===================
budMaterial::SetDefaultText
===================
*/
bool budMaterial::SetDefaultText()
{
...
/*
===================
budMaterial::DefaultDefinition
===================
*/
const char* budMaterial::DefaultDefinition() const
{
	return
		"{\n"
		"\t"	"{\n"
...
/*
===================
budMaterial::GetBumpStage
===================
*/
const shaderStage_t* budMaterial::GetBumpStage() const
{
	for( int i = 0 ; i < numStages ; i++ )
	{
...
/*
===================
budMaterial::ReloadImages
===================
*/
void budMaterial::ReloadImages( bool force ) const
{
	for( int i = 0 ; i < numStages ; i++ )
	{
...
/*
=============
budMaterial::SetFastPathImages

See if the material is trivial for the fast path
=============
*/
void budMaterial::SetFastPathImages()
{
	fastPathBumpImage = NULL;
	fastPathDiffuseImage = NULL;
	fastPathSpecularImage = NULL;
