...
struct portalStack_t
{
	...
	int						numPortalPlanes;
	budPlane					portalPlanes[MAX_PORTAL_PLANES + 1];
	budScreenRect			rect;
};
...
=============
*/
viewLight_t* R_SetLightDefViewLight( budRenderLightLocal* light )
{
...
================
*/
bool budRenderWorldLocal::CullEntityByPortals( const budRenderEntityLocal* entity, const portalStack_t* ps )
{
	if( r_useEntityPortalCulling.GetInteger() == 1 )
	{
	
		ALIGNTYPE16 frustumCorners_t corners;
		budRenderMatrix::GetFrustumCorners( corners, entity->inverseBaseModelProject, bounds_unitCube );
		for( int i = 0; i < ps->numPortalPlanes; i++ )
		{
			if( budRenderMatrix::CullFrustumCornersToPlane( corners, ps->portalPlanes[i] ) == FRUSTUM_CULL_FRONT )
			{
				return true;
			}
		}
	...
	else if( r_useEntityPortalCulling.GetInteger() >= 2 )
	{
		...
		budRenderMatrix baseModelProject;
		budRenderMatrix::Inverse( entity->inverseBaseModelProject, baseModelProject );
		
		budPlane frustumPlanes[6];
		budRenderMatrix::GetFrustumPlanes( frustumPlanes, baseModelProject, false, true );
...
===================
*/
void budRenderWorldLocal::AddAreaViewEntities( int areaNum, const portalStack_t* ps )
{
	portalArea_t* area = &portalAreas[ areaNum ];
	
	for( areaReference_t* ref = area->entityRefs.areaNext; ref != &area->entityRefs; ref = ref->areaNext )
	{
		budRenderEntityLocal*	 entity = ref->entity;
...
================
*/
bool budRenderWorldLocal::CullLightByPortals( const budRenderLightLocal* light, const portalStack_t* ps )
{
	if( r_useLightPortalCulling.GetInteger() == 1 )
	{
	
		ALIGNTYPE16 frustumCorners_t corners;
		budRenderMatrix::GetFrustumCorners( corners, light->inverseBaseLightProject, bounds_zeroOneCube );
		for( int i = 0; i < ps->numPortalPlanes; i++ )
		{
			if( budRenderMatrix::CullFrustumCornersToPlane( corners, ps->portalPlanes[i] ) == FRUSTUM_CULL_FRONT )
			{
	...
	else if( r_useLightPortalCulling.GetInteger() >= 2 )
	{
		...
		budPlane frustumPlanes[6];
		budRenderMatrix::GetFrustumPlanes( frustumPlanes, light->baseLightProject, true, true );
...
===================
*/
void budRenderWorldLocal::AddAreaViewLights( int areaNum, const portalStack_t* ps )
{
	portalArea_t* area = &portalAreas[ areaNum ];
	
	for( areaReference_t* lref = area->lightRefs.areaNext; lref != &area->lightRefs; lref = lref->areaNext )
	{
		budRenderLightLocal* light = lref->light;
...
===================
*/
void budRenderWorldLocal::AddAreaToView( int areaNum, const portalStack_t* ps )
{
...
/*
===================
budRenderWorldLocal::ScreenRectForWinding
===================
*/
budScreenRect budRenderWorldLocal::ScreenRectFromWinding( const idWinding* w, const viewEntity_t* space )
{
...
/*
===================
budRenderWorldLocal::PortalIsFoggedOut
===================
*/
bool budRenderWorldLocal::PortalIsFoggedOut( const portal_t* p )
{
	budRenderLightLocal* ldef = p->doublePortal->fogLight;
	if( ldef == NULL )
	{
		return false;
	}
	...
	const budMaterial* lightShader = ldef->lightShader;
...
/*
=======================
budRenderWorldLocal::FlowViewThroughPortals
...
/*
===================
budRenderWorldLocal::BuildConnectedAreas_r
===================
*/
void budRenderWorldLocal::BuildConnectedAreas_r( int areaNum )
{
...
/*
===================
budRenderWorldLocal::BuildConnectedAreas

This is only valid for a given view, not all views in a frame
===================
*/
void budRenderWorldLocal::BuildConnectedAreas()
{
...
/*
=============
budRenderWorldLocal::FindViewLightsAndEntites
...
=============
*/
void budRenderWorldLocal::FindViewLightsAndEntities()
{
...
/*
===================
budRenderWorldLocal::FloodLightThroughArea_r
===================
*/
void budRenderWorldLocal::FloodLightThroughArea_r( budRenderLightLocal* light, int areaNum,
...
/*
=======================
budRenderWorldLocal::FlowLightThroughPortals
...
=======================
*/
void budRenderWorldLocal::FlowLightThroughPortals( budRenderLightLocal* light )
{
...
==============
*/
int budRenderWorldLocal::NumPortals() const
{
...
==============
*/
qhandle_t budRenderWorldLocal::FindPortal( const budBounds& b ) const
{
	int				i, j;
	budBounds		wb;
...
=============
*/
void budRenderWorldLocal::FloodConnectedAreas( portalArea_t* area, int portalAttributeIndex )
{
...
==============
*/
bool budRenderWorldLocal::AreasAreConnected( int areaNum1, int areaNum2, portalConnection_t connection ) const
{
	...
	{
		common->Error( "budRenderWorldLocal::AreAreasConnected: bad parms: %i, %i", areaNum1, areaNum2 );
	}
	...
	{
		common->Error( "budRenderWorldLocal::AreasAreConnected: bad connection number: %i\n", ( int )connection );
	}
...
==============
*/
void budRenderWorldLocal::SetPortalState( qhandle_t portal, int blockTypes )
{
	if( portal == 0 )
	{
		return;
...
==============
*/
int budRenderWorldLocal::GetPortalState( qhandle_t portal )
{
	if( portal == 0 )
	{
		return 0;
