...
struct srfTriangles_t
{
	...
	budDrawVert* 				verts;					// vertices, allocated with special allocator
	...
	budRenderModelStatic* 		staticModelWithJoints;
...
{
public:
	virtual						~budRenderModel() {};
	
	// Loads static models only, dynamic models must be loaded by the modelManager
	virtual void				InitFromFile( const char* fileName ) = 0;
	
	// Supports reading/writing binary file formats
	virtual bool				LoadBinaryModel( budFile* file, const ID_TIME_T sourceTimeStamp ) = 0;
	virtual void				WriteBinaryModel( budFile* file, ID_TIME_T* _timeStamp = NULL ) const = 0;
	virtual bool				SupportsBinaryModel() = 0;
	
	// RB begin
	virtual void				ExportOBJ( budFile* objFile, budFile* mtlFile, ID_TIME_T* _timeStamp = NULL ) const = 0;
	// RB end
	...
	virtual budBounds			Bounds( const struct renderEntity_s* ent = NULL ) const = 0;
	...
	virtual budRenderModel* 		InstantiateDynamicModel( const struct renderEntity_s* ent, const viewDef_t* view, budRenderModel* cachedModel ) = 0;
	...
	virtual const budMD5Joint* 	GetJoints() const = 0;
	...
	virtual const budJointQuat* 	GetDefaultPose() const = 0;
	...
	virtual void				ReadFromDemoFile( class budDemoFile* f ) = 0;
	virtual void				WriteToDemoFile( class budDemoFile* f ) = 0;
