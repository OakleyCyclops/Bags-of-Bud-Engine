...
#endif
{
	ALIGNTYPE16 budDrawVert	verts[MAX_DECAL_VERTS];
...
public:
	...
	void						CreateDeferredDecals( const budRenderModel* model );
...
private:
	...
	const budMaterial* 			decalMaterials[MAX_DECALS];
	unsigned int				numDecalMaterials;
	
	void						CreateDecalFromWinding( const idWinding& w, const budMaterial* decalMaterial, const budPlane fadePlanes[2], float fadeDepth, int startTime );
	void						CreateDecal( const budRenderModel* model, const decalProjectionParms_t& localParms );
};
