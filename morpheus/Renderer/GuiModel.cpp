...
const float budGuiModel::STEREO_DEPTH_NEAR = 0.0f;
const float budGuiModel::STEREO_DEPTH_MID  = 0.5f;
const float budGuiModel::STEREO_DEPTH_FAR  = 1.0f;

/*
================
budGuiModel::budGuiModel
================
*/
budGuiModel::budGuiModel()
{
...
/*
================
budGuiModel::WriteToDemo
================
*/
void budGuiModel::WriteToDemo( budDemoFile* demo )
{
}

/*
================
budGuiModel::ReadFromDemo
================
*/
void budGuiModel::ReadFromDemo( budDemoFile* demo )
{
}

/*
================
budGuiModel::BeginFrame
================
*/
void budGuiModel::BeginFrame()
{
	vertexBlock = vertexCache.AllocVertex( NULL, MAX_VERTS );
	indexBlock = vertexCache.AllocIndex( NULL, MAX_INDEXES );
	vertexPointer = ( budDrawVert* )vertexCache.MappedVertexBuffer( vertexBlock );
...
================
*/
void budGuiModel::EmitSurfaces( float modelMatrix[16], float modelViewMatrix[16],
							   bool depthHack, bool allowFullScreenStereoDepth, bool linkAsEntity )
{
	...
	budRenderMatrix::Transpose( *( budRenderMatrix* )modelViewMatrix, viewMat );
	budRenderMatrix::Multiply( tr.viewDef->projectionRenderMatrix, viewMat, guiSpace->mvp );
	if( depthHack )
	{
		budRenderMatrix::ApplyDepthHack( guiSpace->mvp );
	}
	...
	for( int i = 0; i < surfaces.Num(); i++ )
	{
		...
		const budMaterial* shader = guiSurf.material;
...
====================
*/
void budGuiModel::EmitToCurrentView( float modelMatrix[16], bool depthHack )
{
...
void budGuiModel::EmitFullScreen()
{
	...
	budRenderMatrix::Transpose( *( budRenderMatrix* )viewDef->projectionMatrix, viewDef->projectionRenderMatrix );
...
=============
*/
budDrawVert* budGuiModel::AllocTris( int vertCount, const triIndex_t* tempIndexes, int indexCount, const budMaterial* material, const uint64 glState, const stereoDepthType_t stereoType )
{
	if( material == NULL )
	{
		return NULL;
	...
	if( numIndexes + indexCount > MAX_INDEXES )
	{
		...
		if( warningFrame != tr.frameCount )
		{
			...
			libBud::Warning( "budGuiModel::AllocTris: MAX_INDEXES exceeded" );
		}
		return NULL;
	}
	if( numVerts + vertCount > MAX_VERTS )
	{
		...
		{
			warningFrame = tr.frameCount;
			libBud::Warning( "budGuiModel::AllocTris: MAX_VERTS exceeded" );
		}
		return NULL;
	}
