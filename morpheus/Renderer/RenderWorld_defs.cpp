...
=================
*/
void R_DeriveEntityData( budRenderEntityLocal* entity )
{
	R_AxisToModelMatrix( entity->parms.axis, entity->parms.origin, entity->modelMatrix );
	
	budRenderMatrix::CreateFromOriginAxis( entity->parms.origin, entity->parms.axis, entity->modelRenderMatrix );
	
	// calculate the matrix that transforms the unit cube to exactly cover the model in world space
	budRenderMatrix::OffsetScaleForBounds( entity->modelRenderMatrix, entity->localReferenceBounds, entity->inverseBaseModelProject );
	
	// calculate the global model bounds by inverse projecting the unit cube with the 'inverseBaseModelProject'
	budRenderMatrix::ProjectedBounds( entity->globalReferenceBounds, entity->inverseBaseModelProject, bounds_unitCube, false );
}
...
===============
*/
void R_CreateEntityRefs( budRenderEntityLocal* entity )
{
...
========================
*/
static float R_ComputePointLightProjectionMatrix( budRenderLightLocal* light, budRenderMatrix& localProject )
{
...
========================
*/
static float R_ComputeParallelLightProjectionMatrix( budRenderLightLocal* light, budRenderMatrix& localProject )
{
...
void R_DeriveLightData( budRenderLightLocal* light )
{
	...
	if( light->falloffImage == NULL )
	{
		...
		const budMaterial* defaultShader;
		...
		if( light->parms.pointLight )
		{
			...
			declManager->Touch( static_cast< const budDecl*>( defaultShader ) );
	...
	for( int i = 0; i < 4; i++ )
	{
		budPlane temp = light->lightProject[i];
	...
	budRenderMatrix lightMatrix;
	budRenderMatrix::CreateFromOriginAxis( light->parms.origin, light->parms.axis, lightMatrix );
	
	budRenderMatrix inverseLightMatrix;
	if( !budRenderMatrix::Inverse( lightMatrix, inverseLightMatrix ) )
	{
		libBud::Warning( "lightMatrix invert failed" );
	}
	
	// 'baseLightProject' goes from global space -> light local space -> light projective space
	budRenderMatrix::Multiply( localProject, inverseLightMatrix, light->baseLightProject );
	
	// Invert the light projection so we can deform zero-to-one cubes into
	// the light model and calculate global bounds.
	if( !budRenderMatrix::Inverse( light->baseLightProject, light->inverseBaseLightProject ) )
	{
		libBud::Warning( "baseLightProject invert failed" );
	}
	
	// calculate the global light bounds by inverse projecting the zero to one cube with the 'inverseBaseLightProject'
	budRenderMatrix::ProjectedBounds( light->globalLightBounds, light->inverseBaseLightProject, bounds_zeroOneCube, false );
}
...
====================
*/
void R_FreeLightDefDerivedData( budRenderLightLocal* ldef )
{
...
{
	budRenderLightLocal	fakeLight;
	
	fakeLight.parms = renderLight;
	
	R_DeriveLightData( &fakeLight );
	
	budRenderMatrix::GetFrustumPlanes( lightFrustum, fakeLight.baseLightProject, true, true );
...
srfTriangles_t* R_PolytopeSurface( int numPlanes, const budPlane* planes, idWinding** windings )
{
	...
	budFixedWinding planeWindings[MAX_POLYTOPE_PLANES];
	...
	{
		const budPlane& plane = planes[i];
		budFixedWinding& w = planeWindings[i];
		
		w.BaseForPlane( plane );
		for( j = 0; j < numPlanes; j++ )
		{
			const budPlane& plane2 = planes[j];
			if( j == i )
			{
				continue;
			}
...
===============
*/
static bool WindingCompletelyInsideLight( const idWinding* w, const budRenderLightLocal* ldef )
{
	for( int i = 0; i < w->GetNumPoints(); i++ )
	{
		if( budRenderMatrix::CullPointToMVP( ldef->baseLightProject, ( *w )[i].ToVec3(), true ) )
		{
			return false;
		}
	}
...
======================
*/
static void R_CreateLightDefFogPortals( budRenderLightLocal* ldef )
{
...
void R_FreeDerivedData()
{
	...
	{
		budRenderWorldLocal* rw = tr.worlds[j];
		
		for( int i = 0; i < rw->entityDefs.Num(); i++ )
		{
			budRenderEntityLocal* def = rw->entityDefs[i];
			if( def == NULL )
			{
				continue;
			}
		...
		{
			budRenderLightLocal* light = rw->lightDefs[i];
			if( light == NULL )
			{
				continue;
			}
