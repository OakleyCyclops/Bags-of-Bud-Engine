...
budCVar r_showVertexCache( "r_showVertexCache", "0", CVAR_RENDERER | CVAR_BOOL, "Print stats about the vertex cache every frame" );
budCVar r_showVertexCacheTimings( "r_showVertexCacheTimings", "0", CVAR_RENDERER | CVAR_BOOL, "Print stats about the vertex cache every frame" );
...
vertCacheHandle_t idVertexCache::ActuallyAlloc( geoBufferSet_t& vcs, const void* data, int bytes, cacheType_t type )
{
	...
	switch( type )
	{
		case CACHE_INDEX:
		{
			...
			{
				libBud::Error( "Out of index cache" );
			}
		...
		case CACHE_VERTEX:
		{
			...
			{
				libBud::Error( "Out of vertex cache" );
			}
		...
		case CACHE_JOINT:
		{
			...
			{
				libBud::Error( "Out of joint buffer cache" );
			}
...
==============
*/
vertCacheHandle_t idVertexCache::AllocVertex( const void* data, int num, size_t size /*= sizeof( budDrawVert ) */ )
{
...
==============
*/
vertCacheHandle_t idVertexCache::AllocJoint( const void* data, int num, size_t size /*= sizeof( budJointMat ) */ )
{
...
vertCacheHandle_t idVertexCache::AllocStaticVertex( const void* data, int bytes )
{
	...
	{
		libBud::FatalError( "AllocStaticVertex failed, increase STATIC_VERTEX_MEMORY" );
	}
...
vertCacheHandle_t idVertexCache::AllocStaticIndex( const void* data, int bytes )
{
	...
	{
		libBud::FatalError( "AllocStaticIndex failed, increase STATIC_INDEX_MEMORY" );
	}
...
void idVertexCache::BeginBackEnd()
{
	...
	{
		libBud::Printf( "%08d: %d allocations, %dkB vertex, %dkB index, %ikB joint : %dkB vertex, %dkB index, %ikB joint\n",
	...
	{
		libBud::PrintfIf( r_showVertexCacheTimings.GetBool(), "idVertexCache::unmap took %i msec\n", endUnmap - startUnmap );
	}
	drawListNum = listNum;
	...
	{
		libBud::PrintfIf( r_showVertexCacheTimings.GetBool(), "idVertexCache::map took %i msec\n", endMap - startMap );
	}
