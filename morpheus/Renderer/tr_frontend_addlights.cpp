...
/*
============================
budRenderEntityLocal::IsDirectlyVisible()
============================
*/
bool budRenderEntityLocal::IsDirectlyVisible() const
{
...
static void R_AddSingleLight( viewLight_t* vLight )
{
	...
	const budRenderLightLocal* light = vLight->lightDef;
	const budMaterial* lightShader = light->lightShader;
	if( lightShader == NULL )
	{
	...
	budPlane fogPlane(	light->baseLightProject[2][0] - light->baseLightProject[3][0],
	...
	if( r_useLightScissors.GetInteger() != 0 )
	{
		...
		budRenderMatrix::Multiply( viewDef->worldSpace.mvp, light->inverseBaseLightProject, invProjectMVPMatrix );
		
		// Calculate the projected bounds, either not clipped at all, near clipped, or fully clipped.
		budBounds projected;
		if( r_useLightScissors.GetInteger() == 1 )
		{
			budRenderMatrix::ProjectedBounds( projected, invProjectMVPMatrix, bounds_zeroOneCube );
		}
		else if( r_useLightScissors.GetInteger() == 2 )
		{
			budRenderMatrix::ProjectedNearClippedBounds( projected, invProjectMVPMatrix, bounds_zeroOneCube );
		}
		else
		{
			budRenderMatrix::ProjectedFullyClippedBounds( projected, invProjectMVPMatrix, bounds_zeroOneCube );
		}
	...
	budInteraction** const interactionTableRow = light->world->interactionTable + light->index * light->world->interactionTableWidth;
	...
	for( areaReference_t* lref = light->references; lref != NULL; lref = lref->ownerNext )
	{
		...
		for( areaReference_t* eref = area->entityRefs.areaNext; eref != &area->entityRefs; eref = eref->areaNext )
		{
			...
			const budInteraction* inter = interactionTableRow[ edef->index ];
			
			const renderEntity_t& eParms = edef->parms;
			const budRenderModel* eModel = eParms.hModel;
	...
	if( light->parms.prelightModel != NULL && !r_useShadowMapping.GetBool() )
	{
		...
		if( budRenderMatrix::CullBoundsToMVP( viewDef->worldSpace.mvp, tri->bounds ) )
		{
			return;
		}
