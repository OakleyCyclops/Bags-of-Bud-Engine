...
========================
*/
BUD_INLINE void budImage::DeriveOpts()
{
...
========================
*/
void budImage::AllocImage( const budImageOpts& imgOpts, textureFilter_t tf, textureRepeat_t tr )
{
	filter = tf;
	repeat = tr;
	opts = imgOpts;
...
On exit, the budImage will have a valid OpenGL texture number that can be bound
===============
*/
void budImage::ActuallyLoadImage( bool fromBackEnd )
{
	...
	else
	{
		...
		if( cubeFiles != CF_2D )
		{
			...
			{
				libBud::Warning( "Couldn't load cube image: %s", GetName() );
				return;
			}
		...
		else
		{
			...
			if( pic == NULL )
			{
				libBud::Warning( "Couldn't load image: %s : %s", GetName(), generatedName.c_str() );
...
==================
*/
int budImage::StorageSize() const
{
...
===============
*/
void budImage::Reload( bool force )
{
...
================
*/
void budImage::GenerateImage( const byte* pic, int width, int height, textureFilter_t filterParm, textureRepeat_t repeatParm, textureUsage_t usageParm, textureSamples_t samples )
{
	PurgeImage();
...
====================
*/
void budImage::GenerateCubeImage( const byte* pic[6], int size, textureFilter_t filterParm, textureUsage_t usageParm )
{
	PurgeImage();
...
// RB begin
void budImage::GenerateShadowArray( int width, int height, textureFilter_t filterParm, textureRepeat_t repeatParm, textureUsage_t usageParm )
{
	PurgeImage();
...
=============
*/
void budImage::UploadScratch( const byte* data, int cols, int rows )
{
	// if rows = cols * 6, assume it is a cube map animation
	if( rows == cols * 6 )
	{
