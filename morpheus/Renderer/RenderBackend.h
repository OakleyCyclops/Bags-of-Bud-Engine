...
private:
	...
	void				T_BasicFog( const drawSurf_t* drawSurfs, const budPlane fogPlanes[ 4 ], const budRenderMatrix* inverseBaseLightProject );
...
//	void				GL_BindTexture( budImage* image );

//	void				GL_CopyFrameBuffer( budImage* image, int x, int y, int imageWidth, int imageHeight );
//	void				GL_CopyDepthBuffer( budImage* image, int x, int y, int imageWidth, int imageHeight );
	...
	BUD_INLINE void		GL_Scissor( const budScreenRect& rect )
	{
		GL_Scissor( rect.x1, rect.y1, rect.x2 - rect.x1 + 1, rect.y2 - rect.y1 + 1 );
	}
	BUD_INLINE void		GL_Viewport( const budScreenRect& rect )
	{
		GL_Viewport( rect.x1, rect.y1, rect.x2 - rect.x1 + 1, rect.y2 - rect.y1 + 1 );
	}
	
	BUD_INLINE void	GL_ViewportAndScissor( int x, int y, int w, int h )
	{
		GL_Viewport( x, y, w, h );
		GL_Scissor( x, y, w, h );
	}
	
	BUD_INLINE void	GL_ViewportAndScissor( const budScreenRect& rect )
	{
		GL_Viewport( rect );
		GL_Scissor( rect );
	}
	...
	BUD_INLINE void		GL_Color( float r, float g, float b )
	{
		GL_Color( r, g, b, 1.0f );
	}
...
private:
	...
	budScreenRect		currentScissor;			// for scissor clipping, local inside renderView viewport
	
	bool				currentRenderCopied;	// true if any material has already referenced _currentRender
	
	budRenderMatrix		prevMVP[2];				// world MVP from previous frame for motion blur
	
	// RB begin
	budRenderMatrix		shadowV[6];				// shadow depth view matrix
	budRenderMatrix		shadowP[6];				// shadow depth projection matrix
