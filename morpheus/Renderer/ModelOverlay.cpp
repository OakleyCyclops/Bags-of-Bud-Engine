...
#include "../libBud/geometry/DrawVert_intrinsics.h"

/*
====================
budRenderModelOverlay::budRenderModelOverlay
====================
*/
budRenderModelOverlay::budRenderModelOverlay() :
	firstOverlay( 0 ),
...
/*
=================
budRenderModelOverlay::ReUse
=================
*/
void budRenderModelOverlay::ReUse()
{
	firstOverlay = 0;
	nextOverlay = 0;
	firstDeferredOverlay = 0;
...
/*
====================
budRenderModelOverlay::FreeOverlay
====================
*/
void budRenderModelOverlay::FreeOverlay( overlay_t& overlay )
{
...
====================
*/
static void R_OverlayPointCullStatic( byte* cullBits, halfFloat_t* texCoordS, halfFloat_t* texCoordT, const budPlane* planes, const budDrawVert* verts, const int numVerts )
{
	...
	idODSStreamedArray< budDrawVert, 16, SBT_DOUBLE, 4 > vertsODS( verts, numVerts );
	...
	idODSStreamedArray< budDrawVert, 16, SBT_DOUBLE, 1 > vertsODS( verts, numVerts );
...
====================
*/
static void R_OverlayPointCullSkinned( byte* cullBits, halfFloat_t* texCoordS, halfFloat_t* texCoordT, const budPlane* planes, const budDrawVert* verts, const int numVerts, const budJointMat* joints )
{
	assert_16_byte_aligned( cullBits );
	assert_16_byte_aligned( texCoordS );
	assert_16_byte_aligned( texCoordT );
...
/*
=====================
budRenderModelOverlay::CreateOverlay
...
=====================
*/
void budRenderModelOverlay::CreateOverlay( const budRenderModel* model, const budPlane localTextureAxis[2], const budMaterial* material )
{
...
/*
====================
budRenderModelOverlay::CreateDeferredOverlays
====================
*/
void budRenderModelOverlay::CreateDeferredOverlays( const budRenderModel* model )
{
...
/*
====================
budRenderModelOverlay::AddDeferredOverlay
====================
*/
void budRenderModelOverlay::AddDeferredOverlay( const overlayProjectionParms_t& localParms )
{
...
====================
*/
static void R_CopyOverlaySurface( budDrawVert* verts, int numVerts, triIndex_t* indexes, int numIndexes, const overlay_t* overlay, const budDrawVert* sourceVerts )
{
	...
	assert( ( ( overlay->numVerts * sizeof( budDrawVert ) ) & 15 ) == 0 );
	...
	for( int i = 0; i < overlay->numVerts; i++ )
	{
		...
		const budDrawVert& srcVert = sourceVerts[overlayVert.vertexNum];
		budDrawVert& dstVert = verts[numVerts + i];
...
/*
=====================
budRenderModelOverlay::GetNumOverlayDrawSurfs
=====================
*/
unsigned int budRenderModelOverlay::GetNumOverlayDrawSurfs()
{
	numOverlayMaterials = 0;
...
/*
====================
budRenderModelOverlay::CreateOverlayDrawSurf
====================
*/
drawSurf_t* budRenderModelOverlay::CreateOverlayDrawSurf( const viewEntity_t* space, const budRenderModel* baseModel, unsigned int index )
{
	...
	const budRenderModelStatic* staticModel = static_cast< const budRenderModelStatic* >( baseModel );
	
	const budMaterial* material = overlayMaterials[index];
	...
	newTri->staticModelWithJoints = ( staticModel->jointsInverted != NULL ) ? const_cast< budRenderModelStatic* >( staticModel ) : NULL;	// allow GPU skinning
	
	newTri->ambientCache = vertexCache.AllocVertex( NULL, maxVerts );
	newTri->indexCache = vertexCache.AllocIndex( NULL, maxIndexes );
	
	budDrawVert* mappedVerts = ( budDrawVert* )vertexCache.MappedVertexBuffer( newTri->ambientCache );
	...
	for( unsigned int i = firstOverlay; i < nextOverlay; i++ )
	{
		...
		if( overlay.maxReferencedVertex >= baseTri->numVerts )
		{
			...
			common->Warning( "budRenderModelOverlay::CreateOverlayDrawSurf: overlay vertex out of range.  Model has probably changed since generating the overlay." );
			FreeOverlay( overlay );
			if( i == firstOverlay )
			{
				firstOverlay++;
...
/*
====================
budRenderModelOverlay::ReadFromDemoFile
====================
*/
void budRenderModelOverlay::ReadFromDemoFile( budDemoFile* f )
{
...
/*
====================
budRenderModelOverlay::WriteToDemoFile
====================
*/
void budRenderModelOverlay::WriteToDemoFile( budDemoFile* f ) const
{
