...
================
*/
void R_CalcInteractionFacing( const budRenderEntityLocal* ent, const srfTriangles_t* tri, const budRenderLightLocal* light, srfCullInfo_t& cullInfo )
{
	...
	{
		const budDrawVert& v0 = tri->verts[tri->indexes[i + 0]];
		const budDrawVert& v1 = tri->verts[tri->indexes[i + 1]];
		const budDrawVert& v2 = tri->verts[tri->indexes[i + 2]];
		
		const budPlane plane( v0.xyz, v1.xyz, v2.xyz );
...
=====================
*/
void R_CalcInteractionCullBits( const budRenderEntityLocal* ent, const srfTriangles_t* tri, const budRenderLightLocal* light, srfCullInfo_t& cullInfo )
{
...
====================
*/
static srfTriangles_t* R_CreateInteractionLightTris( const budRenderEntityLocal* ent,
		const srfTriangles_t* tri, const budRenderLightLocal* light,
...
=====================
*/
static srfTriangles_t* R_CreateInteractionShadowVolume( const budRenderEntityLocal* ent,
		const srfTriangles_t* tri, const budRenderLightLocal* light )
{
...
/*
===============
budInteraction::AllocAndLink
===============
*/
budInteraction* budInteraction::AllocAndLink( budRenderEntityLocal* edef, budRenderLightLocal* ldef )
{
	if( edef == NULL || ldef == NULL )
	{
		common->Error( "budInteraction::AllocAndLink: NULL parm" );
		return NULL;
	}
	
	budRenderWorldLocal* renderWorld = edef->world;
	
	budInteraction* interaction = renderWorld->interactionAllocator.Alloc();
	...
	if( renderWorld->interactionTable != NULL )
	{
		...
		{
			common->Error( "budInteraction::AllocAndLink: non NULL table entry" );
		}
...
/*
===============
budInteraction::Unlink
===============
*/
void budInteraction::Unlink()
{
...
/*
===============
budInteraction::UnlinkAndFree

Removes links and puts it back on the free list.
===============
*/
void budInteraction::UnlinkAndFree()
{
	// clear the table pointer
	budRenderWorldLocal* renderWorld = this->lightDef->world;
	...
	if( renderWorld->interactionTable != NULL )
	{
		...
		{
			common->Error( "budInteraction::UnlinkAndFree: interactionTable wasn't set" );
		}
...
/*
===============
budInteraction::HasShadows
===============
*/
bool budInteraction::HasShadows() const
{
...
Called by budRenderWorldLocal::GenerateAllInteractions
======================
*/
void budInteraction::CreateStaticInteraction()
{
	// note that it is a static interaction
	staticInteraction = true;
	const budRenderModel* model = entityDef->parms.hModel;
	...
	const budBounds bounds = model->Bounds( &entityDef->parms );
	...
	for( int c = 0 ; c < model->NumSurfaces() ; c++ )
	{
		...
		const budMaterial* const shader = R_RemapShaderBySkin( surf->shader,
...
void R_ShowInteractionMemory_f( const budCmdArgs& args )
{
	...
	{
		budRenderLightLocal* light = tr.primaryWorld->lightDefs[i];
		if( light == NULL )
		{
			continue;
		}
		int numInteractionsForLight = 0;
		for( budInteraction* inter = light->firstInteraction; inter != NULL; inter = inter->lightNext )
		{
			if( !inter->IsEmpty() )
			{
	...
	{
		budRenderEntityLocal*	def = tr.primaryWorld->entityDefs[i];
		if( def == NULL )
		{
			continue;
		}
		...
		for( budInteraction* inter = def->firstInteraction; inter != NULL; inter = inter->entityNext )
		{
			interactions++;
