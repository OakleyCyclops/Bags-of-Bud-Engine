...
BUD_INLINE int SIMD_ROUND_JOINTS( int numJoints )
{
	return ( ( numJoints + 1 ) & ~1 );
}
BUD_INLINE void SIMD_INIT_LAST_JOINT( budJointMat* joints, int numJoints )
{
	if( numJoints & 1 )
	{
...
{
	budRenderModel* 			hModel;				// this can only be null if callback is set
	...
	budBounds				bounds;					// only needs to be set for deferred models and md5s
	...
	const budMaterial* 		referenceShader;		// used so flares can reference the proper light shader
	const budDeclSkin* 		customSkin;				// 0 for no remappings
	...
	class budUserInterface* gui[ MAX_RENDERENTITY_GUI ];
	
	struct renderView_s*		remoteRenderView;		// any remote camera surfaces will use this
	
	int						numJoints;
	budJointMat* 			joints;					// array of joints that will modify vertices.
...
typedef struct renderLight_s
{
	...
	budRenderModel* 			prelightModel;
	
	// muzzle flash lights will not cast shadows from player and weapon world models
	int						lightId;
	
	
	const budMaterial* 		shader;				// NULL = either lights/defaultPointLight or lights/defaultProjectedLight
...
typedef struct renderView_s
{
	...
	const budMaterial*		globalMaterial;							// used to override everything draw
...
// exitPortal_t is returned by budRenderWorld::GetPortal()
typedef struct
{
...
// guiPoint_t is returned by budRenderWorld::GuiTrace()
typedef struct
{
...
public:
	...
	virtual	qhandle_t		FindPortal( const budBounds& b ) const = 0;
	...
	virtual int				BoundsInAreas( const budBounds& bounds, int* areas, int maxAreas ) const = 0;
	...
	virtual void			StartWritingDemo( budDemoFile* demo ) = 0;
	...
	virtual bool			ProcessDemoCommand( budDemoFile* readDemo, renderView_t* demoRenderView, int* demoTimeOffset ) = 0;
