...
void R_AllocStaticTriSurfVerts( srfTriangles_t* tri, int numVerts )
{
	...
	tri->verts = ( budDrawVert* )Mem_Alloc16( numVerts * sizeof( budDrawVert ), TAG_TRI_VERTS );
}
...
{
	budDrawVert* newVerts = ( budDrawVert* )Mem_Alloc16( numVerts * sizeof( budDrawVert ), TAG_TRI_VERTS );
	const int copy = std::min( numVerts, tri->numVerts );
	memcpy( newVerts, tri->verts, copy * sizeof( budDrawVert ) );
	Mem_Free( tri->verts );
	tri->verts = newVerts;
}
...
static int* R_CreateSilRemap( const srfTriangles_t* tri )
{
	...
	budHashIndex		hash( 1024, tri->numVerts );
...
{
	const budDrawVert* a = tri->verts + tri->indexes[firstIndex + 0];
...
void R_DeriveNormalsAndTangents( srfTriangles_t* tri )
{
	...
	for( int i = 0; i < tri->numIndexes; i += 3 )
	{
		...
		const budDrawVert* a = tri->verts + v0;
...
void R_CreateVertexNormals( srfTriangles_t* tri )
{
	...
	for( int i = 0; i < tri->numIndexes; i += 3 )
	{
		...
		const budDrawVert& v0 = tri->verts[i0];
		const budDrawVert& v1 = tri->verts[i1];
		const budDrawVert& v2 = tri->verts[i2];
...
void R_BuildDominantTris( srfTriangles_t* tri )
{
	...
	if( ind == NULL )
	{
		libBud::Error( "Couldn't allocate index sort array" );
		return;
	}
...
void R_TestDegenerateTextureSpace( srfTriangles_t* tri )
{
	...
	for( i = 0; i < tri->numIndexes; i += 3 )
	{
		const budDrawVert& a = tri->verts[tri->indexes[i + 0]];
		const budDrawVert& b = tri->verts[tri->indexes[i + 1]];
		const budDrawVert& c = tri->verts[tri->indexes[i + 2]];
