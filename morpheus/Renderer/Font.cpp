...
==============================
*/
BUD_INLINE float Old_SelectValueForScale( float scale, float v0, float v1, float v2 )
{
...
budFont* budFont::RemapFont( const char* baseName )
{
	...
	const char* remapped = budLocalization::FindString( "#font_" + cleanName );
	if( remapped != NULL )
	{
		return renderSystem->RegisterFont( remapped );
	}
	
	const char* wildcard = budLocalization::FindString( "#font_*" );
...
==============================
*/
budFont::budFont( const char* n ) : name( n )
{
	fontInfo = NULL;
	alias = RemapFont( n );
	...
	if( alias != NULL )
	{
		...
		for( budFont* f = alias; f != NULL; f = f->alias )
		{
			if( f == this )
			{
				libBud::FatalError( "Font alias \"%s\" is a circular reference!", n );
			}
		}
		return;
	}
	...
	if( !LoadFont() )
	{
		...
		{
			libBud::FatalError( "Could not load default font \"%s\"", DEFAULT_FONT );
		}
		else
		{
			libBud::Warning( "Could not load font %s", n );
...
{
	budFile* fd = fileSystem->OpenFileRead( filename );
	if( fd == NULL )
	{
		return false;
	}
...
bool budFont::LoadFont()
{
	...
	{
		libBud::Warning( "Wrong version in %s", GetName() );
		delete fd;
		return false;
	}
...
/*
==============================
budFont::GetGlyphIndex
==============================
*/
int	budFont::GetGlyphIndex( uint32 idx ) const
{
	if( idx < 128 )
	{
...
/*
==============================
budFont::GetLineHeight
==============================
*/
float budFont::GetLineHeight( float scale ) const
{
	if( alias != NULL )
	{
...
==============================
*/
float budFont::GetAscender( float scale ) const
{
	if( alias != NULL )
	{
...
/*
==============================
budFont::GetMaxCharWidth
==============================
*/
float budFont::GetMaxCharWidth( float scale ) const
{
	if( alias != NULL )
	{
...
/*
==============================
budFont::GetGlyphWidth
==============================
*/
float budFont::GetGlyphWidth( float scale, uint32 idx ) const
{
	if( alias != NULL )
	{
...
/*
==============================
budFont::GetScaledGlyph
==============================
*/
void budFont::GetScaledGlyph( float scale, uint32 idx, scaledGlyphInfo_t& glyphInfo ) const
{
	if( alias != NULL )
	{
...
void budFont::Touch()
{
	...
	if( fontInfo != NULL )
	{
		const_cast<budMaterial*>( fontInfo->material )->EnsureNotPurged();
		fontInfo->material->SetSort( SS_GUI );
	}
}
