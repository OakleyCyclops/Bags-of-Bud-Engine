...
/*
================================================
budBinaryImage is used by the budImage class for constructing mipmapped
textures and for loading and saving generated files by budImage.
Also used in a memory-mapped form for imageCPU for offline megatexture
generation.
================================================
*/
class budBinaryImage
{
public:
	budBinaryImage( const char* name ) : imgName( name ) { }
...
private:
	...
	public:
		...
		budBinaryImageData() : data( NULL ) { }
		~budBinaryImageData()
		{
			Free();
		}
		budBinaryImageData& operator=( budBinaryImageData& other )
		{
			if( this == &other )
			{
				return *this;
