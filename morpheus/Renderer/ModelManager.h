...
class budRenderModelManager
{
public:
	virtual					~budRenderModelManager() {}
	...
	virtual budRenderModel* 	AllocModel() = 0;
	
	// frees a render model
	virtual void			FreeModel( budRenderModel* model ) = 0;
	
	// returns NULL if modelName is NULL or an empty string, otherwise
	// it will create a default model if not loadable
	virtual	budRenderModel* 	FindModel( const char* modelName ) = 0;
	
	// returns NULL if not loadable
	virtual	budRenderModel* 	CheckModel( const char* modelName ) = 0;
	
	// returns the default cube model
	virtual	budRenderModel* 	DefaultModel() = 0;
	
	// world map parsing will add all the inline models with this call
	virtual	void			AddModel( budRenderModel* model ) = 0;
	...
	virtual	void			RemoveModel( budRenderModel* model ) = 0;
...
extern	budRenderModelManager*	renderModelManager;
