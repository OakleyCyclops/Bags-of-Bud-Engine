...
struct shortBounds_t
{
	...
	shortBounds_t( const budBounds& b )
	{
	...
	void SetFromReferenceBounds( const budBounds& set )
	{
	...
	int* const					returnedList )
{
	...
	int hits = 0;
	budBounds	testF = testBounds.ToFloatBounds();
	for( int i = 0 ; i < numBounds ; i++ )
	{
		budBounds	listF = boundsList[i].ToFloatBounds();
...
void	budBoundsTrack::SetIndex( const int index, const budBounds& bounds )
{
...
void	budBoundsTrack::ClearIndex( const int index )
{
	assert( ( unsigned )index < MAX_BOUNDS_TRACK_INDEXES );
	boundsList[index].SetToEmpty();
}

int		budBoundsTrack::FindIntersections( const budBounds& testBounds, int intersectedIndexes[ MAX_BOUNDS_TRACK_INDEXES ] ) const
{
...
void	budBoundsTrack::Test()
{
	...
	libBud::Printf( "%i intersections\n", numHits1 );
	if( numHits1 != numHits2 )
	{
		libBud::Printf( "different results\n" );
	}
	else
	{
	...
	libBud::Printf( "%lli microseconds for 40 itterations\n", stop - start );
}
