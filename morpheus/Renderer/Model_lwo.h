...
int lwGetPoints( budFile* fp, int cksize, lwPointList* point );
void lwGetBoundingBox( lwPointList* point, float bbox[] );
int lwAllocPolygons( lwPolygonList* plist, int npols, int nverts );
int lwGetPolygons( budFile* fp, int cksize, lwPolygonList* plist, int ptoffset );
...
int lwGetTags( budFile* fp, int cksize, lwTagList* tlist );
int lwGetPolygonTags( budFile* fp, int cksize, lwTagList* tlist,
					  lwPolygonList* plist );

/* vmap.c */

void lwFreeVMap( lwVMap* vmap );
lwVMap* lwGetVMap( budFile* fp, int cksize, int ptoffset, int poloffset,
				   int perpoly );
...
lwClip* lwGetClip( budFile* fp, int cksize );
lwClip* lwFindClip( lwClip* list, int index );

/* envelope.c */

void lwFreeEnvelope( lwEnvelope* env );
lwEnvelope* lwGetEnvelope( budFile* fp, int cksize );
...
int lwGetTHeader( budFile* fp, int hsz, lwTexture* tex );
int lwGetTMap( budFile* fp, int tmapsz, lwTMap* tmap );
int lwGetImageMap( budFile* fp, int rsz, lwTexture* tex );
int lwGetProcedural( budFile* fp, int rsz, lwTexture* tex );
int lwGetGradient( budFile* fp, int rsz, lwTexture* tex );
lwTexture* lwGetTexture( budFile* fp, int bloksz, unsigned int type );
lwPlugin* lwGetShader( budFile* fp, int bloksz );
lwSurface* lwGetSurface( budFile* fp, int cksize );
lwSurface* lwDefaultSurface();

/* lwob.c */

lwSurface* lwGetSurface5( budFile* fp, int cksize, lwObject* obj );
int lwGetPolygons5( budFile* fp, int cksize, lwPolygonList* plist, int ptoffset );
...
#define vecangle( a, b ) ( float ) budMath::ACos( dot( a, b ) )

/* lwio.c */

void  set_flen( int i );
int   get_flen();
void* getbytes( budFile* fp, int size );
void  skipbytes( budFile* fp, int n );
int   getI1( budFile* fp );
short getI2( budFile* fp );
int   getI4( budFile* fp );
unsigned char  getU1( budFile* fp );
unsigned short getU2( budFile* fp );
unsigned int   getU4( budFile* fp );
int   getVX( budFile* fp );
float getF4( budFile* fp );
char* getS0( budFile* fp );
