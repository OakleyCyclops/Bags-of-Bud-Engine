...
/*
==================
budRenderModelDecal::budRenderModelDecal
==================
*/
budRenderModelDecal::budRenderModelDecal() :
	firstDecal( 0 ),
...
bool budRenderModelDecal::CreateProjectionParms( decalProjectionParms_t& parms, const budFixedWinding& winding, const Vector3& projectionOrigin, const bool parallel, const float fadeDepth, const budMaterial* material, const int startTime )
{
	...
	{
		common->Printf( "budRenderModelDecal::CreateProjectionInfo: winding must have %d points\n", NUM_DECAL_BOUNDING_PLANES - 2 );
		return false;
	}
...
/*
=================
budRenderModelDecal::ReUse
=================
*/
void budRenderModelDecal::ReUse()
{
	firstDecal = 0;
	nextDecal = 0;
	firstDeferredDecal = 0;
...
/*
=================
budRenderModelDecal::CreateDecalFromWinding
=================
*/
void budRenderModelDecal::CreateDecalFromWinding( const idWinding& w, const budMaterial* decalMaterial, const budPlane fadePlanes[2], float fadeDepth, int startTime )
{
...
============
*/
static void R_DecalPointCullStatic( byte* cullBits, const budPlane* planes, const budDrawVert* verts, const int numVerts )
{
...
/*
=================
budRenderModelDecal::CreateDecal
=================
*/
void budRenderModelDecal::CreateDecal( const budRenderModel* model, const decalProjectionParms_t& localParms )
{
	int maxVerts = 0;
	...
	for( int surfNum = 0; surfNum < model->NumSurfaces(); surfNum++ )
	{
		...
		for( int i = 0; i < tri->numIndexes; )
		{
			...
			for( ; i <= nextNumIndexes; i += 3 )
			{
				...
				const budPlane plane( verts[0]->xyz, verts[1]->xyz, verts[2]->xyz );
...
/*
=====================
budRenderModelDecal::CreateDeferredDecals
=====================
*/
void budRenderModelDecal::CreateDeferredDecals( const budRenderModel* model )
{
...
/*
=====================
budRenderModelDecal::AddDeferredDecal
=====================
*/
void budRenderModelDecal::AddDeferredDecal( const decalProjectionParms_t& localParms )
{
...
/*
=====================
budRenderModelDecal::RemoveFadedDecals
=====================
*/
void budRenderModelDecal::RemoveFadedDecals( int time )
{
...
=====================
*/
static void R_CopyDecalSurface( budDrawVert* verts, int numVerts, triIndex_t* indexes, int numIndexes,
								const decal_t* decal, const float fadeColor[4] )
{
	...
	assert( ( ( decal->numVerts * sizeof( budDrawVert ) ) & 15 ) == 0 );
	...
	assert_offsetof( budDrawVert, color, 6 * 4 );
	for( int i = 0; i < decal->numVerts; i++ )
	{
		const budDrawVert& srcVert = decal->verts[i];
...
/*
=====================
budRenderModelDecal::GetNumDecalDrawSurfs
=====================
*/
unsigned int budRenderModelDecal::GetNumDecalDrawSurfs()
{
	numDecalMaterials = 0;
...
/*
=====================
budRenderModelDecal::CreateDecalDrawSurf
=====================
*/
drawSurf_t* budRenderModelDecal::CreateDecalDrawSurf( const viewEntity_t* space, unsigned int index )
{
	...
	const budMaterial* material = decalMaterials[index];
...
/*
====================
budRenderModelDecal::ReadFromDemoFile
====================
*/
void budRenderModelDecal::ReadFromDemoFile( budDemoFile* f )
{
	...
	for( unsigned int i = firstDecal; i < nextDecal; i++ )
	{
		...
		{
			f->Read( &decal.verts[ j ], sizeof( budDrawVert ) );
		}
	...
	for( unsigned int i = firstDeferredDecal; i < nextDeferredDecal; i++ )
	{
		...
		f->Read( deferredDecal.boundingPlanes, sizeof( budPlane )*NUM_DECAL_BOUNDING_PLANES );
...
/*
====================
budRenderModelDecal::WriteToDemoFile
====================
*/
void budRenderModelDecal::WriteToDemoFile( budDemoFile* f ) const
{
	unsigned int i = 0;
	int j = 0;
	...
	for( unsigned int i = firstDecal; i < nextDecal; i++ )
	{
		...
		if( decal.numVerts )
		{
			...
			{
				f->Write( &decal.verts[ j ], sizeof( budDrawVert ) );
			}
		}
	...
	for( i = firstDeferredDecal; i < nextDeferredDecal; i++ )
	{
		...
		f->Write( deferredDecal.boundingPlanes, sizeof( budPlane )*NUM_DECAL_BOUNDING_PLANES );
