...
#include "../../../libBud/sys/sys_intrinsics.h"
		...
		bool* insideShadowVolume, const float radius )
{
	...
	idODSStreamedIndexedArray< budDrawVert, triIndex_t, 32, SBT_QUAD, 4 * 3 > indexedVertsODS( verts, numVerts, indexes, numIndexes );
	...
	idODSStreamedIndexedArray< budDrawVert, triIndex_t, 32, SBT_QUAD, 1 > indexedVertsODS( verts, numVerts, indexes, numIndexes );
		...
		bool* insideShadowVolume, const float radius )
{
	...
	idODSStreamedArray< budDrawVert, 32, SBT_DOUBLE, 1 > vertsODS( verts, numVerts );
...
void DynamicShadowVolumeJob( const dynamicShadowVolumeParms_t* parms )
{
	...
	{
		budRenderMatrix::DepthBoundsForShadowBounds( shadowZMin, shadowZMax, parms->triangleMVP, parms->triangleBounds, parms->localLightOrigin, true );
