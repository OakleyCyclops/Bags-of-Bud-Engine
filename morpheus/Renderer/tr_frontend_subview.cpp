...
=============
*/
static void R_PlaneForSurface( const srfTriangles_t* tri, budPlane& plane )
{
	budDrawVert* v1 = tri->verts + tri->indexes[0];
	budDrawVert* v2 = tri->verts + tri->indexes[1];
	budDrawVert* v3 = tri->verts + tri->indexes[2];
...
=================
*/
void R_MirrorRender( const drawSurf_t* surf, textureStage_t* stage, budScreenRect scissor )
{
	// remote views can be reused in a single frame
	if( stage->dynamicFrameCount == tr.frameCount )
	{
...
=================
*/
void R_XrayRender( const drawSurf_t* surf, textureStage_t* stage, budScreenRect scissor )
{
	// remote views can be reused in a single frame
	if( stage->dynamicFrameCount == tr.frameCount )
	{
