...
void R_ReloadImages_f( const budCmdArgs& args )
{
	...
	if( args.Argc() == 2 )
	{
		...
		{
			libBud::Printf( "USAGE: reloadImages <all>\n" );
			return;
		}
	}
...
void R_ListImages_f( const budCmdArgs& args )
{
	...
	if( failed )
	{
		libBud::Printf( "usage: listImages [ sorted | namesort | unloaded | duplicated | showOverSized ]\n" );
		return;
	}
	...
	if( sorted || sortByName )
	{
		...
		for( i = 0 ; i < count ; i++ )
		{
			libBud::Printf( "%4i:",	sortedArray[i].index );
			sortedArray[i].image->Print();
			partialSize += sortedArray[i].image->StorageSize();
			if( ( ( i + 1 ) % 10 ) == 0 )
			{
				libBud::Printf( "-------- %5.1f of %5.1f megs --------\n",
							   partialSize / ( 1024 * 1024.0 ), totalSize / ( 1024 * 1024.0 ) );
			}
		}
	}
	
	libBud::Printf( "%s", header );
	libBud::Printf( " %i images (%i total)\n", count, numImages );
	libBud::Printf( " %5.1f total megabytes of images\n\n\n", totalSize / ( 1024 * 1024.0 ) );
}
...
Allocates an budImage, adds it to the list,
copies the name, and adds it to the hash chain.
==============
*/
budImage* budImageManager::AllocImage( const char* name )
{
...
Allocates an budImage,does not add it to the list or hash chain

==============
*/
budImage* budImageManager::AllocStandaloneImage( const char* name )
{
	if( strlen( name ) >= MAX_IMAGE_NAME )
	{
		common->Error( "budImageManager::AllocImage: \"%s\" is too long\n", name );
	}
...
==================
*/
budImage* budImageManager::ImageFromFunction( const char* _name, void ( *generatorFunction )( budImage* image ) )
{
	...
	budImage*	 image = AllocImage( name );
...
==============
*/
budImage*	budImageManager::ImageFromFile( const char* _name, textureFilter_t filter,
		textureRepeat_t repeat, textureUsage_t usage, cubeFiles_t cubeMap )
{
	...
	for( int i = imageHash.First( hash ); i != -1; i = imageHash.Next( i ) )
	{
		...
		if( name.Icmp( image->GetName() ) == 0 )
		{
			...
			{
				libBud::Error( "Image '%s' has been referenced with conflicting cube map states", _name );
			}
...
/*
========================
budImageManager::ScratchImage
========================
*/
budImage* budImageManager::ScratchImage( const char* _name, budImageOpts* imgOpts, textureFilter_t filter, textureRepeat_t repeat, textureUsage_t usage )
{
	if( !_name || !_name[0] )
	{
		libBud::FatalError( "budImageManager::ScratchImage called with empty name" );
	}
	...
	// create a new image
	//
	budImage* newImage = AllocImage( name );
	if( newImage != NULL )
	{
...
===============
*/
budImage* budImageManager::ScratchImage( const char* name, const budImageOpts& opts )
{
	if( !name || !name[0] )
	{
		libBud::FatalError( "budImageManager::ScratchImage" );
	}
	
	budImage* image = GetImage( name );
	if( image == NULL )
	{
...
===============
*/
void budImageManager::PurgeAllImages()
{
...
===============
*/
void budImageManager::ReloadImages( bool all )
{
	for( int i = 0 ; i < images.Num() ; i++ )
	{
		images[ i ]->Reload( all );
...
void R_CombineCubeImages_f( const budCmdArgs& args )
{
	if( args.Argc() != 2 )
	{
		libBud::Printf( "usage: combineCubeImages <baseName>\n" );
		libBud::Printf( " combines basename[1-6][0001-9999].tga to basenameCM[0001-9999].tga\n" );
		libBud::Printf( " 1: forward 2:right 3:back 4:left 5:up 6:down\n" );
		return;
	}
...
/*
====================
budImageManager::BeginLevelLoad
Frees all images used by the previous level
====================
*/
void budImageManager::BeginLevelLoad()
{
	...
	for( int i = 0 ; i < images.Num() ; i++ )
	{
		...
		{
			image->PurgeImage();
			//libBud::Printf( "purging %s\n", image->GetName() );
		}
		else
		{
			//libBud::Printf( "not purging %s\n", image->GetName() );
		}
...
/*
====================
budImageManager::ExcludePreloadImage
====================
*/
...
/*
====================
budImageManager::Preload
====================
*/
void budImageManager::Preload( const idPreloadManifest& manifest, const bool& mapPreload )
{
	if( preLoad_Images.GetBool() && manifest.NumResources() > 0 )
	{
		// preload this levels images
		libBud::Printf( "Preloading images...\n" );
		...
		libBud::Printf( "%05d images preloaded ( or were already loaded ) in %5.1f seconds\n", numLoaded, ( end - start ) * 0.001 );
		libBud::Printf( "----------------------------------------\n" );
...
/*
===============
budImageManager::LoadLevelImages
===============
*/
int budImageManager::LoadLevelImages( bool pacifier )
{
	int	loadCount = 0;
	for( int i = 0 ; i < images.Num() ; i++ )
	{
...
/*
===============
budImageManager::EndLevelLoad
===============
*/
void budImageManager::EndLevelLoad()
{
	insideLevelLoad = false;
	
	libBud::Printf( "----- budImageManager::EndLevelLoad -----\n" );
	int start = Sys_Milliseconds();
...
/*
===============
budImageManager::PrintMemInfo
===============
*/
void budImageManager::PrintMemInfo( MemInfo_t* mi )
{
	int i, j, total = 0;
	int* sortIndex;
	budFile* f;
	...
	for( i = 0; i < images.Num(); i++ )
	{
		budImage* im = images[sortIndex[i]];
		int size;
