...
budFrameData		smpFrameData[NUM_FRAME_DATA];
budFrameData* 	frameData;
unsigned int	smpFrame;

//#define TRACK_FRAME_ALLOCS

#if defined( TRACK_FRAME_ALLOCS )
budSysInterlockedInteger frameAllocTypeCount[FRAME_ALLOC_MAX];
...
void* R_FrameAlloc( int bytes, frameAllocType_t type )
{
	...
	{
		libBud::Error( "R_FrameAlloc ran out of memory. bytes = %d, end = %d, highWaterAllocated = %d\n", bytes, end, frameData->highWaterAllocated );
	}
...
static void R_SetupSplitFrustums( viewDef_t* viewDef )
{
	...
	for( int i = 1; i <= ( r_shadowMapSplits.GetInteger() + 1 ) && i < MAX_FRUSTUMS; i++ )
	{
		...
		budRenderMatrix projectionRenderMatrix;
		budRenderMatrix::Transpose( *( budRenderMatrix* )projectionMatrix, projectionRenderMatrix );
		budRenderMatrix viewRenderMatrix;
		budRenderMatrix::Transpose( *( budRenderMatrix* )tr.viewDef->worldSpace.modelViewMatrix, viewRenderMatrix );
		budRenderMatrix::Multiply( projectionRenderMatrix, viewRenderMatrix, tr.viewDef->frustumMVPs[i] );
		
		// the planes of the view frustum are needed for portal visibility culling
		budRenderMatrix::GetFrustumPlanes( tr.viewDef->frustums[i], tr.viewDef->frustumMVPs[i], false, true );
...
void R_RenderView( viewDef_t* parms )
{
	...
	// setup render matrices for faster culling
	budRenderMatrix::Transpose( *( budRenderMatrix* )tr.viewDef->projectionMatrix, tr.viewDef->projectionRenderMatrix );
	budRenderMatrix viewRenderMatrix;
	budRenderMatrix::Transpose( *( budRenderMatrix* )tr.viewDef->worldSpace.modelViewMatrix, viewRenderMatrix );
	budRenderMatrix::Multiply( tr.viewDef->projectionRenderMatrix, viewRenderMatrix, tr.viewDef->worldSpace.mvp );
	
	// the planes of the view frustum are needed for portal visibility culling
	budRenderMatrix::GetFrustumPlanes( tr.viewDef->frustums[FRUSTUM_PRIMARY], tr.viewDef->worldSpace.mvp, false, true );
	...
	static_cast<budRenderWorldLocal*>( parms->renderWorld )->FindViewLightsAndEntities();
	...
	if( common->WriteDemo() )
	{
		static_cast<budRenderWorldLocal*>( parms->renderWorld )->WriteVisibleDefs( tr.viewDef );
	}
