...
class budRenderModelOverlay
{
public:
	budRenderModelOverlay();
	~budRenderModelOverlay();
	
	void						ReUse();
	
	void						AddDeferredOverlay( const overlayProjectionParms_t& localParms );
	void						CreateDeferredOverlays( const budRenderModel* model );
	
	unsigned int				GetNumOverlayDrawSurfs();
	struct drawSurf_t* 			CreateOverlayDrawSurf( const struct viewEntity_t* space, const budRenderModel* baseModel, unsigned int index );
...
private:
	...
	const budMaterial* 			overlayMaterials[MAX_OVERLAYS];
	unsigned int				numOverlayMaterials;
	
	void						CreateOverlay( const budRenderModel* model, const budPlane localTextureAxis[2], const budMaterial* material );
