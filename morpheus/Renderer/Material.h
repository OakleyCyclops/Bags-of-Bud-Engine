...
typedef struct
{
	...
	budImage* 			fragmentProgramImages[MAX_FRAGMENT_IMAGES];
} newShaderStage_t;
...
class budMaterial : public budDecl
{
public:
	budMaterial();
	virtual				~budMaterial();
	...
	budImage* 			GetFastPathBumpImage() const
	{
		return fastPathBumpImage;
	};
	budImage* 			GetFastPathDiffuseImage() const
	{
		return fastPathDiffuseImage;
	};
	budImage* 			GetFastPathSpecularImage() const
	{
	...
	bool				HasHigherDmapPriority( const budMaterial& other ) const
	{
		return ( IsDrawn() && !other.IsDrawn() ) ||
			   ( Coverage() < other.Coverage() );
	}
	
	// returns a budUserInterface if it has a global gui, or NULL if no gui
	budUserInterface*		GlobalGui() const
	{
		return gui;
	}
	...
	budImage*				LightFalloffImage() const
	{
	...
	const budDecl*		GetDeformDecl() const
	{
		return deformDecl;
	}
...
private:
	...
	void				CommonInit();
	void				ParseMaterial( budLexer& src );
	bool				MatchToken( budLexer& src, const char* match );
	void				ParseSort( budLexer& src );
	void				ParseStereoEye( budLexer& src );
	void				ParseBlend( budLexer& src, shaderStage_t* stage );
	void				ParseVertexParm( budLexer& src, newShaderStage_t* newStage );
	void				ParseVertexParm2( budLexer& src, newShaderStage_t* newStage );
	void				ParseFragmentMap( budLexer& src, newShaderStage_t* newStage );
	void				ParseStage( budLexer& src, const textureRepeat_t trpDefault = TR_REPEAT );
	void				ParseDeform( budLexer& src );
	void				ParseDecalInfo( budLexer& src );
	bool				CheckSurfaceParm( budToken* token );
	...
	int					ParseEmitOp( budLexer& src, int a, expOpType_t opType, int priority );
	int					ParseTerm( budLexer& src );
...
private:
	...
	budImage* 			fastPathBumpImage;	// if any of these are set, they all will be
	budImage* 			fastPathDiffuseImage;
	budImage* 			fastPathSpecularImage;
	
	int					entityGui;			// draw a gui with the budUserInterface from the renderEntity_t
	// non zero will draw gui, gui2, or gui3 from renderEnitty_t
	mutable budUserInterface*	gui;			// non-custom guis are shared by all users of a material
	...
	const budDecl*		deformDecl;			// for surface emitted particle deforms and tables
