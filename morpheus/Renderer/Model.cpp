...
budCVar budRenderModelStatic::r_mergeModelSurfaces( "r_mergeModelSurfaces", "1", CVAR_BOOL | CVAR_RENDERER, "combine model surfaces with the same material" );
budCVar budRenderModelStatic::r_slopVertex( "r_slopVertex", "0.01", CVAR_RENDERER, "merge xyz coordinates this far apart" );
budCVar budRenderModelStatic::r_slopTexCoord( "r_slopTexCoord", "0.001", CVAR_RENDERER, "merge texture coordinates this far apart" );
budCVar budRenderModelStatic::r_slopNormal( "r_slopNormal", "0.02", CVAR_RENDERER, "merge normals that dot less than this" );
...
================
*/
static void AddCubeFace( srfTriangles_t* tri, budVec3 v1, budVec3 v2, budVec3 v3, budVec3 v4 )
{
...
void budRenderModelStatic::MakeDefaultModel()
{
	...
	AddCubeFace( tri, budVec3( -1, 1, 1 ), budVec3( 1, 1, 1 ), budVec3( 1, -1, 1 ), budVec3( -1, -1, 1 ) );
	AddCubeFace( tri, budVec3( -1, 1, -1 ), budVec3( -1, -1, -1 ), budVec3( 1, -1, -1 ), budVec3( 1, 1, -1 ) );
	...
	for( int i = 0; i < surfaces.Num(); i++ )
	{
		...
		if( ( surfaces[i].geometry != NULL ) && ( surfaces[i].geometry->numVerts > 0 ) && ( surfaces[i].geometry->numIndexes > 0 ) && ( surfaces[i].geometry->verts != NULL ) )
		{
			...
			if( tri.numVerts > 0 && tri.verts != NULL )
			{
				...
				for( int j = 0; j < tri.numVerts; j++ )
				{
					const budVec2 vST =  tri.verts[j].GetTexCoord();
				...
				for( int j = 0; j < tri.numVerts; j++ )
				{
					const budVec3 n = tri.verts[j].GetNormalRaw();
...
void budRenderModelStatic::InitEmpty( const char* fileName )
{
	...
	// shadowed normally
	if( !budStr::Cmpn( fileName, "_area", 5 ) )
	{
...
void budRenderModelStatic::FinishSurfaces()
{
	...
	else
	{
		...
		for( i = 0; i < surfaces.Num(); i++ )
		{
			...
			{
				srfTriangles_t*	tri = surf->geometry;
				budVec3	mid = ( tri->bounds[1] + tri->bounds[0] ) * 0.5f;
...
//	Collada::Material**	material;
	...
	budVectorSubset<budVec3, 3> vertexSubset;
	budVectorSubset<budVec2, 2> texCoordSubset;
	...
	for( objectNum = 0; objectNum < dae->mNodeLibrary.Num(); objectNum++ )
	{
		...
		if( meshInstance->mMaterials.Num() )
		{
			...
			budStr materialId = mappingTable->mMatName;
		...
		budStr meshId = ( meshInstance )->mMeshOrController;
		...
		budMat4 nodeTransform = dae->CalculateResultTransform( ( *object )->mTransforms );
		...
		{
			const budVec3& p = ( *mesh )->mPositions[j];
		...
		{
			const budVec3& n = ( *mesh )->mNormals[j];
			
			budVec4 n2( n.x, n.y, n.z, 0 );
			n2 = nodeTransform * n2;
			n2.Normalize();
		...
		else
		{
			...
			mins -= budVec3( expand, expand, expand );
			maxs += budVec3( expand, expand, expand );
		...
		else
		{
			...
			mins -= budVec2( expand, expand );
			maxs += budVec2( expand, expand );
		...
		else
		{
			...
			textureSin = budMath::Sin( material->angle );
			textureCos = budMath::Cos( material->angle );
		}
		*/
		...
		for( j = 0; j < tri->numVerts; j++ )
		{
			...
			*( unsigned* )tri->verts[j].color = *( unsigned* )mv->color;
			...
			{
				const budVec2& tv = ( *mesh )->mTexCoords[ mv->tv ];
	...
	for( objectNum = 0; objectNum < ase->objects.Num(); objectNum++ )
	{
		...
		for( j = 0; j < tri->numVerts; j++ )
		{
			...
			*( unsigned* )tri->verts[j].color = *( unsigned* )mv->color;
			if( mesh->numTVFaces == mesh->numFaces && mesh->numTVertexes != 0 )
			{
				const budVec2& tv = mesh->tvertexes[ mv->tv ];
				float u = tv.x * uTiling + uOffset;
				float v = tv.y * vTiling + vOffset;
				tri->verts[j].SetTexCoord( u * textureCos + v * textureSin, u * -textureSin + v * textureCos );
			}
...
bool budRenderModelStatic::ConvertLWOToModelSurfaces( const struct st_lwObject* lwo )
{
	...
	vList = ( budVec3* )R_StaticAlloc( layer->point.count * sizeof( vList[0] ), TAG_MODEL );
	...
	if( numTVertexes )
	{
		tvList = ( budVec2* )Mem_Alloc( numTVertexes * sizeof( tvList[0] ), TAG_MODEL );
		int offset = 0;
		for( lwVMap* vm = layer->vmap; vm; vm = vm->next )
		{
			if( vm->type == LWID_( 'T', 'X', 'U', 'V' ) )
	...
	else
	{
		...
		numTVertexes = 1;
		tvList = ( budVec2* )Mem_ClearedAlloc( numTVertexes * sizeof( tvList[0] ), TAG_MODEL );
	}
...
struct aseModel_s* budRenderModelStatic::ConvertLWOToASE( const struct st_lwObject* obj, const char* fileName )
{
	...
	for( lwSurface* surf = obj->surf; surf; surf = surf->next )
	{
		...
		mesh->vertexes = ( budVec3* )Mem_Alloc( mesh->numVertexes * sizeof( mesh->vertexes[0] ), TAG_MODEL );
		...
		{
			mesh->tvertexes = ( budVec2* )Mem_Alloc( mesh->numTVertexes * sizeof( mesh->tvertexes[0] ), TAG_MODEL );
			int offset = 0;
			for( lwVMap* vm = layer->vmap; vm; vm = vm->next )
			{
				if( vm->type == LWID_( 'T', 'X', 'U', 'V' ) )
		...
		else
		{
			...
			mesh->numTVertexes = 1;
			mesh->tvertexes = ( budVec2* )Mem_ClearedAlloc( mesh->numTVertexes * sizeof( mesh->tvertexes[0] ), TAG_MODEL );
		}
...
void budRenderModelStatic::ReadFromDemoFile( class budDemoFile* f )
{
	...
	for( i = 0; i < numSurfaces; i++ )
	{
		...
		budVec3 tNormal, tTangent, tBiTangent;
		for( j = 0; j < tri->numVerts; ++j )
		{
