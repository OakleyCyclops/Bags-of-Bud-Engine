...
CVar budRenderModelStatic::r_mergeModelSurfaces( "r_mergeModelSurfaces", "1", CVAR_BOOL | CVAR_RENDERER, "combine model surfaces with the same material" );
CVar budRenderModelStatic::r_slopVertex( "r_slopVertex", "0.01", CVAR_RENDERER, "merge xyz coordinates this far apart" );
CVar budRenderModelStatic::r_slopTexCoord( "r_slopTexCoord", "0.001", CVAR_RENDERER, "merge texture coordinates this far apart" );
CVar budRenderModelStatic::r_slopNormal( "r_slopNormal", "0.02", CVAR_RENDERER, "merge normals that dot less than this" );
...
/*
================
budRenderModelStatic::budRenderModelStatic
================
*/
budRenderModelStatic::budRenderModelStatic()
{
...
/*
==============
budRenderModelStatic::Print
==============
*/
void budRenderModelStatic::Print() const
{
	common->Printf( "%s\n", name.c_str() );
	...
	for( int i = 0; i < NumSurfaces(); i++ )
	{
		...
		const budMaterial* material = surf->shader;
...
/*
==============
budRenderModelStatic::Memory
==============
*/
int budRenderModelStatic::Memory() const
{
	int	totalBytes = 0;
...
/*
==============
budRenderModelStatic::List
==============
*/
void budRenderModelStatic::List() const
{
	int	totalTris = 0;
	int	totalVerts = 0;
	int	totalBytes = 0;
...
/*
================
budRenderModelStatic::IsDefaultModel
================
*/
bool budRenderModelStatic::IsDefaultModel() const
{
	return defaulted;
}
...
================
*/
static void AddCubeFace( srfTriangles_t* tri, Vector3 v1, Vector3 v2, Vector3 v3, Vector3 v4 )
{
...
/*
================
budRenderModelStatic::MakeDefaultModel
================
*/
void budRenderModelStatic::MakeDefaultModel()
{
	...
	AddCubeFace( tri, Vector3( -1, 1, 1 ), Vector3( 1, 1, 1 ), Vector3( 1, -1, 1 ), Vector3( -1, -1, 1 ) );
	AddCubeFace( tri, Vector3( -1, 1, -1 ), Vector3( -1, -1, -1 ), Vector3( 1, -1, -1 ), Vector3( 1, 1, -1 ) );
...
/*
================
budRenderModelStatic::PartialInitFromFile
================
*/
void budRenderModelStatic::PartialInitFromFile( const char* fileName )
{
	fastLoad = true;
...
/*
================
budRenderModelStatic::InitFromFile
================
*/
void budRenderModelStatic::InitFromFile( const char* fileName )
{
	bool loaded;
	String extension;
...
// RB end
	...
	{
		common->Warning( "budRenderModelStatic::InitFromFile: unknown type for model: \'%s\'", name.c_str() );
		loaded		= false;
	}
...
/*
========================
budRenderModelStatic::LoadBinaryModel
========================
*/
bool budRenderModelStatic::LoadBinaryModel( budFile* file, const ID_TIME_T sourceTimeStamp )
{
	if( file == NULL )
	{
		return false;
...
/*
========================
budRenderModelStatic::WriteBinaryModel
========================
*/
void budRenderModelStatic::WriteBinaryModel( budFile* file, ID_TIME_T* _timeStamp ) const
{
	if( file == NULL )
	{
...
// RB begin
void budRenderModelStatic::ExportOBJ( budFile* objFile, budFile* mtlFile, ID_TIME_T* _timeStamp ) const
{
	...
	for( int i = 0; i < surfaces.Num(); i++ )
	{
		...
		if( ( surfaces[i].geometry != NULL ) && ( surfaces[i].geometry->numVerts > 0 ) && ( surfaces[i].geometry->numIndexes > 0 ) && ( surfaces[i].geometry->verts != NULL ) )
		{
			...
			if( tri.numVerts > 0 && tri.verts != NULL )
			{
				...
				for( int j = 0; j < tri.numVerts; j++ )
				{
					const Vector2 vST =  tri.verts[j].GetTexCoord();
				...
				for( int j = 0; j < tri.numVerts; j++ )
				{
					const Vector3 n = tri.verts[j].GetNormalRaw();
	...
	for( int i = 0; i < materials.Num(); i++ )
	{
		...
		{
			String path = material->GetFastPathDiffuseImage()->GetName();
		...
		{
			String path = material->GetEditorImage()->GetName();
			path.SlashesToBackSlashes();
			path.DefaultFileExtension( ".tga" );
...
/*
================
budRenderModelStatic::LoadModel
================
*/
void budRenderModelStatic::LoadModel()
{
	PurgeModel();
	InitFromFile( name );
}
...
/*
================
budRenderModelStatic::InitEmpty
================
*/
void budRenderModelStatic::InitEmpty( const char* fileName )
{
	...
	// shadowed normally
	if( !String::Cmpn( fileName, "_area", 5 ) )
	{
...
/*
================
budRenderModelStatic::AddSurface
================
*/
void budRenderModelStatic::AddSurface( modelSurface_t surface )
{
...
/*
================
budRenderModelStatic::Name
================
*/
const char* budRenderModelStatic::Name() const
{
	return name;
}
...
/*
================
budRenderModelStatic::Timestamp
================
*/
ID_TIME_T budRenderModelStatic::Timestamp() const
{
	return timeStamp;
}
...
/*
================
budRenderModelStatic::NumSurfaces
================
*/
int budRenderModelStatic::NumSurfaces() const
{
	return surfaces.Num();
}
...
/*
================
budRenderModelStatic::NumBaseSurfaces
================
*/
int budRenderModelStatic::NumBaseSurfaces() const
{
...
/*
================
budRenderModelStatic::Surface
================
*/
const modelSurface_t* budRenderModelStatic::Surface( int surfaceNum ) const
{
...
/*
================
budRenderModelStatic::AllocSurfaceTriangles
================
*/
srfTriangles_t* budRenderModelStatic::AllocSurfaceTriangles( int numVerts, int numIndexes ) const
{
	srfTriangles_t* tri = R_AllocStaticTriSurf();
...
/*
================
budRenderModelStatic::FreeSurfaceTriangles
================
*/
void budRenderModelStatic::FreeSurfaceTriangles( srfTriangles_t* tris ) const
{
...
/*
================
budRenderModelStatic::IsStaticWorldModel
================
*/
bool budRenderModelStatic::IsStaticWorldModel() const
{
...
/*
================
budRenderModelStatic::IsDynamicModel
================
*/
dynamicModel_t budRenderModelStatic::IsDynamicModel() const
{
...
/*
================
budRenderModelStatic::IsReloadable
================
*/
bool budRenderModelStatic::IsReloadable() const
{
	return reloadable;
}
...
/*
================
budRenderModelStatic::Bounds
================
*/
budBounds budRenderModelStatic::Bounds( const struct renderEntity_s* mdef ) const
{
	return bounds;
}
...
/*
================
budRenderModelStatic::DepthHack
================
*/
float budRenderModelStatic::DepthHack() const
{
	return 0.0f;
}
...
/*
================
budRenderModelStatic::InstantiateDynamicModel
================
*/
budRenderModel* budRenderModelStatic::InstantiateDynamicModel( const struct renderEntity_s* ent, const viewDef_t* view, budRenderModel* cachedModel )
{
	if( cachedModel )
	{
		delete cachedModel;
...
/*
================
budRenderModelStatic::NumJoints
================
*/
int budRenderModelStatic::NumJoints() const
{
	return 0;
}
...
/*
================
budRenderModelStatic::GetJoints
================
*/
const budMD5Joint* budRenderModelStatic::GetJoints() const
{
	return NULL;
}
...
/*
================
budRenderModelStatic::GetJointHandle
================
*/
jointHandle_t budRenderModelStatic::GetJointHandle( const char* name ) const
{
...
/*
================
budRenderModelStatic::GetJointName
================
*/
const char* budRenderModelStatic::GetJointName( jointHandle_t handle ) const
{
	return "";
}
...
/*
================
budRenderModelStatic::GetDefaultPose
================
*/
const budJointQuat* budRenderModelStatic::GetDefaultPose() const
{
	return NULL;
}
...
/*
================
budRenderModelStatic::NearestJoint
================
*/
int budRenderModelStatic::NearestJoint( int surfaceNum, int a, int b, int c ) const
{
	return INVALID_JOINT;
}
...
/*
================
budRenderModelStatic::FinishSurfaces
...
================
*/
void budRenderModelStatic::FinishSurfaces()
{
	int			i;
	...
	for( i = 0; i < surfaces.Num(); i++ )
	{
		...
		for( int j = 0; j < tri->numIndexes; j += 3 )
		{
			...
			const_cast<budMaterial*>( surf->shader )->AddToSurfaceArea( area );
		}
	}
	...
	else
	{
		...
		for( i = 0; i < surfaces.Num(); i++ )
		{
			...
			{
				srfTriangles_t*	tri = surf->geometry;
				Vector3	mid = ( tri->bounds[1] + tri->bounds[0] ) * 0.5f;
...
bool budRenderModelStatic::ConvertDAEToModelSurfaces( const ColladaParser* dae )
{
	Collada::Node**		object;
	Collada::Mesh**		mesh;
	Collada::MeshInstance* meshInstance;
//	Collada::Material**	material;
	const budMaterial*	im1, *im2;
	srfTriangles_t*		tri;
	int					objectNum;
	int					i, j, k;
	int					index, v, tv;
	...
	VectortorSubset<Vector3, 3> vertexSubset;
	VectortorSubset<Vector2, 2> texCoordSubset;
	...
	for( objectNum = 0; objectNum < dae->mNodeLibrary.Num(); objectNum++ )
	{
		...
		if( meshInstance->mMaterials.Num() )
		{
			...
			String materialId = mappingTable->mMatName;
		...
		String meshId = ( meshInstance )->mMeshOrController;
		...
		Matrix4 nodeTransform = dae->CalculateResultTransform( ( *object )->mTransforms );
		...
		{
			const Vector3& p = ( *mesh )->mPositions[j];
		...
		{
			const Vector3& n = ( *mesh )->mNormals[j];
			
			Vector4 n2( n.x, n.y, n.z, 0 );
			n2 = nodeTransform * n2;
			n2.Normalize();
		...
		else
		{
			...
			mins -= Vector3( expand, expand, expand );
			maxs += Vector3( expand, expand, expand );
		...
		else
		{
			...
			mins -= Vector2( expand, expand );
			maxs += Vector2( expand, expand );
		...
		else
		{
			...
			textureSin = Math::Sin( material->angle );
			textureCos = Math::Cos( material->angle );
		}
		*/
		...
		for( j = 0; j < tri->numVerts; j++ )
		{
			...
			*( unsigned* )tri->verts[j].color = *( unsigned* )mv->color;
			...
			{
				const Vector2& tv = ( *mesh )->mTexCoords[ mv->tv ];
...
/*
=================
budRenderModelStatic::ConvertASEToModelSurfaces
=================
*/
bool budRenderModelStatic::ConvertASEToModelSurfaces( const struct aseModel_s* ase )
{
	aseObject_t* 	object;
	aseMesh_t* 		mesh;
	...
	for( objectNum = 0; objectNum < ase->objects.Num(); objectNum++ )
	{
		...
		for( j = 0; j < tri->numVerts; j++ )
		{
			...
			*( unsigned* )tri->verts[j].color = *( unsigned* )mv->color;
			if( mesh->numTVFaces == mesh->numFaces && mesh->numTVertexes != 0 )
			{
				const Vector2& tv = mesh->tvertexes[ mv->tv ];
				float u = tv.x * uTiling + uOffset;
				float v = tv.y * vTiling + vOffset;
				tri->verts[j].SetTexCoord( u * textureCos + v * textureSin, u * -textureSin + v * textureCos );
			}
...
/*
=================
budRenderModelStatic::ConvertLWOToModelSurfaces
=================
*/
bool budRenderModelStatic::ConvertLWOToModelSurfaces( const struct st_lwObject* lwo )
{
	const budMaterial* im1, *im2;
	srfTriangles_t*	tri;
	lwSurface* 		lwoSurf;
	...
	vList = ( Vector3* )R_StaticAlloc( layer->point.count * sizeof( vList[0] ), TAG_MODEL );
	...
	if( numTVertexes )
	{
		tvList = ( Vector2* )Mem_Alloc( numTVertexes * sizeof( tvList[0] ), TAG_MODEL );
		int offset = 0;
		for( lwVMap* vm = layer->vmap; vm; vm = vm->next )
		{
			if( vm->type == LWID_( 'T', 'X', 'U', 'V' ) )
	...
	else
	{
		...
		numTVertexes = 1;
		tvList = ( Vector2* )Mem_ClearedAlloc( numTVertexes * sizeof( tvList[0] ), TAG_MODEL );
	}
...
/*
=================
budRenderModelStatic::ConvertLWOToASE
=================
*/
struct aseModel_s* budRenderModelStatic::ConvertLWOToASE( const struct st_lwObject* obj, const char* fileName )
{
	int j, k;
	aseModel_t* ase;
	...
	for( lwSurface* surf = obj->surf; surf; surf = surf->next )
	{
		...
		mesh->vertexes = ( Vector3* )Mem_Alloc( mesh->numVertexes * sizeof( mesh->vertexes[0] ), TAG_MODEL );
		...
		{
			mesh->tvertexes = ( Vector2* )Mem_Alloc( mesh->numTVertexes * sizeof( mesh->tvertexes[0] ), TAG_MODEL );
			int offset = 0;
			for( lwVMap* vm = layer->vmap; vm; vm = vm->next )
			{
				if( vm->type == LWID_( 'T', 'X', 'U', 'V' ) )
		...
		else
		{
			...
			mesh->numTVertexes = 1;
			mesh->tvertexes = ( Vector2* )Mem_ClearedAlloc( mesh->numTVertexes * sizeof( mesh->tvertexes[0] ), TAG_MODEL );
		}
...
/*
=================
budRenderModelStatic::ConvertMAToModelSurfaces
=================
*/
bool budRenderModelStatic::ConvertMAToModelSurfaces( const struct maModel_s* ma )
{
...
/*
=================
budRenderModelStatic::LoadASE
=================
*/
bool budRenderModelStatic::LoadASE( const char* fileName )
{
	aseModel_t* ase;
...
/*
=================
budRenderModelStatic::LoadLWO
=================
*/
bool budRenderModelStatic::LoadLWO( const char* fileName )
{
	unsigned int failID;
	int failPos;
	lwObject* lwo;
...
/*
=================
budRenderModelStatic::LoadMA
=================
*/
bool budRenderModelStatic::LoadMA( const char* fileName )
{
	maModel_t* ma;
...
bool budRenderModelStatic::LoadDAE( const char* fileName )
{
	bool loaded = false;
...
/*
================
budRenderModelStatic::PurgeModel
================
*/
void budRenderModelStatic::PurgeModel()
{
	for( int i = 0; i < surfaces.Num(); i++ )
	{
		modelSurface_t* surf = &surfaces[i];
...
/*
==============
budRenderModelStatic::FreeVertexCache

We are about to restart the vertex cache, so dump everything
==============
*/
void budRenderModelStatic::FreeVertexCache()
{
...
/*
================
budRenderModelStatic::ReadFromDemoFile
================
*/
void budRenderModelStatic::ReadFromDemoFile( class budDemoFile* f )
{
	PurgeModel();
	...
	for( i = 0; i < numSurfaces; i++ )
	{
		...
		Vector3 tNormal, tTangent, tBiTangent;
		for( j = 0; j < tri->numVerts; ++j )
		{
...
/*
================
budRenderModelStatic::WriteToDemoFile
================
*/
void budRenderModelStatic::WriteToDemoFile( class budDemoFile* f )
{
...
/*
================
budRenderModelStatic::IsLoaded
================
*/
bool budRenderModelStatic::IsLoaded()
{
	return !purged;
}
...
/*
================
budRenderModelStatic::SetLevelLoadReferenced
================
*/
void budRenderModelStatic::SetLevelLoadReferenced( bool referenced )
{
...
/*
================
budRenderModelStatic::IsLevelLoadReferenced
================
*/
bool budRenderModelStatic::IsLevelLoadReferenced()
{
...
/*
=================
budRenderModelStatic::TouchData
=================
*/
void budRenderModelStatic::TouchData()
{
	for( int i = 0; i < surfaces.Num(); i++ )
	{
		const modelSurface_t*	surf = &surfaces[i];
...
/*
=================
budRenderModelStatic::DeleteSurfaceWithId
=================
*/
bool budRenderModelStatic::DeleteSurfaceWithId( int id )
{
	int i;
...
/*
=================
budRenderModelStatic::DeleteSurfacesWithNegativeId
=================
*/
void budRenderModelStatic::DeleteSurfacesWithNegativeId()
{
	for( int i = 0; i < surfaces.Num(); i++ )
	{
		if( surfaces[i].id < 0 )
...
/*
=================
budRenderModelStatic::FindSurfaceWithId
=================
*/
bool budRenderModelStatic::FindSurfaceWithId( int id, int& surfaceNum ) const
{
	for( int i = 0; i < surfaces.Num(); i++ )
	{
		if( surfaces[i].id == id )
