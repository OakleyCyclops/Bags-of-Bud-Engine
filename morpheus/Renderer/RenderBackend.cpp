...
================
*/
static BUD_INLINE void SetVertexParm( renderParm_t rp, const float* value )
{
...
================
*/
static BUD_INLINE void SetVertexParms( renderParm_t rp, const float* value, int num )
{
	for( int i = 0; i < num; i++ )
	{
...
================
*/
static BUD_INLINE void SetFragmentParm( renderParm_t rp, const float* value )
{
	renderProgManager.SetUniformValue( rp, value );
}
...
================
*/
static void RB_SetMVPWithStereoOffset( const budRenderMatrix& mvp, const float stereoOffset )
{
	budRenderMatrix offset = mvp;
...
=====================
*/
void RB_BakeTextureMatrixIntoTexgen( budPlane lightProject[3], const float* textureMatrix )
{
	float genMatrix[16];
	float final[16];
...
/*
======================
budRenderBackend::BindVariableStageImage

Handles generating a cinematic frame if needed
======================
*/
void budRenderBackend::BindVariableStageImage( const textureStage_t* texture, const float* shaderRegisters )
{
...
/*
================
budRenderBackend::PrepareStageTexturing
================
*/
void budRenderBackend::PrepareStageTexturing( const shaderStage_t* pStage,  const drawSurf_t* surf )
{
	...
	else if( pStage->texture.texgen == TG_DIFFUSE_CUBE )
	{
		...
		libBud::Warning( "Using Diffuse Cube! Please contact Brian!" );
	...
	else if( pStage->texture.texgen == TG_GLASSWARP )
	{
		...
		// As far as I can tell, this is never used
		libBud::Warning( "Using GlassWarp! Please contact Brian!" );
	}
...
/*
================
budRenderBackend::FinishStageTexturing
================
*/
void budRenderBackend::FinishStageTexturing( const shaderStage_t* pStage, const drawSurf_t* surf )
{
...
void budRenderBackend::ResetViewportAndScissorToDefaultCamera( const viewDef_t* _viewDef )
{
...
/*
==================
budRenderBackend::FillDepthBufferGeneric
==================
*/
void budRenderBackend::FillDepthBufferGeneric( const drawSurf_t* const* drawSurfs, int numDrawSurfs )
{
	for( int i = 0; i < numDrawSurfs; i++ )
	{
		const drawSurf_t* drawSurf = drawSurfs[i];
		const budMaterial* shader = drawSurf->material;
...
/*
=====================
budRenderBackend::FillDepthBufferFast
...
=====================
*/
void budRenderBackend::FillDepthBufferFast( drawSurf_t** drawSurfs, int numDrawSurfs )
{
	if( numDrawSurfs == 0 )
	{
		return;
	...
	for( ; surfNum < numDrawSurfs; surfNum++ )
	{
		...
		const budMaterial* shader = surf->material;
...
/*
==================
budRenderBackend::SetupInteractionStage
==================
*/
...
/*
=================
budRenderBackend::DrawSingleInteraction
=================
*/
void budRenderBackend::DrawSingleInteraction( drawInteraction_t* din )
{
...
/*
=============
budRenderBackend::RenderInteractions

With added sorting and trivial path work.
=============
*/
void budRenderBackend::RenderInteractions( const drawSurf_t* surfList, const viewLight_t* vLight, int depthFunc, bool performStencilTest, bool useLightDepthBounds )
{
	if( surfList == NULL )
	{
		return;
	...
	const budMaterial* lightShader = vLight->lightShader;
	...
	budStaticList< const drawSurf_t*, MAX_INTERACTIONS_PER_LIGHT > allSurfaces;
	budStaticList< const drawSurf_t*, MAX_COMPLEX_INTERACTIONS_PER_LIGHT > complexSurfaces;
	for( const drawSurf_t* walk = surfList; walk != NULL; walk = walk->nextOnLight )
	{
		...
		const budMaterial* surfaceShader = walk->material;
	...
	for( int lightStageNum = 0; lightStageNum < lightShader->GetNumStages(); lightStageNum++ )
	{
		...
		for( int sortedSurfNum = 0; sortedSurfNum < allSurfaces.Num(); sortedSurfNum++ )
		{
			...
			const budMaterial* surfaceShader = surf->material;
			...
			if( surf->space != currentSpace )
			{
				...
				// RB begin
				budRenderMatrix modelMatrix;
				budRenderMatrix::Transpose( *( budRenderMatrix* )surf->space->modelMatrix, modelMatrix );
				
				SetVertexParms( RENDERPARM_MODELMATRIX_X, modelMatrix[0], 4 );
				
				// for determining the shadow mapping cascades
				budRenderMatrix modelViewMatrix, tmp;
				...
				if( r_useShadowMapping.GetBool() )
				{
					if( vLight->parallel )
					{
						...
						{
							budRenderMatrix modelToShadowMatrix;
							budRenderMatrix::Multiply( shadowV[i], modelMatrix, modelToShadowMatrix );
							
							budRenderMatrix shadowClipMVP;
							budRenderMatrix::Multiply( shadowP[i], modelToShadowMatrix, shadowClipMVP );
							
							budRenderMatrix shadowWindowMVP;
							budRenderMatrix::Multiply( renderMatrix_clipSpaceToWindowSpace, shadowClipMVP, shadowWindowMVP );
					...
					else
					{
						...
						budRenderMatrix modelToShadowMatrix;
						budRenderMatrix::Multiply( shadowV[0], modelMatrix, modelToShadowMatrix );
						
						budRenderMatrix shadowClipMVP;
						budRenderMatrix::Multiply( shadowP[0], modelToShadowMatrix, shadowClipMVP );
...
/*
==================
budRenderBackend::AmbientPass
==================
*/
void budRenderBackend::AmbientPass( const drawSurf_t* const* drawSurfs, int numDrawSurfs, bool fillGbuffer )
{
	if( fillGbuffer )
	{
	...
	*/
	...
	{
		const drawSurf_t* drawSurf = drawSurfs[i];
		const budMaterial* surfaceMaterial = drawSurf->material;
		...
		if( drawSurf->space != currentSpace )
		{
			...
			budRenderMatrix modelMatrix;
			budRenderMatrix::Transpose( *( budRenderMatrix* )drawSurf->space->modelMatrix, modelMatrix );
	...
	if( fillGbuffer )
	{
		...
		const budScreenRect& viewport = viewDef->viewport;
...
/*
=====================
budRenderBackend::StencilShadowPass

The stencil buffer should have been set to 128 on any surfaces that might receive shadows.
=====================
*/
void budRenderBackend::StencilShadowPass( const drawSurf_t* drawSurfs, const viewLight_t* vLight )
{
...
/*
==================
budRenderBackend::StencilSelectLight
...
==================
*/
void budRenderBackend::StencilSelectLight( const viewLight_t* vLight )
{
	...
	budRenderMatrix invProjectMVPMatrix;
	budRenderMatrix::Multiply( viewDef->worldSpace.mvp, vLight->inverseBaseLightProject, invProjectMVPMatrix );
...
/*
=====================
budRenderBackend::ShadowMapPass
=====================
*/
void budRenderBackend::ShadowMapPass( const drawSurf_t* drawSurfs, const viewLight_t* vLight, int side )
{
	if( r_skipShadows.GetBool() )
	{
		return;
	...
	budRenderMatrix lightProjectionRenderMatrix;
	budRenderMatrix lightViewRenderMatrix;
	...
	if( vLight->parallel && side >= 0 )
	{
		...
		budRenderMatrix::Transpose( *( budRenderMatrix* )lightViewMatrix, lightViewRenderMatrix );
#endif
		
		budBounds lightBounds;
		lightBounds.Clear();
		...
		budRenderMatrix::Transpose( *( budRenderMatrix* )lightProjectionMatrix, lightProjectionRenderMatrix );
		...
		budRenderMatrix splitFrustumInverse;
		if( !budRenderMatrix::Inverse( viewDef->frustumMVPs[FRUSTUM_CASCADE1 + side], splitFrustumInverse ) )
		{
			libBud::Warning( "splitFrustumMVP invert failed" );
		}
		
		// splitFrustumCorners in global space
		ALIGNTYPE16 frustumCorners_t splitFrustumCorners;
		budRenderMatrix::GetFrustumCorners( splitFrustumCorners, splitFrustumInverse, bounds_unitCube );
		
#if 0
		budBounds splitFrustumBounds;
		...
		budRenderMatrix lightViewProjectionRenderMatrix;
		budRenderMatrix::Multiply( lightProjectionRenderMatrix, lightViewRenderMatrix, lightViewProjectionRenderMatrix );
		
		// find the bounding box of the current split in the light's clip space
		budBounds cropBounds;
		cropBounds.Clear();
		for( int j = 0; j < 8; j++ )
		{
			point[0] = splitFrustumCorners.x[j];
		...
		//budRenderMatrix cropRenderMatrix;
		//budRenderMatrix::Transpose( *( budRenderMatrix* )cropMatrix, cropRenderMatrix );
		
		//budRenderMatrix tmp = lightProjectionRenderMatrix;
		//budRenderMatrix::Multiply( cropRenderMatrix, tmp, lightProjectionRenderMatrix );
	...
	for( const drawSurf_t* drawSurf = drawSurfs; drawSurf != NULL; drawSurf = drawSurf->nextOnLight )
	{
		...
		if( drawSurf->space != currentSpace )
		{
			...
			budRenderMatrix::Transpose( *( budRenderMatrix* )drawSurf->space->modelMatrix, modelRenderMatrix );
			
			budRenderMatrix modelToLightRenderMatrix;
			budRenderMatrix::Multiply( lightViewRenderMatrix, modelRenderMatrix, modelToLightRenderMatrix );
			
			budRenderMatrix clipMVP;
			budRenderMatrix::Multiply( lightProjectionRenderMatrix, modelToLightRenderMatrix, clipMVP );
			
			if( vLight->parallel )
			{
				budRenderMatrix MVP;
				budRenderMatrix::Multiply( renderMatrix_clipSpaceToWindowSpace, clipMVP, MVP );
			...
			else if( side < 0 )
			{
				// from OpenGL view space to OpenGL NDC ( -1 : 1 in XYZ )
				budRenderMatrix MVP;
				budRenderMatrix::Multiply( renderMatrix_windowSpaceToClipSpace, clipMVP, MVP );
...
/*
==================
budRenderBackend::DrawInteractions
==================
*/
void budRenderBackend::DrawInteractions( const viewDef_t* _viewDef )
{
...
/*
=====================
budRenderBackend::DrawShaderPasses
...
=====================
*/
int budRenderBackend::DrawShaderPasses( const drawSurf_t* const* const drawSurfs, const int numDrawSurfs,
	...
	for( ; i < numDrawSurfs; i++ )
	{
		...
		for( int stage = 0; stage < shader->GetNumStages(); stage++ )
		{
			...
			if( newStage != NULL )
			{
				...
				{
					budImage* image = newStage->fragmentProgramImages[j];
					if( image != NULL )
					{
						GL_SelectTexture( j );
						image->Bind();
...
/*
=====================
budRenderBackend::T_BlendLight
=====================
*/
void budRenderBackend::T_BlendLight( const drawSurf_t* drawSurfs, const viewLight_t* vLight )
{
	currentSpace = NULL;
	...
	for( const drawSurf_t* drawSurf = drawSurfs; drawSurf != NULL; drawSurf = drawSurf->nextOnLight )
	{
		...
		if( drawSurf->space != currentSpace )
		{
			...
			budPlane	lightProjectInCurrentSpace[4];
			for( int i = 0; i < 4; i++ )
			{
...
/*
=====================
budRenderBackend::BlendLight

Dual texture together the falloff and projection texture with a blend
mode to the framebuffer, instead of interacting with the surface texture
=====================
*/
void budRenderBackend::BlendLight( const drawSurf_t* drawSurfs, const drawSurf_t* drawSurfs2, const viewLight_t* vLight )
{
	if( drawSurfs == NULL )
	{
		return;
...
/*
=====================
budRenderBackend::T_BasicFog
=====================
*/
void budRenderBackend::T_BasicFog( const drawSurf_t* drawSurfs, const budPlane fogPlanes[4], const budRenderMatrix* inverseBaseLightProject )
{
	currentSpace = NULL;
	...
	for( const drawSurf_t* drawSurf = drawSurfs; drawSurf != NULL; drawSurf = drawSurf->nextOnLight )
	{
		...
		if( drawSurf->space != currentSpace )
		{
			...
			{
				budRenderMatrix invProjectMVPMatrix;
				budRenderMatrix::Multiply( viewDef->worldSpace.mvp, *inverseBaseLightProject, invProjectMVPMatrix );
				RB_SetMVP( invProjectMVPMatrix );
				for( int i = 0; i < 4; i++ )
				{
...
/*
==================
budRenderBackend::FogPass
==================
*/
void budRenderBackend::FogPass( const drawSurf_t* drawSurfs,  const drawSurf_t* drawSurfs2, const viewLight_t* vLight )
{
	renderLog.OpenBlock( vLight->lightShader->GetName() );
...
/*
==================
budRenderBackend::FogAllLights
==================
*/
void budRenderBackend::FogAllLights()
{
...
// RB begin
void budRenderBackend::CalculateAutomaticExposure()
{
	int				i;
	...
	{
		libBud::Printf( "HDR luminance avg = %f, max = %f, key = %f\n", hdrAverageLuminance, hdrMaxLuminance, hdrKey );
	}
...
void budRenderBackend::Tonemap( const viewDef_t* _viewDef )
{
	...
	//const budScreenRect& viewport = cmd->viewDef->viewport;
...
void budRenderBackend::Bloom( const viewDef_t* _viewDef )
{
...
void budRenderBackend::DrawScreenSpaceAmbientOcclusion( const viewDef_t* _viewDef )
{
#if !defined(USE_VULKAN)
	...
	const budScreenRect& viewport = backEnd.viewDef->viewport;
	globalImages->currentRenderImage->CopyFramebuffer( viewport.x1, viewport.y1, viewport.GetWidth(), viewport.GetHeight() );
	...
	budRenderMatrix::Inverse( backEnd.viewDef->worldSpace.mvp, inverseMVP );
	
	budRenderMatrix	motionMatrix;
	budRenderMatrix::Multiply( backEnd.prevMVP[mvpIndex], inverseMVP, motionMatrix );
	...
	budRenderMatrix modelViewMatrix;
	budRenderMatrix::Transpose( *( budRenderMatrix* )backEnd.viewDef->worldSpace.modelViewMatrix, modelViewMatrix );
	budRenderMatrix cameraToWorldMatrix;
	if( !budRenderMatrix::Inverse( modelViewMatrix, cameraToWorldMatrix ) )
	{
		libBud::Warning( "cameraToWorldMatrix invert failed" );
	}
...
void budRenderBackend::DrawScreenSpaceGlobalIllumination( const viewDef_t* _viewDef )
{
#if !defined(USE_VULKAN)
	if( !_viewDef->viewEntitys || _viewDef->is2Dgui )
	{
...
====================
*/
void budRenderBackend::ExecuteBackEndCommands( const emptyCommand_t* cmds )
{
...
/*
==================
budRenderBackend::DrawViewInternal
==================
*/
void budRenderBackend::DrawViewInternal( const viewDef_t* _viewDef, const int stereoEye )
{
	...
	for( int i = 0; i < numDrawSurfs; i++ )
	{
		...
		if( ds->material != NULL )
		{
			const_cast<budMaterial*>( ds->material )->EnsureNotPurged();
		}
	}
...
==================
*/
void budRenderBackend::MotionBlur()
{
...
// FIXME
	...
	// derive the matrix to go from current pixels to previous frame pixels
	budRenderMatrix	inverseMVP;
	budRenderMatrix::Inverse( viewDef->worldSpace.mvp, inverseMVP );
	
	budRenderMatrix	motionMatrix;
	budRenderMatrix::Multiply( prevMVP[mvpIndex], inverseMVP, motionMatrix );
...
==================
*/
void budRenderBackend::DrawView( const void* data, const int stereoEye )
{
	...
	{
		const budScreenRect& r = viewDef->viewport;
...
/*
==================
budRenderBackend::CopyRender

Copy part of the current framebuffer to an image
==================
*/
void budRenderBackend::CopyRender( const void* data )
{
