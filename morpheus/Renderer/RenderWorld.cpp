...
void R_ListRenderLightDefs_f( const CmdArgs& args )
{
	...
	for( i = 0; i < tr.primaryWorld->lightDefs.Num(); i++ )
	{
		...
		int	iCount = 0;
		for( budInteraction* inter = ldef->firstInteraction; inter != NULL; inter = inter->lightNext )
		{
			iCount++;
		}
		totalIntr += iCount;
...
void R_ListRenderEntityDefs_f( const CmdArgs& args )
{
	...
	for( i = 0; i < tr.primaryWorld->entityDefs.Num(); i++ )
	{
		...
		// count up the interactions
		int	iCount = 0;
		for( budInteraction* inter = mdef->firstInteraction; inter != NULL; inter = inter->entityNext )
		{
			iCount++;
		}
		totalIntr += iCount;
...
/*
===================
budRenderWorldLocal::budRenderWorldLocal
===================
*/
budRenderWorldLocal::budRenderWorldLocal()
{
	...
	for( int i = 0; i < decals.Num(); i++ )
	{
		...
		decals[i].decals = new( TAG_MODEL ) budRenderModelDecal();
	...
	for( int i = 0; i < overlays.Num(); i++ )
	{
		...
		overlays[i].overlays = new( TAG_MODEL ) budRenderModelOverlay();
...
===================
*/
void budRenderWorldLocal::ResizeInteractionTable()
{
	// we overflowed the interaction table, so make it larger
	common->Printf( "budRenderWorldLocal::ResizeInteractionTable: overflowed interactionTable, resizing\n" );
	
	const int oldInteractionTableWidth = interactionTableWidth;
	const int oldIinteractionTableHeight = interactionTableHeight;
	budInteraction** oldInteractionTable = interactionTable;
	...
	interactionTable = ( budInteraction** )R_ClearedStaticAlloc( size );
...
===================
*/
qhandle_t budRenderWorldLocal::AddEntityDef( const renderEntity_t* re )
{
...
void budRenderWorldLocal::UpdateEntityDef( qhandle_t entityHandle, const renderEntity_t* re )
{
	...
	{
		common->Error( "budRenderWorld::UpdateEntityDef: NULL hModel" );
	}
	
	// create new slots if needed
	if( entityHandle < 0 || entityHandle > LUDICROUS_INDEX )
	{
		common->Error( "budRenderWorld::UpdateEntityDef: index = %i", entityHandle );
	}
	...
	budRenderEntityLocal*	def = entityDefs[entityHandle];
	if( def != NULL )
	{
	...
	{
		// creating a new one
		def = new( TAG_RENDER_ENTITY ) budRenderEntityLocal;
...
===================
*/
void budRenderWorldLocal::FreeEntityDef( qhandle_t entityHandle )
{
	budRenderEntityLocal*	def;
	
	if( entityHandle < 0 || entityHandle >= entityDefs.Num() )
	{
		common->Printf( "budRenderWorld::FreeEntityDef: handle %i > %i\n", entityHandle, entityDefs.Num() );
		return;
	}
	...
	if( !def )
	{
		common->Printf( "budRenderWorld::FreeEntityDef: handle %i is NULL\n", entityHandle );
		return;
	}
...
==================
*/
const renderEntity_t* budRenderWorldLocal::GetRenderEntity( qhandle_t entityHandle ) const
{
	budRenderEntityLocal*	def;
	
	if( entityHandle < 0 || entityHandle >= entityDefs.Num() )
	{
		common->Printf( "budRenderWorld::GetRenderEntity: invalid handle %i [0, %i]\n", entityHandle, entityDefs.Num() );
		return NULL;
	}
	...
	if( !def )
	{
		common->Printf( "budRenderWorld::GetRenderEntity: handle %i is NULL\n", entityHandle );
		return NULL;
	}
...
==================
*/
qhandle_t budRenderWorldLocal::AddLightDef( const renderLight_t* rlight )
{
	// try and reuse a free spot
...
=================
*/
void budRenderWorldLocal::UpdateLightDef( qhandle_t lightHandle, const renderLight_t* rlight )
{
	if( r_skipUpdates.GetBool() )
	{
		return;
	...
	{
		common->Error( "budRenderWorld::UpdateLightDef: index = %i", lightHandle );
	}
	...
	budRenderLightLocal* light = lightDefs[lightHandle];
	if( light )
	{
	...
	{
		// create a new one
		light = new( TAG_RENDER_LIGHT ) budRenderLightLocal;
...
====================
*/
void budRenderWorldLocal::FreeLightDef( qhandle_t lightHandle )
{
	budRenderLightLocal*	light;
	
	if( lightHandle < 0 || lightHandle >= lightDefs.Num() )
	{
		common->Printf( "budRenderWorld::FreeLightDef: invalid handle %i [0, %i]\n", lightHandle, lightDefs.Num() );
		return;
	}
	...
	if( !light )
	{
		common->Printf( "budRenderWorld::FreeLightDef: handle %i is NULL\n", lightHandle );
		return;
	}
...
==================
*/
const renderLight_t* budRenderWorldLocal::GetRenderLight( qhandle_t lightHandle ) const
{
	budRenderLightLocal* def;
	
	if( lightHandle < 0 || lightHandle >= lightDefs.Num() )
	{
		common->Printf( "budRenderWorld::GetRenderLight: handle %i > %i\n", lightHandle, lightDefs.Num() );
		return NULL;
	}
	...
	if( !def )
	{
		common->Printf( "budRenderWorld::GetRenderLight: handle %i is NULL\n", lightHandle );
		return NULL;
	}
...
void budRenderWorldLocal::ProjectDecalOntoWorld( const budFixedWinding& winding, const Vector3& projectionOrigin, const bool parallel, const float fadeDepth, const budMaterial* material, const int startTime )
{
	...
	if( !budRenderModelDecal::CreateProjectionParms( globalParms, winding, projectionOrigin, parallel, fadeDepth, material, startTime ) )
	{
		return;
	}
	...
	for( int i = 0; i < numAreas; i++ )
	{
		...
		{
			budRenderEntityLocal* def = ref->entity;
			...
			const budRenderModel* model = def->parms.hModel;
			...
			budRenderModelDecal::GlobalProjectionParmsToLocal( localParms, globalParms, def->parms.origin, def->parms.axis );
...
void budRenderWorldLocal::ProjectDecal( qhandle_t entityHandle, const budFixedWinding& winding, const Vector3& projectionOrigin, const bool parallel, const float fadeDepth, const budMaterial* material, const int startTime )
{
	if( entityHandle < 0 || entityHandle >= entityDefs.Num() )
	{
		common->Error( "budRenderWorld::ProjectOverlay: index = %i", entityHandle );
		return;
	}
...
/*
====================
budRenderWorldLocal::ProjectOverlay
====================
*/
void budRenderWorldLocal::ProjectOverlay( qhandle_t entityHandle, const budPlane localTextureAxis[2], const budMaterial* material, const int startTime )
{
	if( entityHandle < 0 || entityHandle >= entityDefs.Num() )
	{
		common->Error( "budRenderWorld::ProjectOverlay: index = %i", entityHandle );
...
/*
====================
budRenderWorldLocal::AllocDecal
====================
*/
budRenderModelDecal* budRenderWorldLocal::AllocDecal( qhandle_t newEntityHandle, int startTime )
{
	int oldest = 0;
	...
	{
		budRenderEntityLocal*	def = entityDefs[decals[oldest].entityHandle];
...
/*
====================
budRenderWorldLocal::AllocOverlay
====================
*/
budRenderModelOverlay* budRenderWorldLocal::AllocOverlay( qhandle_t newEntityHandle, int startTime )
{
	int oldest = 0;
	int oldestTime = MAX_TYPE( oldestTime );
	for( int i = 0; i < overlays.Num(); i++ )
	...
	{
		budRenderEntityLocal*	def = entityDefs[overlays[oldest].entityHandle];
...
/*
====================
budRenderWorldLocal::RemoveDecals
====================
*/
void budRenderWorldLocal::RemoveDecals( qhandle_t entityHandle )
{
	if( entityHandle < 0 || entityHandle >= entityDefs.Num() )
	{
		common->Error( "budRenderWorld::ProjectOverlay: index = %i", entityHandle );
...
/*
====================
budRenderWorldLocal::SetRenderView

Sets the current view so any calls to the render world will use the correct parms.
====================
*/
void budRenderWorldLocal::SetRenderView( const renderView_t* renderView )
{
...
/*
====================
budRenderWorldLocal::RenderScene
...
====================
*/
void budRenderWorldLocal::RenderScene( const renderView_t* renderView )
{
	if( !tr.IsInitialized() )
	{
		return;
	...
	{
		common->Error( "budRenderWorld::RenderScene: bad FOVs: %f, %f", renderView->fov_x, renderView->fov_y );
	}
	...
	{
		budRenderEntityLocal*	def = entityDefs[i];
		if( !def )
		{
			continue;
		}
...
/*
===================
budRenderWorldLocal::NumAreas
===================
*/
int budRenderWorldLocal::NumAreas() const
{
...
/*
===================
budRenderWorldLocal::NumPortalsInArea
===================
*/
int budRenderWorldLocal::NumPortalsInArea( int areaNum )
{
	portalArea_t*	area;
	int				count;
	portal_t*		portal;
	...
	{
		common->Error( "budRenderWorld::NumPortalsInArea: bad areanum %i", areaNum );
	}
...
/*
===================
budRenderWorldLocal::GetPortal
===================
*/
exitPortal_t budRenderWorldLocal::GetPortal( int areaNum, int portalNum )
{
	portalArea_t*	area;
	int				count;
	portal_t*		portal;
	...
	{
		common->Error( "budRenderWorld::GetPortal: areaNum > numAreas" );
	}
	area = &portalAreas[areaNum];
	...
	common->Error( "budRenderWorld::GetPortal: portalNum > numPortals" );
...
/*
===============
budRenderWorldLocal::PointInAreaNum
...
int budRenderWorldLocal::PointInArea( const Vector3& point ) const
{
	...
	while( 1 )
	{
		...
		if( nodeNum < 0 )
		{
			...
			{
				common->Error( "budRenderWorld::PointInArea: area out of range" );
			}
			return nodeNum;
		}
...
/*
===================
budRenderWorldLocal::BoundsInAreas_r
===================
*/
void budRenderWorldLocal::BoundsInAreas_r( int nodeNum, const budBounds& bounds, int* areas, int* numAreas, int maxAreas ) const
{
	int side, i;
	areaNode_t* node;
...
/*
===================
budRenderWorldLocal::BoundsInAreas

  fills the *areas array with the number of the areas the bounds are in
  returns the total number of areas the bounds are in
===================
*/
int budRenderWorldLocal::BoundsInAreas( const budBounds& bounds, int* areas, int maxAreas ) const
{
	int numAreas = 0;
...
guiPoint_t budRenderWorldLocal::GuiTrace( qhandle_t entityHandle, const Vector3 start, const Vector3 end ) const
{
	...
	if( ( entityHandle < 0 ) || ( entityHandle >= entityDefs.Num() ) )
	{
		common->Printf( "budRenderWorld::GuiTrace: invalid handle %i\n", entityHandle );
		return pt;
	}
	...
	if( def == NULL )
	{
		common->Printf( "budRenderWorld::GuiTrace: handle %i is NULL\n", entityHandle );
		return pt;
	}
	
	budRenderModel* model = def->parms.hModel;
	if( model == NULL || model->IsDynamicModel() != DM_STATIC || def->parms.callback != NULL )
	{
		return pt;
	}
	...
	for( int i = 0; i < model->NumSurfaces(); i++ )
	{
		...
		const budMaterial* shader = R_RemapShaderBySkin( surf->shader, def->parms.customSkin, def->parms.customShader );
		if( shader == NULL )
		{
			continue;
		}
...
bool budRenderWorldLocal::ModelTrace( modelTrace_t& trace, qhandle_t entityHandle, const Vector3& start, const Vector3& end, const float radius ) const
{
	...
	budRenderModel* model = R_EntityDefDynamicModel( def );
	if( model == NULL )
	{
		return false;
	}
...
/*
=================
budRenderWorldLocal::AddEntityRefToArea

This is called by R_PushVolumeIntoTree and also directly
for the world model references that are precalculated.
=================
*/
void budRenderWorldLocal::AddEntityRefToArea( budRenderEntityLocal* def, portalArea_t* area )
{
	areaReference_t*	ref;
	
	if( def == NULL )
	{
		common->Error( "budRenderWorldLocal::AddEntityRefToArea: NULL def" );
		return;
	}
...
/*
===================
budRenderWorldLocal::AddLightRefToArea
===================
*/
void budRenderWorldLocal::AddLightRefToArea( budRenderLightLocal* light, portalArea_t* area )
{
...
/*
===================
budRenderWorldLocal::GenerateAllInteractions

Force the generation of all light / surface interactions at the start of a level
If this isn't called, they will all be dynamically generated
===================
*/
void budRenderWorldLocal::GenerateAllInteractions()
{
	if( !tr.IsInitialized() )
	{
		return;
	...
	{
		budRenderLightLocal*	ldef = this->lightDefs[i];
		if( ldef == NULL )
		{
			continue;
		}
		...
		for( areaReference_t* lref = ldef->references; lref; lref = lref->ownerNext )
		{
			...
			for( areaReference_t* eref = area->entityRefs.areaNext; eref != &area->entityRefs; eref = eref->areaNext )
			{
				...
				inter = budInteraction::AllocAndLink( edef, ldef );
				count++;
	...
	common->Printf( "budRenderWorld::GenerateAllInteractions, msec = %i\n", msec );
	common->Printf( "interactionTable size: %i bytes\n", size );
	common->Printf( "%i interactions take %i bytes\n", count, count * sizeof( budInteraction ) );
...
/*
===================
budRenderWorldLocal::FreeInteractions
===================
*/
void budRenderWorldLocal::FreeInteractions()
{
	int			i;
	budRenderEntityLocal*	def;
...
/*
==================
budRenderWorldLocal::PushFrustumIntoTree_r
...
==================
*/
void budRenderWorldLocal::PushFrustumIntoTree_r( budRenderEntityLocal* def, budRenderLightLocal* light,
		const frustumCorners_t& corners, int nodeNum )
{
	...
	frustumCull_t cull = budRenderMatrix::CullFrustumCornersToPlane( corners, node->plane );
...
/*
==============
budRenderWorldLocal::PushFrustumIntoTree
==============
*/
void budRenderWorldLocal::PushFrustumIntoTree( budRenderEntityLocal* def, budRenderLightLocal* light, const budRenderMatrix& frustumTransform, const budBounds& frustumBounds )
{
	if( areaNodes == NULL )
	{
		return;
	...
	budRenderMatrix::GetFrustumCorners( corners, frustumTransform, frustumBounds );
...
/*
====================
budRenderWorldLocal::DebugClearLines
====================
*/
void budRenderWorldLocal::DebugClearLines( int time )
{
...
/*
====================
budRenderWorldLocal::DebugClearPolygons
====================
*/
void budRenderWorldLocal::DebugClearPolygons( int time )
{
...
/*
================
budRenderWorldLocal::DrawTextLength

  returns the length of the given text
================
*/
float budRenderWorldLocal::DrawTextLength( const char* text, float scale, int len )
{
...
/*
===============
budRenderWorldLocal::RegenerateWorld
===============
*/
void budRenderWorldLocal::RegenerateWorld()
{
	R_FreeDerivedData();
...
===============
*/
const budMaterial* R_RemapShaderBySkin( const budMaterial* shader, const budDeclSkin* skin, const budMaterial* customShader )
{
	...
	if( customShader )
	{
		...
		return const_cast<budMaterial*>( customShader );
	}
	
	if( !skin )
	{
		return const_cast<budMaterial*>( shader );
	}
