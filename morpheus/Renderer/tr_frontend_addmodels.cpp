...
viewEntity_t* R_SortViewEntities( viewEntity_t* vEntities )
{
	...
	for( viewEntity_t* vEntity = vEntities; vEntity != NULL; )
	{
		...
		const budRenderModel* model = vEntity->entityDef->parms.hModel;
...
void R_ClearEntityDefDynamicModel( budRenderEntityLocal* def )
{
	...
	for( budInteraction* inter = def->firstInteraction; inter != NULL && !inter->IsEmpty(); inter = inter->entityNext )
	{
		inter->FreeSurfaces();
	}
...
{
	budBounds oldBounds = def->localReferenceBounds;
...
This is also called by the game code for budRenderWorldLocal::ModelTrace(), and budRenderWorldLocal::Trace() which is bad for performance...
...
budRenderModel* R_EntityDefDynamicModel( budRenderEntityLocal* def )
{
	...
	if( def->dynamicModel == NULL )
	{
		...
		{
			budBounds b = def->cachedDynamicModel->Bounds();
...
void R_SetupDrawSurfJoints( drawSurf_t* drawSurf, const srfTriangles_t* tri, const budMaterial* shader )
{
	...
	budRenderModelStatic* model = tri->staticModelWithJoints;
...
void R_AddSingleModel( viewEntity_t* vEntity )
{
	...
	budRenderEntityLocal* entityDef = vEntity->entityDef;
	const renderEntity_t* renderEntity = &entityDef->parms;
	const budRenderWorldLocal* world = entityDef->world;
	...
	budInteraction* staticInteractions[MAX_CONTACTED_LIGHTS];
			...
			renderEntity->hModel->ModelHasShadowCastingSurfaces() )
	{
		...
		for( viewLight_t* vLight = viewDef->viewLights; vLight != NULL; vLight = vLight->next )
		{
			...
			const budRenderLightLocal* lightDef = vLight->lightDef;
			...
			if( !modelIsVisible )
			{
				...
				if( budRenderMatrix::CullBoundsToMVP( viewDef->worldSpace.mvp, shadowBounds ) )
				{
					continue;
				}
			}
	...
	//---------------------------
	budRenderModel* model = R_EntityDefDynamicModel( entityDef );
	...
	budRenderMatrix viewMat;
	budRenderMatrix::Transpose( *( budRenderMatrix* )vEntity->modelViewMatrix, viewMat );
	budRenderMatrix::Multiply( viewDef->projectionRenderMatrix, viewMat, vEntity->mvp );
	if( renderEntity->weaponDepthHack )
	{
		budRenderMatrix::ApplyDepthHack( vEntity->mvp );
	}
	if( renderEntity->modelDepthHack != 0.0f )
	{
		budRenderMatrix::ApplyModelDepthHack( vEntity->mvp, renderEntity->modelDepthHack );
	}
	...
	for( int surfaceNum = 0; surfaceNum < model->NumSurfaces(); surfaceNum++ )
	{
		...
		const budMaterial* shader = surf->shader;
		if( shader == NULL )
		{
			continue;
		}
		...
		if( shader->IsLOD() )
		{
			...
			budBounds& localBounds = tri->bounds;
		...
		// individual surfaces.
		const bool surfaceDirectlyVisible = modelIsVisible && !budRenderMatrix::CullBoundsToMVP( vEntity->mvp, tri->bounds );
		...
		for( int contactedLight = 0; contactedLight < numContactedLights; contactedLight++ )
		{
			...
			const budRenderLightLocal* lightDef = vLight->lightDef;
			const budInteraction* interaction = staticInteractions[contactedLight];
						...
						else
						{
							...
							if( r_cullDynamicLightTriangles.GetBool() && !r_skipDynamicShadows.GetBool() && !r_useShadowMapping.GetBool() && shader->SurfaceCastsShadow() )
							{
								...
								if( vertexCache.CacheIsCurrent( lightIndexCache ) )
								{
									...
									budRenderMatrix::Multiply( vLight->lightDef->baseLightProject, entityDef->modelRenderMatrix, dynamicShadowParms->localLightProject );
