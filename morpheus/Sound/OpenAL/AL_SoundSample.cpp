...
========================
*/
void idSoundSample_OpenAL::WriteGeneratedSample( budFile* fileOut )
{
...
void idSoundSample_OpenAL::WriteAllSamples( const budStr& sampleName )
{
	idSoundSample_OpenAL* samplePC = new idSoundSample_OpenAL();
	{
		...
		{
			budFile* fileOut = fileSystem->OpenFileWrite( outName, "fs_basepath" );
...
bool idSoundSample_OpenAL::LoadWav( const budStr& filename )
{
	...
	if( formatError != NULL )
	{
		libBud::Warning( "LoadWav( %s ) : %s", filename.c_str(), formatError );
		MakeDefault();
		return false;
	}
	...
	if( format.basic.formatTag == idWaveFile::FORMAT_PCM || format.basic.formatTag == idWaveFile::FORMAT_EXTENSIBLE )
	{
		...
		if( format.basic.bitsPerSample != 16 )
		{
			libBud::Warning( "LoadWav( %s ) : %s", filename.c_str(), "Not a 16 bit PCM wav file" );
			MakeDefault();
			return false;
		}
	...
	else if( format.basic.formatTag == idWaveFile::FORMAT_XMA2 )
	{
		...
		{
			libBud::Warning( "LoadWav( %s ) : %s", filename.c_str(), "No data blocks in file" );
			MakeDefault();
			return false;
		}
		...
		{
			libBud::Warning( "LoadWav( %s ) : %s", filename.c_str(), "Wrong number of entries in seek table" );
			MakeDefault();
			return false;
		}
	...
	{
		libBud::Warning( "LoadWav( %s ) : Unsupported wave format %d", filename.c_str(), format.basic.formatTag );
		MakeDefault();
		return false;
	}
