...
void idSoundHardware_OpenAL::PrintALCInfo( ALCdevice* device )
{
	...
	if( device )
	{
		...
		libBud::Printf( "** Info for device \"%s\" **\n", devname );
	}
	alcGetIntegerv( device, ALC_MAJOR_VERSION, 1, &major );
	alcGetIntegerv( device, ALC_MINOR_VERSION, 1, &minor );
	
	if( CheckALCErrors( device ) == ALC_NO_ERROR )
		libBud::Printf( "ALC version: %d.%d\n", major, minor );
		
	if( device )
	{
		libBud::Printf( "OpenAL extensions: %s", alGetString( AL_EXTENSIONS ) );
		
		//libBud::Printf("ALC extensions:");
...
{
	libBud::Printf( "OpenAL vendor string: %s\n", alGetString( AL_VENDOR ) );
	libBud::Printf( "OpenAL renderer string: %s\n", alGetString( AL_RENDERER ) );
	libBud::Printf( "OpenAL version string: %s\n", alGetString( AL_VERSION ) );
	libBud::Printf( "OpenAL extensions: %s", alGetString( AL_EXTENSIONS ) );
...
void listDevices_f( const CmdArgs& args )
{
	...
	//libBud::Printf("Default capture device: %s\n", alcGetString(NULL, ALC_CAPTURE_DEFAULT_DEVICE_SPECIFIER));
...
idSoundVoice* idSoundHardware_OpenAL::AllocateVoice( const idSoundSample* leadinSample, const idSoundSample* loopingSample )
{
	...
	if( loopingSample != NULL )
	{
		...
		{
			libBud::Warning( "Leadin/looping format mismatch: %s & %s", leadinSample->GetName(), loopingSample->GetName() );
			loopingSample = NULL;
		}
	}
