...
void TestSound_f( const CmdArgs& args )
{
	if( args.Argc() != 2 )
	{
		libBud::Printf( "Usage: testSound <file>\n" );
		return;
	}
...
void idSoundSystemLocal::Restart()
{
	...
	{
		budSoundWorldLocal* sw = soundWorlds[i];
...
void idSoundSystemLocal::Init()
{
	...
	libBud::Printf( "----- Initializing Sound System ------\n" );
	...
	libBud::Printf( "sound system initialized.\n" );
	libBud::Printf( "--------------------------------------\n" );
}
...
========================
*/
budSoundWorld* idSoundSystemLocal::AllocSoundWorld( budRenderWorld* rw )
{
	budSoundWorldLocal* local = new( TAG_AUDIO ) budSoundWorldLocal;
...
========================
*/
void idSoundSystemLocal::FreeSoundWorld( budSoundWorld* sw )
{
	budSoundWorldLocal* local = static_cast<budSoundWorldLocal*>( sw );
...
========================
*/
void idSoundSystemLocal::SetPlayingSoundWorld( budSoundWorld* soundWorld )
{
	if( currentSoundWorld == soundWorld )
	{
		return;
	}
	budSoundWorldLocal* oldSoundWorld = currentSoundWorld;
	
	currentSoundWorld = static_cast<budSoundWorldLocal*>( soundWorld );
...
========================
*/
budSoundWorld* idSoundSystemLocal::GetPlayingSoundWorld()
{
...
========================
*/
void idSoundSystemLocal::OnReloadSound( const budDecl* sound )
{
	for( int i = 0; i < soundWorlds.Num(); i++ )
	{
...
void idSoundSystemLocal::StopAllSounds()
{
	for( int i = 0; i < soundWorlds.Num(); i++ )
	{
		budSoundWorld* sw = soundWorlds[i];
		if( sw )
		{
			sw->StopAllSounds();
		}
...
void idSoundSystemLocal::StopVoicesWithSample( const idSoundSample* const sample )
{
	...
	{
		budSoundWorldLocal* sw = soundWorlds[w];
		if( sw == NULL )
		{
			continue;
		}
