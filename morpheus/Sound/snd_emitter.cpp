...
{
	budSoundWorldLocal* soundWorld = emitter->soundWorld;
...
========================
*/
void idSoundEmitterLocal::Init( int i, budSoundWorldLocal* sw )
{
	index = i;
	soundWorld = sw;
...
int idSoundEmitterLocal::StartSound( const idSoundShader* shader, const s_channelType channel, float diversity, int shaderFlags, bool allowSlow )
{
	...
	if( showStartSound )
	{
		libBud::Printf( "%dms: StartSound(%d:%d): %s: ", currentTime, index, channel, shader->GetName() );
	}
	...
	if( shader->entries.Num() == 0 )
	{
		if( showStartSound )
		{
			libBud::Printf( S_COLOR_RED "No Entries\n" );
		}
		return 0;
	}
	...
	if( chanParms.soundShaderFlags & SSF_PLAY_ONCE )
	{
		for( int i = 0; i < channels.Num(); i++ )
		{
			...
			if( chan->soundShader == shader && !chan->CheckForCompletion( currentTime ) )
			{
				if( showStartSound )
				{
					libBud::Printf( S_COLOR_YELLOW "Not started because of playOnce\n" );
				}
				return 0;
			}
		}
	...
	for( int i = 0; i < channels.Num(); i++ )
	{
		...
		if( chan->soundShader == shader && chan->startTime == currentTime && chan->endTime != 1 )
		{
			if( showStartSound )
			{
				libBud::Printf( S_COLOR_RED "Already started this frame\n" );
			}
			return 0;
		}
	}
	...
	if( channel != SCHANNEL_ANY )
	{
		for( int i = 0; i < channels.Num(); i++ )
		{
			...
			if( chan->soundShader && chan->logicalChannel == channel )
			{
				if( showStartSound )
				{
					libBud::Printf( S_COLOR_YELLOW "OVERRIDE %s: ", chan->soundShader->GetName() );
				}
				channels.RemoveIndex( i );
				soundWorld->FreeSoundChannel( chan );
				break;
	...
	if( channels.Num() == channels.Max() )
	{
		...
		if( channels.Num() == channels.Max() )
		{
			if( showStartSound )
			{
				libBud::Printf( S_COLOR_RED "No free emitter channels!\n" );
			}
			return 0;
		}
	}
	...
	if( chan == NULL )
	{
		if( showStartSound )
		{
			libBud::Printf( S_COLOR_RED "No free global channels!\n" );
		}
		return 0;
	}
...
========================
*/
void idSoundEmitterLocal::OnReloadSound( const budDecl* decl )
{
}
...
void idSoundEmitterLocal::ModifySound( const s_channelType channel, const soundShaderParms_t* parms )
{
	...
	for( int i = channels.Num() - 1; i >= 0; i-- )
	{
		...
		if( s_showStartSound.GetBool() )
		{
			libBud::Printf( "%dms: ModifySound(%d:%d): %s\n", soundWorld->GetSoundTime(), index, channel, chan->soundShader->GetName() );
		}
