...
bool idWaveFile::Open( const char* filename )
{
	...
	{
		Close();
		libBud::Warning( "Header is not RIFF WAVE in %s", filename );
		return false;
	}
	...
	while( offset < riffSize )
	{
		...
		{
			Close();
			libBud::Warning( "More than %d chunks in %s", chunks.Max(), filename );
			return false;
		}
...
========================
*/
bool idWaveFile::ReadWaveFormatDirect( waveFmt_t& format, budFile* file )
{
...
========================
*/
bool idWaveFile::WriteWaveFormatDirect( waveFmt_t& format, budFile* file )
{
	//idSwapClass<waveFmt_t::basic_t> swap;
	//swap.Little( format.basic.formatTag );
	//swap.Little( format.basic.numChannels );
...
bool idWaveFile::WriteDataDirect( char* _data, uint32 size, budFile* file )
{
...
bool idWaveFile::WriteHeaderDirect( uint32 fileSize, budFile* file )
{
...
bool idWaveFile::ReadLoopData( int& start, int& end )
{
	...
	if( smplData.type != 0 )
	{
		libBud::Warning( "Invalid loop type in %s", file->GetName() );
		return false;
	}
