...
/*
================
budCollisionModelManagerLocal::TestTrmVertsInBrush

  returns true if any of the trm vertices is inside the brush
================
*/
bool budCollisionModelManagerLocal::TestTrmVertsInBrush( cm_traceWork_t* tw, cm_brush_t* b )
{
	...
	b->checkcount = budCollisionModelManagerLocal::checkCount;
...
/*
================
budCollisionModelManagerLocal::TestTrmInPolygon

  returns true if the trm intersects the polygon
================
*/
bool budCollisionModelManagerLocal::TestTrmInPolygon( cm_traceWork_t* tw, cm_polygon_t* p )
{
...
int budCollisionModelManagerLocal::PointContents( const budVec3 p, cmHandle_t model )
{
	...
	node = budCollisionModelManagerLocal::PointNode( p, budCollisionModelManagerLocal::models[model] );
...
int	budCollisionModelManagerLocal::TransformedPointContents( const budVec3& p, cmHandle_t model, const budVec3& origin, const budMat3& modelAxis )
{
	...
	return budCollisionModelManagerLocal::PointContents( p_l, model );
}
		...
		cmHandle_t model, const budVec3& modelOrigin, const budMat3& modelAxis )
{
				  ...
				  trm->bounds[1][2] - trm->bounds[0][2] <= 0.0f ) )
	{
		...
		results->c.contents = budCollisionModelManagerLocal::TransformedPointContents( start, model, modelOrigin, modelAxis );
		...
		cmHandle_t model, const budVec3& modelOrigin, const budMat3& modelAxis )
{
	...
	if( model < 0 || model > budCollisionModelManagerLocal::maxModels || model > MAX_SUBMODELS )
	{
		common->Printf( "budCollisionModelManagerLocal::Contents: invalid model handle\n" );
		return 0;
	}
	if( !budCollisionModelManagerLocal::models || !budCollisionModelManagerLocal::models[model] )
	{
		common->Printf( "budCollisionModelManagerLocal::Contents: invalid model\n" );
		return 0;
	}
