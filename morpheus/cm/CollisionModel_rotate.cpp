...
/*
================
budCollisionModelManagerLocal::CollisionBetweenEdgeBounds
...
/*
================
budCollisionModelManagerLocal::RotateEdgeThroughEdge
...
/*
================
budCollisionModelManagerLocal::EdgeFurthestFromEdge
...
/*
================
budCollisionModelManagerLocal::RotateTrmEdgeThroughPolygon
================
*/
void budCollisionModelManagerLocal::RotateTrmEdgeThroughPolygon( cm_traceWork_t* tw, cm_polygon_t* poly, cm_trmEdge_t* trmEdge )
{
	int i, j, edgeNum;
...
/*
================
budCollisionModelManagerLocal::RotatePointThroughPlane
...
/*
================
budCollisionModelManagerLocal::PointFurthestFromPlane
...
/*
================
budCollisionModelManagerLocal::RotateTrmVertexThroughPolygon
================
*/
void budCollisionModelManagerLocal::RotateTrmVertexThroughPolygon( cm_traceWork_t* tw, cm_polygon_t* poly, cm_trmVertex_t* v, int vertexNum )
{
	int i;
	float tanHalfAngle;
...
/*
================
budCollisionModelManagerLocal::RotateTrmThroughPolygon

  returns true if the polygon blocks the complete rotation
================
*/
bool budCollisionModelManagerLocal::RotateTrmThroughPolygon( cm_traceWork_t* tw, cm_polygon_t* p )
{
	int i, j, k, edgeNum;
	float d;
	cm_trmVertex_t* bv;
...
/*
================
budCollisionModelManagerLocal::BoundsForRotation
		...
		cmHandle_t model, const Vector3& modelOrigin, const Matrix3& modelAxis )
{
	...
	{
		common->Printf( "budCollisionModelManagerLocal::Rotation180: invalid model handle\n" );
		return;
	}
	if( !budCollisionModelManagerLocal::models[model] )
	{
		common->Printf( "budCollisionModelManagerLocal::Rotation180: invalid model\n" );
		return;
	}
...
/*
================
budCollisionModelManagerLocal::Rotation
================
*/
#ifdef _DEBUG
		...
		cmHandle_t model, const Vector3& modelOrigin, const Matrix3& modelAxis )
{
	...
	if( rotation.GetAngle() >= 180.0f || rotation.GetAngle() <= -180.0f )
	{
		...
		{
			// partial rotation
			budCollisionModelManagerLocal::Rotation180( results, rotation.GetOrigin(), rotation.GetVec(), lasta, a, start, trm, trmAxis, contentMask, model, modelOrigin, modelAxis );
	...
	budCollisionModelManagerLocal::Rotation180( results, rotation.GetOrigin(), rotation.GetVec(), 0.0f, rotation.GetAngle(), start, trm, trmAxis, contentMask, model, modelOrigin, modelAxis );
	...
	if( cm_debugCollision.GetBool() )
	{
		if( !entered )
		{
			...
			if( budCollisionModelManagerLocal::Contents( results->endpos, trm, results->endAxis, -1, model, modelOrigin, modelAxis ) & contentMask )
			{
				trace_t tr;
				
				// test where the trm is stuck in the model
				budCollisionModelManagerLocal::Contents( results->endpos, trm, results->endAxis, -1, model, modelOrigin, modelAxis );
				// re-run collision detection to find out where it failed
				budCollisionModelManagerLocal::Rotation( &tr, start, rotation, trm, trmAxis, contentMask, model, modelOrigin, modelAxis );
			}
			entered = 0;
		}
	}
