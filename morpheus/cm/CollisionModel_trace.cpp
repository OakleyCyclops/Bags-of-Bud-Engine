...
/*
================
budCollisionModelManagerLocal::TraceTrmThroughNode
================
*/
void budCollisionModelManagerLocal::TraceTrmThroughNode( cm_traceWork_t* tw, cm_node_t* node )
{
	...
	if( tw->positionTest )
	{
		...
		{
			if( budCollisionModelManagerLocal::TestTrmVertsInBrush( tw, bref->b ) )
			{
				return;
			}
		}
		...
		{
			if( budCollisionModelManagerLocal::TestTrmInPolygon( tw, pref->p ) )
			{
				return;
			}
		}
	...
	else if( tw->rotation )
	{
		...
		for( pref = node->polygons; pref; pref = pref->next )
		{
			if( budCollisionModelManagerLocal::RotateTrmThroughPolygon( tw, pref->p ) )
			{
				return;
			}
		}
	...
	else
	{
		...
		for( pref = node->polygons; pref; pref = pref->next )
		{
			if( budCollisionModelManagerLocal::TranslateTrmThroughPolygon( tw, pref->p ) )
			{
				return;
			}
		}
...
/*
================
budCollisionModelManagerLocal::TraceThroughAxialBSPTree_r
================
*/
...
void budCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( cm_traceWork_t* tw, cm_node_t* node, float p1f, float p2f, budVec3& p1, budVec3& p2 )
{
	...
	if( node->polygons || ( tw->positionTest && node->brushes ) )
	{
		...
		budCollisionModelManagerLocal::TraceTrmThroughNode( tw, node );
	}
	// if already stuck in solid
	...
	budCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, node->children[0], p1f, p2f, p1, p2 );
	budCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, node->children[1], p1f, p2f, p1, p2 );
	return;
#endif
	...
	budCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, node->children[side], p1f, midf, p1, mid );
	...
	budCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, node->children[side ^ 1], midf, p2f, mid, p2 );
}

/*
================
budCollisionModelManagerLocal::TraceThroughModel
================
*/
void budCollisionModelManagerLocal::TraceThroughModel( cm_traceWork_t* tw )
{
	float d;
	int i, numSteps;
	budVec3 start, end;
	...
	if( !tw->rotation )
	{
		...
		budCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, tw->model->node, 0, 1, tw->start, tw->end );
	}
	else
	{
		...
		if( d > CIRCLE_APPROXIMATION_LENGTH )
		{
			...
			for( i = 0; i < numSteps; i++ )
			{
				...
				// trace through spatial subdivision and then through leafs
				budCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, tw->model->node, 0, 1, start, end );
		...
		budCollisionModelManagerLocal::TraceThroughAxialBSPTree_r( tw, tw->model->node, 0, 1, start, tw->end );
	}
}
