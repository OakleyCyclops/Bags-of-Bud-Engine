...
class budCollisionModelManager
{
public:
	virtual					~budCollisionModelManager() {}
	
	// Loads collision models from a map file.
	virtual void			LoadMap( const budMapFile* mapFile ) = 0;
	...
	virtual cmHandle_t		SetupTrmModel( const budTraceModel& trm, const budMaterial* material ) = 0;
	// Creates a trace model from a collision model, returns true if succesfull.
	virtual bool			TrmFromModel( const char* modelName, budTraceModel& trm ) = 0;
	
	// Gets the name of a model.
	virtual const char* 	GetModelName( cmHandle_t model ) const = 0;
	// Gets the bounds of a model.
	virtual bool			GetModelBounds( cmHandle_t model, budBounds& bounds ) const = 0;
...
extern budCollisionModelManager* 		collisionModelManager;
