...
/*
================
budCollisionModelManagerLocal::TranslateEdgeThroughEdge
...
================
*/
BUD_INLINE void CM_AddContact( cm_traceWork_t* tw )
{
...
================
*/
BUD_INLINE void CM_SetVertexSidedness( cm_vertex_t* v, const idPluecker& vpl, const idPluecker& epl, const int bitNum )
{
...
================
*/
BUD_INLINE void CM_SetEdgeSidedness( cm_edge_t* edge, const idPluecker& vpl, const idPluecker& epl, const int bitNum )
{
	const int mask = 1 << bitNum;
...
/*
================
budCollisionModelManagerLocal::TranslateTrmEdgeThroughPolygon
================
*/
void budCollisionModelManagerLocal::TranslateTrmEdgeThroughPolygon( cm_traceWork_t* tw, cm_polygon_t* poly, cm_trmEdge_t* trmEdge )
{
	int i, edgeNum;
	...
	for( i = 0; i < poly->numEdges; i++ )
	{
		...
		if( edge->checkcount == budCollisionModelManagerLocal::checkCount )
		{
			continue;
		}
		...
		if( !budCollisionModelManagerLocal::TranslateEdgeThroughEdge( trmEdge->cross, trmEdge->pl, *pl, &f1 ) )
		{
			continue;
		}
		...
		if( !budCollisionModelManagerLocal::TranslateEdgeThroughEdge( trmEdge->cross, trmEdge->pl, epsPl, &f2 ) )
		{
			continue;
		}
...
/*
================
budCollisionModelManagerLocal::TranslateTrmVertexThroughPolygon
================
*/
void budCollisionModelManagerLocal::TranslateTrmVertexThroughPolygon( cm_traceWork_t* tw, cm_polygon_t* poly, cm_trmVertex_t* v, int bitNum )
{
	int i, edgeNum;
	float f;
	cm_edge_t* edge;
...
/*
================
budCollisionModelManagerLocal::TranslatePointThroughPolygon
================
*/
void budCollisionModelManagerLocal::TranslatePointThroughPolygon( cm_traceWork_t* tw, cm_polygon_t* poly, cm_trmVertex_t* v )
{
	int i, edgeNum;
	float f;
	cm_edge_t* edge;
	...
	if( f < tw->trace.fraction )
	{
		...
		for( i = 0; i < poly->numEdges; i++ )
		{
			...
			{
				float fl;
				edge->checkcount = budCollisionModelManagerLocal::checkCount;
...
/*
================
budCollisionModelManagerLocal::TranslateTrmThroughPolygon

  returns true if the polygon blocks the complete translation
================
*/
bool budCollisionModelManagerLocal::TranslateTrmThroughPolygon( cm_traceWork_t* tw, cm_polygon_t* p )
{
	int i, j, k, edgeNum;
	float fraction, d;
	budVec3 endp;
	...
	p->checkcount = budCollisionModelManagerLocal::checkCount;
	...
	if( tw->pointTrace )
	{
		budCollisionModelManagerLocal::TranslatePointThroughPolygon( tw, p, &tw->vertices[0] );
	}
	else
	{
		...
		for( i = 0; i < p->numEdges; i++ )
		{
			...
			if( e->checkcount != budCollisionModelManagerLocal::checkCount )
			{
				e->sideSet = 0;
			}
			...
			if( v->checkcount != budCollisionModelManagerLocal::checkCount )
			{
				v->sideSet = 0;
			}
		...
		for( i = 0; i < tw->numVerts; i++ )
		{
			...
			if( bv->used )
			{
				budCollisionModelManagerLocal::TranslateTrmVertexThroughPolygon( tw, p, bv, i );
			}
		}
		...
		for( i = 1; i <= tw->numEdges; i++ )
		{
			...
			if( be->used )
			{
				budCollisionModelManagerLocal::TranslateTrmEdgeThroughPolygon( tw, p, be );
			}
		}
		...
		for( i = 0; i < p->numEdges; i++ )
		{
			...
			e->checkcount = budCollisionModelManagerLocal::checkCount;
			// can never collide with internal edges
			if( e->internal )
			{
				continue;
			...
			for( k = 0; k < 2; k++ )
			{
				...
				v->checkcount = budCollisionModelManagerLocal::checkCount;
				...
				for( j = 0; j < tw->numPolys; j++ )
				{
					...
					if( bp->used )
					{
						budCollisionModelManagerLocal::TranslateVertexThroughTrmPolygon( tw, bp, p, v, endp, *pl );
					}
				}
			}
		}
...
/*
================
budCollisionModelManagerLocal::SetupTrm
================
*/
void budCollisionModelManagerLocal::SetupTrm( cm_traceWork_t* tw, const budTraceModel* trm )
{
	int i, j;
...
/*
================
budCollisionModelManagerLocal::SetupTranslationHeartPlanes
================
*/
...
/*
================
budCollisionModelManagerLocal::Translation
================
*/
#ifdef _DEBUG
		...
		cmHandle_t model, const budVec3& modelOrigin, const budMat3& modelAxis )
{
	...
	if( model < 0 || model > MAX_SUBMODELS || model > budCollisionModelManagerLocal::maxModels )
	{
		common->Printf( "budCollisionModelManagerLocal::Translation: invalid model handle\n" );
		return;
	}
	if( !budCollisionModelManagerLocal::models[model] )
	{
		common->Printf( "budCollisionModelManagerLocal::Translation: invalid model\n" );
		return;
	}
	...
	{
		budCollisionModelManagerLocal::ContentsTrm( results, start, trm, trmAxis, contentMask, model, modelOrigin, modelAxis );
		return;
	}
	...
	{
		if( !entered && !budCollisionModelManagerLocal::getContacts )
		{
			entered = 1;
			// if already messed up to begin with
			if( budCollisionModelManagerLocal::Contents( start, trm, trmAxis, -1, model, modelOrigin, modelAxis ) & contentMask )
			{
				startsolid = true;
			}
			entered = 0;
	...
	budCollisionModelManagerLocal::checkCount++;
	...
	tw.quickExit = false;
	tw.getContacts = budCollisionModelManagerLocal::getContacts;
	tw.contacts = budCollisionModelManagerLocal::contacts;
	tw.maxContacts = budCollisionModelManagerLocal::maxContacts;
	tw.numContacts = 0;
				  ...
				  trm->bounds[1][2] - trm->bounds[0][2] <= 0.0f ) )
	{
		...
		budCollisionModelManagerLocal::SetupTranslationHeartPlanes( &tw );
		...
		budCollisionModelManagerLocal::TraceThroughModel( &tw );
		// store results
		*results = tw.trace;
		...
		budCollisionModelManagerLocal::numContacts = tw.numContacts;
		return;
	}
	...
	if( tw.dir.LengthSqr() > Square( CM_MAX_TRACE_DIST ) )
	{
		...
		common->Printf( "budCollisionModelManagerLocal::Translation: huge translation\n" );
		return;
	}
	...
	// setup trm structure
	budCollisionModelManagerLocal::SetupTrm( &tw, trm );
	...
	if( cm_debugCollision.GetBool() )
	{
		if( !entered && !budCollisionModelManagerLocal::getContacts )
		{
			...
			if( budCollisionModelManagerLocal::Contents( results->endpos, trm, trmAxis, -1, model, modelOrigin, modelAxis ) & contentMask )
			{
				trace_t tr;
				
				// test where the trm is stuck in the model
				budCollisionModelManagerLocal::Contents( results->endpos, trm, trmAxis, -1, model, modelOrigin, modelAxis );
				// re-run collision detection to find out where it failed
				budCollisionModelManagerLocal::Translation( &tr, start, end, trm, trmAxis, contentMask, model, modelOrigin, modelAxis );
			}
			entered = 0;
		}
	}
