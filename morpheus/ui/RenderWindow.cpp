...
budRenderWindow::budRenderWindow( budUserInterfaceLocal* g ) : idWindow( g )
{
	gui = g;
	CommonInit();
}

budRenderWindow::~budRenderWindow()
{
	renderSystem->FreeRenderWorld( world );
}

void budRenderWindow::CommonInit()
{
...
void budRenderWindow::BuildAnimation( int time )
{
	...
	if( animName.Length() && animClass.Length() )
	{
		...
		worldEntity.joints = ( budJointMat* )Mem_Alloc16( SIMD_ROUND_JOINTS( worldEntity.numJoints ) * sizeof( *worldEntity.joints ), TAG_JOINTMAT );
...
void budRenderWindow::PreRender()
{
	if( needsRender )
	{
...
void budRenderWindow::Render( int time )
{
...
void budRenderWindow::Draw( int time, float x, float y )
{
	PreRender();
	Render( time );
...
void budRenderWindow::PostParse()
{
	idWindow::PostParse();
}
