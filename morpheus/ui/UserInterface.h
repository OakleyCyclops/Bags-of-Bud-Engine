...
public:
	...
	virtual bool				WriteToSaveGame( budFile* savefile ) const = 0;
	virtual bool				ReadFromSaveGame( budFile* savefile ) = 0;
...
class budUserInterfaceManager
{
public:
	virtual						~budUserInterfaceManager() {};
	...
	// Allocates a new gui.
	virtual budUserInterface* 	Alloc() const = 0;
	
	// De-allocates a gui.. ONLY USE FOR PRECACHING
	virtual void				DeAlloc( budUserInterface* gui ) = 0;
	
	// Returns NULL if gui by that name does not exist.
	virtual budUserInterface* 	FindGui( const char* qpath, bool autoLoad = false, bool needUnique = false, bool forceUnique = false ) = 0;
