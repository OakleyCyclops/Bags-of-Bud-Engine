...
=================
*/
void idRegister::ReadFromDemoFile( budDemoFile* f )
{
	f->ReadBool( enabled );
	f->ReadShort( type );
	f->ReadInt( regCount );
...
=================
*/
void idRegister::WriteToDemoFile( budDemoFile* f )
{
	f->WriteBool( enabled );
	f->WriteShort( type );
	f->WriteInt( regCount );
...
=================
*/
void idRegister::WriteToSaveGame( budFile* savefile )
{
	int len;
...
================
*/
void idRegister::ReadFromSaveGame( budFile* savefile )
{
	int len;
...
====================
*/
void idRegisterList::AddReg( const char* name, int type, budTokenParser* src, idWindow* win, idWinVar* var )
{
	idRegister* reg;
	...
	if( reg == NULL )
	{
		...
		if( type == idRegister::STRING )
		{
			...
			{
				tok = budLocalization::GetString( tok );
				var->Init( tok, win );
			}
		}
		else
...
====================
*/
void idRegisterList::ReadFromDemoFile( budDemoFile* f )
{
	int c;
...
====================
*/
void idRegisterList::WriteToDemoFile( budDemoFile* f )
{
	int c = regs.Num();
...
=====================
*/
void idRegisterList::WriteToSaveGame( budFile* savefile )
{
	int i, num;
...
====================
*/
void idRegisterList::ReadFromSaveGame( budFile* savefile )
{
	int i, num;
