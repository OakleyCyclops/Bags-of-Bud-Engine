...
void ListWindow::PostParse()
{
	...
	{
		budParser src( tabAlignStr, tabAlignStr.Length(), "tabaligns", LEXFL_NOFATALERRORS | LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS );
		budToken tok;
		while( src.ReadToken( &tok ) )
		{
			if( tok == "," )
...
void ListWindow::InitScroller( bool horizontal )
{
	...
	const budMaterial* mat = declManager->FindMaterial( barImage );
	mat->SetSort( SS_GUI );
		...
		if( tabInfo.Num() > 0 )
		{
			...
			while( start < listItems[i].Length() )
			{
				...
				else if( tabInfo[tab].type == TAB_TYPE_ICON )
				{
					...
					const budMaterial**	hashMat;
					const budMaterial*	iconMat;
