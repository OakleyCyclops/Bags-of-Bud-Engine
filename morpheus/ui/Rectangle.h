...
BUD_INLINE idRectangle& idRectangle::operator+=( const idRectangle& a )
{
	x += a.x;
	y += a.y;
	w += a.w;
...
BUD_INLINE idRectangle& idRectangle::operator/=( const float a )
{
	float inva = 1.0f / a;
	x *= inva;
	y *= inva;
...
BUD_INLINE int idRectangle::operator==( const idRectangle& a ) const
{
	return ( x == a.x && y == a.y && w == a.w && a.h );
}

BUD_INLINE float& idRectangle::operator[]( int index )
{
	return ( &x )[ index ];
}
