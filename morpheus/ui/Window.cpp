...
================
*/
idWindow::idWindow( budUserInterfaceLocal* ui )
{
	gui = ui;
	CommonInit();
}
...
================
*/
bool idWindow::ParseScript( budTokenParser* src, budGuiScriptList& list, int* timeParm, bool elseBlock )
{
	...
	while( 1 )
	{
		...
		budGuiScript* gs = new( TAG_OLD_UI ) budGuiScript();
		if( token.Icmp( "if" ) == 0 )
		{
			gs->conditionReg = ParseExpression( src );
			gs->ifList = new( TAG_OLD_UI ) budGuiScriptList();
			ParseScript( src, *gs->ifList, NULL );
			if( src->ReadToken( &token ) )
			{
				if( token == "else" )
				{
					gs->elseList = new( TAG_OLD_UI ) budGuiScriptList();
...
================
*/
bool idWindow::ParseScriptEntry( const char* name, budTokenParser* src )
{
	for( int i = 0; i < SCRIPT_COUNT; i++ )
	{
...
bool idWindow::Parse( budTokenParser* src, bool rebuild )
{
	...
	while( token != "}" )
	{
		...
		else if( token == "renderDef" )
		{
			...
			budRenderWindow* win = new( TAG_OLD_UI ) budRenderWindow( gui );
			SaveExpressionParseState();
			win->Parse( src, rebuild );
			RestoreExpressionParseState();
			AddChild( win );
		...
		{
			budGameSSDWindow* win = new( TAG_OLD_UI ) budGameSSDWindow( gui );
			SaveExpressionParseState();
			win->Parse( src, rebuild );
			RestoreExpressionParseState();
			AddChild( win );
		...
		{
			budGameBearShootWindow* win = new( TAG_OLD_UI ) budGameBearShootWindow( gui );
			SaveExpressionParseState();
			win->Parse( src, rebuild );
			RestoreExpressionParseState();
			AddChild( win );
		...
		{
			budGameBustOutWindow* win = new( TAG_OLD_UI ) budGameBustOutWindow( gui );
			SaveExpressionParseState();
			win->Parse( src, rebuild );
			RestoreExpressionParseState();
			AddChild( win );
...
================
*/
bool idWindow::RunScriptList( budGuiScriptList* src )
{
	if( src == NULL )
	{
		return false;
...
*/
// DG: a, b and the return value are really pointers, so use intptr_t
intptr_t idWindow::ParseEmitOp( budTokenParser* src, intptr_t a, wexpOpType_t opType, int priority, wexpOp_t** opp )
{
...
intptr_t idWindow::ParseTerm( budTokenParser* src,	idWinVar* var, intptr_t component )
{
	// DG end
	budToken token;
...
intptr_t idWindow::ParseExpressionPriority( budTokenParser* src, int priority, idWinVar* var, intptr_t component )
{
	budToken token;
	intptr_t a;
...
*/
// DG: component and the return value are really pointers, so use intptr_t
intptr_t idWindow::ParseExpression( budTokenParser* src, idWinVar* var, intptr_t component )
{
	// DG end
...
================
*/
void idWindow::ParseBracedExpression( budTokenParser* src )
{
	src->ExpectTokenString( "{" );
	ParseExpression( src );
	src->ExpectTokenString( "}" );
...
================
*/
void idWindow::ReadFromDemoFile( class budDemoFile* f, bool rebuild )
{
...
================
*/
void idWindow::WriteToDemoFile( class budDemoFile* f )
{
	// should never hit unless we re-enable WRITE_GUIS
#ifndef WRITE_GUIS
	assert( false );
...
===============
*/
void idWindow::WriteSaveGameString( const char* string, budFile* savefile )
{
...
===============
*/
void idWindow::ReadSaveGameTransition( idTransitionData& trans, budFile* savefile )
{
	int offset;
...
===============
*/
void idWindow::WriteToSaveGame( budFile* savefile )
{
	int i;
...
===============
*/
void idWindow::ReadFromSaveGame( budFile* savefile )
{
	int i;
...
bool idWindow::UpdateFromDictionary( Dict& dict )
{
	...
	for( i = 0; i < dict.GetNumKeyVals(); i ++ )
	{
		...
		budParser src( kv->GetValue().c_str(), kv->GetValue().Length(), "",
