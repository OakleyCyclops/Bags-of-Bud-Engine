...
=============
*/
void idDeviceContext::DrawWinding( idWinding& w, const budMaterial* mat )
{
	...
	budDrawVert* verts = renderSystem->AllocTris( w.GetNumPoints(), tempIndexes, numIndexes, mat );
	if( verts == NULL )
	{
		return;
	}
...
void idDeviceContext::DrawStretchPic( float x, float y, float w, float h, float s1, float t1, float s2, float t2, const budMaterial* shader )
{
	if( matIsIdentity )
	{
...
void idDeviceContext::DrawStretchPicRotated( float x, float y, float w, float h, float s1, float t1, float s2, float t2, const budMaterial* shader, float angle )
{
...
void idDeviceContext::PaintChar( float x, float y, const scaledGlyphInfo_t& glyphInfo )
{
	...
	const budMaterial* hShader = glyphInfo.material;
...
int idDeviceContextOptimized::DrawText( float x, float y, float scale, budVec4 color, const char* text, float adjust, int limit, int style, int cursor )
{
	...
	while( charIndex < drawText.Length() )
	{
		...
		if( !ClippedCoords( &drawX, &drawY, &w, &h, &s, &t, &s2, &t2 ) )
		{
			...
			budDrawVert* verts = tr_guiModel->AllocTris( 4, quadPicIndexes, 6, glyphInfo.material, 0, STEREO_DEPTH_TYPE_NONE );
			if( verts != NULL )
			{
				verts[0].xyz[0] = x1;
				verts[0].xyz[1] = y1;
