	...
	switch( actionType )
	{
		...
		case WIDGET_ACTION_PRESS_FOCUSED:
		{
			if( menuData->GetPlatform() == 2 )
			{
				...
				idWidgetEvent pressEvent( WIDGET_EVENT_PRESS, 0, NULL, budSWFParmList() );
			...
			switch( parms[0].ToInteger() )
			{
				...
				case ROOT_CMD_MULTIPLAYER:
				{
					...
					else if( !masterUser->CanPlayOnline() )
					{
						...
						common->Dialog().AddDialog( GDM_PLAY_ONLINE_NO_PROFILE, DIALOG_ACCEPT_CANCEL, new( TAG_SWF ) budSWFScriptFunction_Accept(), new( TAG_SWF ) budSWFScriptFunction_Cancel(), false );
					}
					else
					{
