...
void idMenuScreen::UpdateCmds()
{
	...
	budSWFScriptVar clearFunc = shortcutKeys->Get( "clear" );
	if( clearFunc.IsFunction() )
	{
		clearFunc.GetFunction()->Call( NULL, budSWFParmList() );
	}
	
	// NAVIGATION: UP/DOWN, etc.
	budSWFScriptObject* const buttons = gui->GetRootObject().GetObject( "buttons" );
	if( buttons != NULL )
	{
	
		budSWFScriptObject* const btnUp = buttons->GetObject( "btnUp" );
		if( btnUp != NULL )
		{
		...
		budSWFScriptObject* const btnDown = buttons->GetObject( "btnDown" );
		if( btnDown != NULL )
		{
		...
		budSWFScriptObject* const btnUp_LStick = buttons->GetObject( "btnUp_LStick" );
		...
		budSWFScriptObject* const btnDown_LStick = buttons->GetObject( "btnDown_LStick" );
		...
		budSWFScriptObject* const btnUp_RStick = buttons->GetObject( "btnUp_RStick" );
		...
		budSWFScriptObject* const btnDown_RStick = buttons->GetObject( "btnDown_RStick" );
		...
		budSWFScriptObject* const btnPageUp = buttons->GetObject( "btnPageUp" );
		if( btnPageUp != NULL )
		{
		...
		budSWFScriptObject* const btnPageDown = buttons->GetObject( "btnPageDown" );
		if( btnPageDown != NULL )
		{
		...
		budSWFScriptObject* const btnHome = buttons->GetObject( "btnHome" );
		if( btnHome != NULL )
		{
		...
		budSWFScriptObject* const btnEnd = buttons->GetObject( "btnEnd" );
		if( btnEnd != NULL )
		{
		...
		budSWFScriptObject* const btnLeft = buttons->GetObject( "btnLeft" );
		if( btnLeft != NULL )
		{
		...
		budSWFScriptObject* const btnRight = buttons->GetObject( "btnRight" );
		if( btnRight != NULL )
		{
		...
		budSWFScriptObject* const btnLeft_LStick = buttons->GetObject( "btnLeft_LStick" );
		...
		budSWFScriptObject* const btnRight_LStick = buttons->GetObject( "btnRight_LStick" );
		...
		budSWFScriptObject* const btnLeft_RStick = buttons->GetObject( "btnLeft_RStick" );
		...
		budSWFScriptObject* const btnRight_RStick = buttons->GetObject( "btnRight_RStick" );
	...
	budSWFScriptObject* const navigation = gui->GetRootObject().GetObject( "navBar" );
	if( navigation != NULL )
	{
		// TAB NEXT
		budSWFScriptObject* const btnTabNext = navigation->GetNestedObj( "options", "btnTabNext" );
		if( btnTabNext != NULL )
		{
		...
		// TAB PREV
		budSWFScriptObject* const btnTabPrev = navigation->GetNestedObj( "options", "btnTabPrev" );
		if( btnTabPrev != NULL )
		{
...
void idMenuScreen::HandleMenu( const mainMenuTransition_t type )
{
	...
	{
		ReceiveEvent( idWidgetEvent( WIDGET_EVENT_PRESS, 0, NULL, budSWFParmList() ) );
	}
	else if( type == MENU_TRANSITION_BACK )
	{
		ReceiveEvent( idWidgetEvent( WIDGET_EVENT_BACK, 0, NULL, budSWFParmList() ) );
	}
