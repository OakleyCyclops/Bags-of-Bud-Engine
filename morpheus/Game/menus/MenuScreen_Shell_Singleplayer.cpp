...
void idMenuScreen_Shell_Singleplayer::ContinueGame()
{
	...
	if( sortedSaves.Num() > 0 )
	{
		...
		{
			class budSWFScriptFunction_ContinueDamaged : public budSWFScriptFunction_RefCounted
			{
			public:
	...
	switch( actionType )
	{
		...
		case WIDGET_ACTION_PRESS_FOCUSED:
		{
			...
			if( canContinue )
			{
				...
				{
					class budSWFScriptFunction_NewGame : public budSWFScriptFunction_RefCounted
					{
					public:
						budSWFScriptFunction_NewGame( idMenuHandler* _menuData, bool _accept )
						{
							menuData = _menuData;
							accept = _accept;
						}
					...
					common->Dialog().AddDialog( GDM_DELETE_AUTOSAVE, DIALOG_ACCEPT_CANCEL, new budSWFScriptFunction_NewGame( menuData, true ), new budSWFScriptFunction_NewGame( menuData, false ), true );
				}
