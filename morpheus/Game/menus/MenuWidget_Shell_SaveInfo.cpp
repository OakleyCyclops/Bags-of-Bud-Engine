...
void idMenuWidget_Shell_SaveInfo::Update()
{
	...
	if( loadIndex >= 0 && sortedSaves.Num() != 0 && loadIndex < sortedSaves.Num() )
	{
		...
		{
			case GAME_D3XP:
				expansionStr = budLocalization::GetString( "#str_swf_resurrection" );
				break;
			case GAME_D3LE:
				expansionStr = budLocalization::GetString( "#str_swf_lost_episodes" );
				break;
			case GAME_BASE:
				expansionStr = budLocalization::GetString( "#str_swf_doom3" );
				break;
			default:
				expansionStr = budLocalization::GetString( "#str_savegame_title" );
				break;
		}
		...
		{
			case 0:
				difficultyStr = budLocalization::GetString( "#str_04089" );
				break;
			case 1:
				difficultyStr = budLocalization::GetString( "#str_04091" );
				break;
			case 2:
				difficultyStr = budLocalization::GetString( "#str_04093" );
				break;
			case 3:
				difficultyStr = budLocalization::GetString( "#str_02357" );
				break;
		}
		...
		{
			info.Append( "\n" );
			info.Append( va( "^1%s^0", budLocalization::GetString( "#str_swf_damaged" ) ) );
		}
		else if( details.GetSaveVersion() > BUILD_NUMBER )
		{
			info.Append( "\n" );
			info.Append( va( "^1%s^0", budLocalization::GetString( "#str_swf_wrong_version" ) ) );
		}
	}
	
	budSWFTextInstance* infoSprite = GetSprite()->GetScriptObject()->GetNestedText( "txtDesc" );
	if( infoSprite != NULL )
	{
		infoSprite->SetText( info );
	}
	
	budSWFSpriteInstance* img = GetSprite()->GetScriptObject()->GetNestedSprite( "img" );
	if( img != NULL )
	{
