...
void idMenuScreen_PDA_VideoDisks::Update()
{
	...
	if( player != NULL )
	{
		...
		const budDeclVideo* video = player->GetVideo( index );
		...
		if( player->GetVideoMaterial() != NULL )
		{
			...
			{
				budSWFSpriteInstance* videoSprite = GetSprite()->GetScriptObject()->GetNestedSprite( "info", "details", "video", "img" );
				const budMaterial* mat = player->GetVideoMaterial();
...
void idMenuScreen_PDA_VideoDisks::SelectedVideoToPlay( int index )
{
	...
	const budDeclVideo* video = player->GetVideo( pdaVideoList.GetViewOffset() + index );
