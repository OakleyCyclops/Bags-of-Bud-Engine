...
void idMenuWidget_PDA_Objective::Update()
{
	...
	budSWFScriptObject* dataObj = GetSprite()->GetScriptObject()->GetNestedObj( "info" );
	budSWFSpriteInstance* dataSprite = dataObj->GetSprite();
	
	if( dataObj != NULL && dataSprite != NULL )
	{
	
		budSWFSpriteInstance* img = dataObj->GetNestedSprite( "objImg", "img" );
		...
		else
		{
			...
			budSWFTextInstance* txtDesc = dataObj->GetNestedText( "txtDesc" );
			...
			for( int index = numObjectives - 1; displayCount < 2 && index >= 0; --index )
			{
				...
				budSWFSpriteInstance* objSel = dataObj->GetNestedSprite( va( "obj%d", objStartIndex - displayCount ), "sel" );
				budSWFTextInstance* txtNote = dataObj->GetNestedText( va( "obj%d", objStartIndex - displayCount ), "txtVal" );
		...
		budSWFTextInstance* txtMainObj = dataObj->GetNestedText( "txtObj" );
		if( txtMainObj != NULL )
		{
			if( mainObj != NULL )
			{
				txtMainObj->SetText( mainObj->spawnArgs.GetString( "text", budLocalization::GetString( "#str_04253" ) ) );
			}
			else
			{
				txtMainObj->SetText( budLocalization::GetString( "#str_02526" ) );
			}
		}
	}
}
