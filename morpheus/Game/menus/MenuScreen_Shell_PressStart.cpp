...
================================================
*/
class UICmd_RegisterUser : public budSWFScriptFunction_RefCounted
{
public:
	budSWFScriptVar Call( budSWFScriptObject* thisObject, const budSWFParmList& parms )
	{
		if( parms.Num() != 1 )
		{
			libBud::Warning( "No device specified when registering mouse user" );
	...
	if( BindSprite( root ) )
	{
		if( g_demoMode.GetBool() )
		{
			...
			if( startButton != NULL )
			{
				...
				startButton->SetLabel( budLocalization::GetString( "#str_swf_press_start" ) );
			}
