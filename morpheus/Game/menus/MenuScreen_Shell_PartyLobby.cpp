...
void idMenuScreen_Shell_PartyLobby::ShowScreen( const mainMenuTransition_t transitionType )
{
	...
	{
		budMatchParameters matchParameters = session->GetPartyLobbyBase().GetMatchParms();
...
bool idMenuScreen_Shell_PartyLobby::HandleAction( idWidgetAction& action, const idWidgetEvent& event, idMenuWidget* widget, bool forceHandled )
{
	...
	switch( actionType )
	{
		...
		case WIDGET_ACTION_GO_BACK:
		{
			...
			{
				common->Dialog().AddDialog( GDM_LEAVE_LOBBY_RET_MAIN, DIALOG_ACCEPT_CANCEL, new( TAG_SWF ) budSWFScriptFunction_Accept(), new( TAG_SWF ) budSWFScriptFunction_Cancel(), false );
			}
			else
			{
				session->Cancel();
		...
		case WIDGET_ACTION_COMMAND:
		{
			...
			switch( parms[0].ToInteger() )
			{
				...
				{
					budMatchParameters matchParameters = budMatchParameters( session->GetPartyLobbyBase().GetMatchParms() );
