	...
	else
	{
		...
		if( options != NULL )
		{
			...
			for( int slot = 0; slot < sortedSaves.Num(); ++slot )
			{
				...
				if( details.damaged )
				{
					slotSaveName =  va( S_COLOR_RED "%s", budLocalization::GetString( "#str_swf_corrupt_file" ) );
				}
				else if( details.GetSaveVersion() > BUILD_NUMBER )
				{
					slotSaveName =  va( S_COLOR_RED "%s", budLocalization::GetString( "#str_swf_wrong_version" ) );
				}
				else
				{
					if( details.slotName.Icmp( "autosave" ) == 0 )
...
void idMenuScreen_Shell_Save::SaveGame( int index )
{
	...
	else
	{
		...
		class budSWFScriptFunction_OverwriteSave : public budSWFScriptFunction_RefCounted
		{
		public:
			budSWFScriptFunction_OverwriteSave( gameDialogMessages_t _msg, bool _accept, int _index, idMenuScreen_Shell_Save* _screen )
			{
				msg = _msg;
				accept = _accept;
				index = _index;
		...
		common->Dialog().AddDialog( GDM_OVERWRITE_SAVE, DIALOG_ACCEPT_CANCEL, new budSWFScriptFunction_OverwriteSave( GDM_OVERWRITE_SAVE, true, index, this ), new budSWFScriptFunction_OverwriteSave( GDM_OVERWRITE_SAVE, false, index, this ), false );
	}
...
void idMenuScreen_Shell_Save::DeleteGame( int index )
{
	...
	class budSWFScriptFunction_DeleteGame : public budSWFScriptFunction_RefCounted
	{
	public:
		budSWFScriptFunction_DeleteGame( gameDialogMessages_t _msg, bool _accept, int _index, idMenuScreen_Shell_Save* _screen )
		{
			msg = _msg;
			accept = _accept;
			index = _index;
	...
	common->Dialog().AddDialog( GDM_DELETE_SAVE, DIALOG_ACCEPT_CANCEL, new budSWFScriptFunction_DeleteGame( GDM_DELETE_SAVE, true, index, this ), new budSWFScriptFunction_DeleteGame( GDM_DELETE_SAVE, false, index, this ), false );
