...
void idMenuWidget_ScrollBar::Update()
{
	...
	budSWFScriptObject* node = GetSprite()->GetScriptObject()->GetNestedObj( "node" );
	budSWFSpriteInstance* nodeSprite = GetSprite()->GetScriptObject()->GetNestedSprite( "node" );
...
void idMenuWidget_ScrollBar::CalcTopAndBottom()
{
	...
	int tempPos = 0.0f;
	budSWFSpriteInstance* curMC = GetSprite()->GetScriptObject()->GetNestedSprite( "top" );
	if( curMC != NULL )
	{
...
void idMenuWidget_ScrollBar::CalculatePosition( float x, float y )
{
	...
	if( y >= yTop && y <= yBot )
	{
		...
		budSWFSpriteInstance* node = GetSprite()->GetScriptObject()->GetNestedSprite( "node" );
		if( node != NULL )
		{
