...
void idMenuScreen_Shell_Credits::SetupCreditList()
{
	...
	class idRefreshCredits : public budSWFScriptFunction_RefCounted
	{
	public:
...
void idMenuScreen_Shell_Credits::UpdateCredits()
{
	...
	budSWFScriptObject* options = GetSWFObject()->GetRootObject().GetNestedObj( "menuCredits", "info", "options" );
	if( options != NULL )
	{
		for( int i = 15; i >= 0; --i )
		{
			int curIndex = creditIndex - i;
			budSWFTextInstance* heading = options->GetNestedText( va( "item%d", 15 - i ), "heading" );
			budSWFTextInstance* subHeading = options->GetNestedText( va( "item%d", 15 - i ), "subHeading" );
			budSWFTextInstance* title = options->GetNestedText( va( "item%d", 15 - i ), "title" );
			budSWFTextInstance* txtEntry = options->GetNestedText( va( "item%d", 15 - i ), "entry" );
