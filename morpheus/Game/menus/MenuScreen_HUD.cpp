...
void idMenuScreen_HUD::ShowScreen( const mainMenuTransition_t transitionType )
{
	...
	budSWFScriptObject& root = menuGUI->GetRootObject();
	...
	class idTriggerNewPDAOrVideo : public budSWFScriptFunction_RefCounted
	{
	public:
...
void idMenuScreen_HUD::UpdateHealthArmor( budPlayer* player )
{
	...
	budSWFTextInstance* txtVal = playerInfo->GetNestedText( "health", "txtVal" );
	if( txtVal != NULL )
	{
...
void idMenuScreen_HUD::UpdateStamina( budPlayer* player )
{
	...
	budSWFSpriteInstance* stamSprite = stamina->GetSprite();
	if( stamSprite != NULL )
	{
...
void idMenuScreen_HUD::UpdateWeaponInfo( budPlayer* player )
{
	...
	else
	{
		...
		if( showClip )
		{
			...
			budSWFSpriteInstance* txtClipSprite = ammoInfo->GetScriptObject()->GetNestedSprite( "info", "clip" );
			budSWFSpriteInstance* clipLowSprite = ammoInfo->GetScriptObject()->GetNestedSprite( "info", "lowAmmo" );
			budSWFSpriteInstance* clipEmptySprite = ammoInfo->GetScriptObject()->GetNestedSprite( "info", "clipEmpty" );
			budSWFSpriteInstance* ammoEmptySprite = ammoInfo->GetScriptObject()->GetNestedSprite( "info", "noAmmo" );
			budSWFSpriteInstance* txtAmmoSprite = ammoInfo->GetScriptObject()->GetNestedSprite( "info", "ammoCount" );
			
			budSWFTextInstance* txtClip = ammoInfo->GetScriptObject()->GetNestedText( "info", "clip", "clipCount", "txtVal" );
			budSWFTextInstance* txtAmmo = ammoInfo->GetScriptObject()->GetNestedText( "info", "ammoCount", "txtVal" );
		...
		else
		{
			...
			budSWFTextInstance* txtAmmo = ammoInfo->GetScriptObject()->GetNestedText( "info", "txtVal" );
...
void idMenuScreen_HUD::GiveWeapon( budPlayer* player, int weaponIndex )
{
	...
	if( weap != NULL && *weap != '\0' )
	{
		const budDeclEntityDef* weaponDef = gameLocal.FindEntityDef( weap, false );
		if( weaponDef != NULL )
		{
			const char* hudIconName = weaponDef->dict.GetString( "hudIcon" );
			if( hudIconName[ 0 ] == '\0' )
			{
				libBud::Warning( "idMenuScreen_HUD: Missing hudIcon for weapon %s", weap );
				return;
			}
			
			const budMaterial* hudIcon = declManager->FindMaterial( hudIconName, false );
			if( newWeapon != NULL )
			{
				newWeapon->SetVisible( true );
				newWeapon->PlayFrame( 2 );
				
				budSWFSpriteInstance* topImg = newWeapon->GetScriptObject()->GetNestedSprite( "topImg" );
				budSWFSpriteInstance* botImg = newWeapon->GetScriptObject()->GetNestedSprite( "botImg" );
...
void idMenuScreen_HUD::UpdatePickupInfo( int index, const budStr& name )
{
	...
	budSWFTextInstance* txtItem = pickupInfo->GetScriptObject()->GetNestedText( va( "item%d", index ), "txtVal" );
	if( txtItem != NULL )
	{
		...
		if( cursorTalking )
		{
			...
			if( cursorState != CURSOR_TALK )
			{
				...
				if( talkCursor )
				{
					...
					budSWFSpriteInstance* topBacking = talkCursor->GetScriptObject()->GetNestedSprite( "backing", "topBar" );
					budSWFSpriteInstance* bottomBacking = talkCursor->GetScriptObject()->GetNestedSprite( "backing", "botBar" );
					
					budSWFTextInstance* txtAction = talkCursor->GetScriptObject()->GetNestedText( "info", "txtAction" );
					budSWFTextInstance* txtFocus = talkCursor->GetScriptObject()->GetNestedText( "info", "txtFocus" );
					
					budSWFTextInstance* txtPrompt = talkCursor->GetScriptObject()->GetNestedText( "talkPrompt", "txtPrompt" );
...
void idMenuScreen_HUD::ShowRespawnMessage( bool show )
{
	...
	if( show )
	{
		...
		budSWFTextInstance* message = respawnMessage->GetScriptObject()->GetNestedText( "info", "txtMessage" );
		if( message != NULL )
		{
...
void idMenuScreen_HUD::UpdateWeaponStates( budPlayer* player, bool weaponChanged )
{
	...
	if( common->IsMultiplayer() )
	{
		...
		if( weaponChanged )
		{
			...
			for( int i = -2; i < 3; ++i )
			{
				...
				budSWFSpriteInstance* botValid = mpWeapons->GetNestedSprite( "list", va( "weapon%i", index ), "botValid" );
				budSWFSpriteInstance* topInvalid = mpWeapons->GetNestedSprite( "list", va( "weapon%i", index ), "topInvalid" );
				budSWFSpriteInstance* botInvalid = mpWeapons->GetNestedSprite( "list", va( "weapon%i", index ), "botInvalid" );
				...
				int weapState = 1;
				const budMaterial* hudIcon = NULL;
	...
	else
	{
		...
		for( int i = 0; i < MAX_WEAPONS; i++ )
		{
			...
			budSWFSpriteInstance* pill = weaponPills->GetNestedSprite( va( "pill%d", i ) );
			if( pill )
			{
		...
		if( weaponImg )
		{
			if( weaponChanged && hudIcon != NULL )
			{
				...
				budSWFSpriteInstance* topImg = weaponImg->GetScriptObject()->GetNestedSprite( "topImg" );
				budSWFSpriteInstance* botImg = weaponImg->GetScriptObject()->GetNestedSprite( "botImg" );
				...
				/*if ( weaponName && weaponName->GetSprite() ) {
					...
					budSWFTextInstance * txtVal = weaponName->GetNestedText( "info", "txtVal" );
					if ( txtVal != NULL ) {
...
void idMenuScreen_HUD::UpdateLocation( budPlayer* player )
{
	...
	{
		locationName->SetText( budLocalization::GetString( "#str_02911" ) );
	}
...
void idMenuScreen_HUD::ShowTip( const char* title, const char* tip )
{
	...
	budSWFSpriteInstance* tipSprite = tipInfo->GetSprite();
	...
	budSWFTextInstance* txtTitle = tipInfo->GetNestedText( "info", "txtTitle" );
	budSWFTextInstance* txtTip = tipInfo->GetNestedText( "info", "txtTip" );
	...
	if( txtTip != NULL )
	{
		...
		budSWFSpriteInstance* backing = tipInfo->GetNestedSprite( "info", "backing" );
		if( backing != NULL )
		{
...
========================
*/
void idMenuScreen_HUD::DownloadPDA( const budDeclPDA* pda, bool newSecurity )
{
...
void  idMenuScreen_HUD::UpdateAudioLog( bool show )
{
	...
	if( show && !audioLog->IsVisible() )
	{
		...
		{
			budSWFSpriteInstance* node = audioLog->GetScriptObject()->GetNestedSprite( "bar", va( "node%d", index ) );
			if( node != NULL )
			{
...
void  idMenuScreen_HUD::UpdateCommunication( bool show, budPlayer* player )
{
	...
	if( show && !communication->IsVisible() )
	{
		...
		{
			budSWFSpriteInstance* node = communication->GetScriptObject()->GetNestedSprite( "info", "bar", va( "node%d", index ) );
			if( node != NULL )
			{
				int frame = gameLocal.random.RandomInt( 100 );
				node->SetScale( 100.0f, frame );
...
void  idMenuScreen_HUD::UpdateOxygen( bool show, int val )
{
	...
	if( show )
	{
		...
		budSWFSpriteInstance* info = oxygen->GetScriptObject()->GetNestedSprite( "info" );
		if( info != NULL )
		{
			info->StopFrame( val + 1 );
		}
		
		budSWFSpriteInstance* goodFrame = oxygen->GetScriptObject()->GetNestedSprite( "goodFrame" );
		budSWFSpriteInstance* badFrame = oxygen->GetScriptObject()->GetNestedSprite( "badFrame" );
		...
		budSWFTextInstance* txtVal = oxygen->GetScriptObject()->GetNestedText( "info", "txtHeading" );
		if( txtVal != NULL )
		{
...
void idMenuScreen_HUD::ShowObjective( bool complete )
{
	...
	if( complete )
	{
		...
		budSWFTextInstance* txtComplete = objectiveComplete->GetScriptObject()->GetNestedText( "info", "txtComplete" );
		budSWFTextInstance* txtTitle = objectiveComplete->GetScriptObject()->GetNestedText( "info", "txtTitle" );
		budSWFSpriteInstance* rightArrow = objectiveComplete->GetScriptObject()->GetNestedSprite( "info", "right_arrows" );
	...
	else
	{
		...
		budSWFTextInstance* txtNew = objective->GetScriptObject()->GetNestedText( "info", "txtComplete" );
		budSWFTextInstance* txtTitle = objective->GetScriptObject()->GetNestedText( "info", "txtTitle" );
		budSWFTextInstance* txtDesc = objective->GetScriptObject()->GetNestedText( "info", "txtDesc" );
		budSWFSpriteInstance* img = objective->GetScriptObject()->GetNestedSprite( "info", "img" );
		budSWFSpriteInstance* rightArrow = objective->GetScriptObject()->GetNestedSprite( "info", "right_arrows" );
...
void idMenuScreen_HUD::ToggleMPInfo( bool show, bool showTeams, bool isCTF )
{
	...
	if( show )
	{
		...
		budSWFSpriteInstance* redTeam = mpInfo->GetScriptObject()->GetNestedSprite( "redTeam" );
		budSWFSpriteInstance* blueTeam = mpInfo->GetScriptObject()->GetNestedSprite( "blueTeam" );
		budSWFSpriteInstance* redFlag = mpInfo->GetScriptObject()->GetNestedSprite( "redFlag" );
		budSWFSpriteInstance* blueFlag = mpInfo->GetScriptObject()->GetNestedSprite( "blueFlag" );
...
void idMenuScreen_HUD::SetFlagState( int team, int state )
{
	...
	budSWFSpriteInstance* flag = NULL;
	if( team == 0 )
	{
...
void idMenuScreen_HUD::SetTeamScore( int team, int score )
{
	...
	budSWFTextInstance* txtScore = NULL;
...
void idMenuScreen_HUD::SetTeam( int team )
{
	...
	budSWFSpriteInstance* teamBacking = mpInfo->GetScriptObject()->GetNestedSprite( "teamBack" );
...
void idMenuScreen_HUD::TriggerHitTarget( bool show, const budStr& target, int color )
{
	...
	if( show )
	{
		...
		budSWFSpriteInstance* backing = mpHitInfo->GetScriptObject()->GetNestedSprite( "bgColor" );
		if( backing )
		{
...
void idMenuScreen_HUD::ShowNewItem( const char* name, const char* icon )
{
	...
	budSWFSpriteInstance* topImg = newItem->GetScriptObject()->GetNestedSprite( "info", "topImg" );
	budSWFSpriteInstance* botImg = newItem->GetScriptObject()->GetNestedSprite( "info", "botImg" );
	budSWFTextInstance* heading = newItem->GetScriptObject()->GetNestedText( "info", "txtTitle" );
	budSWFTextInstance* itemName = newItem->GetScriptObject()->GetNestedText( "info", "txtItem" );
	
	const budMaterial* mat = declManager->FindMaterial( icon, false );
...
void idMenuScreen_HUD::UpdateFlashlight( budPlayer* player )
{
	...
	if( player->flashlightBattery != flashlight_batteryDrainTimeMS.GetInteger() )
	{
		...
		budSWFSpriteInstance* batteryLife = flashlight->GetScriptObject()->GetNestedSprite( "info" );
		if( batteryLife )
		{
...
void idMenuScreen_HUD::UpdateChattingHud( budPlayer* player )
{
	...
	else
	{
		if( !mpChatObject->IsVisible() )
		{
			...
			budSWFTextInstance* txtType = mpChatObject->GetScriptObject()->GetNestedText( "info", "saybox" );
			int length = 0;
			if( txtType )
			{
			...
			budSWFSpriteInstance* sayBox = mpChatObject->GetScriptObject()->GetNestedSprite( "info", "textEntry" );
			if( sayBox )
			{
				sayBox->SetXPos( length + 10 );
			}
			
			budSWFTextInstance* say = mpChatObject->GetScriptObject()->GetNestedText( "info", "textEntry", "txtVal" );
			if( say != NULL )
			{
			...
			budSWFScriptObject* const sayObj = mpChatObject->GetScriptObject()->GetNestedObj( "info", "textEntry", "txtVal" );
			if( sayObj != NULL )
			{
				...
				budSWFScriptObject* const shortcutKeys = gui->GetGlobal( "shortcutKeys" ).GetObject();
