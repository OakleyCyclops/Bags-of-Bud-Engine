...
void idMenuWidget_DynamicList::Update()
{
	...
	for( int optionIndex = 0; optionIndex < GetNumVisibleOptions(); ++optionIndex )
	{
		...
		{
			budSWFSpriteInstance* item = GetSprite()->GetScriptObject()->GetNestedSprite( va( "item%d", optionIndex ) );
			if( item != NULL )
			{
	...
	budSWFSpriteInstance* const upSprite = GetSprite()->GetScriptObject()->GetSprite( "upIndicator" );
	if( upSprite != NULL )
	{
		upSprite->SetVisible( GetViewOffset() > 0 );
	}
	
	budSWFSpriteInstance* const downSprite = GetSprite()->GetScriptObject()->GetSprite( "downIndicator" );
	if( downSprite != NULL )
	{
...
========================
*/
void idMenuWidget_DynamicList::SetListData( List< List< String, TAG_LIBBUD_LIST_MENU >, TAG_LIBBUD_LIST_MENU >& list )
{
	listItemInfo.Clear();
...
void idMenuWidget_DynamicList::Recalculate()
{
	...
	budSWFScriptObject& root = swf->GetRootObject();
