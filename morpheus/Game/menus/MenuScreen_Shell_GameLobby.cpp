...
void idMenuScreen_Shell_GameLobby::ShowScreen( const mainMenuTransition_t transitionType )
{
	...
	if( BindSprite( root ) )
	{
		budSWFSpriteInstance* waitTime = GetSprite()->GetScriptObject()->GetNestedSprite( "waitTime" );
		if( waitTime != NULL )
		{
...
bool idMenuScreen_Shell_GameLobby::HandleAction( idWidgetAction& action, const idWidgetEvent& event, idMenuWidget* widget, bool forceHandled )
{
	...
	switch( actionType )
	{
		...
		case WIDGET_ACTION_GO_BACK:
		{
			...
			class budSWFScriptFunction_Cancel : public budSWFScriptFunction_RefCounted
			{
			public:
				budSWFScriptFunction_Cancel() { }
				budSWFScriptVar Call( budSWFScriptObject* thisObject, const budSWFParmList& parms )
				{
					common->Dialog().ClearDialog( GDM_LEAVE_LOBBY_RET_NEW_PARTY );
					return budSWFScriptVar();
			...
			{
				common->Dialog().AddDialog( GDM_LEAVE_LOBBY_RET_NEW_PARTY, DIALOG_ACCEPT_CANCEL, new( TAG_SWF ) budSWFScriptFunction_Accept(), new( TAG_SWF ) budSWFScriptFunction_Cancel(), false );
			}
			else
			{
				session->Cancel();
		...
		case WIDGET_ACTION_COMMAND:
		{
			...
			switch( parms[0].ToInteger() )
			{
				...
				{
					budMatchParameters matchParameters = budMatchParameters( session->GetActivePlatformLobbyBase().GetMatchParms() );
					...
					budMatchParameters partyParms = session->GetPartyLobbyBase().GetMatchParms();
...
void idMenuScreen_Shell_GameLobby::UpdateLobby()
{
	...
	if( session->GetState() == budSession::GAME_LOBBY )
	{
		...
		budSWFTextInstance* mapName = GetSprite()->GetScriptObject()->GetNestedText( "matchInfo", "txtMapName" );
		budSWFTextInstance* modeName = GetSprite()->GetScriptObject()->GetNestedText( "matchInfo", "txtModeName" );
		...
		budSWFTextInstance* privacy = GetSprite()->GetScriptObject()->GetNestedText( "matchInfo", "txtPrivacy" );
		if( privacy != NULL )
		{
