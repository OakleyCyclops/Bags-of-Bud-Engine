...
void idMenuHandler_PDA::Update()
{
	...
	if( activeScreen != nextScreen )
	{
		...
		if( nextScreen == PDA_AREA_INVALID )
		{
			...
			budSWFSpriteInstance* menu = gui->GetRootObject().GetNestedSprite( "navBar" );
...
void idMenuHandler_PDA::ActivateMenu( bool show )
{
	...
	if( show )
	{
		...
		navOptions.Clear();
		navOptions.Append( budLocalization::GetString( "#str_04190" ) );
		navOptions.Append( budLocalization::GetString( "#str_01442" ) );
		navOptions.Append( budLocalization::GetString( "#str_01440" ) );
		navOptions.Append( budLocalization::GetString( "#str_01414" ) );
...
========================
*/
void idMenuHandler_PDA::Initialize( const char* swfFile, budSoundWorld* sw )
{
...
#define BIND_PDA_SCREEN( screenId, className, menuHandler )				\
	...
	class idPDAGGUIClose : public budSWFScriptFunction_RefCounted
	{
	public:
...
bool idMenuHandler_PDA::PlayPDAAudioLog( int pdaIndex, int audioIndex )
{
	...
	if( player != NULL )
	{
		...
		{
			const budDeclAudio* aud = pda->GetAudioByIndex( audioIndex );
