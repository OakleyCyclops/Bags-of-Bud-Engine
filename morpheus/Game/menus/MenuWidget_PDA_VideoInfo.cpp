...
void idMenuWidget_PDA_VideoInfo::Update()
{
	...
	budSWFTextInstance* txtHeading = GetSprite()->GetScriptObject()->GetNestedText( "txtName" );
	budSWFTextInstance* txtInfo = GetSprite()->GetScriptObject()->GetNestedText( "txtInfo" );
	
	int numVideos = player->GetInventory().videos.Num();
	if( numVideos != 0 )
	{
		const budDeclVideo* video = player->GetVideo( videoIndex );
		if( video != NULL )
		{
			if( txtHeading != NULL )
			{
	...
	switch( event.type )
	{
		case WIDGET_EVENT_FOCUS_ON:
		{
			...
			if( player != NULL )
			{
				...
				if( video != NULL )
				{
					budSWFSpriteInstance* videoSprite = GetSprite()->GetScriptObject()->GetNestedSprite( "video", "img" );
					if( videoSprite != NULL )
					{
