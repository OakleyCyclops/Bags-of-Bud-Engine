...
void idMenuWidget_CommandBar::Update()
{
	...
	budStaticList< button_t, MAX_BUTTONS > buttonOrder;
	...
	for( int i = 0; i < buttonOrder.Num(); ++i )
	{
		...
		budSWFSpriteInstance* const buttonSprite = GetSprite()->GetScriptObject()->GetSprite( buttonName );
		if( buttonSprite == NULL )
		{
			continue;
		}
		budSWFTextInstance* const buttonText = buttonSprite->GetScriptObject()->GetText( "txt_info" );
		if( buttonText == NULL )
		{
			continue;
		}
		budSWFSpriteInstance* const imageSprite = buttonSprite->GetScriptObject()->GetSprite( "img" );
		if( imageSprite == NULL )
		{
			continue;
		}
		...
		{
			budSWFScriptObject* const shortcutKeys = GetSWFObject()->GetGlobal( "shortcutKeys" ).GetObject();
