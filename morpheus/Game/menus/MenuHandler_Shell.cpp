...
//#endif
	...
	if( nextState != state )
	{
		...
		else if( nextState == SHELL_STATE_IDLE )
		{
			...
			if( menuBar != NULL && gui != NULL )
			{
				budSWFScriptObject& root = gui->GetRootObject();
	...
	if( activeScreen != nextScreen )
	{
		...
		if( nextScreen == SHELL_AREA_INVALID )
		{
			...
			budSWFSpriteInstance* bg = gui->GetRootObject().GetNestedSprite( "pause_bg" );
			budSWFSpriteInstance* edging = gui->GetRootObject().GetNestedSprite( "_fullscreen" );
			...
			else
			{
				...
				else  	// binding found prompt to change
				{
					...
					else
					{
						...
						if( bindScreen != NULL )
						{
							class budSWFScriptFunction_RebindKey : public budSWFScriptFunction_RefCounted
							{
							public:
								budSWFScriptFunction_RebindKey( idMenuScreen_Shell_Bindings* _menu, gameDialogMessages_t _msg, bool _accept, idMenuHandler_Shell* _mgr, int _key, const char* _bind )
								{
									menu = _menu;
									msg = _msg;
									accept = _accept;
							...
							common->Dialog().AddDialog( GDM_BINDING_ALREDY_SET, DIALOG_ACCEPT_CANCEL, new budSWFScriptFunction_RebindKey( bindScreen, GDM_BINDING_ALREDY_SET, true, this, sev->evValue, waitBind ), new budSWFScriptFunction_RebindKey( bindScreen, GDM_BINDING_ALREDY_SET, false, this, sev->evValue, waitBind ), false );
						}
...
========================
*/
void idMenuHandler_Shell::Initialize( const char* swfFile, budSoundWorld* sw )
{
...
#define BIND_SHELL_SCREEN( screenId, className, menuHandler )				\
	...
	delete new budSWF( "doomIntro", NULL );
	delete new budSWF( "roeIntro", NULL );
	delete new budSWF( "leIntro", NULL );
	...
	class idPauseGUIClose : public budSWFScriptFunction_RefCounted
	{
	public:
		budSWFScriptVar Call( budSWFScriptObject* thisObject, const budSWFParmList& parms )
		{
	...
	if( show )
	{
		...
		if( !inGame )
		{
			...
			if( gui != NULL )
			{
				...
				budSWFSpriteInstance* mars = gui->GetRootObject().GetNestedSprite( "mars" );
				if( mars )
				{
					mars->stereoDepth = STEREO_DEPTH_TYPE_FAR;
					
					budSWFSpriteInstance* planet = mars->GetScriptObject()->GetNestedSprite( "planet" );
					
					if( marsRotation != NULL && planet != NULL )
					{
						const budMaterial* mat = marsRotation;
						if( mat != NULL )
						{
...
{
	class budSWFScriptFunction_QuitDialog : public budSWFScriptFunction_RefCounted
	{
	public:
		budSWFScriptFunction_QuitDialog( gameDialogMessages_t _msg, int _accept )
		{
			msg = _msg;
			accept = _accept;
		}
...
bool idMenuHandler_Shell::HandleAction( idWidgetAction& action, const idWidgetEvent& event, idMenuWidget* widget, bool forceHandled )
{
	...
	const budSWFParmList& parms = action.GetParms();
...
void idMenuHandler_Shell::ShowSmallFrame( bool show )
{
	...
	budSWFSpriteInstance* smallFrame = gui->GetRootObject().GetNestedSprite( "smallFrame" );
	if( smallFrame == NULL )
	{
		return;
	}
...
void idMenuHandler_Shell::ShowMPFrame( bool show )
{
	...
	budSWFSpriteInstance* smallFrame = gui->GetRootObject().GetNestedSprite( "smallFrameMP" );
	if( smallFrame == NULL )
	{
		return;
	}
...
void idMenuHandler_Shell::ShowLogo( bool show )
{
	...
	budSWFSpriteInstance* logo = gui->GetRootObject().GetNestedSprite( "logoInfo" );
	budSWFSpriteInstance* bg = gui->GetRootObject().GetNestedSprite( "background" );
...
void idMenuHandler_Shell::UpdateLobby( idMenuWidget_LobbyList* lobbyList )
{
	...
	budStaticList< lobbyPlayerInfo_t, MAX_PLAYERS > lobbyPlayers;
...
void idMenuHandler_Shell::ShowDoomIntro()
{
	...
	delete introGui;
	introGui = new budSWF( "doomIntro", common->MenuSW() );
	
	if( introGui != NULL )
	{
		const budMaterial* mat = doom3Intro;
		if( mat != NULL )
		{
			int c = mat->GetNumStages();
			for( int i = 0; i < c; i++ )
		...
		budStr textEntries[NUM_DOOM_INTRO_LINES] = { va( "%s %s", budLocalization::GetString( "#str_04052" ), budLocalization::GetString( "#str_04053" ) ),
													...
													budLocalization::GetString( "#str_04057" ),
													va( "%s %s", budLocalization::GetString( "#str_04058" ), budLocalization::GetString( "#str_04059" ) ),
													va( "%s %s", budLocalization::GetString( "#str_04060" ), budLocalization::GetString( "#str_04061" ) )
												  };
												  
		for( int i = 0; i < numTextFields; ++i )
		{
		
			budSWFTextInstance* txtVal = introGui->GetRootObject().GetNestedText( va( "info%d", i ), "txtInfo", "txtVal" );
			if( txtVal != NULL )
			{
			...
			budSWFSpriteInstance* infoSprite = introGui->GetRootObject().GetNestedSprite( va( "info%d", i ) );
			if( infoSprite != NULL && txtVal != NULL )
			{
				class idIntroTextUpdate : public budSWFScriptFunction_RefCounted
				{
				public:
					idIntroTextUpdate( budSWFTextInstance* _txtVal, int _numLines, int _nextIndex, idMenuHandler_Shell* _shell, budSWF* _gui )
					{
						txtVal = _txtVal;
						generating = false;
						numLines = _numLines;
						...
						else if( generating )
						{
							if( !txtVal->generatingText )
							{
								...
								else if( newYPos <= 665.0f - ( numLines * 36.0f ) )
								{
									if( nextIndex < NUM_DOOM_INTRO_LINES )
									{
										budSWFSpriteInstance* nextInfo = gui->GetRootObject().GetNestedSprite( va( "info%d", nextIndex ) );
		...
		class idIntroVOStart : public budSWFScriptFunction_RefCounted
		{
		public:
			idIntroVOStart( budSWF* gui )
			{
				introGui = gui;
			}
			budSWFScriptVar Call( budSWFScriptObject* thisObject, const budSWFParmList& parms )
		...
		budSWFSpriteInstance* img = introGui->GetRootObject().GetNestedSprite( "intro", "img" );
		if( img != NULL )
		{
			if( mat != NULL )
			{
...
void idMenuHandler_Shell::ShowROEIntro()
{
	...
	delete introGui;
	introGui = new budSWF( "roeIntro", common->MenuSW() );
	
	if( introGui != NULL )
	{
		const budMaterial* mat = roeIntro;
		if( mat != NULL )
		{
			int c = mat->GetNumStages();
			for( int i = 0; i < c; i++ )
		...
		budStr textEntries[NUM_ROE_INTRO_LINES] =
		{
			...
			budLocalization::GetString( "#str_00100879" ),
			budLocalization::GetString( "#str_00100855" ),
			budLocalization::GetString( "#str_00100890" ),
			budLocalization::GetString( "#str_00100856" ),
		};
							...
							if( !txtVal->generatingText && thisObject->GetSprite()->IsVisible() )
							{
								if( nextIndex % 2 != 0 )
								{
									if( nextIndex < NUM_ROE_INTRO_LINES )
									{
										...
										{
											budSWFTextInstance* txtData = nextInfo->GetScriptObject()->GetNestedText( "txtInfo", "txtVal" );
											if( txtData != NULL && !txtData->generatingText )
											{
												...
												else
												{
													if( Sys_Milliseconds() - startFade >= 3000 )
													{
														...
														{
															budSWFSpriteInstance* nextInfo = gui->GetRootObject().GetNestedSprite( va( "info%d", nextDateIndex ) );
															if( nextInfo != NULL && nextInfo->GetCurrentFrame() != nextInfo->FindFrame( "active" ) )
															{
																nextInfo->StopFrame( "active" );
																return budSWFScriptVar();
...
void idMenuHandler_Shell::ShowLEIntro()
{
	...
	delete introGui;
	introGui = new budSWF( "leIntro", common->MenuSW() );
	...
	if( introGui != NULL )
	{
		...
		budSWFSpriteInstance* infoSprite = introGui->GetRootObject().GetNestedSprite( "info0" );
		if( infoSprite != NULL )
		{
			class idIntroTextUpdate : public budSWFScriptFunction_RefCounted
			{
			public:
				idIntroTextUpdate( budSWFTextInstance* _txtVal, idMenuHandler_Shell* _shell )
				{
					txtVal = _txtVal;
					generating = false;
					shell = _shell;
