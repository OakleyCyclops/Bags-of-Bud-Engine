		...
		case WIDGET_ACTION_PRESS_FOCUSED:
		{
			if( options != NULL )
			{
				...
				else
				{
					...
					class budSWFFuncAcceptVideoChanges : public budSWFScriptFunction_RefCounted
					{
					public:
						budSWFFuncAcceptVideoChanges( idMenuHandler* _menu, gameDialogMessages_t _msg, const optionData_t& _optionData, bool _accept )
						{
							menuHandler = _menu;
							msg = _msg;
					...
					common->Dialog().AddDialog( GDM_CONFIRM_VIDEO_CHANGES, DIALOG_TIMER_ACCEPT_REVERT, new( TAG_SWF ) budSWFFuncAcceptVideoChanges( menuData, GDM_CONFIRM_VIDEO_CHANGES, currentOption, true ), new( TAG_SWF ) budSWFFuncAcceptVideoChanges( menuData, GDM_CONFIRM_VIDEO_CHANGES, originalOption, false ), false );
				}
				return true;
			}
			return true;
