...
void idMenuScreen_Shell_Stereoscopics::ShowScreen( const mainMenuTransition_t transitionType )
{
	...
	if( GetSprite() != NULL )
	{
		budSWFSpriteInstance* leftEye = GetSprite()->GetScriptObject()->GetNestedSprite( "info", "leftEye" );
		budSWFSpriteInstance* rightEye = GetSprite()->GetScriptObject()->GetNestedSprite( "info", "rightEye" );
...
void idMenuScreen_Shell_Stereoscopics::HideScreen( const mainMenuTransition_t transitionType )
{
	...
	{
		class budSWFScriptFunction_Restart : public budSWFScriptFunction_RefCounted
		{
		public:
			budSWFScriptFunction_Restart( gameDialogMessages_t _msg, bool _restart )
			{
				msg = _msg;
				restart = _restart;
			}
...
========================
*/
budSWFScriptVar idMenuScreen_Shell_Stereoscopics::idMenuDataSource_StereoSettings::GetField( const int fieldIndex ) const
{
	if( fieldIndex == STEREO_FIELD_ENABLE )
	{
		return budSWFScriptVar( stereoRender_enable_text[fields[fieldIndex].ToInteger()] );
	}
