...
================================================
*/
void idMenuHandler::Initialize( const char* swfFile, budSoundWorld* sw )
{
	Cleanup();
	gui = new budSWF( swfFile, sw );
...
void idMenuHandler::ActivateMenu( bool show )
{
	...
	class budSWFScriptFunction_updateMenuDisplay : public budSWFScriptFunction_RefCounted
	{
	public:
		budSWFScriptFunction_updateMenuDisplay( budSWF* _gui, idMenuHandler* _handler )
		{
			gui = _gui;
			handler = _handler;
		}
	...
	class budSWFScriptFunction_activateMenu : public budSWFScriptFunction_RefCounted
	{
	public:
		budSWFScriptFunction_activateMenu( idMenuHandler* _handler )
		{
			handler = _handler;
		}
		budSWFScriptVar Call( budSWFScriptObject* thisObject, const budSWFParmList& parms )
	...
	gui->SetGlobal( "updateMenuDisplay", new( TAG_SWF ) budSWFScriptFunction_updateMenuDisplay( gui, this ) );
	gui->SetGlobal( "activateMenus", new( TAG_SWF ) budSWFScriptFunction_activateMenu( this ) );
...
bool idMenuHandler::HandleAction( idWidgetAction& action, const idWidgetEvent& event, idMenuWidget* widget, bool forceHandled )
{
	...
	switch( actionType )
	{
		...
		case WIDGET_ACTION_PRESS_FOCUSED:
		{
			...
			if( screen != NULL )
			{
				idWidgetEvent pressEvent( WIDGET_EVENT_PRESS, 0, event.thisObject, budSWFParmList() );
