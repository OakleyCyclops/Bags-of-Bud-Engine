...
void idMenuScreen_Shell_ControllerLayout::Update()
{
	...
	if( BindSprite( root ) )
	{
		...
		if( menuData != NULL )
		{
			budSWFSpriteInstance* layout = NULL;
...
void idMenuScreen_Shell_ControllerLayout::ShowScreen( const mainMenuTransition_t transitionType )
{
	...
	if( GetSprite() != NULL )
	{
		...
		budSWFSpriteInstance* layout360 = NULL;
		budSWFSpriteInstance* layoutPS3 = NULL;
...
void idMenuScreen_Shell_ControllerLayout::idMenuDataSource_LayoutSettings::LoadData()
{
	...
	fields[ LAYOUT_FIELD_LAYOUT ].SetString( budLocalization::GetString( va( "#str_swf_config_360_%i", configSet ) ) );
