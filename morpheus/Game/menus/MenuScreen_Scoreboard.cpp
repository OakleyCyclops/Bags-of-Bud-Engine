...
void idMenuScreen_Scoreboard::ShowScreen( const mainMenuTransition_t transitionType )
{
	...
	if( GetSWFObject() )
	{
		...
		if( BindSprite( root ) )
		{
			...
			budSWFTextInstance* txtVal = GetSprite()->GetScriptObject()->GetNestedText( "info", "txtScoreboard" );
			if( txtVal != NULL )
			{
			...
			if( txtVal != NULL )
			{
				...
				{
					txtVal->SetText( budLocalization::GetString( "#str_04242" ) );
				}
				else if( gameLocal.gameType == GAME_CTF )
				{
					txtVal->SetText( budLocalization::GetString( "#str_11112" ) );
				}
				else
				{
					txtVal->SetText( budLocalization::GetString( "#str_04243" ) );
				}
				txtVal->SetStrokeInfo( true, 0.75f, 1.75f );
			}
...
void idMenuScreen_Scoreboard::UpdateGameInfo( budStr gameInfo )
{
	...
	if( GetSWFObject() )
	{
		...
		if( BindSprite( root ) )
		{
			...
			budSWFTextInstance* txtVal = GetSprite()->GetScriptObject()->GetNestedText( "info", "txtGameInfo" );
			if( txtVal != NULL )
			{
...
void idMenuScreen_Scoreboard::UpdateSpectating( budStr spectating, budStr follow )
{
	...
	if( GetSWFObject() )
	{
		...
		if( BindSprite( root ) )
		{
			...
			budSWFTextInstance* txtVal = GetSprite()->GetScriptObject()->GetNestedText( "info", "txtSpectating" );
			if( txtVal != NULL )
			{
				txtVal->tooltip = true;
...
void idMenuScreen_Scoreboard::UpdateTeamScores( int r, int b )
{
	...
	if( GetSWFObject() )
	{
		...
		if( BindSprite( root ) )
		{
			...
			budSWFTextInstance* txtVal = GetSprite()->GetScriptObject()->GetNestedText( "info", "txtRedScore" );
			if( txtVal != NULL )
			{
