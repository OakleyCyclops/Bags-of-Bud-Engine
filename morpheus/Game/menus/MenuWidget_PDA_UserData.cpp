...
void idMenuWidget_PDA_UserData::Update()
{
	...
	budSWFScriptObject* dataObj = GetSprite()->GetScriptObject();
	
	if( dataObj != NULL && pda != NULL )
	{
	
		budSWFTextInstance* txtName = dataObj->GetNestedText( "txtName" );
		budSWFTextInstance* txtId = dataObj->GetNestedText( "txtId" );
		budSWFTextInstance* txtLocation = dataObj->GetNestedText( "txtLocation" );
		budSWFTextInstance* txtRank = dataObj->GetNestedText( "txtRank" );
		budSWFTextInstance* txtClearance = dataObj->GetNestedText( "txtClearance" );
		budSWFTextInstance* txtLocHeading = dataObj->GetNestedText( "txtLocHeading" );
		...
		if( txtLocHeading != NULL )
		{
			if( pdaIndex == 0 )
			{
				txtLocHeading->SetText( budLocalization::GetString( "#str_02516" ) );	// location
			}
			else
			{
				txtLocHeading->SetText( budLocalization::GetString( "#str_02515" ) );	// post
			}
		}
		...
		if( txtLocation != NULL )
		{
			if( pdaIndex == 0 )
			{
				...
				{
					txtLocation->SetText( budLocalization::GetString( "#str_02911" ) );
				}
			}
			else
			{
		...
		if( txtClearance != NULL )
		{
			...
			if( *security == '\0' )
			{
				txtClearance->SetText( budLocalization::GetString( "#str_00066" ) );
			}
			else
			{
