...
void idMenuWidget_Button::Update()
{
	...
	budSWFScriptObject* const spriteObject = GetSprite()->GetScriptObject();
	...
	if( btnLabel.IsEmpty() )
	{
		if( values.Num() > 0 )
		{
			...
			{
				budSWFScriptObject* const textObject = spriteObject->GetNestedObj( va( "label%d", val ), "txtVal" );
				if( textObject != NULL )
				{
					budSWFTextInstance* const text = textObject->GetText();
		...
		{
			budSWFSpriteInstance* btnImg = spriteObject->GetNestedSprite( "img" );
			if( btnImg != NULL )
			{
	...
	{
		budSWFScriptObject* const textObject = spriteObject->GetNestedObj( "label0", "txtVal" );
		if( textObject != NULL )
		{
			budSWFTextInstance* const text = textObject->GetText();
			text->SetIgnoreColor( ignoreColor );
	...
	budSWFScriptObject* hitBox = spriteObject->GetObject( "hitBox" );
	if( hitBox == NULL )
	{
		hitBox = spriteObject;
	}
...
void idMenuWidget_Button::AnimateToState( const animState_t targetAnimState, const bool force )
{
	...
	if( GetSprite() != NULL )
	{
		...
		budSWFSpriteInstance* const focusSprite = GetSprite()->GetScriptObject()->GetSprite( "focusIndicator" );
		if( focusSprite != NULL )
		{
...
void idMenuWidget_ControlButton::Update()
{
	...
	budSWFScriptObject* const spriteObject = GetSprite()->GetScriptObject()->GetNestedObj( "type" );
	if( spriteObject == NULL )
	{
		return;
	}
	budSWFSpriteInstance* type = spriteObject->GetSprite();
	...
	budSWFTextInstance* text = spriteObject->GetNestedText( "label0", "txtVal" );
	if( text != NULL )
	{
	...
	{
		budSWFSpriteInstance* controlSprite = NULL;
		if( CONTROL_SPRITE_NAMES[ GetOptionType() ] != NULL )
		{
			...
			if( verify( controlSprite != NULL ) )
			{
				...
				{
					budSWFScriptVar fieldValue = GetDataSource()->GetField( GetDataSourceFieldIndex() );
					...
					{
						budSWFTextInstance* const txtInfo = controlSprite->GetScriptObject()->GetNestedText( "txtVal" );
				...
				budSWFScriptObject* const btnLess = GetSprite()->GetScriptObject()->GetObject( "btnLess" );
				budSWFScriptObject* const btnMore = GetSprite()->GetScriptObject()->GetObject( "btnMore" );
	...
	budSWFScriptObject* hitBox = GetSprite()->GetScriptObject()->GetObject( "hitBox" );
	if( hitBox == NULL )
	{
...
void idMenuWidget_ServerButton::Update()
{
	...
	budSWFScriptObject* const spriteObject = GetSprite()->GetScriptObject();
	budSWFTextInstance* const txtName = spriteObject->GetNestedText( "label0", "txtVal" );
...
void idMenuWidget_LobbyButton::Update()
{
	...
	budSWFScriptObject* const spriteObject = GetSprite()->GetScriptObject();
	budSWFTextInstance* const txtName = spriteObject->GetNestedText( "itemName", "txtVal" );
	budSWFSpriteInstance* talkIcon = spriteObject->GetNestedSprite( "chaticon" );
