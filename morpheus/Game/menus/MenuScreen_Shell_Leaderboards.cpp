...
static budSWFScriptVar FormatColumn( const columnDef_t* columnDef, int64 score )
{
	switch( columnDef->displayType )
	{
		case STATS_COLUMN_DISPLAY_TIME_MILLISECONDS:
			return budSWFScriptVar( FormatTime( score ) );
		case STATS_COLUMN_DISPLAY_CASH:
			return budSWFScriptVar( FormatCash( score ) );
		default:
			return budSWFScriptVar( FormatNumber( score ) );
	}
}
...
void idMenuScreen_Shell_Leaderboards::Update()
{
	...
	budSWFScriptObject& root = GetSWFObject()->GetRootObject();
	if( BindSprite( root ) )
	{
		...
		budSWFSpriteInstance* gradient = GetSprite()->GetScriptObject()->GetNestedSprite( "info", "gradient" );
...
void idMenuScreen_Shell_Leaderboards::ShowScreen( const mainMenuTransition_t transitionType )
{
	...
	if( GetSprite() != NULL )
	{
		...
		{
			budSWFScriptObject* const shortcutKeys = menuData->GetGUI()->GetGlobal( "shortcutKeys" ).GetObject();
			if( verify( shortcutKeys != NULL ) )
			{
			
				// TAB NEXT
				budSWFScriptObject* const btnTabNext = GetSprite()->GetScriptObject()->GetNestedObj( "info", "btnNext" );
				if( btnTabNext != NULL )
				{
				...
				// TAB PREV
				budSWFScriptObject* const btnTabPrev = GetSprite()->GetScriptObject()->GetNestedObj( "info", "btnPrevious" );
				if( btnTabPrev != NULL )
				{
				...
				// TAB NEXT
				budSWFScriptObject* const btnDwn = GetSprite()->GetScriptObject()->GetNestedObj( "info", "btnPageDwn" );
				if( btnDwn != NULL )
				{
				...
				// TAB PREV
				budSWFScriptObject* const btnUp = GetSprite()->GetScriptObject()->GetNestedObj( "info", "btnPageUp" );
				if( btnUp != NULL )
				{
	...
	{
		budSWFSpriteInstance* btnImg = btnNext->GetSprite()->GetScriptObject()->GetNestedSprite( "btnImg" );
	...
	{
		budSWFSpriteInstance* btnImg = btnPrev->GetSprite()->GetScriptObject()->GetNestedSprite( "btnImg" );
	...
	{
		budSWFSpriteInstance* btnImg = btnPageDwn->GetSprite()->GetScriptObject()->GetNestedSprite( "btnImg" );
	...
	{
		budSWFSpriteInstance* btnImg = btnPageUp->GetSprite()->GetScriptObject()->GetNestedSprite( "btnImg" );
...
void idMenuScreen_Shell_Leaderboards::ShowMessage( bool show, String message, bool spinner )
{
	...
	budSWFSpriteInstance* pacifier = menuData->GetGUI()->GetRootObject().GetNestedSprite( "menuLeaderboards", "info", "pacifier" );
	...
	if( show )
	{
		...
		budSWFTextInstance* txtMsg = pacifier->GetScriptObject()->GetNestedText( "message" );
		if( txtMsg != NULL )
		{
		...
		budSWFSpriteInstance* spriteSpinner = pacifier->GetScriptObject()->GetNestedSprite( "graphic" );
...
void idLBCache::SetLeaderboard( const leaderboardDefinition_t* def_, leaderboardFilterMode_t filter_ )
{
	...
	//libBud::Printf( "SetLeaderboard 0x%p.\n", def_ );
...
String idLBCache::GetFilterStrType()
{
	...
	{
		return budLocalization::GetString( "#str_swf_leaderboards_friends_heading" );
	}
	else if( filter == LEADERBOARD_FILTER_MYSCORE )
	{
		return budLocalization::GetString( "#str_swf_leaderboards_global_self_heading" );
	}
	
	return budLocalization::GetString( "#str_swf_leaderboards_global_heading" );
}
...
void idLBCache::Update( const idLeaderboardCallback* callback )
{
	...
	if( callback->GetDef() != def )
	{
		...
		libBud::Printf( "Wrong leaderboard.\n" );
		numRowsInLeaderboard = 0;
		loadingNewLeaderboard = false;
		return;
	}
