...
void idMenuWidget_NavBar::Update()
{
	...
	{
		budSWFSpriteInstance* const rightOption = GetSprite()->GetScriptObject()->GetSprite( va( "optionRight%d", index ) );
		rightOption->SetVisible( false );
		budSWFSpriteInstance* const leftOption = GetSprite()->GetScriptObject()->GetSprite( va( "optionLeft%d", index ) );
...
bool idMenuWidget_NavBar::PrepareListElement( idMenuWidget& widget, const int navIndex )
{
	...
	budSWFTextInstance* ti = button->GetSprite()->GetScriptObject()->GetNestedText( "txtVal" );
	if( ti != NULL )
	{
