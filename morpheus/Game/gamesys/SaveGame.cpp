...
void idSaveGame::Close()
{
	...
	// read trace models
	budClipModel::SaveTraceModels( this );
	...
	{
		libBud::FatalError( "OVERFLOWED SAVE GAME FILE BUFFER" );
	}
...
void idSaveGame::WriteDecls()
{
	...
	for( int t = 0; t < declManager->GetNumDeclTypes(); t++ )
	{
		...
		{
			const budDecl* decl = declManager->DeclByIndex( ( declType_t )t, d, false );
...
================
*/
void idSaveGame::WriteBounds( const budBounds& bounds )
{
...
================
*/
void idSaveGame::WriteMaterial( const budMaterial* material )
{
	if( !material )
	{
		WriteString( "" );
...
================
*/
void idSaveGame::WriteSkin( const budDeclSkin* skin )
{
	if( !skin )
	{
		WriteString( "" );
...
================
*/
void idSaveGame::WriteParticle( const budDeclParticle* particle )
{
	if( !particle )
	{
		WriteString( "" );
...
================
*/
void idSaveGame::WriteFX( const budDeclFX* fx )
{
	if( !fx )
	{
		WriteString( "" );
...
================
*/
void idSaveGame::WriteModelDef( const budDeclModelDef* modelDef )
{
	if( !modelDef )
	{
		WriteString( "" );
...
================
*/
void idSaveGame::WriteModel( const budRenderModel* model )
{
	const char* name;
...
================
*/
void idSaveGame::WriteUserInterface( const budUserInterface* ui, bool unique )
{
	const char* name;
...
/*
 ...
 ===================
 */
void idSaveGame::WriteTraceModel( const budTraceModel& trace )
{
	int j, k;
...
===================
*/
void idSaveGame::WriteClipModel( const budClipModel* clipModel )
{
	if( clipModel != NULL )
	{
		WriteBool( true );
...
================
*/
idRestoreGame::idRestoreGame( budFile* savefile, budFile* stringTableFile, int saveVersion )
{
	file = savefile;
	stringFile = stringTableFile;
	version = saveVersion;
...
void idRestoreGame::RestoreObjects()
{
	...
	// read trace models
	budClipModel::RestoreTraceModels( this );
...
================
*/
void idRestoreGame::ReadBounds( budBounds& bounds )
{
	file->ReadBig( bounds );
}
...
void idRestoreGame::ReadParticle( const budDeclParticle*& particle )
{
	...
	{
		particle = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, name ) );
	}
}
...
void idRestoreGame::ReadFX( const budDeclFX*& fx )
{
	...
	{
		fx = static_cast<const budDeclFX*>( declManager->FindType( DECL_FX, name ) );
	}
}
...
/*
 ...
 ===================
 */
void idRestoreGame::ReadTraceModel( budTraceModel& trace )
{
	int j, k;
...
=====================
*/
void idRestoreGame::ReadClipModel( budClipModel*& clipModel )
{
	...
	if( restoreClipModel )
	{
		clipModel = new( TAG_SAVEGAMES ) budClipModel();
