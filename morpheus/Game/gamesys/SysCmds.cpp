...
void Cmd_KillRagdolls_f( const budCmdArgs& args )
{
	...
	KillEntities( args, budAFEntity_Generic::Type );
	KillEntities( args, budAFEntity_WithAttachedHead::Type );
}
...
void Cmd_Give_f( const budCmdArgs& args )
{
	...
	if( budStr::Icmp( name, "pda" ) == 0 )
	{
		...
		else if( budStr::Icmp( args.Argv( 2 ), "all" ) == 0 )
		{
			...
			{
				player->GivePDA( static_cast<const budDeclPDA*>( declManager->DeclByIndex( DECL_PDA, i ) ), NULL );
			}
		}
		else
		{
			const budDeclPDA* pda = static_cast<const budDeclPDA*>( declManager->FindType( DECL_PDA, args.Argv( 2 ), false ) );
			if( pda == NULL )
			{
...
static void Cmd_Say( bool team, const budCmdArgs& args )
{
	...
	else
	{
		...
		budPlayer* player = static_cast<budPlayer*>( gameLocal.entities[ gameLocal.GetLocalClientNum() ] );
...
static void Cmd_AASStats_f( const budCmdArgs& args )
{
	...
	budAAS* aas = gameLocal.GetAAS( aasNum );
	if( !aas )
	{
...
static void Cmd_SaveSelected_f( const budCmdArgs& args )
{
	...
	else if( s->IsType( budAFEntity_Generic::Type ) || s->IsType( budAFEntity_WithAttachedHead::Type ) )
	{
		// save the articulated figure state
		dict.Clear();
		static_cast<budAFEntity_Base*>( s )->SaveState( dict );
...
static void Cmd_SaveRagdolls_f( const budCmdArgs& args )
{
	...
	for( e = 0; e < MAX_GENTITIES; e++ )
	{
		af = static_cast<budAFEntity_Base*>( gameLocal.entities[ e ] );
		...
		if( !af->IsType( budAFEntity_WithAttachedHead::Type ) && !af->IsType( budAFEntity_Generic::Type ) )
		{
			continue;
		}
...
void Cmd_TestId_f( const budCmdArgs& args )
{
	...
	gameLocal.mpGame.AddChatLine( budLocalization::GetString( id ), "<nothing>", "<nothing>", "<nothing>" );
}


/*
=================
budGameLocal::InitConsoleCommands

Let the system know about all of our commands
so it can perform tab completion
=================
*/
void budGameLocal::InitConsoleCommands()
{
	...
	cmdSystem->AddCommand( "listMonsters",			budAI::List_f,				CMD_FL_GAME | CMD_FL_CHEAT,	"lists monsters" );
	cmdSystem->AddCommand( "listSpawnArgs",			Cmd_ListSpawnArgs_f,		CMD_FL_GAME | CMD_FL_CHEAT,	"list the spawn args of an entity", budGameLocal::ArgCompletion_EntityName );
...
//	cmdSystem->AddCommand( "clientCallVote",		idMultiplayerGame::CallVote_f,	CMD_FL_GAME,			"call a vote: clientCallVote si_.. proposed_value" );
	...
	cmdSystem->AddCommand( "serverMapRestart",		budGameLocal::MapRestart_f,	CMD_FL_GAME,				"restart the current game" );
	...
	cmdSystem->AddCommand( "setActorState",			Cmd_SetActorState_f,		CMD_FL_GAME | CMD_FL_CHEAT,	"Manually sets an actors script state", budGameLocal::ArgCompletion_EntityName );
}

/*
=================
budGameLocal::ShutdownConsoleCommands
=================
*/
void budGameLocal::ShutdownConsoleCommands()
{
