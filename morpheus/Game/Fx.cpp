...
void idEntityFx::Setup( const char* fx )
{
	...
	fxEffect = static_cast<const budDeclFX*>( declManager->FindType( DECL_FX, systemName.c_str() ) );
...
=================
*/
void idEntityFx::WriteToSnapshot( budBitMsg& msg ) const
{
...
=================
*/
void idEntityFx::ReadFromSnapshot( const budBitMsg& msg )
{
	...
	if( fx_index != -1 && start_time > 0 && !fxEffect && started < 0 )
	{
		...
		const budDeclFX* fx = static_cast<const budDeclFX*>( declManager->DeclByIndex( DECL_FX, fx_index ) );
		if( !fx )
		{
