...
================
*/
void idItem::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idItem::ReadFromSnapshot( const budBitMsg& msg )
{
	if( msg.ReadBits( 1 ) )
	{
		Hide();
...
================
*/
bool idItem::ClientReceiveEvent( int event, int time, const budBitMsg& msg )
{
...
===============
*/
bool idItemTeam::ClientReceiveEvent( int event, int time, const budBitMsg& msg )
{
...
================
*/
void idItemTeam::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idItemTeam::ReadFromSnapshot( const budBitMsg& msg )
{
...
bool idVideoCDItem::GiveToPlayer( budPlayer* player, unsigned int giveFlags )
{
	...
	const budDeclVideo* video = static_cast<const budDeclVideo* >( declManager->FindType( DECL_VIDEO, spawnArgs.GetString( "video" ), false ) );
	if( video == NULL )
	{
		return false;
	}
...
bool idPDAItem::GiveToPlayer( budPlayer* player, unsigned int giveFlags )
{
	...
	if( pdaName != NULL && pdaName[0] != 0 )
	{
		...
		pda = static_cast<const budDeclPDA*>( declManager->FindType( DECL_PDA, pdaName, false ) );
		if( pda == NULL )
		{
			libBud::Warning( "PDA Item '%s' references unknown PDA %s", GetName(), pdaName );
			return false;
		}
	}
	if( giveFlags & ITEM_GIVE_UPDATE_STATE )
...
void idMoveableItem::Spawn()
{
	...
	physicsObj.SetClipModel( new( TAG_PHYSICS_CLIP_ENTITY ) budClipModel( trm ), density );
...
======================
*/
void idMoveableItem::WriteToSnapshot( budBitMsg& msg ) const
{
...
======================
*/
void idMoveableItem::ReadFromSnapshot( const budBitMsg& msg )
{
...
void idMoveableItem::Gib( const budVec3& dir, const char* damageDefName )
{
	...
	if( *smokeName != '\0' )
	{
		const budDeclParticle* smoke = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, smokeName ) );
