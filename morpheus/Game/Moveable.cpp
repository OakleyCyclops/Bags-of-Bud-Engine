...
void idMoveable::Spawn()
{
	...
	physicsObj.SetClipModel( new( TAG_PHYSICS_CLIP_MOVER ) budClipModel( trm ), density );
...
================
*/
const budMaterial* idMoveable::GetRenderModelMaterial() const
{
...
================
*/
void idMoveable::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idMoveable::ReadFromSnapshot( const budBitMsg& msg )
{
...
void idExplodingBarrel::AddParticles( const char* name, bool burn )
{
	if( name && *name )
	{
		...
		memset( &particleRenderEntity, 0, sizeof( particleRenderEntity ) );
		budRenderModel* modelDef = renderModelManager->FindModel( name );
		if( modelDef )
		{
			particleRenderEntity.origin = physicsObj.GetAbsBounds().GetCenter();
			particleRenderEntity.axis = mat3_identity;
...
================
*/
void idExplodingBarrel::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idExplodingBarrel::ReadFromSnapshot( const budBitMsg& msg )
{
...
================
*/
bool idExplodingBarrel::ClientReceiveEvent( int event, int time, const budBitMsg& msg )
{
