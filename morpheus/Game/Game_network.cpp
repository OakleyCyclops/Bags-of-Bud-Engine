...
===============================================================================
	...
	- entities should never be spawned outside budGameLocal::ClientReadSnapshot
...
/*
================
budGameLocal::InitAsyncNetwork
================
*/
void budGameLocal::InitAsyncNetwork()
{
	eventQueue.Init();
...
/*
================
budGameLocal::ShutdownAsyncNetwork
================
*/
void budGameLocal::ShutdownAsyncNetwork()
{
	eventQueue.Shutdown();
...
/*
================
budGameLocal::ServerRemapDecl
================
*/
int budGameLocal::ServerRemapDecl( int clientNum, declType_t type, int index )
{
	return index;
}
...
/*
================
budGameLocal::ClientRemapDecl
================
*/
int budGameLocal::ClientRemapDecl( declType_t type, int index )
{
	return index;
}
...
/*
================
budGameLocal::SyncPlayersWithLobbyUsers
================
*/
void budGameLocal::SyncPlayersWithLobbyUsers( bool initial )
{
	...
	budStaticList< lobbyUserID_t, MAX_CLIENTS > newLobbyUsers;
...
/*
================
budGameLocal::ServerSendNetworkSyncCvars
================
*/
void budGameLocal::ServerSendNetworkSyncCvars()
{
	...
	libBud::Printf( "Sending networkSync cvars:\n" );
	syncedCvars.Print();
}

/*
================
budGameLocal::ServerWriteInitialReliableMessages

  Send reliable messages to initialize the client game up to a certain initial state.
================
*/
void budGameLocal::ServerWriteInitialReliableMessages( int clientNum, lobbyUserID_t lobbyUserID )
{
	...
	libBud::Printf( "Sending initial networkSync cvars:\n" );
	syncedCvars.Print();
...
/*
================
budGameLocal::SaveEntityNetworkEvent
================
*/
void budGameLocal::SaveEntityNetworkEvent( const idEntity* ent, int eventId, const budBitMsg* msg )
{
...
/*
================
budGameLocal::ServerWriteSnapshot

  Write a snapshot of the current game state
================
*/
void budGameLocal::ServerWriteSnapshot( budSnapShot& ss )
{
...
/*
================
budGameLocal::NetworkEventWarning
================
*/
void budGameLocal::NetworkEventWarning( const entityNetEvent_t* event, const char* fmt, ... )
{
	char buf[1024];
	int length = 0;
	va_list argptr;
...
/*
================
budGameLocal::ServerProcessEntityNetworkEventQueue
================
*/
void budGameLocal::ServerProcessEntityNetworkEventQueue()
{
...
/*
================
budGameLocal::ProcessReliableMessage
================
*/
void budGameLocal::ProcessReliableMessage( int clientNum, int type, const budBitMsg& msg )
{
...
/*
================
budGameLocal::ServerProcessReliableMessage
================
*/
void budGameLocal::ServerProcessReliableMessage( int clientNum, int type, const budBitMsg& msg )
{
	if( clientNum < 0 )
	{
		return;
	...
	switch( type )
	{
		...
		case GAME_RELIABLE_MESSAGE_CLIENT_HITSCAN_HIT:
		{
			...
			const budDeclEntityDef* damageDef = static_cast<const budDeclEntityDef*>( declManager->DeclByIndex( DECL_ENTITYDEF, damageDefIndex, false ) );
...
/*
================
budGameLocal::ClientReadSnapshot
================
*/
void budGameLocal::ClientReadSnapshot( const budSnapShot& ss )
{
	...
	for( int o = 0; o < ss.NumObjects(); o++ )
	{
		...
		if( !typeInfo )
		{
			libBud::Error( "Unknown type number %d for entity %d with class number %d", typeNum, entityNumber, entityDefNumber );
		}
		...
		if( entities[entityNumber] == NULL )
		{
			...
			{
				libBud::PrintfIf( debug, "Looking for predicted key %d.\n", predictedKey );
				...
				/* else {
					libBud::Warning( "Could not find predicted entity - key: %d. EntityIndex: %d", predictedKey, entityNum );
				} */
			}
		}
...
/*
================
budGameLocal::ClientProcessEntityNetworkEventQueue
================
*/
void budGameLocal::ClientProcessEntityNetworkEventQueue()
{
	while( eventQueue.Start() )
	{
		entityNetEvent_t* event = eventQueue.Start();
...
/*
================
budGameLocal::ClientProcessReliableMessage
================
*/
void budGameLocal::ClientProcessReliableMessage( int type, const budBitMsg& msg )
{
	switch( type )
	{
...
/*
================
budGameLocal::ClientRunFrame
================
*/
void budGameLocal::ClientRunFrame( budUserCmdMgr& cmdMgr, bool lastPredictFrame, gameReturn_t& ret )
{
	idEntity* ent;
...
/*
========================
budGameLocal::FindPredictedEntity
========================
*/
idEntity*   budGameLocal::FindPredictedEntity( uint32 predictedKey, idTypeInfo* type )
{
	for( idEntity* predictedEntity = activeEntities.Next(); predictedEntity != NULL; predictedEntity = predictedEntity->activeNode.Next() )
	{
		...
		if( !predictedEntity->IsReplicated() && predictedEntity->GetPredictedKey() == predictedKey )
		{
			...
			{
				libBud::Warning( "Mismatched presentable type. Predicted: %s Actual: %s", predictedEntity->GetType()->classname, type->classname );
			}
...
uint32  budGameLocal::GeneratePredictionKey( idWeapon* weapon, budPlayer* playerAttacker, int overrideKey )
{
	...
	{
		libBud::Printf( "GeneratePredictionKey. predictedKey: %d peedIndex: %d\n", predictedKey, peerIndex );
	}
