...
void idMover::Spawn()
{
	...
	physicsObj.SetClipModel( new( TAG_PHYSICS_CLIP_MOVER ) budClipModel( GetPhysics()->GetClipModel() ), 1.0f );
...
================
*/
void idMover::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idMover::ReadFromSnapshot( const budBitMsg& msg )
{
...
===============
*/
bool idElevator::HandleSingleGuiCommand( idEntity* entityGui, budLexer* src )
{
	budToken token;
...
================
*/
void idMover_Binary::WriteToSnapshot( budBitMsg& msg ) const
{
	physicsObj.WriteToSnapshot( msg );
...
================
*/
void idMover_Binary::ReadFromSnapshot( const budBitMsg& msg )
{
...
======================
*/
void idDoor::CalcTriggerBounds( float size, budBounds& bounds )
{
	idMover_Binary*	other;
	int				i;
	int				best;
...
void idDoor::Event_SpawnDoorTrigger()
{
	...
	trigger = new( TAG_PHYSICS_CLIP_MOVER ) budClipModel( budTraceModel( bounds ) );
...
void idDoor::Event_SpawnSoundTrigger()
{
	...
	// create a trigger clip model
	sndTrigger = new( TAG_PHYSICS_CLIP_MOVER ) budClipModel( budTraceModel( bounds ) );
...
void idPlat::SpawnPlatTrigger( budVec3& pos )
{
	...
	trigger = new( TAG_PHYSICS_CLIP_MOVER ) budClipModel( budTraceModel( budBounds( tmin, tmax ) ) );
	trigger->Link( gameLocal.clip, this, 255, GetPhysics()->GetOrigin(), mat3_identity );
	trigger->SetContents( CONTENTS_TRIGGER );
}
...
================
*/
void idMover_Periodic::WriteToSnapshot( budBitMsg& msg ) const
{
	physicsObj.WriteToSnapshot( msg );
	WriteBindToSnapshot( msg );
}
...
================
*/
void idMover_Periodic::ReadFromSnapshot( const budBitMsg& msg )
{
	physicsObj.ReadFromSnapshot( msg );
	ReadBindFromSnapshot( msg );
