...
CLASS_DECLARATION( budAnimatedEntity, idTestModel )
...
void idTestModel::Spawn()
{
	...
	if( headModel[ 0 ] )
	{
		...
		else
		{
			...
			head = gameLocal.SpawnEntityType( budAnimatedEntity::Type, &args );
...
void idTestModel::ArgCompletion_TestAnim( const CmdArgs& args, void( *callback )( const char* s ) )
{
	if( gameLocal.testmodel )
	{
		budAnimator* animator = gameLocal.testmodel->GetAnimator();
