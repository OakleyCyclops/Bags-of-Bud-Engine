...
// This is used in MP to simulate frames to catchup a projectile's state. Similiar to how players work much lighter weight.
budArray< idProjectile::simulatedProjectile_t, idProjectile::MAX_SIMULATED_PROJECTILES >	idProjectile::projectilesToSimulate;
...
void idProjectile::Launch( const budVec3& start, const budVec3& dir, const budVec3& pushVelocity, const float timeSinceFire, const float launchPower, const float dmgPower )
{
	...
	if( *smokeName != '\0' )
	{
		smokeFly = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, smokeName ) );
...
bool idProjectile::Collide( const trace_t& collision, const budVec3& velocity )
{
	...
	if( ent->IsType( budActor::Type ) || ( ent->IsType( budAFAttachment::Type ) && static_cast<const budAFAttachment*>( ent )->GetBody()->IsType( budActor::Type ) ) )
	{
...
void idProjectile::CatchProjectile( idEntity* o, const char* reflectName )
{
	...
	if( this->IsType( budGuidedProjectile::Type ) )
	{
		budGuidedProjectile* proj = static_cast<budGuidedProjectile*>( this );
...
================
*/
void idProjectile::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idProjectile::ReadFromSnapshot( const budBitMsg& msg )
{
...
bool idProjectile::ClientReceiveEvent( int event, int time, const budBitMsg& msg )
{
	...
	switch( event )
	{
		case EVENT_DAMAGE_EFFECT:
		{
			...
			collision.c.material = ( index != -1 ) ? static_cast<const budMaterial*>( declManager->DeclByIndex( DECL_MATERIAL, index ) ) : NULL;
	...
	for( int i = 0; i < MAX_SIMULATED_PROJECTILES; i++ )
	{
		if( projectilesToSimulate[i].projectile == NULL )
		{
			...
			if( g_projectileDebug.GetBool() )
			{
				...
				libBud::Printf( "Simulating projectile %d. Approx %d delay.\n", GetEntityNumber(), delta );
			}
			return;
		}
	}
	
	libBud::Warning( "Unable to simulate more projectiles this frame" );
}
...
void idProjectile::SimulateProjectileFrame( int msec, int endTime )
{
	...
	if( g_projectileDebug.GetBool() )
	{
		...
		libBud::Printf( "Simulated projectile %d. Delta: %.2f \n", GetEntityNumber(), delta );
...
CLASS_DECLARATION( idProjectile, budGuidedProjectile )
EVENT( EV_SetEnemy,		budGuidedProjectile::Event_SetEnemy )
END_CLASS

/*
================
budGuidedProjectile::budGuidedProjectile
================
*/
budGuidedProjectile::budGuidedProjectile()
{
...
/*
================
budGuidedProjectile::Spawn
================
*/
void budGuidedProjectile::Spawn()
{
}

/*
================
budGuidedProjectile::Save
================
*/
void budGuidedProjectile::Save( idSaveGame* savefile ) const
{
	enemy.Save( savefile );
...
/*
================
budGuidedProjectile::Restore
================
*/
void budGuidedProjectile::Restore( idRestoreGame* savefile )
{
...
void budGuidedProjectile::GetSeekPos( budVec3& out )
{
	...
	if( enemyEnt )
	{
		if( enemyEnt->IsType( budActor::Type ) )
		{
			out = static_cast<budActor*>( enemyEnt )->GetEyePosition();
			out.z -= 12.0f;
		}
		else
		{
...
void budGuidedProjectile::Launch( const budVec3& start, const budVec3& dir, const budVec3& pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower )
{
	...
	if( owner.GetEntity() )
	{
		if( owner.GetEntity()->IsType( budAI::Type ) )
		{
		...
		else if( owner.GetEntity()->IsType( budPlayer::Type ) )
		{
			...
			if( enemy.GetEntity() == NULL || !enemy.GetEntity()->IsType( budActor::Type ) || ( static_cast<budActor*>( enemy.GetEntity() )->team == player->team ) )
			{
...
void budGuidedProjectile::SetEnemy( idEntity* ent )
{
	enemy = ent;
}
void budGuidedProjectile::Event_SetEnemy( idEntity* ent )
{
	SetEnemy( ent );
}
...
CLASS_DECLARATION( budGuidedProjectile, idSoulCubeMissile )
END_CLASS
...
void idSoulCubeMissile::KillTarget( const budVec3& dir )
{
	...
	ReturnToOwner();
	if( enemy.GetEntity() && enemy.GetEntity()->IsType( budActor::Type ) )
	{
		act = static_cast<budActor*>( enemy.GetEntity() );
		killPhase = true;
		...
		if( *smokeName != '\0' )
		{
			smokeKill = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, smokeName ) );
...
void idSoulCubeMissile::GetSeekPos( budVec3& out )
{
	...
	{
		budActor* act = static_cast<budActor*>( owner.GetEntity() );
	...
	budGuidedProjectile::GetSeekPos( out );
}
...
void idSoulCubeMissile::Launch( const budVec3& start, const budVec3& dir, const budVec3& pushVelocity, const float timeSinceFire, const float launchPower, float dmgPower )
{
	...
	newStart += offs;
	budGuidedProjectile::Launch( newStart, dir, pushVelocity, timeSinceFire, launchPower, dmgPower );
	if( enemy.GetEntity() == NULL || !enemy.GetEntity()->IsType( budActor::Type ) )
	{
...
void idBFGProjectile::Launch( const budVec3& start, const budVec3& dir, const budVec3& pushVelocity, const float timeSinceFire, const float power, const float dmgPower )
{
	...
	bounds = budBounds( GetPhysics()->GetOrigin() ).Expand( radius );
	...
	for( int e = 0; e < numListedEntities; e++ )
	{
		...
		if( ent == this || ent == owner.GetEntity() || ent->IsHidden() || !ent->IsActive() || !ent->fl.takedamage || ent->health <= 0 || !ent->IsType( budActor::Type ) )
		{
			continue;
		}
	...
	// Major hack for end boss.  :(
	budAnimatedEntity* maledict = static_cast<budAnimatedEntity*>( gameLocal.FindEntity( "monster_boss_d3xp_maledict_1" ) );
...
void idBFGProjectile::Explode( const trace_t& collision, idEntity* ignore )
{
	...
	for( i = 0; i < beamTargets.Num(); i++ )
	{
		...
		{
			// if the projectile hit an actor
			if( beamTargets[i].target.GetEntity()->IsType( budActor::Type ) )
			{
...
void idDebris::Launch()
{
	...
	{
		physicsObj.SetClipModel( new( TAG_PHYSICS_CLIP_ENTITY ) budClipModel( trm ), 1.0f );
	}
