...
/*
============
budAASLocal::FloorEdgeSplitPoint
...
Vector3 budAASLocal::SubSampleWalkPath( int areaNum, const Vector3& origin, const Vector3& start, const Vector3& end, int travelFlags, int& endAreaNum ) const
{
	...
	for( i = 1; i < numSamples; i++ )
	{
		...
		if( !budAASLocal::WalkPathValid( areaNum, origin, 0, nextPoint, travelFlags, endPos, curAreaNum ) )
		{
			return point;
		}
		point = nextPoint;
...
bool budAASLocal::WalkPathToGoal( aasPath_t& path, int areaNum, const Vector3& origin, int goalAreaNum, const Vector3& goalOrigin, int travelFlags ) const
{
	...
	for( i = 0; i < maxWalkPathIterations; i++ )
	{
		...
		if( !budAASLocal::RouteToGoalArea( curAreaNum, path.moveGoal, goalAreaNum, travelFlags, travelTime, &reach ) )
		{
			break;
		}
		...
		if( areaNum != curAreaNum )
		{
			...
			if( !budAASLocal::WalkPathValid( areaNum, origin, 0, reach->start, travelFlags, endPos, endAreaNum ) )
			{
#if SUBSAMPLE_WALK_PATH
				path.moveGoal = SubSampleWalkPath( areaNum, origin, path.moveGoal, reach->start, travelFlags, path.moveAreaNum );
#endif
		...
		if( !budAASLocal::WalkPathValid( areaNum, origin, 0, reach->end, travelFlags, endPos, endAreaNum ) )
		{
			return true;
		}
		...
		{
			if( !budAASLocal::WalkPathValid( areaNum, origin, 0, goalOrigin, travelFlags, endPos, endAreaNum ) )
			{
#if SUBSAMPLE_WALK_PATH
		...
		{
			common->Warning( "budAASLocal::WalkPathToGoal: local routing minimum going from area %d to area %d", areaNum, goalAreaNum );
			break;
		}
	}
...
Vector3 budAASLocal::SubSampleFlyPath( int areaNum, const Vector3& origin, const Vector3& start, const Vector3& end, int travelFlags, int& endAreaNum ) const
{
	...
	for( i = 1; i < numSamples; i++ )
	{
		...
		if( !budAASLocal::FlyPathValid( areaNum, origin, 0, nextPoint, travelFlags, endPos, curAreaNum ) )
		{
			return point;
		}
		point = nextPoint;
...
bool budAASLocal::FlyPathToGoal( aasPath_t& path, int areaNum, const Vector3& origin, int goalAreaNum, const Vector3& goalOrigin, int travelFlags ) const
{
	...
	for( i = 0; i < maxFlyPathIterations; i++ )
	{
		...
		if( areaNum != curAreaNum )
		{
			...
			if( !budAASLocal::FlyPathValid( areaNum, origin, 0, reach->start, travelFlags, endPos, endAreaNum ) )
			{
#if SUBSAMPLE_FLY_PATH
				path.moveGoal = SubSampleFlyPath( areaNum, origin, path.moveGoal, reach->start, travelFlags, path.moveAreaNum );
#endif
		...
		if( !budAASLocal::FlyPathValid( areaNum, origin, 0, reach->end, travelFlags, endPos, endAreaNum ) )
		{
			return true;
		}
		...
		if( reach->toAreaNum == goalAreaNum )
		{
			if( !budAASLocal::FlyPathValid( areaNum, origin, 0, goalOrigin, travelFlags, endPos, endAreaNum ) )
			{
#if SUBSAMPLE_FLY_PATH
		...
		if( curAreaNum == lastAreas[0] || curAreaNum == lastAreas[1] ||
				curAreaNum == lastAreas[2] || curAreaNum == lastAreas[3] )
		{
			common->Warning( "budAASLocal::FlyPathToGoal: local routing minimum going from area %d to area %d", areaNum, goalAreaNum );
			break;
		}
	}
...
/*
============
budAASLocal::SortWallEdges
============
*/
void budAASLocal::SortWallEdges( int* edges, int numEdges ) const
{
...
/*
============
budAASLocal::GetWallEdges
============
*/
int budAASLocal::GetWallEdges( int areaNum, const budBounds& bounds, int travelFlags, int* edges, int maxEdges ) const
{
