...
{
	int							type;			// path type
	Vector3						moveGoal;		// point the AI should move towards
	int							moveAreaNum;	// number of the area the AI should move towards
	Vector3						secondaryGoal;	// secondary move goal for complex navigation
...
typedef struct aasGoal_s
{
	...
	Vector3						origin;			// position of goal
} aasGoal_t;
...
public:
	...
	virtual bool				Init( const String& mapName, unsigned int mapFileCRC ) = 0;
	// Print AAS stats.
	virtual void				Stats() const = 0;
	// Test from the given origin.
	virtual void				Test( const Vector3& origin ) = 0;
	...
	virtual int					PointReachableAreaNum( const Vector3& origin, const budBounds& bounds, const int areaFlags ) const = 0;
	...
	virtual void				PushPointIntoAreaNum( int areaNum, Vector3& origin ) const = 0;
	...
	// Get an edge.
	virtual void				GetEdge( int edgeNum, Vector3& start, Vector3& end ) const = 0;
	...
	virtual int					TravelTimeToGoalArea( int areaNum, const Vector3& origin, int goalAreaNum, int travelFlags ) const = 0;
	// Get the travel time and first reachability to be used towards the goal, returns true if there is a path.
	virtual bool				RouteToGoalArea( int areaNum, const Vector3 origin, int goalAreaNum, int travelFlags, int& travelTime, idReachability** reach ) const = 0;
	// Creates a walk path towards the goal.
	virtual bool				WalkPathToGoal( aasPath_t& path, int areaNum, const Vector3& origin, int goalAreaNum, const Vector3& goalOrigin, int travelFlags ) const = 0;
	// Returns true if one can walk along a straight line from the origin to the goal origin.
	virtual bool				WalkPathValid( int areaNum, const Vector3& origin, int goalAreaNum, const Vector3& goalOrigin, int travelFlags, Vector3& endPos, int& endAreaNum ) const = 0;
	// Creates a fly path towards the goal.
	virtual bool				FlyPathToGoal( aasPath_t& path, int areaNum, const Vector3& origin, int goalAreaNum, const Vector3& goalOrigin, int travelFlags ) const = 0;
	// Returns true if one can fly along a straight line from the origin to the goal origin.
	virtual bool				FlyPathValid( int areaNum, const Vector3& origin, int goalAreaNum, const Vector3& goalOrigin, int travelFlags, Vector3& endPos, int& endAreaNum ) const = 0;
	// Show the walk path from the origin towards the area.
	virtual void				ShowWalkPath( const Vector3& origin, int goalAreaNum, const Vector3& goalOrigin ) const = 0;
	// Show the fly path from the origin towards the area.
	virtual void				ShowFlyPath( const Vector3& origin, int goalAreaNum, const Vector3& goalOrigin ) const = 0;
	// Find the nearest goal which satisfies the callback.
	virtual bool				FindNearestGoal( aasGoal_t& goal, int areaNum, const Vector3 origin, const Vector3& target, int travelFlags, aasObstacle_t* obstacles, int numObstacles, budAASCallback& callback ) const = 0;
};
