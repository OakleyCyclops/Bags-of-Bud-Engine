...
class budAI_Vagary : public budAI
{
public:
...
CLASS_DECLARATION( budAI, budAI_Vagary )
EVENT( AI_Vagary_ChooseObjectToThrow,	budAI_Vagary::Event_ChooseObjectToThrow )
EVENT( AI_Vagary_ThrowObjectAtEnemy,	budAI_Vagary::Event_ThrowObjectAtEnemy )
END_CLASS
...
void budAI_Vagary::Event_ChooseObjectToThrow( const budVec3& mins, const budVec3& maxs, float speed, float minDist, float offset )
{
	...
	for( i = 0; i < numListedEntities; i++, index++ )
	{
		...
		budBounds expandedBounds = myBounds.Expand( entPhys->GetBounds().GetRadius() );
...
/*
================
budAI_Vagary::Event_ThrowObjectAtEnemy
================
*/
