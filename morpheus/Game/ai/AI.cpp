...
bool budAASFindCover::TestArea( const budAAS* aas, int areaNum )
{
	...
	numPVSAreas = gameLocal.pvs.GetPVSAreas( budBounds( areaCenter ).Expand( 16.0f ), PVSAreas, idEntity::MAX_PVS_AREAS );
...
/*
============
budAASFindAreaOutOfRange::TestArea
============
*/
...
/*
============
budAASFindAttackPosition::TestArea
============
*/
...
=====================
*/
void budAI::Save( idSaveGame* savefile ) const
{
	int i;
...
=====================
*/
void budAI::Restore( idRestoreGame* savefile )
{
	bool		restorePhysics;
	int			i;
	int			num;
...
void budAI::Spawn()
{
	...
	if( head.GetEntity() )
	{
		budAnimator* headAnimator = head.GetEntity()->GetAnimator();
	...
	physicsObj.SetClipModel( new( TAG_PHYSICS_CLIP_ENTITY ) budClipModel( GetPhysics()->GetClipModel() ), 1.0f );
...
void budAI::Gib( const budVec3& dir, const char* damageDefName )
{
	...
	budActor::Gib( dir, damageDefName );
}

/*
===================
budAI::InitMuzzleFlash
===================
*/
...
void budAI::List_f( const budCmdArgs& args )
{
	...
	{
		check = static_cast<budAI*>( gameLocal.entities[ e ] );
		if( !check || !check->IsType( budAI::Type ) )
		{
			continue;
		}
...
/*
=====================
budAI::LinkScriptVariables
=====================
*/
void budAI::LinkScriptVariables()
{
...
/*
=====================
budAI::UpdateAIScript
=====================
*/
void budAI::UpdateAIScript()
{
	UpdateScript();
...
============
*/
bool ValidForBounds( const budAASSettings* settings, const budBounds& bounds )
{
	int i;
...
void budAI::SetAAS()
{
	...
	if( aas )
	{
		const budAASSettings* settings = aas->GetSettings();
		if( settings )
		{
...
bool budAI::ReachedPos( const budVec3& pos, const moveCommand_t moveCommand ) const
{
	...
	else
	{
		...
		{
			if( physicsObj.GetAbsBounds().IntersectsBounds( budBounds( pos ).Expand( 8.0f ) ) )
			{
				return true;
			}
		}
...
=====================
*/
void budAI::StopMove( moveStatus_t status )
{
	AI_MOVE_DONE		= true;
	AI_FORWARD			= false;
...
=====================
*/
bool budAI::FaceEntity( idEntity* ent )
{
	if( !ent )
	{
		StopMove( MOVE_STATUS_DEST_NOT_FOUND );
...
/*
=====================
budAI::MoveToEnemyHeight
=====================
*/
bool budAI::MoveToEnemyHeight()
{
	budActor*	enemyEnt = enemy.GetEntity();
...
=====================
*/
bool budAI::MoveToEntity( idEntity* ent )
{
	int			areaNum;
	aasPath_t	path;
	budVec3		pos;
...
/*
=====================
budAI::MoveOutOfRange
=====================
*/
bool budAI::MoveOutOfRange( idEntity* ent, float range )
{
	int				areaNum;
	...
	budAASFindAreaOutOfRange findGoal( pos, range );
...
/*
=====================
budAI::MoveToAttackPosition
=====================
*/
bool budAI::MoveToAttackPosition( idEntity* ent, int attack_anim )
{
	int				areaNum;
	aasObstacle_t	obstacle;
	aasGoal_t		goal;
	...
	budAASFindAttackPosition findGoal( this, physicsObj.GetGravityAxis(), ent, pos, missileLaunchOffset[ attack_anim ] );
	if( !aas->FindNearestGoal( goal, areaNum, org, pos, travelFlags, &obstacle, 1, findGoal ) )
	{
		StopMove( MOVE_STATUS_DEST_UNREACHABLE );
		AI_DEST_UNREACHABLE = true;
...
bool budAI::MoveToCover( idEntity* entity, const budVec3& hideFromPos )
{
	...
	budAASFindCover findCover( hideFromPos );
...
bool budAI::StepDirection( float dir )
{
	...
	budAI::PredictPath( this, aas, org, move.moveDir * 48.0f, 1000, 1000, ( move.moveType == MOVETYPE_FLY ) ? SE_BLOCKED : ( SE_ENTER_OBSTACLE | SE_BLOCKED | SE_ENTER_LEDGE_AREA ), path );
	...
	if( ( move.moveType == MOVETYPE_FLY ) && ( path.endEvent == SE_BLOCKED ) )
	{
		...
		for( z = org.z; z <= ceilingPos.z + 64.0f; z += 64.0f )
		{
			...
			budAI::PredictPath( this, aas, start, move.moveDir * 48.0f, 1000, 1000, SE_BLOCKED, path );
			if( !path.endEvent )
			{
				move.moveDir.z = 1.0f;
				return true;
...
bool budAI::EntityCanSeePos( budActor* actor, const budVec3& actorOrigin, const budVec3& pos )
{
	...
	const budBounds& bounds = physicsObj.GetBounds();
...
/*
=====================
budAI::BlockedFailSafe
=====================
*/
void budAI::BlockedFailSafe()
{
...
void budAI::ApplyImpulse( idEntity* ent, int id, const budVec3& point, const budVec3& impulse )
{
	...
	{
		budActor::ApplyImpulse( ent, id, point, impulse );
	}
}
...
void budAI::CheckObstacleAvoidance( const budVec3& goalPos, budVec3& newPos )
{
	...
	if( obstacle )
	{
		if( obstacle->IsType( budActor::Type ) )
		{
...
=====================
*/
int budAI::ReactionTo( const idEntity* ent )
{
...
bool budAI::Pain( idEntity* inflictor, idEntity* attacker, int damage, const budVec3& dir, int location )
{
	...
	AI_PAIN = budActor::Pain( inflictor, attacker, damage, dir, location );
	AI_DAMAGE = true;
	...
	if( attacker != this )
	{
		...
		if( enemy.GetEntity() != attacker && attacker->IsType( budActor::Type ) )
		{
			actor = ( budActor* )attacker;
...
/*
=====================
budAI::SpawnParticles
=====================
*/
void budAI::SpawnParticles( const char* keyName )
{
...
const budDeclParticle* budAI::SpawnParticlesOnJoint( particleEmitter_t& pe, const char* particleName, const char* jointName )
{
	...
	else
	{
		...
		pe.particle = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, particleName ) );
...
void budAI::Killed( idEntity* inflictor, idEntity* attacker, int damage, const budVec3& dir, int location )
{
	...
	if( attacker && attacker->IsType( budActor::Type ) )
	{
		gameLocal.AlertAI( ( budActor* )attacker );
	}
...
=====================
*/
void budAI::TalkTo( budActor* actor )
{
...
=====================
*/
talkState_t budAI::GetTalkState() const
{
...
/*
=====================
budAI::TouchedByFlashlight
=====================
*/
void budAI::TouchedByFlashlight( budActor* flashlight_owner )
{
	if( wakeOnFlashlight )
	{
...
/*
=====================
budAI::SetEnemyPosition
=====================
*/
void budAI::SetEnemyPosition()
{
	budActor*		enemyEnt = enemy.GetEntity();
	int			enemyAreaNum;
	int			areaNum;
...
/*
=====================
budAI::UpdateEnemyPosition
=====================
*/
void budAI::UpdateEnemyPosition()
{
	budActor* enemyEnt = enemy.GetEntity();
	int				enemyAreaNum;
	int				areaNum;
...
=====================
*/
void budAI::SetEnemy( budActor* newEnemy )
{
	int enemyAreaNum;
...
/*
===================
budAI::CalculateAttackOffsets

calculate joint positions on attack frames so we can do proper "can hit" tests
===================
*/
void budAI::CalculateAttackOffsets()
{
...
/*
=====================
budAI::CreateProjectileClipModel
=====================
*/
void budAI::CreateProjectileClipModel() const
{
...
bool budAI::GetAimDir( const budVec3& firePos, idEntity* aimAtEnt, const idEntity* ignore, budVec3& aimDir ) const
{
	...
	{
		static_cast<budActor*>( aimAtEnt )->GetAIAimTargets( lastVisibleEnemyPos, targetPos1, targetPos2 );
	}
	else if( aimAtEnt->IsType( budActor::Type ) )
	{
		static_cast<budActor*>( aimAtEnt )->GetAIAimTargets( aimAtEnt->GetPhysics()->GetOrigin(), targetPos1, targetPos2 );
	}
	else
	{
	...
	if( result || !aimAtEnt->IsType( budActor::Type ) )
	{
		return result;
	}
...
=====================
*/
void budAI::BeginAttack( const char* name )
{
	attack = name;
...
/*
=====================
budAI::RemoveProjectile
=====================
*/
void budAI::RemoveProjectile()
{
	if( projectile.GetEntity() )
	{
...
================
*/
void budAI::DamageFeedback( idEntity* victim, idEntity* inflictor, int& damage )
{
...
void budAI::PushWithAF()
{
	...
	for( i = 0; i < num; i++ )
	{
		...
		if( j >= num_pushed )
		{
			...
			vel.Normalize();
			if( attack.Length() && ent->IsType( budActor::Type ) )
			{
...
/*
================
budAI::UpdateMuzzleFlash
================
*/
void budAI::UpdateMuzzleFlash()
{
	if( worldMuzzleFlashHandle != -1 )
	{
...
/*
================
budAI::CanPlayChatterSounds

Used for playing chatter sounds on monsters.
================
*/
bool budAI::CanPlayChatterSounds() const
{
	if( AI_DEAD )
	{
		return false;
...
/*
=====================
budAI::UpdateParticles
=====================
*/
void budAI::UpdateParticles()
{
...
/*
=====================
budAI::TriggerParticles
=====================
*/
void budAI::TriggerParticles( const char* jointName )
{
...
void budAI::TriggerFX( const char* joint, const char* fx )
{
...
idEntity* budAI::StartEmitter( const char* name, const char* joint, const char* particle )
{
...
idEntity* budAI::GetEmitter( const char* name )
{
...
void budAI::StopEmitter( const char* name )
{
	funcEmitter_t* emitter;
	funcEmitters.Get( name, &emitter );
	if( emitter )
...
/*
================
budAI::UpdateAnimationControllers
================
*/
bool budAI::UpdateAnimationControllers()
{
	...
	if( AI_DEAD )
	{
		return budActor::UpdateAnimationControllers();
	}
	...
	else if( focusEnt->IsType( budActor::Type ) )
	{
		focusPos = static_cast<budActor*>( focusEnt )->GetEyePosition() - eyeVerticalOffset * focusEnt->GetPhysics()->GetGravityNormal();
	}
	else
	{
	...
	if( headEnt )
	{
		budAnimator* headAnimator = headEnt->GetAnimator();
...
bool idCombatNode::EntityInView( budActor* actor, const budVec3& pos )
{
	...
	const budBounds& bounds = actor->GetPhysics()->GetBounds();
