...
void idGrabber::StartDrag( idEntity* grabEnt, int id )
{
	...
	else if( grabEnt->IsType( budAFEntity_Gibbable::Type ) )
	{
		holdingAF = true;
		clipModelId = 0;
		...
		{
			budAI* aiEnt = static_cast<budAI*>( grabEnt );
...
void idGrabber::StopDrag( bool dropOnly )
{
	...
	if( dragEnt.IsValid() )
	{
		...
		if( holdingAF )
		{
			budAFEntity_Gibbable* af = static_cast<budAFEntity_Gibbable*>( ent );
	...
	if( !dragEnt.GetEntity() )
	{
		...
		if( trace.fraction < 1.0f )
		{
			...
			if( newEnt && ( newEnt->IsType( idMoveable::Type ) ||
							...
							newEnt->IsType( budAFEntity_Gibbable::Type )
						  ) &&
					newEnt->noGrab == false &&
					...
					newEnt->GetPhysics()->GetLinearVelocity().LengthSqr() < MAX_PICKUP_VELOCITY )
			{
				...
				if( newEnt->IsType( budAFEntity_Gibbable::Type ) )
				{
					budAFEntity_Gibbable* afEnt = static_cast<budAFEntity_Gibbable*>( newEnt );
				...
				if( valbudAF && player->usercmd.buttons & BUTTON_ATTACK )
				{
