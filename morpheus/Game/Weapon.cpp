...
// class def
//
CLASS_DECLARATION( budAnimatedEntity, idWeapon )
...
void idWeapon::Spawn()
{
	if( !common->IsClient() )
	{
		...
		worldModel = static_cast< budAnimatedEntity* >( gameLocal.SpawnEntityType( budAnimatedEntity::Type, NULL ) );
...
void idWeapon::CacheWeapon( const char* weaponName )
{
	...
	if( brassDefName[0] )
	{
		const budDeclEntityDef* brassDef = gameLocal.FindEntityDef( brassDefName, false );
		if( brassDef )
		{
...
void idWeapon::Restore( idRestoreGame* savefile )
{
	...
	const budDeclEntityDef* projectileDef = gameLocal.FindEntityDef( weaponDef->dict.GetString( "def_projectile" ), false );
	if( projectileDef )
	{
	...
	const budDeclEntityDef* brassDef = gameLocal.FindEntityDef( weaponDef->dict.GetString( "def_ejectBrass" ), false );
	if( brassDef )
	{
	...
	for( int i = 0; i < particleCount; i++ )
	{
		...
		{
			newParticle.particle = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, particlename, false ) );
		}
		else
		{
...
================
*/
void idWeapon::InitWorldModel( const budDeclEntityDef* def )
{
	idEntity* ent;
...
void idWeapon::GetWeaponDef( const char* objectname, int ammoinclip )
{
	...
	if( *smokeName != '\0' )
	{
		weaponSmoke = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, smokeName ) );
	}
	else
	{
		weaponSmoke = NULL;
	...
	if( *smokeName != '\0' )
	{
		strikeSmoke = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, smokeName ) );
	}
	else
	{
		strikeSmoke = NULL;
	...
	{
		const budDeclEntityDef* projectileDef = gameLocal.FindEntityDef( projectileName, false );
		if( !projectileDef )
		{
	...
	if( !common->IsMultiplayer() )
	{
		...
		while( pkv )
		{
			...
			if( newParticle.smoke )
			{
				newParticle.particle = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, particle, false ) );
			}
			else
			{
				idDict args;
...
{
	return budLocalization::GetString( displayName );
}
...
{
	return budLocalization::GetString( itemDesc );
}
...
bool idWeapon::BloodSplat( float size )
{
	...
	const budMaterial* mtr = declManager->FindMaterial( "textures/decals/duffysplatgun" );
...
void idWeapon::AlertMonsters()
{
	...
	if( tr.fraction < 1.0f )
	{
		...
		{
			static_cast<budAI*>( ent )->TouchedByFlashlight( owner );
		}
...
================
*/
void idWeapon::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idWeapon::ReadFromSnapshot( const budBitMsg& msg )
{
...
================
*/
bool idWeapon::ClientReceiveEvent( int event, int time, const budBitMsg& msg )
{
	...
	switch( event )
	{
		...
		case EVENT_CHANGESKIN:
		{
			...
			renderEntity.customSkin = ( index != -1 ) ? static_cast<const budDeclSkin*>( declManager->DeclByIndex( DECL_SKIN, index ) ) : NULL;
			UpdateVisuals();
			if( worldModel.GetEntity() )
			{
...
{
	status = WP_READY;
	libBud::PrintfIf( g_debugWeapon.GetBool(), "Weapon Status WP_READY \n" );
	if( isLinked )
	{
		WEAPON_RAISEWEAPON = false;
	}
...
{
	status = WP_OUTOFAMMO;
	libBud::PrintfIf( g_debugWeapon.GetBool(), "Weapon Status WP_OUTOFAMMO \n" );
	if( isLinked )
	{
		WEAPON_RAISEWEAPON = false;
	}
...
{
	status = WP_RELOAD;
	libBud::PrintfIf( g_debugWeapon.GetBool(), "Weapon Status WP_RELOAD \n" );
}
...
{
	status = WP_HOLSTERED;
	libBud::PrintfIf( g_debugWeapon.GetBool(), "Weapon Status WP_HOLSTERED \n" );
	if( isLinked )
	{
		WEAPON_LOWERWEAPON = false;
	}
...
{
	status = WP_RISING;
	libBud::PrintfIf( g_debugWeapon.GetBool(), "Weapon Status WP_RISING \n" );
	if( isLinked )
	{
		WEAPON_LOWERWEAPON = false;
	}
...
{
	status = WP_LOWERING;
	libBud::PrintfIf( g_debugWeapon.GetBool(), "Weapon Status WP_LOWERING \n" );
	if( isLinked )
	{
		WEAPON_RAISEWEAPON = false;
	}
...
{
	const budDeclSkin* skinDecl = NULL;
	...
	if( actuallySpawnProjectile )
	{
		...
		for( i = 0; i < num_projectiles; i++ )
		{
			...
			if( common->IsServer() && owner != NULL && !owner->IsLocallyControlled() && !projectileDict.GetBool( "net_instanthit" ) )
			{
				...
				{
					libBud::Printf( "Spawning throw item projectile for player %d. PredictiveKey: %d \n", owner->GetEntityNumber(), predictedKey );
				}
			}
			else
			{
