...
/*
================
budGameEdit::ParseSpawnArgsToRenderEntity
...
/*
================
budGameEdit::ParseSpawnArgsToRefSound
...
void idEntity::Spawn()
{
	...
	const budDeclEntityDef* def = gameLocal.FindEntityDef( classname, false );
	if( def )
	{
...
================
*/
void idEntity::SetSkin( const budDeclSkin* skin )
{
...
================
*/
const budDeclSkin* idEntity::GetSkin() const
{
...
void idEntity::UpdateModel()
{
	...
	budAnimator* animator = GetAnimator();
...
void idEntity::UpdatePVSAreas()
{
	...
	{
		localNumPVSAreas = gameLocal.pvs.GetPVSAreas( budBounds( renderEntity.origin ).Expand( 64.0f ), localPVSAreas, sizeof( localPVSAreas ) / sizeof( localPVSAreas[0] ) );
	}
...
void idEntity::UpdatePVSAreas( const Vector3& pos )
{
	...
	numPVSAreas = gameLocal.pvs.GetPVSAreas( budBounds( pos ), PVSAreas, MAX_PVS_AREAS );
	i = numPVSAreas;
...
void idEntity::ProjectOverlay( const Vector3& origin, const Vector3& dir, float size, const char* material )
{
	...
	const budMaterial* mtr = declManager->FindMaterial( material );
...
================
*/
budAnimator* idEntity::GetAnimator()
{
	return NULL;
}
...
bool idEntity::InitBind( idEntity* master )
{
	...
	if( master && IsType( budAFEntity_Base::Type ) )
	{
		static_cast<budAFEntity_Base*>( this )->AddBindConstraints();
	}
...
void idEntity::Unbind()
{
	...
	if( IsType( budAFEntity_Base::Type ) )
	{
		static_cast<budAFEntity_Base*>( this )->RemoveBindConstraints();
	}
...
void idEntity::InitDefaultPhysics( const Vector3& origin, const Matrix3& axis )
{
	...
	{
		if( budClipModel::CheckModel( temp ) )
		{
			clipModel = new( TAG_PHYSICS_CLIP_ENTITY ) budClipModel( temp );
		}
	}
	...
	if( !spawnArgs.GetBool( "noclipmodel", "0" ) )
	{
		...
		if( !clipModel )
		{
			...
			if( setClipModel )
			{
				...
				clipModel = new( TAG_PHYSICS_CLIP_ENTITY ) budClipModel( trm );
			}
		}
...
void idEntity::UpdateFromPhysics( bool moveBack )
{
	...
	if( IsType( budActor::Type ) )
	{
		budActor* actor = static_cast<budActor*>( this );
...
void idEntity::SetAxis( const Matrix3& axis )
{
	...
	{
		static_cast<budActor*>( this )->viewAxis = axis;
	}
	else
	{
...
void idEntity::AddDamageEffect( const trace_t& collision, const Vector3& velocity, const char* damageDefName )
{
	...
	const budDeclEntityDef* def = gameLocal.FindEntityDef( damageDefName, false );
	if( def == NULL )
	{
		return;
	}
...
Can be overridden by subclasses when a thread doesn't need to be allocated.
Not called during budGameLocal::MapShutdown.
================
*/
...
bool idEntity::HandleGuiCommands( idEntity* entityGui, const char* cmds )
{
	...
	{
		budLexer src;
		budToken token, token2, token3, token4;
		...
		while( 1 )
		{
			...
			if( !token.Icmp( "turkeyscore" ) )
			{
				if( src.ReadToken( &token2 ) && entityGui->renderEntity.gui[0] )
				{
					...
					if( gameLocal.GetLocalPlayer() && score >= 25000 )
					{
						...
						gameLocal.GetLocalPlayer()->GiveEmail( static_cast<const budDeclEmail*>( declManager->FindType( DECL_EMAIL, "highScore", false ) ) );
					}
				}
				continue;
			}
			...
			{
				gameLocal.GetLocalPlayer()->GiveEmail( static_cast<const budDeclEmail*>( declManager->FindType( DECL_EMAIL, "MartianBuddyGameComplete", false ) ) );
				continue;
			}
...
================
*/
bool idEntity::HandleSingleGuiCommand( idEntity* entityGui, budLexer* src )
{
	return false;
}
		...
		if( parent )
		{
			...
			if( spawnArgs.GetString( "bindToJoint", "", &joint ) && *joint )
			{
				...
				if( ( parent->spawnArgs.GetString( "bindanim", "", &bindanim ) || parent->spawnArgs.GetString( "anim", "", &bindanim ) ) && *bindanim )
				{
					...
					budJointMat* frame = parent->renderEntity.joints;
...
void idEntity::Event_Touches( idEntity* ent )
{
	...
	const budBounds& myBounds = GetPhysics()->GetAbsBounds();
...
================
*/
void idEntity::WriteBindToSnapshot( budBitMsg& msg ) const
{
	int bindInfo;
...
================
*/
void idEntity::ReadBindFromSnapshot( const budBitMsg& msg )
{
...
================
*/
void idEntity::WriteGUIToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idEntity::ReadGUIFromSnapshot( const budBitMsg& msg )
{
	int state;
...
================
*/
void idEntity::WriteToSnapshot( budBitMsg& msg ) const
{
}
...
================
*/
void idEntity::ReadFromSnapshot( const budBitMsg& msg )
{
}
...
================
*/
void idEntity::ReadFromSnapshot_Ex( const budBitMsg&  msg )
{
	snapshotsReceived += 1;
...
================
*/
void idEntity::ServerSendEvent( int eventId, const budBitMsg* msg, bool saveEvent, lobbyUserID_t excluding ) const
{
	budBitMsg	outMsg;
...
================
*/
void idEntity::ClientSendEvent( int eventId, const budBitMsg* msg ) const
{
	budBitMsg	outMsg;
	byte		msgBuf[MAX_GAME_MESSAGE_SIZE];
...
================
*/
bool idEntity::ServerReceiveEvent( int event, int time, const budBitMsg& msg )
{
	switch( event )
	{
		case 0:
...
================
*/
bool idEntity::ClientReceiveEvent( int event, int time, const budBitMsg& msg )
{
	int					index;
	const idSoundShader*	shader;
...
CLASS_DECLARATION( idEntity, budAnimatedEntity )
EVENT( EV_GetJointHandle,		budAnimatedEntity::Event_GetJointHandle )
EVENT( EV_ClearAllJoints,		budAnimatedEntity::Event_ClearAllJoints )
EVENT( EV_ClearJoint,			budAnimatedEntity::Event_ClearJoint )
EVENT( EV_SetJointPos,			budAnimatedEntity::Event_SetJointPos )
EVENT( EV_SetJointAngle,		budAnimatedEntity::Event_SetJointAngle )
EVENT( EV_GetJointPos,			budAnimatedEntity::Event_GetJointPos )
EVENT( EV_GetJointAngle,		budAnimatedEntity::Event_GetJointAngle )
END_CLASS

/*
================
budAnimatedEntity::budAnimatedEntity
================
*/
budAnimatedEntity::budAnimatedEntity()
{
...
/*
================
budAnimatedEntity::Save

archives object for save game file
================
*/
void budAnimatedEntity::Save( idSaveGame* savefile ) const
{
...
/*
================
budAnimatedEntity::Restore

unarchives object from save game file
================
*/
void budAnimatedEntity::Restore( idRestoreGame* savefile )
{
...
/*
================
budAnimatedEntity::ClientPredictionThink
================
*/
void budAnimatedEntity::ClientPredictionThink()
{
	RunPhysics();
	UpdateAnimation();
	Present();
...
/*
================
budAnimatedEntity::ClientThink
================
*/
void budAnimatedEntity::ClientThink( const int curTime, const float fraction, const bool predict )
{
	InterpolatePhysics( fraction );
	UpdateAnimation();
	Present();
...
/*
================
budAnimatedEntity::UpdateAnimation
================
*/
void budAnimatedEntity::UpdateAnimation()
{
...
/*
================
budAnimatedEntity::GetAnimator
================
*/
budAnimator* budAnimatedEntity::GetAnimator()
{
	return &animator;
}
...
/*
================
budAnimatedEntity::SetModel
================
*/
void budAnimatedEntity::SetModel( const char* modelname )
{
	FreeModelDef();
...
bool budAnimatedEntity::GetJointTransformForAnim( jointHandle_t jointHandle, int animNum, int frameTime, Vector3& offset, Matrix3& axis ) const
{
	...
	frame = ( budJointMat* )_alloca16( numJoints * sizeof( budJointMat ) );
...
/*
==============
budAnimatedEntity::AddDamageEffect
...
/*
==============
budAnimatedEntity::GetDefaultSurfaceType
==============
*/
int	budAnimatedEntity::GetDefaultSurfaceType() const
{
...
void budAnimatedEntity::AddLocalDamageEffect( jointHandle_t jointNum, const Vector3& localOrigin, const Vector3& localNormal, const Vector3& localDir, const budDeclEntityDef* def, const budMaterial* collisionMaterial )
{
	...
	if( *bleed != '\0' )
	{
		...
		de->type = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, bleed ) );
...
/*
==============
budAnimatedEntity::UpdateDamageEffects
==============
*/
void budAnimatedEntity::UpdateDamageEffects()
{
...
/*
================
budAnimatedEntity::ClientReceiveEvent
================
*/
bool budAnimatedEntity::ClientReceiveEvent( int event, int time, const budBitMsg& msg )
{
	int damageDefIndex;
	int materialIndex;
	jointHandle_t jointNum;
	...
	switch( event )
	{
		case EVENT_ADD_DAMAGE_EFFECT:
		{
			...
			const budDeclEntityDef* damageDef = static_cast<const budDeclEntityDef*>( declManager->DeclByIndex( DECL_ENTITYDEF, damageDefIndex ) );
			const budMaterial* collisionMaterial = static_cast<const budMaterial*>( declManager->DeclByIndex( DECL_MATERIAL, materialIndex ) );
...
/*
================
budAnimatedEntity::Event_GetJointHandle

looks up the number of the specified joint.  returns INVALID_JOINT if the joint is not found.
================
*/
void budAnimatedEntity::Event_GetJointHandle( const char* jointname )
{
	jointHandle_t joint;
...
/*
================
budAnimatedEntity::Event_ClearAllJoints

removes any custom transforms on all joints
================
*/
void budAnimatedEntity::Event_ClearAllJoints()
{
...
/*
================
budAnimatedEntity::Event_ClearJoint

removes any custom transforms on the specified joint
================
*/
void budAnimatedEntity::Event_ClearJoint( jointHandle_t jointnum )
{
...
/*
================
budAnimatedEntity::Event_SetJointPos
...
/*
================
budAnimatedEntity::Event_SetJointAngle
...
/*
================
budAnimatedEntity::Event_GetJointPos
...
/*
================
budAnimatedEntity::Event_GetJointAngle
