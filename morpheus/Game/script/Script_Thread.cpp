...
void idThread::Event_Trace( const Vector3& start, const Vector3& end, const Vector3& mins, const Vector3& maxs, int contents_mask, idEntity* passEntity )
{
	...
	{
		gameLocal.clip.TraceBounds( trace, start, end, budBounds( mins, maxs ), contents_mask, passEntity );
	}
...
void idThread::Event_GetTraceJoint()
{
	...
	{
		budAFEntity_Base* af = static_cast<budAFEntity_Base*>( gameLocal.entities[ trace.c.entityNum ] );
...
void idThread::Event_GetTraceBody()
{
	if( trace.fraction < 1.0f && trace.c.id < 0 )
	{
		...
		if( af && af->IsType( budAFEntity_Base::Type ) && af->IsActiveAF() )
		{
			...
			budAFBody* body = af->GetAFPhysics()->GetBody( bodyId );
			if( body )
			{
