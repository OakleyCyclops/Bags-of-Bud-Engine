...
struct idNetEvent
{
	...
	void	Serialize( budSerializer& ser )
	{
		if( count >= max )
		{
			libBud::Warning( "idNetEvent. count %d > max %d", count, max );
		}
...
public:
	...
	void					SetSkin( const budDeclSkin* skin );
	...
	virtual budAnimator* 	GetAnimator();	// returns animator object used by this entity
	...
	void					ReadFromSnapshot_Ex( const budBitMsg& msg );
	...
	void					WriteBindToSnapshot( budBitMsg& msg ) const;
	void					ReadBindFromSnapshot( const budBitMsg& msg );
	void					WriteColorToSnapshot( budBitMsg& msg ) const;
	void					ReadColorFromSnapshot( const budBitMsg& msg );
	void					WriteGUIToSnapshot( budBitMsg& msg ) const;
	void					ReadGUIFromSnapshot( const budBitMsg& msg );
	
	void					ServerSendEvent( int eventId, const budBitMsg* msg, bool saveEvent, lobbyUserID_t excluding = lobbyUserID_t() ) const;
	void					ClientSendEvent( int eventId, const budBitMsg* msg ) const;
...
class budAnimatedEntity : public idEntity
{
public:
	CLASS_PROTOTYPE( budAnimatedEntity );
...
BUD_INLINE SetTimeState::SetTimeState()
{
	activated = false;
}

BUD_INLINE SetTimeState::SetTimeState( int timeGroup )
{
	activated = false;
	PushState( timeGroup );
}

BUD_INLINE void SetTimeState::PushState( int timeGroup )
{
