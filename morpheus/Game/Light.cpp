...
/*
================
budGameEdit::ParseSpawnArgsToRenderLight
...
void idLight::Spawn()
{
	...
	if( health )
	{
		...
		budClipModel::CheckModel( brokenModel );
	}
...
void idLight::BecomeBroken( idEntity* activator )
{
	...
	if( brokenModel.Length() )
	{
		...
		{
			GetPhysics()->SetClipModel( new( TAG_PHYSICS_CLIP_ENTITY ) budClipModel( brokenModel.c_str() ), 1.0f );
...
================
*/
void idLight::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
bool idLight::ClientReceiveEvent( int event, int time, const budBitMsg& msg )
{
