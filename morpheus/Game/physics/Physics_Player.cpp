...
void idPhysics_Player::CheckDuck()
{
	...
	if( clipModel->GetBounds()[1][2] != maxZ )
	{
		...
		{
			clipModel->LoadModel( budTraceModel( bounds, 8 ) );
		}
		else
		{
			clipModel->LoadModel( budTraceModel( bounds ) );
		}
	}
}
...
bool idPhysics_Player::Interpolate( const float fraction )
{
	...
	*/
	...
	{
		libBud::Printf( "Client Interpolation Divergence exceeded, snapping client to next position\n" );
...
================
*/
void idPhysics_Player::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idPhysics_Player::ReadFromSnapshot( const budBitMsg& msg )
{
