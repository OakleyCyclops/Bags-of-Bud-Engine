...
===============================================================
	...
	budClipModel trace model cache
...
===============
*/
void budClipModel::ClearTraceModelCache()
{
...
/*
===============
budClipModel::TraceModelCacheSize
===============
*/
int budClipModel::TraceModelCacheSize()
{
	return traceModelCache.Num() * sizeof( budTraceModel );
}

/*
===============
budClipModel::AllocTraceModel
===============
*/
int budClipModel::AllocTraceModel( const budTraceModel& trm, bool persistantThroughSaves )
{
...
/*
===============
budClipModel::FreeTraceModel
===============
*/
void budClipModel::FreeTraceModel( int traceModelIndex )
{
	...
	if( traceModelIndex & TRACE_MODEL_SAVED )
	{
		...
		{
			gameLocal.Warning( "budClipModel::FreeTraceModel: tried to free uncached trace model" );
			return;
		}
...
/*
===============
budClipModel::GetCachedTraceModel
===============
*/
budTraceModel* budClipModel::GetCachedTraceModel( int traceModelIndex )
{
	int realTraceModelIndex = traceModelIndex & ~TRACE_MODEL_SAVED;
...
===============
*/
trmCache_t* budClipModel::GetTraceModelEntry( int traceModelIndex )
{
...
/*
===============
budClipModel::GetTraceModelHashKey
===============
*/
...
/*
===============
budClipModel::SaveTraceModels
===============
*/
void budClipModel::SaveTraceModels( idSaveGame* savefile )
{
	int i;
...
/*
===============
budClipModel::RestoreTraceModels
===============
*/
void budClipModel::RestoreTraceModels( idRestoreGame* savefile )
{
	int i, num;
...
/*
================
budClipModel::LoadModel
================
*/
bool budClipModel::LoadModel( const char* name )
{
	renderModelHandle = -1;
...
================
*/
void budClipModel::LoadModel( const budTraceModel& trm, bool persistantThroughSave )
{
...
================
*/
void budClipModel::LoadModel( const int renderModelHandle )
{
	collisionModelHandle = 0;
...
================
*/
budClipModel::budClipModel( const char* name )
{
	Init();
	LoadModel( name );
}
...
================
*/
budClipModel::budClipModel( const budTraceModel& trm )
{
	Init();
	LoadModel( trm, true );
}
...
================
*/
budClipModel::budClipModel( const budTraceModel& trm, bool persistantThroughSave )
{
	Init();
...
================
*/
budClipModel::budClipModel( const int renderModelHandle )
{
	Init();
...
================
*/
budClipModel::budClipModel( const budClipModel* model )
{
...
================
*/
void budClipModel::Save( idSaveGame* savefile ) const
{
...
================
*/
cmHandle_t budClipModel::Handle() const
{
	...
	else
	{
		...
		gameLocal.Warning( "budClipModel::Handle: clip model %d on '%s' (%x) is not a collision or trace model", id, entity->name.c_str(), entity->entityNumber );
		return 0;
	}
}
...
void budClipModel::GetMassProperties( const float density, float& mass, budVec3& centerOfMass, budMat3& inertiaTensor ) const
{
	if( traceModelIndex == -1 )
	{
		gameLocal.Error( "budClipModel::GetMassProperties: clip model %d on '%s' is not a trace model\n", id, entity->name.c_str() );
	}
...
===============
*/
void budClipModel::Link_r( struct clipSector_s* node )
{
	clipLink_t* link;
...
===============
*/
void budClipModel::Link( idClip& clp )
{

	assert( budClipModel::entity );
	if( !budClipModel::entity )
	{
		return;
	}
...
/*
============
budClipModel::CheckModel
============
*/
cmHandle_t budClipModel::CheckModel( const char* name )
{
...
void idClip::Shutdown()
{
	...
	{
		budClipModel::FreeTraceModel( temporaryClipModel.traceModelIndex );
	...
	{
		budClipModel::FreeTraceModel( defaultClipModel.traceModelIndex );
...
void idClip::ClipModelsTouchingBounds_r( const struct clipSector_s* node, listParms_t& parms ) const
{
	...
	{
		budClipModel*	check = link->clipModel;
...
================
*/
int idClip::ClipModelsTouchingBounds( const budBounds& bounds, int contentMask, budClipModel** clipModelList, int maxCount ) const
{
	listParms_t parms;
...
================
*/
int idClip::EntitiesTouchingBounds( const budBounds& bounds, int contentMask, idEntity** entityList, int maxCount ) const
{
...
====================
*/
int idClip::GetTraceClipModels( const budBounds& bounds, int contentMask, const idEntity* passEntity, budClipModel** clipModelList ) const
{
	int i, num;
	budClipModel*	cm;
	idEntity* passOwner;
...
============
*/
const budTraceModel* idClip::TraceModelForClipModel( const budClipModel* mdl ) const
{
	if( !mdl )
	{
		return NULL;
	...
	else
	{
		...
		return budClipModel::GetCachedTraceModel( mdl->traceModelIndex );
	}
}
					  ...
					  const budClipModel* mdl, const budMat3& trmAxis, int contentMask, const idEntity* passEntity )
{
	...
	if( !trm )
	{
		traceBounds = budBounds( start ).Expand( depth );
	}
	else
	{
...
============
*/
bool idClip::GetModelContactFeature( const contactInfo_t& contact, const budClipModel* clipModel, budFixedWinding& winding ) const
{
	int i;
	cmHandle_t handle;
	budVec3 start, end;
	...
	else
	{
		...
		{
			handle = collisionModelManager->SetupTrmModel( *budClipModel::GetCachedTraceModel( clipModel->traceModelIndex ), clipModel->material );
		}
		else
		{
...
void idClip::DrawClipModels( const budVec3& eye, const float radius, const idEntity* passEntity )
{
	...
	bounds = budBounds( eye ).Expand( radius );
