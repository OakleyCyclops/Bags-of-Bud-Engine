...
void idPush::SaveEntityPosition( idEntity* ent )
{
	...
	if( ent->IsType( budActor::Type ) )
	{
		...
		pushed[numPushed].deltaViewAngles = static_cast<budActor*>( ent )->GetDeltaViewAngles();
	}
...
void idPush::RestorePushedEntityPositions()
{
	...
	for( i = 0; i < numPushed; i++ )
	{
		...
		// if the entity is an actor
		if( pushed[i].ent->IsType( budActor::Type ) )
		{
			// set back the delta view angles
			static_cast<budActor*>( pushed[i].ent )->SetDeltaViewAngles( pushed[i].deltaViewAngles );
		}
								 ...
								 const Matrix3& newAxis, const Rotation& rotation )
{
	...
	if( check->IsType( budActor::Type ) && physics->IsType( idPhysics_Actor::Type ) )
	{
									 ...
									 const Vector3& newOrigin, const Vector3& translation )
{
	...
	for( i = 0; i < listedEntities; i++ )
	{
		...
		if( check->IsType( budAFEntity_Base::Type ) && check->spawnArgs.GetBool( "gib" ) )
		{
			if( static_cast<budAFEntity_Base*>( check )->IsActiveAF() )
			{
