	...
	for( int mapIdx = 0; mapIdx < maps.Num(); mapIdx++ )
	{
		...
		for( int modeIdx = 0; modeIdx < numModes; modeIdx++ )
		{
			...
			if( maps[ mapIdx ].supportedModes & BIT( modeIdx ) )
			{
				...
				const char* mapname = budLocalization::GetString( maps[ mapIdx ].mapName );
				newLeaderboardDef->boardName.Format( "%s %s", mapname, gamemode.abrevName );
				
				// sanity check.
				if( Sys_FindLeaderboardDef( boardID ) != newLeaderboardDef )
				{
					libBud::Error( "Leaderboards_Init leaderboard creation failed" );
				}
...
void LeaderboardLocal_Upload( lobbyUserID_t lobbyUserID, int gameType, leaderboardStats_t& stats )
{
	...
	{
		libBud::Warning( "LeaderboardLocal_Upload invalid leaderboard with id of %d", boardID );
	}
}
...
class idLeaderboardCallbackTest : public idLeaderboardCallback
{
	void Call()
	{
		libBud::Printf( "Leaderboard information retrieved in user callback.\n" );
		libBud::Printf( "%d total entries in leaderboard %d.\n", numRowsInLeaderboard, def->id );
		for( int i = 0; i < rows.Num(); i++ )
		{
			libBud::Printf( "%d: %s rank:%lld", i, rows[i].name.c_str(), rows[i].rank );
			for( int j = 0; j < def->numColumns; j++ )
			{
				libBud::Printf( ", score[%d]: %lld", j, rows[i].columns[j] );
			}
			libBud::Printf( "\n" );
		}
	}
...
CONSOLE_COMMAND( testLeaderboardDownload, "<id 0 - n > <start = 1> <end = 100>", 0 )
{
	...
	{
		libBud::Warning( "Sys_FindLeaderboardDef() Unable to find board %d\n", leaderboardID );
	}
