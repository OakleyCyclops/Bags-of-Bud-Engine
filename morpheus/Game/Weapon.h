...
typedef struct
{
	...
	const budDeclParticle* particle;		//Used for smoke particles
...
class idWeapon : public budAnimatedEntity
{
public:
	...
	// Flashlight
	budAnimatedEntity* 		GetWorldModel()
	{
...
private:
	...
	budHashTable<WeaponParticle_t>	weaponParticles;
	budHashTable<WeaponLight_t>		weaponLights;
	
	// sound
	const idSoundShader* 	sndHum;
	
	// new style muzzle smokes
	const budDeclParticle* 	weaponSmoke;			// null if it doesn't smoke
...
BUD_INLINE bool idWeapon::IsLinked()
{
	return isLinked;
}

BUD_INLINE bool idWeapon::IsWorldModelReady()
{
