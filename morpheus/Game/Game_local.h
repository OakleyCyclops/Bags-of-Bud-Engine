...
extern budRenderWorld* 				gameRenderWorld;
extern budSoundWorld* 				gameSoundWorld;
...
class budGameLocal : public budGame
{
public:
	...
	budHashIndex				entityHash;				// hash table to quickly find entities by name
	...
	void					RunTimeGroup2( budUserCmdMgr& userCmdMgr );
	...
	virtual void			ServerWriteSnapshot( budSnapShot& ss );
	virtual void			ProcessReliableMessage( int clientNum, int type, const budBitMsg& msg );
	virtual void			ClientReadSnapshot( const budSnapShot& ss );
	virtual void			ClientRunFrame( budUserCmdMgr& cmdMgr, bool lastPredictFrame, gameReturn_t& ret );
	...
	int						NumAAS() const;
	budAAS* 					GetAAS( int num ) const;
	budAAS* 					GetAAS( const char* name ) const;
	void					SetAASAreaState( const budBounds& bounds, const int areaContents, bool closed );
	aasHandle_t				AddAASObstacle( const budBounds& bounds );
	...
	void					ServerProcessReliableMessage( int clientNum, int type, const budBitMsg& msg );
	void					ClientProcessReliableMessage( int type, const budBitMsg& msg );
	...
	void					SetGlobalMaterial( const budMaterial* mat );
	const budMaterial* 		GetGlobalMaterial();
	...
	// MAIN MENU FUNCTIONS
	virtual void					Shell_Init( const char* filename, budSoundWorld* sw );
	...
	virtual void					StartDemoPlayback( budRenderWorld* renderworld );
	...
	virtual bool					ProcessDemoCommand( budDemoFile* readDemo );
...
private:
	...
	budStaticList<spawnSpot_t, MAX_GENTITIES> spawnSpots;
	budStaticList<idEntity*, MAX_GENTITIES> initialSpots;
	int						currentInitialSpot;
	
	budStaticList<spawnSpot_t, MAX_GENTITIES> teamSpawnSpots[2];
	budStaticList<idEntity*, MAX_GENTITIES> teamInitialSpots[2];
	...
	budArray< int, MAX_PLAYERS >	lastCmdRunTimeOnServer;
...
extern budGameLocal			gameLocal;
extern budAnimManager		animationLib;

//============================================================================

class budGameError : public idException
{
public:
	budGameError( const char* text ) : idException( text ) {}
};

//============================================================================

template< class type >
BUD_INLINE idEntityPtr<type>::idEntityPtr()
{
	spawnId = 0;
}

template< class type >
BUD_INLINE void idEntityPtr<type>::Save( idSaveGame* savefile ) const
{
	savefile->WriteInt( spawnId );
}

template< class type >
BUD_INLINE void idEntityPtr<type>::Restore( idRestoreGame* savefile )
{
	savefile->ReadInt( spawnId );
}

template< class type >
BUD_INLINE idEntityPtr<type>& idEntityPtr<type>::operator=( const type* ent )
{
	if( ent == NULL )
	{
		spawnId = 0;
...
template< class type >
BUD_INLINE idEntityPtr< type >& idEntityPtr<type>::operator=( const idEntityPtr& ep )
{
	spawnId = ep.spawnId;
	return *this;
}
...
template< class type >
BUD_INLINE bool idEntityPtr<type>::SetSpawnId( int id )
{
...
template< class type >
BUD_INLINE bool idEntityPtr<type>::IsValid() const
{
	return ( gameLocal.spawnIds[ spawnId & ( ( 1 << GENTITYNUM_BITS ) - 1 ) ] == ( spawnId >> GENTITYNUM_BITS ) );
}

template< class type >
BUD_INLINE type* idEntityPtr<type>::GetEntity() const
{
...
template< class type >
BUD_INLINE int idEntityPtr<type>::GetEntityNum() const
{
