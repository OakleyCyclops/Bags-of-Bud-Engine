...
{
	idEntity* 				touchedEnt;
	budClipModel* 			touchedClipModel;
	budAFBody* 				touchedByBody;
} afTouch_t;
...
public:
	...
	void					SetAnimator( budAnimator* a )
	{
		animator = a;
	}
...
protected:
	void					SetBase( budAFBody* body, const budJointMat* joints );
	void					AddBody( budAFBody* body, const budJointMat* joints, const char* jointName, const AFJointModType_t mod );
	
	bool					LoadBody( const budDeclAF_Body* fb, const budJointMat* joints );
	bool					LoadConstraint( const budDeclAF_Constraint* fc );
