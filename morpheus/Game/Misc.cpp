...
CLASS_DECLARATION( idEntity, budActivator )
EVENT( EV_Activate,		budActivator::Event_Activate )
END_CLASS
...
================
*/
void budActivator::Save( idSaveGame* savefile ) const
{
...
/*
===============
budActivator::Restore
================
*/
void budActivator::Restore( idRestoreGame* savefile )
{
...
/*
===============
budActivator::Activate
================
*/
void budActivator::Event_Activate( idEntity* activator )
{
	if( thinkFlags & TH_THINK )
	{
		BecomeInactive( TH_THINK );
...
void idForceField::Spawn()
{
	...
	forceField.SetClipModel( new( TAG_PHYSICS_CLIP_ENTITY ) budClipModel( GetPhysics()->GetClipModel() ) );
...
CLASS_DECLARATION( budAFEntity_Gibbable, budAnimated )
EVENT( EV_Activate,				budAnimated::Event_Activate )
EVENT( EV_Animated_Start,		budAnimated::Event_Start )
EVENT( EV_StartRagdoll,			budAnimated::Event_StartRagdoll )
EVENT( EV_AnimDone,				budAnimated::Event_AnimDone )
EVENT( EV_Footstep,				budAnimated::Event_Footstep )
EVENT( EV_FootstepLeft,			budAnimated::Event_Footstep )
EVENT( EV_FootstepRight,		budAnimated::Event_Footstep )
EVENT( EV_LaunchMissiles,		budAnimated::Event_LaunchMissiles )
EVENT( EV_LaunchMissilesUpdate,	budAnimated::Event_LaunchMissilesUpdate )
EVENT( EV_SetAnimation,			budAnimated::Event_SetAnimation )
EVENT( EV_GetAnimationLength,	budAnimated::Event_GetAnimationLength )
END_CLASS

/*
===============
budAnimated::budAnimated
================
*/
budAnimated::budAnimated()
{
...
================
*/
void budAnimated::Save( idSaveGame* savefile ) const
{
...
================
*/
void budAnimated::Restore( idRestoreGame* savefile )
{
...
void budAnimated::Spawn()
{
	...
	{
		gameLocal.Warning( "budAnimated '%s' at (%s): cannot find joint '%s' for sound playback", name.c_str(), GetPhysics()->GetOrigin().ToString( 0 ), joint );
	}
	...
	else
	{
		...
		if( !anim )
		{
			gameLocal.Error( "budAnimated '%s' at (%s): cannot find anim '%s'", name.c_str(), GetPhysics()->GetOrigin().ToString( 0 ), animname.c_str() );
		}
	}
...
/*
================
budAnimated::StartRagdoll
================
*/
bool budAnimated::StartRagdoll()
{
	// if no AF loaded
	if( !af.IsLoaded() )
	{
...
/*
=====================
budAnimated::PlayNextAnim
=====================
*/
void budAnimated::PlayNextAnim()
{
	const char* animname;
	int len;
	int cycle;
...
/*
===============
budAnimated::Event_StartRagdoll
================
*/
void budAnimated::Event_StartRagdoll()
{
	StartRagdoll();
}
...
/*
===============
budAnimated::Event_AnimDone
================
*/
void budAnimated::Event_AnimDone( int animindex )
{
	if( g_debugCinematic.GetBool() )
	{
		const budAnim* animPtr = animator.GetAnim( anim );
...
/*
===============
budAnimated::Event_Activate
================
*/
void budAnimated::Event_Activate( idEntity* _activator )
{
	if( num_anims )
	{
		PlayNextAnim();
...
/*
===============
budAnimated::Event_Start
================
*/
void budAnimated::Event_Start()
{
	int cycle;
	int len;
...
/*
===============
budAnimated::Event_Footstep
===============
*/
void budAnimated::Event_Footstep()
{
...
/*
=====================
budAnimated::Event_LaunchMissilesUpdate
=====================
*/
void budAnimated::Event_LaunchMissilesUpdate( int launchjoint, int targetjoint, int numshots, int framedelay )
{
	...
	if( !projectileDef )
	{
		gameLocal.Warning( "budAnimated '%s' at (%s): 'launchMissiles' called with unknown projectile '%s'", name.c_str(), GetPhysics()->GetOrigin().ToString( 0 ), projectilename );
		return;
	}
	...
	{
		gameLocal.Error( "budAnimated '%s' at (%s): in 'launchMissiles' call '%s' is not an idProjectile", name.c_str(), GetPhysics()->GetOrigin().ToString( 0 ), projectilename );
		return;
	}
...
/*
=====================
budAnimated::Event_LaunchMissiles
=====================
*/
void budAnimated::Event_LaunchMissiles( const char* projectilename, const char* sound, const char* launchjoint, const char* targetjoint, int numshots, int framedelay )
{
	...
	if( !projectileDef )
	{
		gameLocal.Warning( "budAnimated '%s' at (%s): unknown projectile '%s'", name.c_str(), GetPhysics()->GetOrigin().ToString( 0 ), projectilename );
		return;
	}
	...
	{
		gameLocal.Warning( "budAnimated '%s' at (%s): unknown launch joint '%s'", name.c_str(), GetPhysics()->GetOrigin().ToString( 0 ), launchjoint );
	...
	{
		gameLocal.Warning( "budAnimated '%s' at (%s): unknown target joint '%s'", name.c_str(), GetPhysics()->GetOrigin().ToString( 0 ), targetjoint );
	}
...
/*
=====================
budAnimated::Event_SetAnimation
=====================
*/
void budAnimated::Event_SetAnimation( const char* animName )
{
	...
	if( !anim )
	{
		gameLocal.Error( "budAnimated '%s' at (%s): cannot find anim '%s'", name.c_str(), GetPhysics()->GetOrigin().ToString( 0 ), animName );
	}
...
/*
=====================
budAnimated::Event_GetAnimationLength
=====================
*/
void budAnimated::Event_GetAnimationLength()
{
	float length = 0;
...
================
*/
void idStaticEntity::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idStaticEntity::ReadFromSnapshot( const budBitMsg& msg )
{
	bool hidden;
...
================
*/
void idFuncEmitter::WriteToSnapshot( budBitMsg& msg ) const
{
...
================
*/
void idFuncEmitter::ReadFromSnapshot( const budBitMsg& msg )
{
...
================
*/
void idFuncShootProjectile::WriteToSnapshot( budBitMsg& msg ) const
{
	// 	msg.WriteBits( hidden ? 1 : 0, 1 );
	// 	msg.WriteFloat( renderEntity.shaderParms[ SHADERPARM_PARTICLE_STOPTIME ] );
	// 	msg.WriteFloat( renderEntity.shaderParms[ SHADERPARM_TIMEOFFSET ] );
...
================
*/
void idFuncShootProjectile::ReadFromSnapshot( const budBitMsg& msg )
{
	// 	hidden = msg.ReadBits( 1 ) != 0;
	// 	renderEntity.shaderParms[ SHADERPARM_PARTICLE_STOPTIME ] = msg.ReadFloat();
	// 	renderEntity.shaderParms[ SHADERPARM_TIMEOFFSET ] = msg.ReadFloat();
...
void idFuncSmoke::Spawn()
{
	...
	if( *smokeName != '\0' )
	{
		smoke = static_cast<const budDeclParticle*>( declManager->FindType( DECL_PARTICLE, smokeName ) );
	}
	else
	{
		smoke = NULL;
...
void idVacuumSeparatorEntity::Spawn()
{
	...
	b = budBounds( spawnArgs.GetVector( "origin" ) ).Expand( 16 );
...
================
*/
void idBeam::WriteToSnapshot( budBitMsg& msg ) const
{
	GetPhysics()->WriteToSnapshot( msg );
	WriteBindToSnapshot( msg );
	WriteColorToSnapshot( msg );
...
================
*/
void idBeam::ReadFromSnapshot( const budBitMsg& msg )
{
	GetPhysics()->ReadFromSnapshot( msg );
	ReadBindFromSnapshot( msg );
	ReadColorFromSnapshot( msg );
...
{
	/*
		model = dynamic_cast<budRenderModelLiquid *>( renderEntity.hModel );
		if ( !model ) {
...
void idPhantomObjects::Event_Activate( idEntity* activator )
{
	...
	if( !activator || !activator->IsType( budActor::Type ) )
	{
		target = gameLocal.GetLocalPlayer();
	}
	else
	{
		target = static_cast<budActor*>( activator );
	}
...
void idPhantomObjects::Think()
{
	...
	for( i = 0; i < targets.Num(); i++ )
	{
		...
		if( time < 0.0f )
		{
			budAI::PredictTrajectory( entPhys->GetOrigin(), lastTargetPos[ i ], speed, entPhys->GetGravity(),
