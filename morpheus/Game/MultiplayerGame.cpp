...
void idMultiplayerGame::SpawnPlayer( int clientNum )
{
	...
	AddChatLine( budLocalization::GetString( "#str_07177" ), lobby.GetLobbyUserName( lobbyUserID ) );
	...
	if( !common->IsClient() )
	{
		...
		budPlayer* p = static_cast< budPlayer* >( gameLocal.entities[ clientNum ] );
...
================
*/
void idMultiplayerGame::UpdateScoreboard( idMenuHandler_Scoreboard* scoreboard, budPlayer* owner )
{
	...
	for( int i = 0; i < MAX_CLIENTS; ++i )
	{
		...
		if( i < gameLocal.numClients )
		{
			...
			if( gameState == WARMUP )
			{
				...
				{
					spectateData = budLocalization::GetString( "#str_04246" );
				}
				else
				{
					spectateData = budLocalization::GetString( "#str_04247" );
				}
			}
			else
			{
				...
				{
					spectateData = budLocalization::GetString( "#str_06736" );
				}
	...
	else
	{
		if( gameLocal.gameType == GAME_LASTMAN )
		{
			...
			{
				gameInfo = va( "%s: %i", budLocalization::GetString( "#str_04264" ), startFragLimit );
			}
			else
			{
				gameInfo = va( "%s: %i", budLocalization::GetString( "#str_04264" ), gameLocal.serverInfo.GetInt( "si_fragLimit" ) );
			}
		}
		else if( gameLocal.gameType == GAME_CTF )
		{
			int captureLimit = gameLocal.serverInfo.GetInt( "si_fragLimit" );
			gameInfo = va( budLocalization::GetString( "#str_11108" ), captureLimit );
		}
		else
		{
			gameInfo = va( "%s: %i", budLocalization::GetString( "#str_01982" ), gameLocal.serverInfo.GetInt( "si_fragLimit" ) );
		}
		
		if( gameLocal.serverInfo.GetInt( "si_timeLimit" ) > 0 )
		{
			gameInfo.Append( va( "        %s: %i", budLocalization::GetString( "#str_01983" ), gameLocal.serverInfo.GetInt( "si_timeLimit" ) ) );
		}
	}
...
================
*/
budPlayer* idMultiplayerGame::FragLimitHit()
{
	int i;
	...
	if( gameLocal.gameType == GAME_LASTMAN )
	{
		...
		assert( !static_cast< budPlayer* >( leader )->lastManOver );
		for( i = 0 ; i < gameLocal.numClients ; i++ )
		{
			idEntity* ent = gameLocal.entities[ i ];
			if( !ent || !ent->IsType( budPlayer::Type ) )
			...
			if( !CanPlay( static_cast< budPlayer* >( ent ) ) )
			{
				continue;
			}
			if( ent == leader )
...
================
*/
budPlayer* idMultiplayerGame::FragLeader()
{
	int i;
	int frags[ MAX_CLIENTS ];
	budPlayer* leader = NULL;
	idEntity* ent;
	budPlayer* p;
	int high = -9999;
	int count = 0;
	...
	for( i = 0 ; i < gameLocal.numClients ; i++ )
	{
		...
		if( static_cast< budPlayer* >( ent )->lastManOver )
		{
			continue;
		}
...
/*
================
budGameLocal::UpdateWinsLosses
================
*/
void idMultiplayerGame::UpdateWinsLosses( budPlayer* winner )
{
	if( winner )
	{
...
================
*/
void idMultiplayerGame::PlayerDeath( budPlayer* dead, budPlayer* killer, bool telefrag )
{
	...
	if( dead == NULL )
	{
		libBud::Warning( "idMultiplayerGame::PlayerDeath dead ptr == NULL, kill will not count" );
		return;
	}
...
void idMultiplayerGame::PlayerStats( int clientNum, char* data, const int len )
{
	...
	*data = 0;
	...
	if( ent && ent->IsType( budPlayer::Type ) )
	{
		team = static_cast< budPlayer* >( ent )->team;
	}
	else
	{
		return;
...
void idMultiplayerGame::DumpTourneyLine()
{
	...
	for( i = 0; i < gameLocal.numClients; i++ )
	{
		...
		{
			common->Printf( "client %d: rank %d\n", i, static_cast< budPlayer* >( gameLocal.entities[ i ] )->tourneyRank );
		}
	}
}
...
================
*/
void idMultiplayerGame::NewState( gameState_t news, budPlayer* player )
{
	budBitMsg	outMsg;
	...
	switch( news )
	{
		case GAMEON:
		{
			...
			for( i = 0; i < gameLocal.numClients; i++ )
			{
				...
				budPlayer* p = static_cast<budPlayer*>( ent );
				...
				if( gameLocal.gameType == GAME_TOURNEY && currentTourneyPlayer[ 0 ] != i && currentTourneyPlayer[ 1 ] != i )
				{
					...
					libBud::Printf( "TOURNEY NewState GAMEON :> Player %d Benched \n", p->entityNumber );
					p->tourneyRank++;
				}
				else
				{
					...
					if( !static_cast<budPlayer*>( ent )->wantSpectate )
					{
						static_cast<budPlayer*>( ent )->ServerSpectate( false );
						libBud::Printf( "TOURNEY NewState :> Player %d On Deck \n", ent->entityNumber );
						if( gameLocal.gameType == GAME_TOURNEY )
						{
							p->tourneyRank = 0;
						}
		...
		case GAMEREVIEW:
		{
			...
			for( i = 0; i < gameLocal.numClients; i++ )
			{
				...
				static_cast<budPlayer*>( ent )->ServerSpectate( true );
				libBud::Printf( "TOURNEY NewState GAMEREVIEW :> Player %d Benched \n", ent->entityNumber );
			}
...
void idMultiplayerGame::FillTourneySlots( )
{
	...
	libBud::Printf( "TOURNEY :> Executing FillTourneySlots  \n" );
	...
	for( i = 0; i < 2; i++ )
	{
		...
		for( j = 0; j < gameLocal.numClients; j++ )
		{
			...
			if( p->wantSpectate )
			{
				libBud::Printf( "FillTourneySlots: Skipping Player %d ( Wants Spectate )\n", p->entityNumber );
				continue;
			}
			...
			if( p->tourneyRank >= rankmax )
			{
				...
				if( p->tourneyRank == rankmax )
				{
					...
					if( p->spawnedTime > static_cast< budPlayer* >( gameLocal.entities[ rankmaxindex ] )->spawnedTime )
					{
						continue;
					}
				}
				rankmax = static_cast< budPlayer* >( ent )->tourneyRank;
				rankmaxindex = j;
			}
		}
...
void idMultiplayerGame::UpdateTourneyLine()
{
	...
	for( int j = 1; j <= gameLocal.numClients; j++ )
	{
		...
		for( int i = 0; i < gameLocal.numClients; i++ )
		{
			...
			budPlayer* p = static_cast< budPlayer* >( gameLocal.entities[ i ] );
			if( !p || p->wantSpectate )
			{
				continue;
			}
...
void idMultiplayerGame::CycleTourneyPlayers( )
{
	...
	for( i = 0 ; i < gameLocal.numClients ; i++ )
	{
		if( currentTourneyPlayer[ 0 ] == i || currentTourneyPlayer[ 1 ] == i )
		{
			...
			libBud::Printf( "TOURNEY CycleTourneyPlayers:> Player %d On Deck \n", player->entityNumber );
		}
		else
		{
			ent = gameLocal.entities[ i ];
			if( ent && ent->IsType( budPlayer::Type ) )
			{
				...
				libBud::Printf( "TOURNEY CycleTourneyPlayers:> Player %d Benched \n", player->entityNumber );
			}
		}
	}
	UpdateTourneyLine();
			...
			if( nextState == INACTIVE )
			{
				...
				for( i = 0; i < gameLocal.numClients; i++ )
				{
					...
					if( ent && ent->IsType( budPlayer::Type ) )
					{
						if( !static_cast< budPlayer* >( ent )->wantSpectate )
						{
							CheckRespawns( static_cast<budPlayer*>( ent ) );
						}
					}
				}
			}
...
bool idMultiplayerGame::Draw( int clientNum )
{
	...
	player = viewPlayer = static_cast<budPlayer*>( gameLocal.entities[ clientNum ] );
	...
	if( player->spectating )
	{
		viewPlayer = static_cast<budPlayer*>( gameLocal.entities[ player->spectator ] );
		if( viewPlayer == NULL )
		{
			return false;
		}
...
void idMultiplayerGame::GetSpectateText( budPlayer* player, String spectatetext[ 2 ], bool scoreboard )
{
	...
	if( player->wantSpectate && !scoreboard )
	{
		if( gameLocal.gameType == GAME_LASTMAN && gameState == GAMEON )
		{
			...
			spectatetext[ 0 ] = budLocalization::GetString( "#str_04246" );
			spectatetext[ 0 ] += budLocalization::GetString( "#str_07003" );
		}
		else
		{
					...
					( currentTourneyPlayer[0] != player->entityNumber && currentTourneyPlayer[1] != player->entityNumber ) )
			{
				...
				switch( player->tourneyLine )
				{
					case 0:
						...
						break;
					case 1:
						spectatetext[ 0 ] += budLocalization::GetString( "#str_07004" );
						break;
					case 2:
						spectatetext[ 0 ] += budLocalization::GetString( "#str_07005" );
						break;
					default:
						spectatetext[ 0 ] += va( budLocalization::GetString( "#str_07006" ), player->tourneyLine );
						break;
				}
			...
			{
				spectatetext[ 0 ] = budLocalization::GetString( "#str_respawn_message" );
			}
		}
	}
	else
	{
		...
		{
			spectatetext[ 0 ] = budLocalization::GetString( "#str_07007" );
		}
		else
		{
			spectatetext[ 0 ] = budLocalization::GetString( "#str_04246" );
	...
	{
		idLobbyBase& lobby = session->GetActingGameStateLobbyBase();
		spectatetext[ 1 ] = va( budLocalization::GetString( "#str_07008" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[ player->spectator ] ) );
	}
}
...
================
*/
void idMultiplayerGame::UpdateHud( budPlayer* player, idMenuHandler_HUD* hudManager )
{
	...
	if( hudManager && hudManager->GetHud() )
	{
		...
		if( gameState == GAMEON || gameState == COUNTDOWN || gameState == WARMUP )
		{
			if( IsGametypeTeamBased() && !IsGametypeFlagBased() )
			{
				...
				{
					idEntity* ent = gameLocal.entities[ i ];
					if( ent == NULL || !ent->IsType( budPlayer::Type ) )
					{
						continue;
					}
...
================
*/
void idMultiplayerGame::DrawScoreBoard( budPlayer* player )
{
...
===============
*/
void idMultiplayerGame::DrawChat( budPlayer* player )
{
	int i, j;
...
================
*/
void idMultiplayerGame::WriteToSnapshot( budBitMsg& msg ) const
{
	int i;
	int value;
...
================
*/
void idMultiplayerGame::ReadFromSnapshot( const budBitMsg& msg )
{
	int i;
...
void idMultiplayerGame::PrintMessageEvent( msg_evt_t evt, int parm1, int parm2 )
{
	...
	switch( evt )
	{
		case MSG_LEFTGAME:
			assert( parm1 >= 0 );
			AddChatLine( budLocalization::GetString( "#str_11604" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ) );
			break;
		case MSG_SUICIDE:
			assert( parm1 >= 0 );
			AddChatLine( budLocalization::GetString( "#str_04293" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ) );
			break;
		case MSG_KILLED:
			assert( parm1 >= 0 && parm2 >= 0 );
			AddChatLine( budLocalization::GetString( "#str_04292" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm2] ) );
			break;
		case MSG_KILLEDTEAM:
			assert( parm1 >= 0 && parm2 >= 0 );
			AddChatLine( budLocalization::GetString( "#str_04291" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm2] ) );
			break;
		case MSG_TELEFRAGGED:
			assert( parm1 >= 0 && parm2 >= 0 );
			AddChatLine( budLocalization::GetString( "#str_04290" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm2] ) );
			break;
		case MSG_DIED:
			assert( parm1 >= 0 );
			AddChatLine( budLocalization::GetString( "#str_04289" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ) );
			break;
		case MSG_SUDDENDEATH:
			AddChatLine( budLocalization::GetString( "#str_04287" ) );
			break;
		case MSG_JOINEDSPEC:
			AddChatLine( budLocalization::GetString( "#str_04285" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ) );
			break;
		case MSG_TIMELIMIT:
			AddChatLine( budLocalization::GetString( "#str_04284" ) );
			break;
		case MSG_FRAGLIMIT:
			if( gameLocal.gameType == GAME_LASTMAN )
			{
				AddChatLine( budLocalization::GetString( "#str_04283" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ) );
			}
			else if( IsGametypeTeamBased() )      /* CTF */
			{
				AddChatLine( budLocalization::GetString( "#str_04282" ), budLocalization::GetString( teamNames[ parm1 ] ) );
			}
			else
			{
				AddChatLine( budLocalization::GetString( "#str_04281" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ) );
			}
			break;
		case MSG_JOINTEAM:
			AddChatLine( budLocalization::GetString( "#str_04280" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm1] ), parm2 ? budLocalization::GetString( "#str_02500" ) : budLocalization::GetString( "#str_02499" ) );
			break;
		case MSG_HOLYSHIT:
			AddChatLine( budLocalization::GetString( "#str_06732" ) );
			break;
		case MSG_POINTLIMIT:
			AddChatLine( budLocalization::GetString( "#str_11100" ), parm1 ? budLocalization::GetString( "#str_11110" ) : budLocalization::GetString( "#str_11111" ) );
			break;
		...
		case MSG_FLAGTAKEN :
			...
			{
				AddChatLine( budLocalization::GetString( "#str_11101" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm2] ) );	// your team
			}
			else
			{
				AddChatLine( budLocalization::GetString( "#str_11102" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm2] ) );	// enemy
			}
			break;
		...
		case MSG_FLAGDROP :
			...
			if( gameLocal.GetLocalPlayer()->team != parm1 )
			{
				AddChatLine( budLocalization::GetString( "#str_11103" ) );	// your team
			}
			else
			{
				AddChatLine( budLocalization::GetString( "#str_11104" ) );	// enemy
			}
			break;
		...
		case MSG_FLAGRETURN :
			...
			if( parm2 >= 0 && parm2 < MAX_CLIENTS )
			{
				if( gameLocal.GetLocalPlayer()->team != parm1 )
				{
					AddChatLine( budLocalization::GetString( "#str_11120" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm2] ) );	// your team
				}
				else
				{
					AddChatLine( budLocalization::GetString( "#str_11121" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm2] ) );	// enemy
				}
			}
			else
			{
				AddChatLine( budLocalization::GetString( "#str_11105" ), parm1 ? budLocalization::GetString( "#str_11110" ) : budLocalization::GetString( "#str_11111" ) );
			}
			break;
		...
		case MSG_FLAGCAPTURE :
			...
			if( gameLocal.GetLocalPlayer()->team != parm1 )
			{
				AddChatLine( budLocalization::GetString( "#str_11122" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm2] ) );	// your team
			}
			else
			{
				AddChatLine( budLocalization::GetString( "#str_11123" ), lobby.GetLobbyUserName( gameLocal.lobbyUserIDs[parm2] ) );	// enemy
			}
			
//			AddChatLine( budLocalization::GetString( "#str_11106" ), parm1 ? budLocalization::GetString( "#str_11110" ) : budLocalization::GetString( "#str_11111" ) );
			break;
			
		case MSG_SCOREUPDATE:
			AddChatLine( budLocalization::GetString( "#str_11107" ), parm1, parm2 );
			break;
		default:
...
void idMultiplayerGame::SuddenRespawn()
{
	...
	for( i = 0; i < gameLocal.numClients; i++ )
	{
		...
		if( !CanPlay( static_cast< budPlayer* >( gameLocal.entities[ i ] ) ) )
		{
			continue;
		}
		if( static_cast< budPlayer* >( gameLocal.entities[ i ] )->lastManOver )
		{
			continue;
		}
		static_cast< budPlayer* >( gameLocal.entities[ i ] )->lastManPlayAgain = true;
	}
}
...
================
*/
void idMultiplayerGame::CheckRespawns( budPlayer* spectator )
{
	for( int i = 0 ; i < gameLocal.numClients ; i++ )
	{
		idEntity* ent = gameLocal.entities[ i ];
			...
			else
			{
				...
				else if( gameLocal.gameType == GAME_TOURNEY )
				{
					if( i == currentTourneyPlayer[ 0 ] || i == currentTourneyPlayer[ 1 ] )
					{
						...
						{
							p->ServerSpectate( false );
							libBud::Printf( "TOURNEY CheckRespawns :> Player %d On Deck \n", p->entityNumber );
						}
					}
					else if( gameState == WARMUP )
					{
						...
						{
							p->ServerSpectate( false );
							libBud::Printf( "TOURNEY CheckRespawns WARMUP :> Player %d On Deck \n", p->entityNumber );
						}
					}
				}
				else if( gameLocal.gameType == GAME_LASTMAN )
							...
							if( !fragLimitTimeout && !p->lastManOver )
							{
								...
								//and the server got stuck
								{
									...
									for( j = 0; j < gameLocal.numClients; j++ )
									{
										...
										if( !CanPlay( static_cast< budPlayer* >( gameLocal.entities[ j ] ) ) )
										{
											continue;
										}
										if( !static_cast< budPlayer* >( gameLocal.entities[ j ] )->lastManOver )
										{
											break;
										}
									}
		...
		else if( p->wantSpectate && !p->spectating )
		{
			...
			p->ServerSpectate( true );
			libBud::Printf( "TOURNEY CheckRespawns :> Player %d Wants Spectate \n", p->entityNumber );
			UpdateTourneyLine();
			CheckAbortGame();
		}
	}
...
void idMultiplayerGame::DropWeapon( int clientNum )
{
	...
	static_cast< budPlayer* >( ent )->DropWeapon( false );
}
...
void idMultiplayerGame::CheckAbortGame()
{
	...
	if( gameLocal.gameType == GAME_TOURNEY && gameState == WARMUP )
	{
		...
		for( i = 0; i < 2; i++ )
		{
			if( !gameLocal.entities[ currentTourneyPlayer[ i ] ] || static_cast< budPlayer* >( gameLocal.entities[ currentTourneyPlayer[ i ] ] )->spectating )
			{
	...
	switch( gameLocal.gameType )
	{
		case GAME_TOURNEY:
			for( i = 0; i < 2; i++ )
			{
				...
				budPlayer* player = static_cast< budPlayer* >( gameLocal.entities[ currentTourneyPlayer[ i ] ] );
...
void idMultiplayerGame::BalanceTeams()
{
	...
	{
		budPlayer* playerA = gameLocal.GetClientByNum( a );
		if( playerA->team == largeTeam && CanPlay( playerA ) )
		{
			for( int b = 0; b < gameLocal.numClients; b++ )
			{
				...
				budPlayer* playerB = gameLocal.GetClientByNum( b );
...
void idMultiplayerGame::SwitchToTeam( int clientNum, int oldteam, int newteam )
{
	...
	for( i = 0; i < gameLocal.numClients; i++ )
	{
		...
		idEntity* ent = gameLocal.entities[ i ];
		if( ent && ent->IsType( budPlayer::Type ) && static_cast< budPlayer* >( ent )->team == newteam )
		{
	...
	if( ( gameState == GAMEON || ( IsGametypeFlagBased() && gameState == SUDDENDEATH ) ) && oldteam != -1 )
	{
		...
		if( p->IsInTeleport() )
		{
			p->ServerSendEvent( budPlayer::EVENT_ABORT_TELEPORTER, NULL, false );
...
void idMultiplayerGame::ProcessChatMessage( int clientNum, bool team, const char* name, const char* text, const char* sound )
{
	...
	if( clientNum >= 0 )
	{
		pfrom = static_cast< budPlayer* >( gameLocal.entities[ clientNum ] );
		if( !( pfrom && pfrom->IsType( budPlayer::Type ) ) )
		{
			return;
		}
	...
	else
	{
		for( i = 0; i < gameLocal.numClients; i++ )
		{
			...
			budPlayer* pent = static_cast< budPlayer* >( ent );
...
void idMultiplayerGame::ToggleSpectate()
{
	...
	budPlayer* player = ( budPlayer* )gameLocal.entities[gameLocal.GetLocalClientNum()];
	...
	{
		gameLocal.mpGame.AddChatLine( budLocalization::GetString( "#str_06747" ) );
		return;
	}
...
================
*/
bool idMultiplayerGame::CanPlay( budPlayer* p )
{
...
================
*/
bool idMultiplayerGame::WantRespawn( budPlayer* p )
{
...
void idMultiplayerGame::ServerWriteInitialReliableMessages( int clientNum, lobbyUserID_t lobbyUserID )
{
	...
	{
		idEntity* ent = gameLocal.entities[ i ];
		if( i != clientNum && ent && ent->IsType( budPlayer::Type ) )
		{
			outMsg.WriteByte( i );
			outMsg.WriteBits( static_cast< budPlayer* >( ent )->inventory.powerups, 15 );
			outMsg.WriteBits( static_cast< budPlayer* >( ent )->spectating, 1 );
		}
	}
...
================
*/
void idMultiplayerGame::ClientReadStartState( const budBitMsg& msg )
{
	...
	while( ( client = msg.ReadByte() ) != MAX_CLIENTS )
	{
		...
		assert( gameLocal.entities[ client ] && gameLocal.entities[ client ]->IsType( budPlayer::Type ) );
		int powerup = msg.ReadBits( 15 );
		for( int i = 0; i < MAX_POWERUPS; i++ )
		{
			if( powerup & ( 1 << i ) )
			{
				static_cast< budPlayer* >( gameLocal.entities[ client ] )->GivePowerUp( i, 0, ITEM_GIVE_FEEDBACK | ITEM_GIVE_UPDATE_STATE );
			}
		}
		bool spectate = ( msg.ReadBits( 1 ) != 0 );
		static_cast< budPlayer* >( gameLocal.entities[ client ] )->Spectate( spectate );
	}
}
...
================
*/
void idMultiplayerGame::ClientReadWarmupTime( const budBitMsg& msg )
{
...
================
*/
void idMultiplayerGame::ClientReadMatchStartedTime( const budBitMsg& msg )
{
	matchStartedTime = msg.ReadLong();
}
...
================
*/
void idMultiplayerGame::ClientReadAchievementUnlock( const budBitMsg& msg )
{

	int playerid = msg.ReadByte();
	achievement_t achieve = ( achievement_t )msg.ReadByte();
	
	budPlayer* player = static_cast<budPlayer*>( gameLocal.entities[ playerid ] );
	
	if( player != NULL )
	{
	
		libBud::Printf( "Client Receiving Achievement\n" );
