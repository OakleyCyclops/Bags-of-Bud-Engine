...
{
	friend class budParallelJobManagerLocal;
public:
	...
	void					Submit( budParallelJobList* waitForJobList = NULL, int parallelism = JOBLIST_PARALLELISM_DEFAULT );
...
private:
	class budParallelJobList_Threads* 	jobListThreads;
	const idColor* 						color;
	
	budParallelJobList( jobListId_t id, jobListPriority_t priority, unsigned int maxJobs, unsigned int maxSyncs, const idColor* color );
	~budParallelJobList();
};

/*
================================================
budParallelJobManager

This is the only interface through which job lists
should be allocated or freed.
================================================
*/
class budParallelJobManager
{
public:
	virtual						~budParallelJobManager() {}
	
	virtual void				Init() = 0;
	virtual void				Shutdown() = 0;
	
	virtual budParallelJobList* 	AllocJobList( jobListId_t id, jobListPriority_t priority, unsigned int maxJobs, unsigned int maxSyncs, const idColor* color ) = 0;
	virtual void				FreeJobList( budParallelJobList* jobList ) = 0;
	
	virtual int					GetNumJobLists() const = 0;
	virtual int					GetNumFreeJobLists() const = 0;
	virtual budParallelJobList* 	GetJobList( int index ) = 0;
...
extern budParallelJobManager* 	parallelJobManager;
...
/*
================================================
budParallelJobRegistration
================================================
*/
class budParallelJobRegistration
{
public:
	budParallelJobRegistration( jobRun_t function, const char* name );
};

#define REGISTER_PARALLEL_JOB( function, name )		static budParallelJobRegistration register_##function( (jobRun_t) function, name )
