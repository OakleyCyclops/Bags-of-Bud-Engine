...
/*
================================================
budSysMutex provides a C++ wrapper to the low level system mutex functions.  A mutex is an
...
private:
	...
	budSysMutex( const budSysMutex& s ) {}
	void			operator=( const budSysMutex& s ) {}
};

/*
================================================
budScopedCriticalSection is a helper class that automagically locks a mutex when it's created
and unlocks it when it goes out of scope.
================================================
*/
class budScopedCriticalSection
{
public:
	budScopedCriticalSection( budSysMutex& m ) : mutex( &m )
	{
		mutex->Lock();
	}
	~budScopedCriticalSection()
	{
		mutex->Unlock();
	}
	
private:
	budSysMutex* 	mutex;	// NOTE: making this a reference causes a TypeInfo crash
};

/*
================================================
budSysSignal is a C++ wrapper for the low level system signal functions.  A signal is an object
...
public:
	...
	budSysSignal( bool manualReset = false )
	{
...
private:
	...
	budSysSignal( const budSysSignal& s ) {}
	void				operator=( const budSysSignal& s ) {}
};

/*
================================================
budSysInterlockedInteger is a C++ wrapper for the low level system interlocked integer
routines to atomically increment or decrement an integer.
================================================
*/
class budSysInterlockedInteger
{
public:
	budSysInterlockedInteger() : value( 0 ) {}
...
/*
================================================
budSysInterlockedPointer is a C++ wrapper around the low level system interlocked pointer
routine to atomically set a pointer while retrieving the previous value of the pointer.
================================================
*/
template< typename T >
class budSysInterlockedPointer
{
public:
	budSysInterlockedPointer() : ptr( NULL ) {}
...
/*
================================================
budSysThread is an abstract base class, to be extended by classes implementing the
budSysThread::Run() method.
...
Note that the Sys_CreateThread function does not support the concept of worker threads.
	...
	class idMyWorkerThread : public budSysThread {
	public:
		virtual int Run() {
			// run thread code here
			return 0;
...
private:
	...
	budSysSignal		signalWorkerDone;
	budSysSignal		signalMoreWorkToDo;
	budSysMutex		signalMutex;
	
	static int		ThreadProc( budSysThread* thread );
	
	budSysThread( const budSysThread& s ) {}
	void			operator=( const budSysThread& s ) {}
};

/*
================================================
budSysWorkerThreadGroup implements a group of worker threads that
typically crunch through a collection of similar tasks.
	...
	budSysWorkerThreadGroup<idMyWorkerThread> workers( "myWorkers", 4 );
	for ( ; ; ) {
...
class budSysWorkerThreadGroup
{
public:
	budSysWorkerThreadGroup( const char* name, int numThreads,
							xthreadPriority priority = THREAD_NORMAL,
							int stackSize = DEFAULT_THREAD_STACK_SIZE );
							
	virtual			~budSysWorkerThreadGroup();
...
private:
	budList<threadType*, TAG_THREAD>	threadList;
...
/*
========================
budSysWorkerThreadGroup<threadType>::budSysWorkerThreadGroup
========================
*/
template<class threadType>
BUD_INLINE budSysWorkerThreadGroup<threadType>::budSysWorkerThreadGroup( const char* name,
...
*/
template<class threadType>
BUD_INLINE budSysWorkerThreadGroup<threadType>::~budSysWorkerThreadGroup()
{
...
/*
========================
budSysWorkerThreadGroup<threadType>::SignalWorkAndWait
========================
*/
template<class threadType>
BUD_INLINE void budSysWorkerThreadGroup<threadType>::SignalWorkAndWait()
{
	if( singleThreaded )
	{
...
/*
================================================
budSysThreadSynchronizer, allows a group of threads to
synchronize with each other half-way through execution.

	budSysThreadSynchronizer sync;
...
================================================
*/
class budSysThreadSynchronizer
{
public:
	static const int	WAIT_INFINITE = -1;
	
	BUD_INLINE	void			SetNumThreads( unsigned int num );
	BUD_INLINE	void			Signal( unsigned int threadNum );
	BUD_INLINE	bool			Synchronize( unsigned int threadNum, int timeout = WAIT_INFINITE );
	
private:
	budList< budSysSignal*, TAG_THREAD >		signals;
	budSysInterlockedInteger		busyCount;
};

/*
========================
budSysThreadSynchronizer::SetNumThreads
========================
*/
BUD_INLINE void budSysThreadSynchronizer::SetNumThreads( unsigned int num )
{
	...
	if( ( int )num != signals.Num() )
	{
		...
		{
			signals[i] = new( TAG_THREAD ) budSysSignal();
		}
...
/*
========================
budSysThreadSynchronizer::Signal
========================
*/
BUD_INLINE void budSysThreadSynchronizer::Signal( unsigned int threadNum )
{
...
/*
========================
budSysThreadSynchronizer::Synchronize
========================
*/
BUD_INLINE bool budSysThreadSynchronizer::Synchronize( unsigned int threadNum, int timeout )
{
