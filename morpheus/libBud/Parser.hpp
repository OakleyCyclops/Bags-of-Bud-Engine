...
typedef struct define_s
{
	...
	budToken* 		parms;						// define parameters
	budToken* 		tokens;						// macro tokens (possibly containing parm tokens)
...
typedef struct indent_s
{
	...
	budLexer* 		script;						// script the indent was in
...
public:
	...
	budParser();
	budParser( int flags );
	budParser( const char* filename, int flags = 0, bool OSPath = false );
	budParser( const char* ptr, int length, const char* name, int flags = 0 );
	// destructor
	~budParser();
	// load a source file
	...
	int				IsLoaded() const
	{
		return budParser::loaded;
	}
	// read a token from the source
	int				ReadToken( budToken* token );
	// expect a certain token, reads the token when available
	int				ExpectTokenString( const char* string );
	// expect a certain token type
	int				ExpectTokenType( int type, int subtype, budToken* token );
	// expect a token
	int				ExpectAnyToken( budToken* token );
	...
	int				CheckTokenType( int type, int subtype, budToken* token );
	...
	int				PeekTokenType( int type, int subtype, budToken* token );
	...
	const char*		ParseBracedSection( budStr& out, int tabs, bool parseFirstBrace, char intro, char outro );
	// parse a braced section into a string, maintaining indents and newlines
	const char* 	ParseBracedSectionExact( budStr& out, int tabs = -1 );
	// parse the rest of the line
	const char* 	ParseRestOfLine( budStr& out );
	// unread the given token
	void			UnreadToken( budToken* token );
	// read a token only if on the current line
	int				ReadTokenOnLine( budToken* token );
...
// RB end
	...
	budStr			filename;					// file name of the script
	budStr			includepath;				// path to include files
	...
	budLexer* 		scriptstack;				// stack with scripts of the source
	budToken* 		tokens;						// tokens to read first
	...
	void			PushScript( budLexer* script );
	int				ReadSourceToken( budToken* token );
	int				ReadLine( budToken* token );
	int				UnreadSourceToken( budToken* token );
	int				ReadDefineParms( define_t* define, budToken** parms, int maxparms );
	int				StringizeTokens( budToken* tokens, budToken* token );
	int				MergeTokens( budToken* t1, budToken* t2 );
	int				ExpandBuiltinDefine( budToken* deftoken, define_t* define, budToken** firsttoken, budToken** lasttoken );
	int				ExpandDefine( budToken* deftoken, define_t* define, budToken** firsttoken, budToken** lasttoken );
	int				ExpandDefineIntoSource( budToken* deftoken, define_t* define );
	...
	// RB: allow override
	virtual int		Directive_include( budToken* token, bool supressWarning = false );
	// RB end
	int				Directive_undef();
	...
	int				EvaluateTokens( budToken* tokens, signed int* intvalue, double* floatvalue, int integer );
...
BUD_INLINE const char* budParser::GetFileName() const
{
	if( budParser::scriptstack )
	{
		return budParser::scriptstack->GetFileName();
	}
	else
	{
		return "";
...
BUD_INLINE const int budParser::GetFileOffset() const
{
	if( budParser::scriptstack )
	{
		return budParser::scriptstack->GetFileOffset();
	}
	else
	{
		return 0;
...
BUD_INLINE const ID_TIME_T budParser::GetFileTime() const
{
	if( budParser::scriptstack )
	{
		return budParser::scriptstack->GetFileTime();
	}
	else
	{
		return 0;
...
BUD_INLINE const int budParser::GetLineNum() const
{
	if( budParser::scriptstack )
	{
		return budParser::scriptstack->GetLineNum();
	}
	else
	{
		return 0;
