...
budBox box_zero( vec3_zero, vec3_zero, mat3_identity );
...
============
*/
bool budBox::AddPoint( const budVec3& v )
{
	budMat3 axis2;
	budBounds bounds1, bounds2;
	...
	bounds1[0] -= extents;
	bounds1[1] += extents;
	if( !bounds1.AddPoint( budVec3( v * axis[0], v * axis[1], v * axis[2] ) ) )
	{
	...
	bounds2.AddPoint( budVec3( v * axis2[0], v * axis2[1], v * axis2[2] ) );
...
============
*/
bool budBox::AddBox( const budBox& a )
{
	int i, besti;
	float v, bestv;
	budVec3 dir;
	budMat3 ax[4];
	budBounds bounds[4], b;
...
/*
================
budBox::PlaneDistance
================
*/
float budBox::PlaneDistance( const budPlane& plane ) const
{
	float d1, d2;
	
	d1 = plane.Distance( center );
	d2 = budMath::Fabs( extents[0] * plane.Normal()[0] ) +
		 budMath::Fabs( extents[1] * plane.Normal()[1] ) +
		 budMath::Fabs( extents[2] * plane.Normal()[2] );
...
================
*/
int budBox::PlaneSide( const budPlane& plane, const float epsilon ) const
{
	float d1, d2;
...
/*
============
budBox::IntersectsBox
============
*/
bool budBox::IntersectsBox( const budBox& a ) const
{
	budVec3 dir;			// vector between centers
	...
	ac[0][0] = budMath::Fabs( c[0][0] );
	ac[0][1] = budMath::Fabs( c[0][1] );
	ac[0][2] = budMath::Fabs( c[0][2] );
	
	d = budMath::Fabs( axisdir[0] );
	e0 = extents[0];
	...
	ac[1][0] = budMath::Fabs( c[1][0] );
	ac[1][1] = budMath::Fabs( c[1][1] );
	ac[1][2] = budMath::Fabs( c[1][2] );
	
	d = budMath::Fabs( axisdir[1] );
	e0 = extents[1];
	...
	ac[2][0] = budMath::Fabs( c[2][0] );
	ac[2][1] = budMath::Fabs( c[2][1] );
	ac[2][2] = budMath::Fabs( c[2][2] );
	
	d = budMath::Fabs( axisdir[2] );
	e0 = extents[2];
	...
	// axis C0 + t * A0xB0
	d = budMath::Fabs( axisdir[2] * c[1][0] - axisdir[1] * c[2][0] );
	...
	// axis C0 + t * A0xB1
	d = budMath::Fabs( axisdir[2] * c[1][1] - axisdir[1] * c[2][1] );
	...
	// axis C0 + t * A0xB2
	d = budMath::Fabs( axisdir[2] * c[1][2] - axisdir[1] * c[2][2] );
	...
	// axis C0 + t * A1xB0
	d = budMath::Fabs( axisdir[0] * c[2][0] - axisdir[2] * c[0][0] );
	...
	// axis C0 + t * A1xB1
	d = budMath::Fabs( axisdir[0] * c[2][1] - axisdir[2] * c[0][1] );
	...
	// axis C0 + t * A1xB2
	d = budMath::Fabs( axisdir[0] * c[2][2] - axisdir[2] * c[0][2] );
	...
	// axis C0 + t * A2xB0
	d = budMath::Fabs( axisdir[1] * c[0][0] - axisdir[0] * c[1][0] );
	...
	// axis C0 + t * A2xB1
	d = budMath::Fabs( axisdir[1] * c[0][1] - axisdir[0] * c[1][1] );
	...
	// axis C0 + t * A2xB2
	d = budMath::Fabs( axisdir[1] * c[0][2] - axisdir[0] * c[1][2] );
...
/*
============
budBox::LineIntersection

  Returns true if the line intersects the box between the start and end point.
============
*/
bool budBox::LineIntersection( const budVec3& start, const budVec3& end ) const
{
	float ld[3];
	budVec3 lineDir = 0.5f * ( end - start );
	budVec3 lineCenter = start + lineDir;
	budVec3 dir = lineCenter - center;
	
	ld[0] = budMath::Fabs( lineDir * axis[0] );
	if( budMath::Fabs( dir * axis[0] ) > extents[0] + ld[0] )
	{
		return false;
	}
	
	ld[1] = budMath::Fabs( lineDir * axis[1] );
	if( budMath::Fabs( dir * axis[1] ) > extents[1] + ld[1] )
	{
		return false;
	}
	
	ld[2] = budMath::Fabs( lineDir * axis[2] );
	if( budMath::Fabs( dir * axis[2] ) > extents[2] + ld[2] )
	{
		return false;
	}
	
	budVec3 cross = lineDir.Cross( dir );
	
	if( budMath::Fabs( cross * axis[0] ) > extents[1] * ld[2] + extents[2] * ld[1] )
	{
		return false;
	}
	
	if( budMath::Fabs( cross * axis[1] ) > extents[0] * ld[2] + extents[2] * ld[0] )
	{
		return false;
	}
	
	if( budMath::Fabs( cross * axis[2] ) > extents[0] * ld[1] + extents[1] * ld[0] )
	{
		return false;
	}
...
============
*/
bool budBox::RayIntersection( const budVec3& start, const budVec3& dir, float& scale1, float& scale2 ) const
{
	budVec3 localStart, localDir;
	
	localStart = ( start - center ) * axis.Transpose();
	localDir = dir * axis.Transpose();
	
	scale1 = -budMath::INFINITY;
	scale2 = budMath::INFINITY;
...
============
*/
void budBox::FromPoints( const budVec3* points, const int numPoints )
{
	int i;
	float invNumPoints, sumXX, sumXY, sumXZ, sumYY, sumYZ, sumZZ;
	budVec3 dir;
	budBounds bounds;
	budMatX eigenVectors;
	budVecX eigenValues;
	...
	for( i = 0; i < numPoints; i++ )
	{
		bounds.AddPoint( budVec3( points[i] * axis[0], points[i] * axis[1], points[i] * axis[2] ) );
	}
...
/*
============
budBox::FromPointTranslation

  Most tight box for the translational movement of the given point.
============
*/
void budBox::FromPointTranslation( const budVec3& point, const budVec3& translation )
{
	// FIXME: implement
}
...
/*
============
budBox::FromBoxTranslation

  Most tight box for the translational movement of the given box.
============
*/
void budBox::FromBoxTranslation( const budBox& box, const budVec3& translation )
{
	// FIXME: implement
}
...
/*
============
budBox::FromPointRotation

  Most tight bounds for the rotational movement of the given point.
============
*/
void budBox::FromPointRotation( const budVec3& point, const budRotation& rotation )
{
	// FIXME: implement
}
...
/*
============
budBox::FromBoxRotation

  Most tight box for the rotational movement of the given box.
============
*/
void budBox::FromBoxRotation( const budBox& box, const budRotation& rotation )
{
	// FIXME: implement
}
...
============
*/
void budBox::ToPoints( budVec3 points[8] ) const
{
	budMat3 ax;
	budVec3 temp[4];
...
/*
============
budBox::GetProjectionSilhouetteVerts
============
*/
int budBox::GetProjectionSilhouetteVerts( const budVec3& projectionOrigin, budVec3 silVerts[6] ) const
{
	float f;
	int i, planeBits, *index;
	budVec3 points[8], dir1, dir2;
...
/*
============
budBox::GetParallelProjectionSilhouetteVerts
============
*/
int budBox::GetParallelProjectionSilhouetteVerts( const budVec3& projectionDir, budVec3 silVerts[6] ) const
{
	float f;
	int i, planeBits, *index;
	budVec3 points[8];
