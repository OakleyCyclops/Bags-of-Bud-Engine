...
class budSortrects : public budSort_Quick< int, budSortrects >
{
public:
	int SizeMetric( budVec2i v ) const
	{
	...
	const budList<budVec2i>* inputSizes;
};
...
void RectAllocator( const budList<budVec2i>& inputSizes, budList<budVec2i>& outputPositions, budVec2i& totalSize )
{
	...
	{
		budVec2i	best( 0, 0 );
		budVec2i	bestMax( START_MAX, START_MAX );
		budVec2i	size = inputSizes[sizeRemap[i]];
		for( int j = 0; j < i; j++ )
		{
			for( int k = 1;  k < 4; k++ )
			{
				...
				for( ; n < i; n++ )
				{
					const budVec2i& check = outputPositions[sizeRemap[n]];
					const budVec2i& checkSize = inputSizes[sizeRemap[n]];
		...
		{
			libBud::FatalError( "RectAllocator: couldn't fit everything" );
		}
