...
============
*/
void VPCALL idSIMD_SSE::BlendJoints( budJointQuat* joints, const budJointQuat* blendJoints, const float lerp, const int* index, const int numJoints )
{
	...
	for( ; i < numJoints; i++ )
	{
		...
		budVec3& jointVert = joints[n].t;
		const budVec3& blendVert = blendJoints[n].t;
		...
		sinom = budMath::InvSqrt( scale0 );
		omega = budMath::ATan16( scale0 * sinom, cosom );
		scale0 = budMath::Sin16( ( 1.0f - lerp ) * omega ) * sinom;
		scale1 = budMath::Sin16( lerp * omega ) * sinom;
...
============
*/
void VPCALL idSIMD_SSE::BlendJointsFast( budJointQuat* joints, const budJointQuat* blendJoints, const float lerp, const int* index, const int numJoints )
{
	...
	assert_sizeof_16_byte_multiple( budJointQuat );
...
============
*/
void VPCALL idSIMD_SSE::ConvertJointQuatsToJointMats( budJointMat* jointMats, const budJointQuat* jointQuats, const int numJoints )
{
	assert( sizeof( budJointQuat ) == JOINTQUAT_SIZE );
	assert( sizeof( budJointMat ) == JOINTMAT_SIZE );
	
	// RB: changed int to intptr_t
	assert( ( intptr_t )( &( ( budJointQuat* )0 )->t ) == ( intptr_t )( &( ( budJointQuat* )0 )->q ) + ( intptr_t )sizeof( ( ( budJointQuat* )0 )->q ) );
	// RB end
...
============
*/
void VPCALL idSIMD_SSE::ConvertJointMatsToJointQuats( budJointQuat* jointQuats, const budJointMat* jointMats, const int numJoints )
{
...
============
*/
void VPCALL idSIMD_SSE::TransformJoints( budJointMat* jointMats, const int* parents, const int firstJoint, const int lastJoint )
{
...
============
*/
void VPCALL idSIMD_SSE::UntransformJoints( budJointMat* jointMats, const int* parents, const int firstJoint, const int lastJoint )
{
	const __m128 vector_float_mask_keep_last	= __m128c( _mm_set_epi32( 0xFFFFFFFF, 0x00000000, 0x00000000, 0x00000000 ) );
