...
int idPolynomial::Laguer( const idComplex* coef, const int degree, idComplex& x ) const
{
	...
	for( i = 1; i <= MAX_ITERATIONS; i++ )
	{
		...
		{
			dx = budMath::Exp( budMath::Log( 1.0f + abx ) ) * idComplex( budMath::Cos( i ), budMath::Sin( i ) );
		}
		cx = x - dx;
		if( x == cx )
		{
...
int idPolynomial::GetRoots( idComplex* roots ) const
{
	...
	for( i = degree - 1; i >= 0; i-- )
	{
		x.Zero();
		Laguer( coef, i + 1, x );
		if( budMath::Fabs( x.i ) < 2.0f * EPSILON * budMath::Fabs( x.r ) )
		{
			x.i = 0.0f;
		}
		roots[i] = x;
...
void idPolynomial::Test()
{
	...
	for( i = 0; i < num; i++ )
	{
		...
		assert( budMath::Fabs( value ) < 1e-4f );
	}
	...
	for( i = 0; i < num; i++ )
	{
		...
		assert( budMath::Fabs( complexValue.r ) < 1e-4f && budMath::Fabs( complexValue.i ) < 1e-4f );
	}
