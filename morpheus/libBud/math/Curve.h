...
protected:
	...
	budList<float>		times;			// knots
	budList<type>		values;			// knot values
...
*/
template< class type >
BUD_INLINE idCurve<type>::idCurve()
{
	currentIndex = -1;
	changed = false;
}
...
*/
template< class type >
BUD_INLINE int idCurve<type>::AddValue( const float time, const type& value )
{
	int i;
...
*/
template< class type >
BUD_INLINE type idCurve<type>::GetCurrentValue( const float time ) const
{
	int i;
...
*/
template< class type >
BUD_INLINE type idCurve<type>::GetCurrentFirstDerivative( const float time ) const
{
...
*/
template< class type >
BUD_INLINE type idCurve<type>::GetCurrentSecondDerivative( const float time ) const
{
	return ( values[0] - values[0] ); //-V501
}
...
*/
template< class type >
BUD_INLINE bool idCurve<type>::IsDone( const float time ) const
{
...
*/
template< class type >
BUD_INLINE float idCurve<type>::GetSpeed( const float time ) const
{
	int i;
	float speed;
	type value;
...
*/
template< class type >
BUD_INLINE float idCurve<type>::RombergIntegral( const float t0, const float t1, const int order ) const
{
	int i, j, k, m, n;
	float sum, delta;
	float* temp[2];
...
*/
template< class type >
BUD_INLINE float idCurve<type>::GetLengthBetweenKnots( const int i0, const int i1 ) const
{
	float length = 0.0f;
	for( int i = i0; i < i1; i++ )
	{
...
*/
template< class type >
BUD_INLINE float idCurve<type>::GetLengthForTime( const float time ) const
{
	float length = 0.0f;
...
*/
template< class type >
BUD_INLINE float idCurve<type>::GetTimeForLength( const float length, const float epsilon ) const
{
	int i, index;
...
*/
template< class type >
BUD_INLINE void idCurve<type>::MakeUniform( const float totalTime )
{
	int i, n;
...
*/
template< class type >
BUD_INLINE void idCurve<type>::SetConstantSpeed( const float totalTime )
{
	int i, j;
...
*/
template< class type >
BUD_INLINE void idCurve<type>::ShiftTime( const float deltaTime )
{
...
*/
template< class type >
BUD_INLINE void idCurve<type>::Translate( const type& translation )
{
...
*/
template< class type >
BUD_INLINE int idCurve<type>::IndexForTime( const float time ) const
{
...
*/
template< class type >
BUD_INLINE type idCurve<type>::ValueForIndex( const int index ) const
{
	int n = values.Num() - 1;
...
*/
template< class type >
BUD_INLINE float idCurve<type>::TimeForIndex( const int index ) const
{
	int n = times.Num() - 1;
...
*/
template< class type >
BUD_INLINE idCurve_Bezier<type>::idCurve_Bezier()
{
}
...
*/
template< class type >
BUD_INLINE type idCurve_Bezier<type>::GetCurrentValue( const float time ) const
{
	int i;
	float* bvals;
	type v;
...
*/
template< class type >
BUD_INLINE type idCurve_Bezier<type>::GetCurrentFirstDerivative( const float time ) const
{
	int i;
	float* bvals, d;
	type v;
...
*/
template< class type >
BUD_INLINE type idCurve_Bezier<type>::GetCurrentSecondDerivative( const float time ) const
{
	int i;
	float* bvals, d;
	type v;
...
*/
template< class type >
BUD_INLINE void idCurve_Bezier<type>::Basis( const int order, const float t, float* bvals ) const
{
	int i, j, d;
...
*/
template< class type >
BUD_INLINE void idCurve_Bezier<type>::BasisFirstDerivative( const int order, const float t, float* bvals ) const
{
	int i;
...
*/
template< class type >
BUD_INLINE void idCurve_Bezier<type>::BasisSecondDerivative( const int order, const float t, float* bvals ) const
{
	int i;
...
*/
template< class type >
BUD_INLINE idCurve_QuadraticBezier<type>::idCurve_QuadraticBezier()
{
}
...
*/
template< class type >
BUD_INLINE type idCurve_QuadraticBezier<type>::GetCurrentValue( const float time ) const
{
	float bvals[3];
...
*/
template< class type >
BUD_INLINE type idCurve_QuadraticBezier<type>::GetCurrentFirstDerivative( const float time ) const
{
	float bvals[3], d;
	assert( this->values.Num() == 3 );
...
*/
template< class type >
BUD_INLINE type idCurve_QuadraticBezier<type>::GetCurrentSecondDerivative( const float time ) const
{
	float bvals[3], d;
	assert( this->values.Num() == 3 );
...
*/
template< class type >
BUD_INLINE void idCurve_QuadraticBezier<type>::Basis( const float t, float* bvals ) const
{
...
*/
template< class type >
BUD_INLINE void idCurve_QuadraticBezier<type>::BasisFirstDerivative( const float t, float* bvals ) const
{
	float s1 = ( float )( t - this->times[0] ) / ( this->times[2] - this->times[0] );
...
*/
template< class type >
BUD_INLINE void idCurve_QuadraticBezier<type>::BasisSecondDerivative( const float t, float* bvals ) const
{
	float s1 = ( float )( t - this->times[0] ) / ( this->times[2] - this->times[0] );
	bvals[0] = 2.0f;
	bvals[1] = -4.0f;
...
*/
template< class type >
BUD_INLINE idCurve_CubicBezier<type>::idCurve_CubicBezier()
{
}
...
*/
template< class type >
BUD_INLINE type idCurve_CubicBezier<type>::GetCurrentValue( const float time ) const
{
	float bvals[4];
...
*/
template< class type >
BUD_INLINE type idCurve_CubicBezier<type>::GetCurrentFirstDerivative( const float time ) const
{
	float bvals[4], d;
	assert( this->values.Num() == 4 );
	BasisFirstDerivative( time, bvals );
...
*/
template< class type >
BUD_INLINE type idCurve_CubicBezier<type>::GetCurrentSecondDerivative( const float time ) const
{
	float bvals[4], d;
	assert( this->values.Num() == 4 );
	BasisSecondDerivative( time, bvals );
...
*/
template< class type >
BUD_INLINE void idCurve_CubicBezier<type>::Basis( const float t, float* bvals ) const
{
...
*/
template< class type >
BUD_INLINE void idCurve_CubicBezier<type>::BasisFirstDerivative( const float t, float* bvals ) const
{
	float s1 = ( float )( t - this->times[0] ) / ( this->times[3] - this->times[0] );
	float s2 = s1 * s1;
...
*/
template< class type >
BUD_INLINE void idCurve_CubicBezier<type>::BasisSecondDerivative( const float t, float* bvals ) const
{
	float s1 = ( float )( t - this->times[0] ) / ( this->times[3] - this->times[0] );
...
*/
template< class type >
BUD_INLINE idCurve_Spline<type>::idCurve_Spline()
{
...
*/
template< class type >
BUD_INLINE type idCurve_Spline<type>::ValueForIndex( const int index ) const
{
...
*/
template< class type >
BUD_INLINE float idCurve_Spline<type>::TimeForIndex( const int index ) const
{
...
*/
template< class type >
BUD_INLINE float idCurve_Spline<type>::ClampedTime( const float t ) const
{
...
*/
template< class type >
BUD_INLINE bool idCurve_Spline<type>::IsDone( const float time ) const
{
...
protected:
	mutable budList<type>b;
	mutable budList<type>c;
	mutable budList<type>d;
...
*/
template< class type >
BUD_INLINE idCurve_NaturalCubicSpline<type>::idCurve_NaturalCubicSpline()
{
}
...
*/
template< class type >
BUD_INLINE type idCurve_NaturalCubicSpline<type>::GetCurrentValue( const float time ) const
{
...
*/
template< class type >
BUD_INLINE type idCurve_NaturalCubicSpline<type>::GetCurrentFirstDerivative( const float time ) const
{
	float clampedTime = this->ClampedTime( time );
	int i = this->IndexForTime( clampedTime );
	float s = time - this->TimeForIndex( i );
...
*/
template< class type >
BUD_INLINE type idCurve_NaturalCubicSpline<type>::GetCurrentSecondDerivative( const float time ) const
{
	float clampedTime = this->ClampedTime( time );
	int i = this->IndexForTime( clampedTime );
	float s = time - this->TimeForIndex( i );
...
*/
template< class type >
BUD_INLINE void idCurve_NaturalCubicSpline<type>::Setup() const
{
	if( this->changed )
	{
...
*/
template< class type >
BUD_INLINE void idCurve_NaturalCubicSpline<type>::SetupFree() const
{
	int i;
	float inv;
...
*/
template< class type >
BUD_INLINE void idCurve_NaturalCubicSpline<type>::SetupClamped() const
{
	int i;
	float inv;
	float* d0, *d1, *beta, *gamma;
...
*/
template< class type >
BUD_INLINE void idCurve_NaturalCubicSpline<type>::SetupClosed() const
{
	int i, j;
	float c0, c1;
	float* d0;
...
*/
template< class type >
BUD_INLINE idCurve_CatmullRomSpline<type>::idCurve_CatmullRomSpline()
{
}
...
*/
template< class type >
BUD_INLINE type idCurve_CatmullRomSpline<type>::GetCurrentValue( const float time ) const
{
	int i, j, k;
...
*/
template< class type >
BUD_INLINE type idCurve_CatmullRomSpline<type>::GetCurrentFirstDerivative( const float time ) const
{
	int i, j, k;
...
*/
template< class type >
BUD_INLINE type idCurve_CatmullRomSpline<type>::GetCurrentSecondDerivative( const float time ) const
{
	int i, j, k;
	float bvals[4], d, clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE void idCurve_CatmullRomSpline<type>::Basis( const int index, const float t, float* bvals ) const
{
...
*/
template< class type >
BUD_INLINE void idCurve_CatmullRomSpline<type>::BasisFirstDerivative( const int index, const float t, float* bvals ) const
{
	float s = ( float )( t - this->TimeForIndex( index ) ) / ( this->TimeForIndex( index + 1 ) - this->TimeForIndex( index ) );
...
*/
template< class type >
BUD_INLINE void idCurve_CatmullRomSpline<type>::BasisSecondDerivative( const int index, const float t, float* bvals ) const
{
	float s = ( float )( t - this->TimeForIndex( index ) ) / ( this->TimeForIndex( index + 1 ) - this->TimeForIndex( index ) );
	bvals[0] = -3.0f * s + 2.0f;
	bvals[1] = 9.0f * s - 5.0f;
...
protected:
	budList<float>		tension;
	budList<float>		continuity;
	budList<float>		bias;
...
*/
template< class type >
BUD_INLINE idCurve_KochanekBartelsSpline<type>::idCurve_KochanekBartelsSpline()
{
}
...
*/
template< class type >
BUD_INLINE int idCurve_KochanekBartelsSpline<type>::AddValue( const float time, const type& value )
{
	int i;
...
*/
template< class type >
BUD_INLINE int idCurve_KochanekBartelsSpline<type>::AddValue( const float time, const type& value, const float tension, const float continuity, const float bias )
{
	int i;
...
*/
template< class type >
BUD_INLINE type idCurve_KochanekBartelsSpline<type>::GetCurrentValue( const float time ) const
{
	int i;
	float bvals[4], clampedTime;
	type v, t0, t1;
...
*/
template< class type >
BUD_INLINE type idCurve_KochanekBartelsSpline<type>::GetCurrentFirstDerivative( const float time ) const
{
	int i;
	float bvals[4], d, clampedTime;
	type v, t0, t1;
...
*/
template< class type >
BUD_INLINE type idCurve_KochanekBartelsSpline<type>::GetCurrentSecondDerivative( const float time ) const
{
	int i;
	float bvals[4], d, clampedTime;
	type v, t0, t1;
...
*/
template< class type >
BUD_INLINE void idCurve_KochanekBartelsSpline<type>::TangentsForIndex( const int index, type& t0, type& t1 ) const
{
...
*/
template< class type >
BUD_INLINE void idCurve_KochanekBartelsSpline<type>::Basis( const int index, const float t, float* bvals ) const
{
	float s = ( float )( t - this->TimeForIndex( index ) ) / ( this->TimeForIndex( index + 1 ) - this->TimeForIndex( index ) );
...
*/
template< class type >
BUD_INLINE void idCurve_KochanekBartelsSpline<type>::BasisFirstDerivative( const int index, const float t, float* bvals ) const
{
	float s = ( float )( t - this->TimeForIndex( index ) ) / ( this->TimeForIndex( index + 1 ) - this->TimeForIndex( index ) );
...
*/
template< class type >
BUD_INLINE void idCurve_KochanekBartelsSpline<type>::BasisSecondDerivative( const int index, const float t, float* bvals ) const
{
	float s = ( float )( t - this->TimeForIndex( index ) ) / ( this->TimeForIndex( index + 1 ) - this->TimeForIndex( index ) );
...
*/
template< class type >
BUD_INLINE idCurve_BSpline<type>::idCurve_BSpline()
{
...
*/
template< class type >
BUD_INLINE type idCurve_BSpline<type>::GetCurrentValue( const float time ) const
{
	int i, j, k;
	float clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE type idCurve_BSpline<type>::GetCurrentFirstDerivative( const float time ) const
{
	int i, j, k;
	float clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE type idCurve_BSpline<type>::GetCurrentSecondDerivative( const float time ) const
{
	int i, j, k;
	float clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE float idCurve_BSpline<type>::Basis( const int index, const int order, const float t ) const
{
	if( order <= 1 )
	{
...
*/
template< class type >
BUD_INLINE float idCurve_BSpline<type>::BasisFirstDerivative( const int index, const int order, const float t ) const
{
...
*/
template< class type >
BUD_INLINE float idCurve_BSpline<type>::BasisSecondDerivative( const int index, const int order, const float t ) const
{
...
*/
template< class type >
BUD_INLINE idCurve_UniformCubicBSpline<type>::idCurve_UniformCubicBSpline()
{
...
*/
template< class type >
BUD_INLINE type idCurve_UniformCubicBSpline<type>::GetCurrentValue( const float time ) const
{
	int i, j, k;
	float bvals[4], clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE type idCurve_UniformCubicBSpline<type>::GetCurrentFirstDerivative( const float time ) const
{
	int i, j, k;
	float bvals[4], d, clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE type idCurve_UniformCubicBSpline<type>::GetCurrentSecondDerivative( const float time ) const
{
	int i, j, k;
	float bvals[4], d, clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE void idCurve_UniformCubicBSpline<type>::Basis( const int index, const float t, float* bvals ) const
{
	float s = ( float )( t - this->TimeForIndex( index ) ) / ( this->TimeForIndex( index + 1 ) - this->TimeForIndex( index ) );
...
*/
template< class type >
BUD_INLINE void idCurve_UniformCubicBSpline<type>::BasisFirstDerivative( const int index, const float t, float* bvals ) const
{
	float s = ( float )( t - this->TimeForIndex( index ) ) / ( this->TimeForIndex( index + 1 ) - this->TimeForIndex( index ) );
...
*/
template< class type >
BUD_INLINE void idCurve_UniformCubicBSpline<type>::BasisSecondDerivative( const int index, const float t, float* bvals ) const
{
	float s = ( float )( t - this->TimeForIndex( index ) ) / ( this->TimeForIndex( index + 1 ) - this->TimeForIndex( index ) );
	bvals[0] = -s + 1.0f;
	bvals[1] = 3.0f * s - 2.0f;
...
*/
template< class type >
BUD_INLINE idCurve_NonUniformBSpline<type>::idCurve_NonUniformBSpline()
{
}
...
*/
template< class type >
BUD_INLINE type idCurve_NonUniformBSpline<type>::GetCurrentValue( const float time ) const
{
	int i, j, k;
	float clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE type idCurve_NonUniformBSpline<type>::GetCurrentFirstDerivative( const float time ) const
{
	int i, j, k;
	float clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE type idCurve_NonUniformBSpline<type>::GetCurrentSecondDerivative( const float time ) const
{
	int i, j, k;
	float clampedTime;
	type v;
...
*/
template< class type >
BUD_INLINE void idCurve_NonUniformBSpline<type>::Basis( const int index, const int order, const float t, float* bvals ) const
{
	int r, s, i;
	float omega;
...
*/
template< class type >
BUD_INLINE void idCurve_NonUniformBSpline<type>::BasisFirstDerivative( const int index, const int order, const float t, float* bvals ) const
{
	int i;
...
*/
template< class type >
BUD_INLINE void idCurve_NonUniformBSpline<type>::BasisSecondDerivative( const int index, const int order, const float t, float* bvals ) const
{
	int i;
...
*/
template< class type >
BUD_INLINE idCurve_NURBS<type>::idCurve_NURBS()
{
}
...
*/
template< class type >
BUD_INLINE int idCurve_NURBS<type>::AddValue( const float time, const type& value )
{
	int i;
...
*/
template< class type >
BUD_INLINE int idCurve_NURBS<type>::AddValue( const float time, const type& value, const float weight )
{
	int i;
...
*/
template< class type >
BUD_INLINE type idCurve_NURBS<type>::GetCurrentValue( const float time ) const
{
	int i, j, k;
...
*/
template< class type >
BUD_INLINE type idCurve_NURBS<type>::GetCurrentFirstDerivative( const float time ) const
{
	int i, j, k;
...
*/
template< class type >
BUD_INLINE type idCurve_NURBS<type>::GetCurrentSecondDerivative( const float time ) const
{
	int i, j, k;
...
*/
template< class type >
BUD_INLINE float idCurve_NURBS<type>::WeightForIndex( const int index ) const
{
	int n = weights.Num() - 1;
