...
*/
template< class type >
BUD_INLINE idExtrapolate<type>::idExtrapolate()
{
...
*/
template< class type >
BUD_INLINE void idExtrapolate<type>::Init( const int startTime, const int duration, const type& startValue, const type& baseSpeed, const type& speed, const extrapolation_t extrapolationType )
{
...
*/
template< class type >
BUD_INLINE type idExtrapolate<type>::GetCurrentValue( int time ) const
{
	if( time < startTime )
	{
		return startValue;
	...
	switch( extrapolationType & ~EXTRAPOLATION_NOSTOP )
	{
		...
		case EXTRAPOLATION_ACCELSINE:
		{
			...
			{
				const float deltaTime = ( time - startTime ) / ( float )duration;
				const float s = ( 1.0f - budMath::Cos( deltaTime * budMath::HALF_PI ) ) * ( float )duration * 0.001f * budMath::SQRT_1OVER2;
				return startValue + deltaTime * baseSpeed + s * speed;
			}
		}
		case EXTRAPOLATION_DECELSINE:
		{
			...
			{
				const float deltaTime = ( time - startTime ) / ( float )duration;
				const float s = budMath::Sin( deltaTime * budMath::HALF_PI ) * ( float )duration * 0.001f * budMath::SQRT_1OVER2;
				return startValue + deltaTime * baseSpeed + s * speed;
			}
		}
	}
...
*/
template< class type >
BUD_INLINE type idExtrapolate<type>::GetCurrentSpeed( int time ) const
{
	...
	switch( extrapolationType & ~EXTRAPOLATION_NOSTOP )
	{
		...
		{
			const float deltaTime = ( time - startTime ) / ( float )duration;
			const float s = budMath::Sin( deltaTime * budMath::HALF_PI );
			return baseSpeed + s * speed;
		}
		case EXTRAPOLATION_DECELSINE:
		{
			const float deltaTime = ( time - startTime ) / ( float )duration;
			const float s = budMath::Cos( deltaTime * budMath::HALF_PI );
			return baseSpeed + s * speed;
		}
		default:
		{
