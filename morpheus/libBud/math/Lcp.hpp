...
public:
	...
	virtual bool	Solve( const budMatX& A, budVecX& x, const budVecX& b, const budVecX& lo,
						   const budVecX& hi, const int* boxIndex = NULL ) = 0;
						   
	virtual void	SetMaxIterations( int max );
	virtual int		GetMaxIterations();
	
	static void		Test_f( const budCmdArgs& args );
