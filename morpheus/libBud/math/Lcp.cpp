...
static budCVar lcp_showFailures( "lcp_showFailures", "0", CVAR_BOOL, "show LCP solver failures" );
...
ALIGN16( const __m128 SIMD_SP_infinity )						= { budMath::INFINITY, budMath::INFINITY, budMath::INFINITY, budMath::INFINITY };
...
========================
*/
static void LowerTriangularSolve_SIMD( const budMatX& L, float* x, const float* b, const int n, int skip )
{
	if( skip >= n )
	{
		return;
...
========================
*/
static void LowerTriangularSolveTranspose_SIMD( const budMatX& L, float* x, const float* b, const int n )
{
	int nc = L.GetNumColumns();
...
========================
*/
static void UpperTriangularSolve_SIMD( const budMatX& U, const float* invDiag, float* x, const float* b, const int n )
{
	for( int i = n - 1; i >= 0; i-- )
	{
		float sum = b[i];
...
========================
*/
static bool LU_Factor_SIMD( budMatX& mat, budVecX& invDiag, const int n )
{
	for( int i = 0; i < n; i++ )
	{
		...
		if( fabs( d1 ) < budMath::FLT_SMALLEST_NON_DENORMAL )
		{
			return false;
		}
...
========================
*/
static bool LDLT_Factor_SIMD( budMatX& mat, budVecX& invDiag, const int n )
{
	float s0, s1, s2, d;
	...
	if( fabs( sum ) < budMath::FLT_SMALLEST_NON_DENORMAL )
	{
		return false;
	}
							 ...
							 int d, float dir, float& maxStep, int& limit, int& limitSide )
{
	...
	// test the current variable
	{
		...
		int m1 = ( fabs( forceLimit ) != budMath::INFINITY );
		int m2 = ( step < maxStep );
		int m3 = ( m0 & m1 & m2 );
...
{
	libBud::Printf( string );
	for( int i = budStr::LengthWithoutColors( string ); i < 48; i++ )
	{
		libBud::Printf( " " );
	}
	if( clocks && otherClocks )
	{
		...
		if( clocks <= otherClocks )
		{
			p = budMath::Ftoi( ( float )( otherClocks - clocks ) * 100.0f / ( float ) otherClocks );
		}
		else
		{
			p = - budMath::Ftoi( ( float )( clocks - otherClocks ) * 100.0f / ( float ) clocks );
		}
		libBud::Printf( "c = %4d, clcks = %5lld, %d%%\n", dataCount, clocks, p );
	}
	else
	{
		libBud::Printf( "c = %4d, clcks = %5lld\n", dataCount, clocks );
	}
}
...
static void DotProduct_Test()
{
	...
	for( int i = 0; i < TEST_TRIANGULAR_SOLVE_SIZE; i++ )
	{
		...
		const char* result = budMath::Fabs( dot1 - dot2 ) < 1e-4f ? "ok" : S_COLOR_RED"X";
...
{
	budMatX src, original, mat1, mat2;
	budVecX invDiag1, invDiag2;
...
{
public:
	virtual bool	Solve( const budMatX& o_m, budVecX& o_x, const budVecX& o_b, const budVecX& o_lo, const budVecX& o_hi, const int* o_boxIndex );
	
private:
	budMatX			m;					// original matrix
	budVecX			b;					// right hand side
	budVecX			lo, hi;				// low and high bounds
	budVecX			f, a;				// force and acceleration
	budVecX			delta_f, delta_a;	// delta force and delta acceleration
	budMatX			clamped;			// LU factored sub matrix for clamped variables
	budVecX			diagonal;			// reciprocal of diagonal of U of the LU factored sub matrix for clamped variables
	...
	bool			FactorClamped();
	void			SolveClamped( budVecX& x, const float* b );
...
========================
*/
void idLCP_Square::SolveClamped( budVecX& x, const float* b )
{
	// solve L
...
void idLCP_Square::RemoveClamped( int r )
{
	...
	for( int i = r; i < numClamped; i++ )
	{
		...
		if( fabs( diag ) < budMath::FLT_SMALLEST_NON_DENORMAL )
		{
			libBud::Printf( "idLCP_Square::RemoveClamped: updating factorization failed\n" );
			diag = budMath::FLT_SMALLEST_NON_DENORMAL;
		}
...
========================
*/
BUD_INLINE void idLCP_Square::CalcAccelDelta( int d )
{
...
========================
*/
BUD_INLINE void idLCP_Square::ChangeForce( int d, float step )
{
...
========================
*/
BUD_INLINE void idLCP_Square::ChangeAccel( int d, float step )
{
...
========================
*/
bool idLCP_Square::Solve( const budMatX& o_m, budVecX& o_x, const budVecX& o_b, const budVecX& o_lo, const budVecX& o_hi, const int* o_boxIndex )
{
	...
	for( int i = 0; i < m.GetNumRows(); i++ )
	{
		if( lo[i] == -budMath::INFINITY && hi[i] == budMath::INFINITY )
		{
			if( numUnbounded != i )
			{
				Swap( numUnbounded, i );
	...
	if( numUnbounded )
	{
		...
		if( !FactorClamped() )
		{
			libBud::Printf( "idLCP_Square::Solve: unbounded factorization failed\n" );
			return false;
		}
	...
	for( int i = numUnbounded; i < m.GetNumRows(); i++ )
	{
		...
		if( i == boxStartIndex )
		{
			...
			for( int j = boxStartIndex; j < m.GetNumRows(); j++ )
			{
				...
				if( lo[j] != -budMath::INFINITY )
				{
					lo[j] = - budMath::Fabs( lo[j] * s );
				}
				if( hi[j] != budMath::INFINITY )
				{
					hi[j] = budMath::Fabs( hi[j] * s );
				}
			}
		}
		...
		if( budMath::Fabs( a[i] ) <= LCP_ACCEL_EPSILON )
		{
			side[i] = 0;
			AddClamped( i );
			continue;
	...
	if( numIgnored )
	{
		...
		{
			libBud::Printf( "idLCP_Square::Solve: %d of %d bounded variables ignored\n", numIgnored, m.GetNumRows() - numUnbounded );
		}
	}
#endif
	...
	if( failed.Length() )
	{
		if( lcp_showFailures.GetBool() )
		{
			libBud::Printf( "idLCP_Square::Solve: %s (%d of %d bounded variables ignored)\n", failed.c_str(), numIgnored, m.GetNumRows() - numUnbounded );
		}
	}
	...
	if( failed.Length() )
	{
		...
		for( int i = 0; i < m.GetNumRows(); i++ )
		{
			...
			else if( f[i] < lo[i] || f[i] > hi[i] || budMath::Fabs( a[i] ) > 1.0f )
			{
				int bah3 = 1;
			}
		}
...
private:
	...
	budVecX			f, a;				// force and acceleration
	budVecX			delta_f, delta_a;	// delta force and delta acceleration
	budMatX			clamped;			// LDLt factored sub matrix for clamped variables
	budVecX			diagonal;			// reciprocal of diagonal of LDLt factored sub matrix for clamped variables
	budVecX			solveCache1;		// intermediate result cached in SolveClamped
	budVecX			solveCache2;		// "
	int				numUnbounded;		// number of unbounded variables
	int				numClamped;			// number of clamped variables
...
========================
*/
void idLCP_Symmetric::SolveClamped( budVecX& x, const float* b )
{
...
void idLCP_Symmetric::AddClamped( int r, bool useSolveCache )
{
	...
	if( fabs( d ) < budMath::FLT_SMALLEST_NON_DENORMAL )
	{
		libBud::Printf( "idLCP_Symmetric::AddClamped: updating factorization failed\n" );
		d = budMath::FLT_SMALLEST_NON_DENORMAL;
	}
...
void idLCP_Symmetric::RemoveClamped( int r )
{
	...
	if( r == 0 )
	{
		...
		if( numClamped == 1 )
		{
			...
			if( fabs( diag ) < budMath::FLT_SMALLEST_NON_DENORMAL )
			{
				libBud::Printf( "idLCP_Symmetric::RemoveClamped: updating factorization failed\n" );
				diag = budMath::FLT_SMALLEST_NON_DENORMAL;
			}
			clamped[0][0] = diag;
	...
	float diag = budMath::SQRT_1OVER2;
	...
	for( int i = r; i < numClamped; i++ )
	{
		...
		if( fabs( newDiag ) < budMath::FLT_SMALLEST_NON_DENORMAL )
		{
			libBud::Printf( "idLCP_Symmetric::RemoveClamped: updating factorization failed\n" );
			newDiag = budMath::FLT_SMALLEST_NON_DENORMAL;
		}
...
========================
*/
BUD_INLINE void idLCP_Symmetric::CalcForceDelta( int d, float dir )
{
...
========================
*/
BUD_INLINE void idLCP_Symmetric::CalcAccelDelta( int d )
{
	// only the not clamped variables, including the current variable, can have a change in acceleration
	for( int j = numClamped; j <= d; j++ )
	{
...
========================
*/
BUD_INLINE void idLCP_Symmetric::ChangeForce( int d, float step )
{
	// only the clamped variables and current variable have a force delta unequal zero
	MultiplyAdd( f.ToFloatPtr(), step, delta_f.ToFloatPtr(), numClamped );
	f[d] += step * delta_f[d];
...
========================
*/
BUD_INLINE void idLCP_Symmetric::ChangeAccel( int d, float step )
{
	// only the not clamped variables, including the current variable, can have an acceleration unequal zero
	MultiplyAdd( a.ToFloatPtr() + numClamped, step, delta_a.ToFloatPtr() + numClamped, d - numClamped + 1 );
}
...
========================
*/
bool idLCP_Symmetric::Solve( const budMatX& o_m, budVecX& o_x, const budVecX& o_b, const budVecX& o_lo, const budVecX& o_hi, const int* o_boxIndex )
{
	...
	if( numUnbounded )
	{
		...
		if( !FactorClamped() )
		{
			libBud::Printf( "idLCP_Symmetric::Solve: unbounded factorization failed\n" );
			return false;
		}
		SolveClamped( f, b.ToFloatPtr() );
	...
	if( numIgnored )
	{
		if( lcp_showFailures.GetBool() )
		{
			libBud::Printf( "idLCP_Symmetric::Solve: %d of %d bounded variables ignored\n", numIgnored, m.GetNumRows() - numUnbounded );
		}
	}
#endif
	...
	if( failed.Length() )
	{
		if( lcp_showFailures.GetBool() )
		{
			libBud::Printf( "idLCP_Symmetric::Solve: %s (%d of %d bounded variables ignored)\n", failed.c_str(), numIgnored, m.GetNumRows() - numUnbounded );
		}
	}
...
========================
*/
void idLCP::Test_f( const budCmdArgs& args )
{
#ifdef ENABLE_TEST_CODE
	DotProduct_Test();
	LowerTriangularSolve_Test();
