...
#include "containers/Array.hpp"	// for idTupleSize
...
public:
	...
	friend budVec3	operator*( const budVec3& a, const idQuat& b );
...
BUD_INLINE idQuat::idQuat()
{
}

BUD_INLINE idQuat::idQuat( float x, float y, float z, float w )
{
	this->x = x;
	this->y = y;
	this->z = z;
...
BUD_INLINE float idQuat::operator[]( int index ) const
{
	assert( ( index >= 0 ) && ( index < 4 ) );
	return ( &x )[ index ];
}

BUD_INLINE float& idQuat::operator[]( int index )
{
	assert( ( index >= 0 ) && ( index < 4 ) );
	return ( &x )[ index ];
}

BUD_INLINE idQuat idQuat::operator-() const
{
	return idQuat( -x, -y, -z, -w );
}

BUD_INLINE idQuat& idQuat::operator=( const idQuat& a )
{
	x = a.x;
	y = a.y;
	z = a.z;
...
BUD_INLINE idQuat idQuat::operator+( const idQuat& a ) const
{
...
BUD_INLINE budVec3 idQuat::operator*( const budVec3& a ) const
{
#if 0
...
BUD_INLINE idQuat idQuat::operator*( float a ) const
{
	return idQuat( x * a, y * a, z * a, w * a );
}

BUD_INLINE idQuat operator*( const float a, const idQuat& b )
{
	return b * a;
}

BUD_INLINE budVec3 operator*( const budVec3& a, const idQuat& b )
{
	return b * a;
}
...
BUD_INLINE idQuat& idQuat::operator*=( float a )
{
	x *= a;
	y *= a;
	z *= a;
...
BUD_INLINE bool idQuat::Compare( const idQuat& a ) const
{
	return ( ( x == a.x ) && ( y == a.y ) && ( z == a.z ) && ( w == a.w ) );
}

BUD_INLINE bool idQuat::Compare( const idQuat& a, const float epsilon ) const
{
...
BUD_INLINE bool idQuat::operator==( const idQuat& a ) const
{
	return Compare( a );
}
...
BUD_INLINE void idQuat::Set( float x, float y, float z, float w )
{
	this->x = x;
	this->y = y;
	this->z = z;
...
BUD_INLINE idQuat idQuat::Inverse() const
{
	return idQuat( -x, -y, -z, w );
}

BUD_INLINE float idQuat::Length() const
{
	float len;
	
	len = x * x + y * y + z * z + w * w;
	return budMath::Sqrt( len );
}

BUD_INLINE idQuat& idQuat::Normalize()
{
	float len;
	float ilength;
...
BUD_INLINE float idQuat::CalcW() const
{
	// take the absolute value because floating point rounding may cause the dot of x,y,z to be larger than 1
	return sqrt( fabs( 1.0f - ( x * x + y * y + z * z ) ) );
}

BUD_INLINE int idQuat::GetDimension() const
{
	return 4;
}

BUD_INLINE const float* idQuat::ToFloatPtr() const
{
	return &x;
}

BUD_INLINE float* idQuat::ToFloatPtr()
{
	return &x;
}
...
BUD_INLINE idCQuat::idCQuat()
{
}

BUD_INLINE idCQuat::idCQuat( float x, float y, float z )
{
	this->x = x;
	this->y = y;
	this->z = z;
...
BUD_INLINE void idCQuat::Set( float x, float y, float z )
{
	this->x = x;
	this->y = y;
	this->z = z;
...
BUD_INLINE float idCQuat::operator[]( int index ) const
{
	assert( ( index >= 0 ) && ( index < 3 ) );
	return ( &x )[ index ];
}

BUD_INLINE float& idCQuat::operator[]( int index )
{
	assert( ( index >= 0 ) && ( index < 3 ) );
	return ( &x )[ index ];
}

BUD_INLINE bool idCQuat::Compare( const idCQuat& a ) const
{
	return ( ( x == a.x ) && ( y == a.y ) && ( z == a.z ) );
}

BUD_INLINE bool idCQuat::Compare( const idCQuat& a, const float epsilon ) const
{
	if( budMath::Fabs( x - a.x ) > epsilon )
	{
		return false;
...
BUD_INLINE bool idCQuat::operator==( const idCQuat& a ) const
{
	return Compare( a );
}
...
BUD_INLINE int idCQuat::GetDimension() const
{
	return 3;
}

BUD_INLINE idQuat idCQuat::ToQuat() const
{
	// take the absolute value because floating point rounding may cause the dot of x,y,z to be larger than 1
	return idQuat( x, y, z, sqrt( fabs( 1.0f - ( x * x + y * y + z * z ) ) ) );
}

BUD_INLINE const float* idCQuat::ToFloatPtr() const
{
	return &x;
}

BUD_INLINE float* idCQuat::ToFloatPtr()
{
	return &x;
}
