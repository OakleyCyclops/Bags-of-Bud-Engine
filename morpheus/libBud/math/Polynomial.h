...
BUD_INLINE idPolynomial::idPolynomial()
{
	degree = -1;
	allocated = 0;
	coefficient = NULL;
...
BUD_INLINE idPolynomial::idPolynomial( int d )
{
	degree = -1;
	allocated = 0;
	coefficient = NULL;
...
BUD_INLINE idPolynomial::idPolynomial( float a, float b )
{
	degree = -1;
	allocated = 0;
	coefficient = NULL;
...
BUD_INLINE idPolynomial::idPolynomial( float a, float b, float c )
{
	degree = -1;
	allocated = 0;
	coefficient = NULL;
...
BUD_INLINE idPolynomial::idPolynomial( float a, float b, float c, float d )
{
	degree = -1;
	allocated = 0;
	coefficient = NULL;
...
BUD_INLINE idPolynomial::idPolynomial( float a, float b, float c, float d, float e )
{
	degree = -1;
	allocated = 0;
	coefficient = NULL;
...
BUD_INLINE float idPolynomial::operator[]( int index ) const
{
	assert( index >= 0 && index <= degree );
	return coefficient[ index ];
}

BUD_INLINE float& idPolynomial::operator[]( int index )
{
	assert( index >= 0 && index <= degree );
	return coefficient[ index ];
}

BUD_INLINE idPolynomial idPolynomial::operator-() const
{
	int i;
	idPolynomial n;
...
BUD_INLINE idPolynomial& idPolynomial::operator=( const idPolynomial& p )
{
...
BUD_INLINE idPolynomial idPolynomial::operator+( const idPolynomial& p ) const
{
	int i;
	idPolynomial n;
...
BUD_INLINE idPolynomial idPolynomial::operator*( const float s ) const
{
	idPolynomial n;
...
BUD_INLINE idPolynomial& idPolynomial::operator*=( const float s )
{
	if( s == 0.0f )
	{
		degree = 0;
...
BUD_INLINE bool idPolynomial::Compare( const idPolynomial& p ) const
{
	if( degree != p.degree )
	{
		return false;
...
BUD_INLINE bool idPolynomial::Compare( const idPolynomial& p, const float epsilon ) const
{
	if( degree != p.degree )
	{
		return false;
	...
	for( int i = 0; i <= degree; i++ )
	{
		if( budMath::Fabs( coefficient[i] - p.coefficient[i] ) > epsilon )
		{
			return false;
		}
	}
...
BUD_INLINE bool idPolynomial::operator==( const idPolynomial& p ) const
{
	return Compare( p );
}
...
BUD_INLINE void idPolynomial::Zero()
{
	degree = 0;
}

BUD_INLINE void idPolynomial::Zero( int d )
{
	Resize( d, false );
	for( int i = 0; i <= degree; i++ )
	{
...
BUD_INLINE int idPolynomial::GetDimension() const
{
	return degree;
}

BUD_INLINE int idPolynomial::GetDegree() const
{
	return degree;
}

BUD_INLINE float idPolynomial::GetValue( const float x ) const
{
	float y, z;
	y = coefficient[0];
	z = x;
...
BUD_INLINE idComplex idPolynomial::GetValue( const idComplex& x ) const
{
	idComplex y, z;
...
BUD_INLINE idPolynomial idPolynomial::GetDerivative() const
{
	idPolynomial n;
...
BUD_INLINE idPolynomial idPolynomial::GetAntiDerivative() const
{
	idPolynomial n;
...
BUD_INLINE int idPolynomial::GetRoots1( float a, float b, float* roots )
{
	assert( a != 0.0f );
	roots[0] = - b / a;
	return 1;
...
BUD_INLINE int idPolynomial::GetRoots2( float a, float b, float c, float* roots )
{
	float inva, ds;
...
BUD_INLINE int idPolynomial::GetRoots3( float a, float b, float c, float d, float* roots )
{
	...
	if( ds < 0.0f )
	{
		dist = budMath::Sqrt( ( -1.0f / 3.0f ) * f );
		angle = ( 1.0f / 3.0f ) * budMath::ATan( budMath::Sqrt( -ds ), -halfg );
		cs = budMath::Cos( angle );
		ss = budMath::Sin( angle );
		roots[0] = 2.0f * dist * cs - ofs;
		roots[1] = -dist * ( cs + budMath::SQRT_THREE * ss ) - ofs;
		roots[2] = -dist * ( cs - budMath::SQRT_THREE * ss ) - ofs;
		return 3;
	}
	else if( ds > 0.0f )
	{
		...
		if( t >= 0.0f )
		{
			roots[0] = budMath::Pow( t, ( 1.0f / 3.0f ) );
		}
		else
		{
			roots[0] = -budMath::Pow( -t, ( 1.0f / 3.0f ) );
...
BUD_INLINE int idPolynomial::GetRoots4( float a, float b, float c, float d, float e, float* roots )
{
	int count;
	...
	else
	{
		...
		if( t2 >= 0.0f )
		{
			t2 = 2.0f * budMath::Sqrt( t2 );
			t1 = 0.75f * b * b - 2.0f * c;
			if( t1 + t2 >= 0.0f )
			{
				s1 = budMath::Sqrt( t1 + t2 );
				roots[count++] = -0.25f * b + 0.5f * s1;
				roots[count++] = -0.25f * b - 0.5f * s1;
			}
			if( t1 - t2 >= 0.0f )
			{
				s2 = budMath::Sqrt( t1 - t2 );
...
BUD_INLINE const float* idPolynomial::ToFloatPtr() const
{
	return coefficient;
}

BUD_INLINE float* idPolynomial::ToFloatPtr()
{
	return coefficient;
}

BUD_INLINE void idPolynomial::Resize( int d, bool keep )
{
	int alloc = ( d + 1 + 3 ) & ~3;
	if( alloc > allocated )
	{
