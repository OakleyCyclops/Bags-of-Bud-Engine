...
budStrPool		idDict::globalKeys;
budStrPool		idDict::globalValues;
...
================
*/
bool idDict::Parse( budParser& parser )
{
	budToken	token;
	budToken	token2;
	bool	errors;
...
{
	return budStr::Cmp( a->GetKey(), b->GetKey() );
}
...
int	idDict::Checksum() const
{
	...
	budList<idKeyValue> sorted = args;
	sorted.SortWithTemplate( budSort_KeyValue() );
	n = sorted.Num();
...
================
*/
bool idDict::GetAngles( const char* key, const char* defaultString, budAngles& out ) const
{
	bool		found;
	const char*	s;
...
================
*/
bool idDict::GetVector( const char* key, const char* defaultString, budVec3& out ) const
{
	bool		found;
	const char*	s;
...
================
*/
bool idDict::GetVec2( const char* key, const char* defaultString, budVec2& out ) const
{
	bool		found;
	const char*	s;
...
================
*/
bool idDict::GetVec4( const char* key, const char* defaultString, budVec4& out ) const
{
	bool		found;
	const char*	s;
...
================
*/
bool idDict::GetMatrix( const char* key, const char* defaultString, budMat3& out ) const
{
	const char*	s;
	bool		found;
...
================
*/
static void WriteString( const char* s, budFile* f )
{
	int	len = strlen( s );
	if( len >= MAX_STRING_CHARS - 1 )
	{
		libBud::common->Error( "idDict::WriteToFileHandle: bad string" );
	}
...
const idKeyValue* idDict::FindKey( const char* key ) const
{
	...
	if( key == NULL || key[0] == '\0' )
	{
		libBud::common->DWarning( "idDict::FindKey: empty key" );
		return NULL;
	}
...
int idDict::FindKeyIndex( const char* key ) const
{
	...
	if( key == NULL || key[0] == '\0' )
	{
		libBud::common->DWarning( "idDict::FindKeyIndex: empty key" );
		return 0;
	}
...
================
*/
void idDict::WriteToFileHandle( budFile* f ) const
{
...
// RB begin
void idDict::WriteJSON( budFile* f, const char* prefix ) const
{
	//f->Printf( "%s[\n", prefix );
...
================
*/
static budStr ReadString( budFile* f )
{
	...
	{
		libBud::common->Error( "idDict::ReadFromFileHandle: bad string" );
	}
...
================
*/
void idDict::ReadFromFileHandle( budFile* f )
{
	int c;
	budStr key, val;
...
========================
*/
void idDict::Serialize( budSerializer& ser )
{
	if( ser.IsReading() )
	{
		Clear();
...
================
*/
void idDict::WriteToIniFile( budFile* f ) const
{
	// make a copy so we don't affect the checksum of the original dict
	budList< idKeyValue > sortedArgs( args );
	sortedArgs.SortWithTemplate( budSort_KeyValue() );
	
	budList< budStr > prefixList;
	...
	for( int i = 0; i < sortedArgs.Num(); i++ )
	{
		...
		if( slashPosition != budStr::INVALID_POSITION )
		{
			budStr prefix = kv->GetKey().Mid( 0, slashPosition );
		...
		else
		{
			...
			budStr str = va( "%s=%s\n", kv->GetKey().c_str(), budStr::CStyleQuote( kv->GetValue() ) );
	...
	for( int i = 0; i < sortedArgs.Num(); i++ )
	{
		...
		const idKeyValue* kv = &sortedArgs[i];
		budStr str = va( "%s=%s\n", kv->GetKey().c_str() + prefixLength, budStr::CStyleQuote( kv->GetValue() ) );
		f->Write( ( void* )str.c_str(), str.Length() );
	}
}
...
================
*/
bool idDict::ReadFromIniFile( budFile* f )
{
	int length = f->Length();
	...
	budLexer parser( LEXFL_NOFATALERRORS | LEXFL_ALLOWPATHNAMES /*| LEXFL_ONLYSTRINGS */ );
	budStr name = f->GetName();
	...
	while( success && !parser.EndOfFile() )
	{
		...
		valueStr = budStr::CStyleUnQuote( valueStr );
		
		budStr key = va( "%s%s", prefix.c_str(), token.c_str() );
...
{
	// Write to the file
	budFile* file = fileSystem->OpenFileWrite( "idDict_ini_test.ini" );
	if( file == NULL )
	{
		libBud::Printf( "[^1FAILED^0] Couldn't open file for writing.\n" );
		return;
	}
	...
	if( file == NULL )
	{
		libBud::Printf( "[^1FAILED^0] Couldn't open file for reading.\n" );
	}
	...
	{
		libBud::Printf( "[^1FAILED^0] Dictionaries do not match.\n" );
	}
	else
	{
		libBud::Printf( "[^2PASSED^0] Dictionaries match.\n" );
	}
...
================
*/
void idDict::ShowMemoryUsage_f( const budCmdArgs& args )
{
	libBud::common->Printf( "%5d KB in %d keys\n", globalKeys.Size() >> 10, globalKeys.Num() );
	libBud::common->Printf( "%5d KB in %d values\n", globalValues.Size() >> 10, globalValues.Num() );
}
...
================
*/
void idDict::ListKeys_f( const budCmdArgs& args )
{
	libBud::Printf( "Not implemented due to sort impl issues.\n" );
	//int i;
	//budList<const idPoolStr *> keyStrings;
	
	//for ( i = 0; i < globalKeys.Num(); i++ ) {
	//	keyStrings.Append( globalKeys[i] );
	//}
	//keyStrings.SortWithTemplate( budSort_PoolStrPtr() );
	//for ( i = 0; i < keyStrings.Num(); i++ ) {
	//	libBud::common->Printf( "%s\n", keyStrings[i]->c_str() );
	//}
	//libBud::common->Printf( "%5d keys\n", keyStrings.Num() );
}
...
================
*/
void idDict::ListValues_f( const budCmdArgs& args )
{
	libBud::Printf( "Not implemented due to sort impl issues.\n" );
	//int i;
	//budList<const idPoolStr *> valueStrings;
	
	//for ( i = 0; i < globalValues.Num(); i++ ) {
	//	valueStrings.Append( globalValues[i] );
	//}
	//valueStrings.SortWithTemplate( budSort_PoolStrPtr() );
	//for ( i = 0; i < valueStrings.Num(); i++ ) {
	//	libBud::common->Printf( "%s\n", valueStrings[i]->c_str() );
	//}
	//libBud::common->Printf( "%5d values\n", valueStrings.Num() );
}
