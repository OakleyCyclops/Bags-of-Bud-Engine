...
class budPlaneSet : public budList<budPlane>
{
public:

	void					Clear()
	{
		budList<budPlane>::Clear();
		hash.Free();
	}
	
	int						FindPlane( const budPlane& plane, const float normalEps, const float distEps );
	
private:
	budHashIndex				hash;
};

BUD_INLINE int budPlaneSet::FindPlane( const budPlane& plane, const float normalEps, const float distEps )
{
	int i, border, hashKey;
	
	assert( distEps <= 0.125f );
	
	hashKey = ( int )( budMath::Fabs( plane.Dist() ) * 0.125f );
