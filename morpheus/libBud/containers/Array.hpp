...
/*
================================================
budArray is a replacement for a normal C array.

int		myArray[ARRAY_SIZE];

becomes:

budArray<int,ARRAY_SIZE>	myArray;
...
Unlike budStaticList, there are no fields other than the
actual raw data, and the size is fixed.
================================================
*/
template<class T_, int numElements > class budArray
{
public:
...
declaring two-dimensional budArrays easier.
...
struct id2DArray
{
	typedef budArray< budArray< _type_, _dim2_ >, _dim1_ > type;
};
...
struct idTupleSize< budArray< _type_, _num_ > >
{
	enum { value = _num_ };
};
