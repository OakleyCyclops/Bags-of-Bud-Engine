...
BUD_INLINE void* 	Mem_Alloc( const size_t size, const memTag_t tag )
{
	return Mem_Alloc16( size, tag );
}

BUD_INLINE void		Mem_Free( void* ptr )
{
	Mem_Free16( ptr );
}
...
BUD_INLINE void* operator new( size_t s )
#if !defined(_MSC_VER)
...
BUD_INLINE void operator delete( void* p )
#if !defined(_MSC_VER)
...
BUD_INLINE void* operator new( size_t s, memTag_t tag )
{
	return Mem_Alloc( s, tag );
}

BUD_INLINE void operator delete( void* p, memTag_t tag )
#if !defined(_MSC_VER)
throw() // DG: delete musn't throw
#endif
{
...
BUD_INLINE void operator delete[]( void* p, memTag_t tag ) throw() // DG: delete musn't throw
{
	Mem_Free( p );
}
...
*/
template < class T >
BUD_INLINE idTempArray<T>::idTempArray( idTempArray<T>& other )
{
	this->num = other.num;
...
*/
template < class T >
BUD_INLINE idTempArray<T>::idTempArray( unsigned int num )
{
	this->num = num;
...
*/
template < class T >
BUD_INLINE idTempArray<T>::~idTempArray()
{
	Mem_Free( buffer );
}
...
{
public:
	BUD_INLINE			idBlockAlloc( bool clear = false );
	BUD_INLINE			~idBlockAlloc();
	...
	BUD_INLINE void		Shutdown();
	BUD_INLINE void		SetFixedBlocks( int numBlocks );
	BUD_INLINE void		FreeEmptyBlocks();
	
	BUD_INLINE _type_* 	Alloc();
	BUD_INLINE void		Free( _type_ *element );
...
BUD_INLINE idBlockAlloc<_type_, _blockSize_, memTag>::idBlockAlloc( bool clear ) :
	blocks( NULL ),
	free( NULL ),
	total( 0 ),
	active( 0 ),
...
*/
template<class _type_, int _blockSize_, memTag_t memTag>
BUD_INLINE idBlockAlloc<_type_, _blockSize_, memTag>::~idBlockAlloc()
{
	Shutdown();
}
...
*/
template<class _type_, int _blockSize_, memTag_t memTag>
BUD_INLINE _type_* idBlockAlloc<_type_, _blockSize_, memTag>::Alloc()
{
...
*/
template<class _type_, int _blockSize_, memTag_t memTag>
BUD_INLINE void idBlockAlloc<_type_, _blockSize_, memTag>::Free( _type_ * t )
{
#ifdef FORCE_DISCRETE_BLOCK_ALLOCS
	// for debugging tools
	delete t;
...
*/
template<class _type_, int _blockSize_, memTag_t memTag>
BUD_INLINE void idBlockAlloc<_type_, _blockSize_, memTag>::Shutdown()
{
	while( blocks != NULL )
	{
...
*/
template<class _type_, int _blockSize_, memTag_t memTag>
BUD_INLINE void idBlockAlloc<_type_, _blockSize_, memTag>::SetFixedBlocks( int numBlocks )
{
...
*/
template<class _type_, int _blockSize_, memTag_t memTag>
BUD_INLINE void idBlockAlloc<_type_, _blockSize_, memTag>::AllocNewBlock()
{
...
*/
template<class _type_, int _blockSize_, memTag_t memTag>
BUD_INLINE void idBlockAlloc<_type_, _blockSize_, memTag>::FreeEmptyBlocks()
{
...
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::Shutdown()
{
	...
	for( block = firstBlock; block != NULL; block = firstBlock )
	{
		...
		if( lockMemory )
		{
			//libBud::sys->UnlockMemory( block, block->GetSize() + (int)sizeof( idDynamicBlock<type> ) );
		}
		Mem_Free16( block );
	}
...
void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::SetFixedBlocks( int numBlocks )
{
	...
	for( int i = numBaseBlocks; i < numBlocks; i++ )
	{
		...
		if( lockMemory )
		{
			//libBud::sys->LockMemory( block, baseBlockSize );
		}
#ifdef DYNAMIC_BLOCK_ALLOC_CHECK
...
template<class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
BUD_INLINE void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::LinkFreeInternal( idDynamicBlock<type>* block )
{
...
template<class type, int baseBlockSize, int minBlockSize, memTag_t _tag_>
BUD_INLINE void idDynamicBlockAlloc<type, baseBlockSize, minBlockSize, _tag_>::UnlinkFreeInternal( idDynamicBlock<type>* block )
{
