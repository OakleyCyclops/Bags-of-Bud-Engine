...
static int Sys_SetThreadName( pthread_t handle, const char* name )
{
	...
	if( ret != 0 )
		libBud::common->Printf( "Setting threadname \"%s\" failed, reason: %s (%i)\n", name, strerror( errno ), errno );
...
static int Sys_GetThreadName( pthread_t handle, char* namebuf, size_t buflen )
{
	...
	if( ret != 0 )
		libBud::common->Printf( "Getting threadname failed, reason: %s (%i)\n", strerror( errno ), errno );
#elif defined(__FreeBSD__)
	// seems like there is no pthread_getname_np equivalent on FreeBSD
	budStr::snPrintf( namebuf, buflen, "Can't read threadname on this platform!" );
#endif
	/* TODO: OSX:
...
uintptr_t Sys_CreateThread( xthread_t function, void* parms, xthreadPriority priority, const char* name, core_t core, int stackSize, bool suspended )
{
	...
	{
		libBud::common->FatalError( "ERROR: pthread_attr_setdetachstate %s failed\n", name );
		return ( uintptr_t )0;
	}
	...
	{
		libBud::common->FatalError( "ERROR: pthread_create %s failed\n", name );
		return ( uintptr_t )0;
	}
	...
	{
		libBud::common->Warning( "Warning: pthread_setname_np %s failed\n", name );
		return ( uintptr_t )0;
	}
#endif
	...
	if( error != 0 )
	{
		libBud::common->FatalError( "ERROR: pthread_getschedparam %s failed: %s\n", name, strerror( error ) );
		return ( uintptr_t )0;
	}
	...
	if( error != 0 )
	{
		libBud::common->FatalError( "ERROR: pthread_setschedparam( name = %s, policy = %i, priority = %i ) failed: %s\n", name, schedulePolicy, scheduleParam.__sched_priority, strerror( error ) );
		return ( uintptr_t )0;
	}
...
void Sys_DestroyThread( uintptr_t threadHandle )
{
	...
	{
		libBud::common->FatalError( "ERROR: pthread_cancel %s failed\n", name );
	}
#endif
	
	if( pthread_join( ( pthread_t )threadHandle, NULL ) != 0 )
	{
		libBud::common->FatalError( "ERROR: pthread_join %s failed\n", name );
	}
}
...
{
	//DWORD result = WaitForSingleObject( handle, timeout == budSysSignal::WAIT_INFINITE ? INFINITE : timeout );
	//assert( result == WAIT_OBJECT_0 || ( timeout != budSysSignal::WAIT_INFINITE && result == WAIT_TIMEOUT ) );
	...
	{
		++handle.waiting;
		if( timeout == budSysSignal::WAIT_INFINITE )
		{
	...
	assert( status == 0 || ( timeout != budSysSignal::WAIT_INFINITE && status == ETIMEDOUT ) );
