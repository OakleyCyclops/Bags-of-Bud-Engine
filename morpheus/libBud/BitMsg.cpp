...
/*
========================
budBitMsg::CheckOverflow
========================
*/
bool budBitMsg::CheckOverflow( int numBits )
{
	if( numBits > GetRemainingWriteBits() )
	{
		if( !allowOverflow )
		{
			libBud::FatalError( "budBitMsg: overflow without allowOverflow set; maxsize=%i size=%i numBits=%i numRemainingWriteBits=%i",
							   GetMaxSize(), GetSize(), numBits, GetRemainingWriteBits() );
		}
		if( numBits > ( maxSize << 3 ) )
		{
			libBud::FatalError( "budBitMsg: %i bits is > full message size", numBits );
		}
		libBud::Printf( "budBitMsg: overflow\n" );
		BeginWriting();
		overflowed = true;
		return true;
	}
...
/*
========================
budBitMsg::GetByteSpace
========================
*/
byte* budBitMsg::GetByteSpace( int length )
{
	byte* ptr;
	
	if( !writeData )
	{
		libBud::FatalError( "budBitMsg::GetByteSpace: cannot write to message" );
	}
...
========================
*/
void budBitMsg::WriteBits( int value, int numBits )
{
	if( !writeData )
	{
		libBud::FatalError( "budBitMsg::WriteBits: cannot write to message" );
	}
	
	// check if the number of bits is valid
	if( numBits == 0 || numBits < -31 || numBits > 32 )
	{
		libBud::FatalError( "budBitMsg::WriteBits: bad numBits %i", numBits );
	}
	...
	if( numBits != 32 )
	{
		if( numBits > 0 )
		{
			...
			{
				libBud::FatalError( "budBitMsg::WriteBits: value overflow %d %d",
								   value, numBits );
...
/*
========================
budBitMsg::WriteString
========================
*/
void budBitMsg::WriteString( const char* s, int maxLength, bool make7Bit )
{
	if( !s )
	{
		WriteData( "", 1 );
...
========================
*/
void budBitMsg::WriteData( const void* data, int length )
{
...
/*
========================
budBitMsg::WriteNetadr
========================
*/
void budBitMsg::WriteNetadr( const netadr_t adr )
{
	WriteData( adr.ip, 4 );
...
/*
========================
budBitMsg::WriteDeltaDict
========================
*/
bool budBitMsg::WriteDeltaDict( const idDict& dict, const idDict* base )
{
	int i;
...
========================
*/
int budBitMsg::ReadBits( int numBits ) const
{
	int		value;
	int		valueBits;
	int		get;
	...
	if( !readData )
	{
		libBud::FatalError( "budBitMsg::ReadBits: cannot read from message" );
	}
	
	// check if the number of bits is valid
	if( numBits == 0 || numBits < -31 || numBits > 32 )
	{
		libBud::FatalError( "budBitMsg::ReadBits: bad numBits %i", numBits );
	}
...
/*
========================
budBitMsg::ReadString
========================
*/
int budBitMsg::ReadString( char* buffer, int bufferSize ) const
{
	int	l, c;
...
========================
*/
int budBitMsg::ReadString( budStr& str ) const
{
	ReadByteAlign();
...
========================
*/
int budBitMsg::ReadData( void* data, int length ) const
{
	int cnt;
...
/*
========================
budBitMsg::ReadNetadr
========================
*/
void budBitMsg::ReadNetadr( netadr_t* adr ) const
{
	ReadData( adr->ip, 4 );
	adr->port = ReadUShort();
...
/*
========================
budBitMsg::ReadDeltaDict
========================
*/
bool budBitMsg::ReadDeltaDict( idDict& dict, const idDict* base ) const
{
...
========================
*/
int budBitMsg::DirToBits( const budVec3& dir, int numBits )
{
	int max, bits;
	float bias;
	...
	bits |= ( budMath::Ftoi( ( budMath::Fabs( dir.x ) + bias ) * max ) ) << ( numBits * 2 );
	bits |= IEEE_FLT_SIGNBITSET( dir.y ) << ( numBits * 2 - 1 );
	bits |= ( budMath::Ftoi( ( budMath::Fabs( dir.y ) + bias ) * max ) ) << ( numBits * 1 );
	bits |= IEEE_FLT_SIGNBITSET( dir.z ) << ( numBits * 1 - 1 );
	bits |= ( budMath::Ftoi( ( budMath::Fabs( dir.z ) + bias ) * max ) ) << ( numBits * 0 );
	return bits;
}
...
========================
*/
budVec3 budBitMsg::BitsToDir( int bits, int numBits )
{
