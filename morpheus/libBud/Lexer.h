...
===============================================================================
	...
	However, budToken may still allocate memory for large strings.
...
public:
	...
	budLexer();
	budLexer( int flags );
	budLexer( const char* filename, int flags = 0, bool OSPath = false );
	budLexer( const char* ptr, int length, const char* name, int flags = 0 );
	// destructor
	~budLexer();
	...
	const char* 	ParseBracedSection( budStr& out );
	...
	const char* 	ParseCompleteLine( budStr& out );
...
private:
	...
	budToken			token;					// available token
	budLexer* 		next;					// next script in a chain
	bool			hadError;				// set by budLexer::Error, even if the error is supressed
...
private:
	...
	int				ReadString( budToken* token, int quote );
	int				ReadName( budToken* token );
	int				ReadNumber( budToken* token );
	int				ReadPunctuation( budToken* token );
	int				ReadPrimitive( budToken* token );
	int				CheckString( const char* str ) const;
	int				NumLinesCrossed();
};

BUD_INLINE const char* budLexer::GetFileName()
{
	return budLexer::filename;
}

BUD_INLINE const int budLexer::GetFileOffset()
{
	return budLexer::script_p - budLexer::buffer;
}

BUD_INLINE const ID_TIME_T budLexer::GetFileTime()
{
	return budLexer::fileTime;
}

BUD_INLINE const int budLexer::GetLineNum()
{
	return budLexer::line;
}

BUD_INLINE void budLexer::SetFlags( int flags )
{
	budLexer::flags = flags;
}

BUD_INLINE int budLexer::GetFlags()
{
	return budLexer::flags;
}
