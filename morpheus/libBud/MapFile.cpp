...
===============
*/
BUD_INLINE unsigned int FloatCRC( float f )
{
...
===============
*/
BUD_INLINE unsigned int StringCRC( const char* str )
{
	unsigned int i, crc;
...
=================
*/
static void ComputeAxisBase( const budVec3& normal, budVec3& texS, budVec3& texT )
{
	float RotY, RotZ;
	budVec3 n;
	
	// do some cleaning
	n[0] = ( budMath::Fabs( normal[0] ) < 1e-6f ) ? 0.0f : normal[0];
	n[1] = ( budMath::Fabs( normal[1] ) < 1e-6f ) ? 0.0f : normal[1];
	n[2] = ( budMath::Fabs( normal[2] ) < 1e-6f ) ? 0.0f : normal[2];
	
	RotY = -atan2( n[2], budMath::Sqrt( n[1] * n[1] + n[0] * n[0] ) );
	RotZ = atan2( n[1], n[0] );
...
=================
*/
void idMapBrushSide::GetTextureVectors( budVec4 v[2] ) const
{
	int i;
	budVec3 texX, texY;
...
=================
*/
idMapPatch* idMapPatch::Parse( budLexer& src, const budVec3& origin, bool patchDef3, float version )
{
	float		info[7];
	budDrawVert* vert;
	budToken		token;
...
============
*/
bool idMapPatch::Write( budFile* fp, int primitiveNum, const budVec3& origin ) const
{
	int i, j;
	const budDrawVert* v;
...
=================
*/
idMapBrush* idMapBrush::Parse( budLexer& src, const budVec3& origin, bool newFormat, float version )
{
	int i;
	budVec3 planepts[3];
	budToken token;
	budList<idMapBrushSide*> sides;
	idMapBrushSide*	side;
	idDict epairs;
...
=================
*/
idMapBrush* idMapBrush::ParseQ3( budLexer& src, const budVec3& origin )
{
	...
	do
	{
		...
		side->texMat[0] = budVec3( 0.03125f, 0.0f, 0.0f );
		side->texMat[1] = budVec3( 0.0f, 0.03125f, 0.0f );
		side->origin = origin;
...
============
*/
bool idMapBrush::Write( budFile* fp, int primitiveNum, const budVec3& origin ) const
{
	int i;
	idMapBrushSide* side;
...
================
*/
idMapEntity* idMapEntity::Parse( budLexer& src, bool worldSpawn, float version )
{
	budToken	token;
	idMapEntity* mapEnt;
	idMapPatch* mapPatch;
	...
	do
	{
		...
		else
		{
			...
			if( !budStr::Icmp( key, "origin" ) )
			{
				// scanf into doubles, then assign, so it is budVec size independent
				v1 = v2 = v3 = 0;
			...
			else if( !budStr::Icmp( key, "classname" ) && !budStr::Icmp( value, "worldspawn" ) )
			{
				worldent = true;
			}
		}
...
============
*/
bool idMapEntity::Write( budFile* fp, int entityNum ) const
{
	int i;
...
// RB begin
bool idMapEntity::WriteJSON( budFile* fp, int entityNum, int numEntities ) const
{
	budVec3 origin;
...
idMapEntity* idMapEntity::ParseJSON( budLexer& src )
{
	budToken	token;
	idMapEntity* mapEnt;
	//idMapPatch* mapPatch;
...
class budSort_CompareMapEntity : public budSort_Quick< idMapEntity*, budSort_CompareMapEntity >
{
public:
	int Compare( idMapEntity* const& a, idMapEntity* const& b ) const
	{
		if( budStr::Icmp( a->epairs.GetString( "name" ), "worldspawn" ) == 0 )
		{
			return 1;
		}
		
		if( budStr::Icmp( b->epairs.GetString( "name" ), "worldspawn" ) == 0 )
		{
			return -1;
		}
		
		return budStr::Icmp( a->epairs.GetString( "name" ), b->epairs.GetString( "name" ) );
	}
};
...
===============
*/
bool budMapFile::Parse( const char* filename, bool ignoreRegion, bool osPath )
{
	// no string concatenation for epairs and allow path names for materials
	budLexer src( LEXFL_NOSTRINGCONCAT | LEXFL_NOSTRINGESCAPECHARS | LEXFL_ALLOWPATHNAMES );
	budToken token;
	budStr fullName;
	idMapEntity* mapEnt;
	int i, j, k;
	...
	if( isJSON )
	{
		while( true )
		{
			...
			if( token == "entities" )
			{
				...
				if( token != ":" )
				{
					src.Error( "budMapFile::Parse: : not found, found %s", token.c_str() );
					return false;
				}
		...
		//entities.SortWithTemplate( budSort_CompareMapEntity() );
		
		if( entities.Num() > 0 && ( budStr::Icmp( entities[0]->epairs.GetString( "name" ), "worldspawn" ) != 0 ) )
		{
			// move world spawn to first place
			for( int i = 1; i < entities.Num(); i++ )
			{
				if( budStr::Icmp( entities[i]->epairs.GetString( "name" ), "worldspawn" ) == 0 )
				{
	...
	if( entities.Num() )
	{
		...
		if( entities[0]->epairs.GetBool( "moveFuncGroups" ) )
		{
			...
			{
				mapEnt = entities[i];
				if( budStr::Icmp( mapEnt->epairs.GetString( "classname" ), "func_group" ) == 0 )
				{
...
============
*/
bool budMapFile::Write( const char* fileName, const char* ext, bool fromBasePath )
{
	int i;
	budStr qpath;
	budFile* fp;
	...
	libBud::common->Printf( "writing %s...\n", qpath.c_str() );
	
	if( fromBasePath )
	{
		fp = libBud::fileSystem->OpenFileWrite( qpath, "fs_basepath" );
	}
	else
	{
		fp = libBud::fileSystem->OpenExplicitFileWrite( qpath );
	}
	
	if( !fp )
	{
		libBud::common->Warning( "Couldn't open %s\n", qpath.c_str() );
		return false;
	}
...
// RB begin
bool budMapFile::WriteJSON( const char* fileName, const char* ext, bool fromBasePath )
{
	int i;
	budStr qpath;
	budFile* fp;
...
/*
===============
budMapFile::SetGeometryCRC
===============
*/
void budMapFile::SetGeometryCRC()
{
	int i;
...
/*
===============
budMapFile::AddEntity
===============
*/
int budMapFile::AddEntity( idMapEntity* mapEnt )
{
...
/*
===============
budMapFile::FindEntity
===============
*/
idMapEntity* budMapFile::FindEntity( const char* name )
{
	for( int i = 0; i < entities.Num(); i++ )
	{
		idMapEntity* ent = entities[i];
		if( budStr::Icmp( ent->epairs.GetString( "name" ), name ) == 0 )
		{
			return ent;
		}
	}
...
/*
===============
budMapFile::RemoveEntity
===============
*/
void budMapFile::RemoveEntity( idMapEntity* mapEnt )
{
...
===============
*/
void budMapFile::RemoveEntities( const char* classname )
{
	for( int i = 0; i < entities.Num(); i++ )
	{
		idMapEntity* ent = entities[i];
		if( budStr::Icmp( ent->epairs.GetString( "classname" ), classname ) == 0 )
		{
			delete entities[i];
...
/*
===============
budMapFile::RemoveAllEntities
===============
*/
void budMapFile::RemoveAllEntities()
{
	entities.DeleteContents( true );
	hasPrimitiveData = false;
}
...
/*
===============
budMapFile::RemovePrimitiveData
===============
*/
void budMapFile::RemovePrimitiveData()
{
	for( int i = 0; i < entities.Num(); i++ )
	{
		idMapEntity* ent = entities[i];
...
/*
===============
budMapFile::NeedsReload
===============
*/
bool budMapFile::NeedsReload()
{
	if( name.Length() )
	{
		ID_TIME_T time = FILE_NOT_FOUND_TIMESTAMP;
		if( libBud::fileSystem->ReadFile( name, NULL, &time ) > 0 )
		{
...
bool MapPolygonMesh::Write( budFile* fp, int primitiveNum, const budVec3& origin ) const
{
	fp->WriteFloatString( "// primitive %d\n{\n meshDef\n {\n", primitiveNum );
	//fp->WriteFloatString( "  \"%s\"\n  ( %d %d 0 0 0 )\n", GetMaterial(), GetWidth(), GetHeight() );
...
bool MapPolygonMesh::WriteJSON( budFile* fp, int primitiveNum, const budVec3& origin ) const
{
	fp->WriteFloatString( "\t\t\t\t{\n\t\t\t\t\t\"primitive\": \"%d\",\n", primitiveNum );
...
MapPolygonMesh* MapPolygonMesh::Parse( budLexer& src, const budVec3& origin, float version )
{
	float		info[7];
	budToken		token;
	int			i;
...
MapPolygonMesh* MapPolygonMesh::ParseJSON( budLexer& src )
{
	budToken		token;
...
bool budMapFile::ConvertToPolygonMeshFormat()
{
