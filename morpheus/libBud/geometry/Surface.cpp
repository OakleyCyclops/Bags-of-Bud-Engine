...
=================
*/
BUD_INLINE int UpdateVertexIndex( int vertexIndexNum[2], int* vertexRemap, int* vertexCopyIndex, int vertNum )
{
...
=================
*/
int idSurface::Split( const budPlane& plane, const float epsilon, idSurface** front, idSurface** back, int* frontOnPlaneEdges, int* backOnPlaneEdges ) const
{
	float* 			dists;
	float			f;
	byte* 			sides;
...
=================
*/
bool idSurface::ClipInPlace( const budPlane& plane, const float epsilon, const bool keepOn )
{
	float* 			dists;
	float			f;
	byte* 			sides;
...
=============
*/
float idSurface::PlaneDistance( const budPlane& plane ) const
{
	int		i;
	float	d, min, max;
...
=============
*/
int idSurface::PlaneSide( const budPlane& plane, const float epsilon ) const
{
	bool	front, back;
	int		i;
	float	d;
...
=================
*/
bool idSurface::LineIntersection( const budVec3& start, const budVec3& end, bool backFaceCull ) const
{
	float scale;
...
=================
*/
bool idSurface::RayIntersection( const budVec3& start, const budVec3& dir, float& scale, bool backFaceCull ) const
{
	...
	for( i = 0; i < edgeIndexes.Num(); i += 3 )
	{
		...
		if( s0 & s1 & s2 )
		{
			...
			if( budMath::Fabs( s ) < budMath::Fabs( scale ) )
			{
				scale = s;
			}
		}
	...
	if( budMath::Fabs( scale ) < budMath::INFINITY )
	{
		return true;
	}
	return false;
