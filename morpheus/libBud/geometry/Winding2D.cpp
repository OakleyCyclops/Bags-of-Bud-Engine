...
============
*/
bool GetAxialBevel( const budVec3& plane1, const budVec3& plane2, const budVec2& point, budVec3& bevel )
{
	if( IEEE_FLT_SIGNBITSET( plane1.x ) ^ IEEE_FLT_SIGNBITSET( plane2.x ) )
	{
		if( budMath::Fabs( plane1.x ) > 0.1f && budMath::Fabs( plane2.x ) > 0.1f )
		{
			bevel.x = 0.0f;
	...
	{
		if( budMath::Fabs( plane1.y ) > 0.1f && budMath::Fabs( plane2.y ) > 0.1f )
		{
			bevel.y = 0.0f;
...
============
*/
void idWinding2D::ExpandForAxialBox( const budVec2 bounds[2] )
{
	int i, j, numPlanes;
	budVec2 v;
	budVec3 planes[MAX_POINTS_ON_WINDING_2D], plane, bevel;
...
{
	int i;
	budVec2 edgeNormals[MAX_POINTS_ON_WINDING_2D];
	
	for( i = 0; i < numPoints; i++ )
	{
		budVec2& start = p[i];
		budVec2& end = p[( i + 1 ) % numPoints];
...
=============
*/
int idWinding2D::Split( const budVec3& plane, const float epsilon, idWinding2D** front, idWinding2D** back ) const
{
...
============
*/
bool idWinding2D::ClipInPlace( const budVec3& plane, const float epsilon, const bool keepOn )
{
	int i, j, maxpts, newNumPoints;
	int sides[MAX_POINTS_ON_WINDING_2D + 1], counts[3];
	float dot, dists[MAX_POINTS_ON_WINDING_2D + 1];
	budVec2* p1, *p2, mid, newPoints[MAX_POINTS_ON_WINDING_2D + 4];
	...
	memcpy( p, newPoints, newNumPoints * sizeof( budVec2 ) );
...
============
*/
budVec2 idWinding2D::GetCenter() const
{
	int i;
	budVec2 center;
...
============
*/
float idWinding2D::GetRadius( const budVec2& center ) const
{
	int i;
	float radius, r;
	budVec2 dir;
...
============
*/
void idWinding2D::GetBounds( budVec2 bounds[2] ) const
{
	int i;
	
	if( !numPoints )
	{
		bounds[0].x = bounds[0].y = budMath::INFINITY;
		bounds[1].x = bounds[1].y = -budMath::INFINITY;
		return;
	}
...
void idWinding2D::Print() const
{
	...
	for( i = 0; i < numPoints; i++ )
	{
		libBud::common->Printf( "(%5.1f, %5.1f)\n", p[i][0], p[i][1] );
	}
}
...
=============
*/
float idWinding2D::PlaneDistance( const budVec3& plane ) const
{
	int		i;
	float	d, min, max;
...
=============
*/
int idWinding2D::PlaneSide( const budVec3& plane, const float epsilon ) const
{
	bool	front, back;
	int		i;
	float	d;
...
============
*/
bool idWinding2D::PointInside( const budVec2& point, const float epsilon ) const
{
	int i;
	float d;
	budVec3 plane;
...
============
*/
bool idWinding2D::LineIntersection( const budVec2& start, const budVec2& end ) const
{
	int i, numEdges;
	int sides[MAX_POINTS_ON_WINDING_2D + 1], counts[3];
	float d1, d2, epsilon = 0.1f;
	budVec3 plane, edges[2];
...
============
*/
bool idWinding2D::RayIntersection( const budVec2& start, const budVec2& dir, float& scale1, float& scale2, int* edgeNums ) const
{
	...
	if( budMath::Fabs( scale1 ) > budMath::Fabs( scale2 ) )
	{
