...
=============
*/
void idWinding::BaseForPlane( const budVec3& normal, const float dist )
{
	budVec3 org, vright, vup;
...
=============
*/
int idWinding::Split( const budPlane& plane, const float epsilon, idWinding** front, idWinding** back ) const
{
	float* 			dists;
	byte* 			sides;
	int				counts[3];
	...
	*back = b = new( TAG_LIBBUD_WINDING ) idWinding( maxpts );
	...
	{
		libBud::common->FatalError( "idWinding::Split: points exceeded estimate." );
	}
...
=============
*/
idWinding* idWinding::Clip( const budPlane& plane, const float epsilon, const bool keepOn )
{
	float* 		dists;
	byte* 		sides;
	budVec5* 	newPoints;
	...
	newPoints = ( budVec5* ) _alloca16( maxpts * sizeof( budVec5 ) );
	newNumPoints = 0;
	...
	memcpy( p, newPoints, newNumPoints * sizeof( budVec5 ) );
...
=============
*/
bool idWinding::ClipInPlace( const budPlane& plane, const float epsilon, const bool keepOn )
{
	float*		dists;
	byte* 		sides;
	budVec5* 	newPoints;
...
bool idWinding::Check( bool print ) const
{
	...
	if( numPoints < 3 )
	{
		if( print )
		{
			libBud::common->Printf( "idWinding::Check: only %i points.", numPoints );
		}
		return false;
	}
	...
	if( area < 1.0f )
	{
		if( print )
		{
			libBud::common->Printf( "idWinding::Check: tiny area: %f", area );
		}
		return false;
	}
	...
	for( i = 0; i < numPoints; i++ )
	{
		...
		for( j = 0; j < 3; j++ )
		{
			if( p1[j] >= MAX_WORLD_COORD || p1[j] <= MIN_WORLD_COORD )
			{
				if( print )
				{
					libBud::common->Printf( "idWinding::Check: point %d outside world %c-axis: %f", i, 'X' + j, p1[j] );
				}
				return false;
			}
		}
		...
		if( d < -ON_EPSILON || d > ON_EPSILON )
		{
			if( print )
			{
				libBud::common->Printf( "idWinding::Check: point %d off plane.", i );
			}
			return false;
		}
		
		// check if the edge isn't degenerate
		const budVec3& p2 = p[j].ToVec3();
		dir = p2 - p1;
		...
		if( dir.Length() < ON_EPSILON )
		{
			if( print )
			{
				libBud::common->Printf( "idWinding::Check: edge %d is degenerate.", i );
			}
			return false;
		}
		...
		for( j = 0; j < numPoints; j++ )
		{
			...
			if( d > edgedist )
			{
				if( print )
				{
					libBud::common->Printf( "idWinding::Check: non-convex." );
				}
				return false;
			}
		}
...
=============
*/
float idWinding::GetRadius( const budVec3& center ) const
{
	int i;
	float radius, r;
	budVec3 dir;
...
=============
*/
budVec3 idWinding::GetCenter() const
{
	int i;
	budVec3 center;
...
=============
*/
void idWinding::GetPlane( budVec3& normal, float& dist ) const
{
	budVec3 v1, v2, center;
...
=============
*/
void idWinding::GetPlane( budPlane& plane ) const
{
	budVec3 v1, v2;
	budVec3 center;
...
=============
*/
void idWinding::GetBounds( budBounds& bounds ) const
{
	int i;
...
=============
*/
void idWinding::RemoveColinearPoints( const budVec3& normal, const float epsilon )
{
	int i, j;
	budVec3 edgeNormal;
	float dist;
	...
	for( i = 0; i < numPoints; i++ )
	{
		...
		if( budMath::Fabs( edgeNormal * p[( i + 1 ) % numPoints].ToVec3() - dist ) > epsilon )
		{
			continue;
		}
...
=============
*/
void idWinding::AddToConvexHull( const idWinding* winding, const budVec3& normal, const float epsilon )
{
	int				i, j, k;
	budVec3			dir;
	float			d;
	...
	newHullPoints = ( budVec5* ) _alloca( maxPts * sizeof( budVec5 ) );
	hullDirs = ( budVec3* ) _alloca( maxPts * sizeof( budVec3 ) );
	hullSide = ( bool* ) _alloca( maxPts * sizeof( bool ) );
	
	for( i = 0; i < winding->numPoints; i++ )
	{
		const budVec5& p1 = winding->p[i];
		...
		memcpy( this->p, newHullPoints, numNewHullPoints * sizeof( budVec5 ) );
	}
}
...
=============
*/
void idWinding::AddToConvexHull( const budVec3& point, const budVec3& normal, const float epsilon )
{
	...
	hullDirs = ( budVec3* ) _alloca( numPoints * sizeof( budVec3 ) );
	...
	hullPoints = ( budVec5* ) _alloca( ( numPoints + 1 ) * sizeof( budVec5 ) );
	...
	memcpy( p, hullPoints, numHullPoints * sizeof( budVec5 ) );
}
...
idWinding* idWinding::TryMerge( const idWinding& w, const budVec3& planenormal, int keep ) const
{
	budVec3*			p1, *p2, *p3, *p4, *back;
	idWinding*		newf;
	const idWinding*	f1, *f2;
	int				i, j, k, l;
	budVec3			normal, delta;
	float			dot;
	bool			keep1, keep2;
	...
	for( i = 0; i < f1->numPoints; i++ )
	{
		...
		for( j = 0; j < f2->numPoints; j++ )
		{
			...
			for( k = 0; k < 3; k++ )
			{
				if( budMath::Fabs( ( *p1 )[k] - ( *p4 )[k] ) > 0.1f )
				{
					break;
				}
				if( budMath::Fabs( ( *p2 )[k] - ( *p3 )[k] ) > 0.1f )
				{
					break;
				}
			}
...
void idWinding::RemovePoint( int point )
{
	...
	{
		libBud::common->FatalError( "idWinding::removePoint: point out of range" );
	}
...
=============
*/
void idWinding::InsertPoint( const budVec5& point, int spot )
{
	int i;
	
	if( spot > numPoints )
	{
		libBud::common->FatalError( "idWinding::insertPoint: spot > numPoints" );
	}
	
	if( spot < 0 )
	{
		libBud::common->FatalError( "idWinding::insertPoint: spot < 0" );
	}
...
=============
*/
bool idWinding::InsertPointIfOnEdge( const budVec5& point, const budPlane& plane, const float epsilon )
{
	int i;
	float dist, dot;
	budVec3 normal;
	...
	if( budMath::Fabs( plane.Distance( point.ToVec3() ) ) > epsilon )
	{
		return false;
	}
	...
	for( i = 0; i < numPoints; i++ )
	{
		...
		if( budMath::Fabs( normal * point.ToVec3() - dist ) > epsilon )
		{
			continue;
		}
...
=============
*/
bool idWinding::InsertPointIfOnEdge( const budVec3& point, const budPlane& plane, const float epsilon )
{
	int i;
	float dist, dot;
	budVec3 normal;
	...
	// point may not be too far from the winding plane
	if( budMath::Fabs( plane.Distance( point ) ) > epsilon )
	{
		return false;
	}
	...
	for( i = 0; i < numPoints; i++ )
	{
		...
		if( budMath::Fabs( normal * point - dist ) > epsilon )
		{
			continue;
		}
		...
		InsertPoint( budVec5( point.x, point.y, point.z, 0, 0 ), i + 1 );
		return true;
	}
	return false;
}
...
void idWinding::Print() const
{
	...
	for( i = 0; i < numPoints; i++ )
	{
		libBud::common->Printf( "(%5.1f, %5.1f, %5.1f)\n", p[i][0], p[i][1], p[i][2] );
	}
}
...
=============
*/
float idWinding::PlaneDistance( const budPlane& plane ) const
{
	int		i;
	float	d, min, max;
...
=============
*/
int idWinding::PlaneSide( const budPlane& plane, const float epsilon ) const
{
	bool	front, back;
	int		i;
	float	d;
...
bool idWinding::PlanesConcave( const idWinding& w2, const budVec3& normal1, const budVec3& normal2, float dist1, float dist2 ) const
{
	int i;
...
=============
*/
bool idWinding::PointInside( const budVec3& normal, const budVec3& point, const float epsilon ) const
{
	int i;
	budVec3 dir, n, pointvec;
...
=============
*/
bool idWinding::LineIntersection( const budPlane& windingPlane, const budVec3& start, const budVec3& end, bool backFaceCull ) const
{
	...
	if( budMath::Fabs( front - back ) < 0.0001f )
	{
		mid = end;
	}
	else
...
=============
*/
bool idWinding::RayIntersection( const budPlane& windingPlane, const budVec3& start, const budVec3& dir, float& scale, bool backFaceCull ) const
{
	int i;
...
=================
*/
float idWinding::TriangleArea( const budVec3& a, const budVec3& b, const budVec3& c )
{
	budVec3	v1, v2;
	budVec3	cross;
...
/*
=============
budFixedWinding::ReAllocate
=============
*/
bool budFixedWinding::ReAllocate( int n, bool keep )
{
	...
	{
		libBud::common->Printf( "WARNING: budFixedWinding -> MAX_POINTS_ON_WINDING overflowed\n" );
		return false;
	}
	return true;
}
...
/*
=============
budFixedWinding::Split
=============
*/
int budFixedWinding::Split( budFixedWinding* back, const budPlane& plane, const float epsilon )
{
	int		counts[3];
