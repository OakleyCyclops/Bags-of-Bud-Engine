...
====================
*/
void idSurface_SweptSpline::SetSpline( idCurve_Spline<budVec4>* spline )
{
	if( this->spline )
	{
		delete this->spline;
...
====================
*/
void idSurface_SweptSpline::SetSweptSpline( idCurve_Spline<budVec4>* sweptSpline )
{
	if( this->sweptSpline )
	{
...
void idSurface_SweptSpline::SetSweptCircle( const float radius )
{
	...
	nurbs->AddValue( 200.0f, budVec4( -radius, -radius, 0.0f, 0.50f ) );
	nurbs->AddValue( 300.0f, budVec4( radius, -radius, 0.0f, 0.75f ) );
	nurbs->SetBoundaryType( idCurve_NURBS<budVec4>::BT_CLOSED );
...
====================
*/
void idSurface_SweptSpline::GetFrame( const budMat3& previousFrame, const budVec3 dir, budMat3& newFrame )
{
	float wx, wy, wz;
	float xx, yy, yz;
	float xy, xz, zz;
	...
	a = budMath::ACos( previousFrame[2] * d ) * 0.5f;
	c = budMath::Cos( a );
	s = budMath::Sqrt( 1.0f - c * c );
...
void idSurface_SweptSpline::Tessellate( const int splineSubdivisions, const int sweptSplineSubdivisions )
{
	...
	sweptSplineDiv = sweptSpline->GetBoundaryType() == idCurve_Spline<budVec4>::BT_CLOSED ? sweptSplineSubdivisions : sweptSplineSubdivisions - 1;
	...
	splineDiv = spline->GetBoundaryType() == idCurve_Spline<budVec4>::BT_CLOSED ? splineSubdivisions : splineSubdivisions - 1;
	splineMat.Identity();
	budVec3 tempNormal;
