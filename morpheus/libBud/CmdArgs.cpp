...
============
*/
void budCmdArgs::operator=( const budCmdArgs& args )
{
	int i;
...
============
*/
const char* budCmdArgs::Args( int start, int end, bool escapeArgs ) const
{
...
============
*/
void budCmdArgs::TokenizeString( const char* text, bool keepAsStrings )
{
	budLexer		lex;
	budToken		token, number;
	int			len, totalLen;
	...
	while( 1 )
	{
		...
		if( token == "$" )
		{
			...
			if( libBud::cvarSystem )
			{
				token = libBud::cvarSystem->GetCVarString( token.c_str() );
			}
			else
			{
				token = "<unknown>";
		...
		budStr::Copynz( tokenized + totalLen, token.c_str(), sizeof( tokenized ) - totalLen );
...
/*
============
budCmdArgs::AppendArg
============
*/
void budCmdArgs::AppendArg( const char* text )
{
	if( argc >= MAX_COMMAND_ARGS )
	{
		return;
	...
	if( !argc )
	{
		argc = 1;
		argv[ 0 ] = tokenized;
		budStr::Copynz( tokenized, text, sizeof( tokenized ) );
	}
	else
	{
		argv[ argc ] = argv[ argc - 1 ] + strlen( argv[ argc - 1 ] ) + 1;
		budStr::Copynz( argv[ argc ], text, sizeof( tokenized ) - ( argv[ argc ] - tokenized ) );
		argc++;
	}
}
...
============
*/
const char* const* budCmdArgs::GetArgs( int* _argc )
{
	*_argc = argc;
