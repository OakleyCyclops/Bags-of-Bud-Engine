...
{
public:
	budSerializer( budBitMsg& msg_, bool writing_ ) : writing( writing_ ), msg( &msg_ )
	...
	void	SerializeRange( int& value, int minSize, int maxSize )  	// Supports signed types
	{
		...
		if( writing )
		{
			msg->WriteBits( value - minSize, budMath::BitsForInteger( maxSize - minSize ) );
		}
		else
		{
			value = minSize + msg->ReadBits( budMath::BitsForInteger( maxSize - minSize ) );
		}
	...
	void	SerializeUMax( int& value, int maxSize )  					// Unsigned only
	{
		...
		if( writing )
		{
			msg->WriteBits( value, budMath::BitsForInteger( maxSize ) );
		}
		else
		{
			value = msg->ReadBits( budMath::BitsForInteger( maxSize ) );
		}
	...
	//void SerializeBitMsg( budBitMsg & inOutMsg, int numBytes ) { SanityCheck(); if ( writing ) { msg->WriteBitMsg( inOutMsg, numBytes ); } else { msg->ReadBitMsg( inOutMsg, numBytes ); } }
	...
	void	SerializeQ( budMat3& axis, int bits = 15 )
	{
		Serialize( axis );
	}
#else
	...
	void	SerializeQ( budVec3& value )
	{
		...
		if( writing )
		{
			msg->WriteQuantizedVector< budVec3, _max_, _numBits_ >( value );
		}
		else
		{
			msg->ReadQuantizedVector< budVec3, _max_, _numBits_ >( value );
		}
	}
	template< int _max_, int _numBits_ >
	void	SerializeQ( float& value )
	...
	void	SerializeQ( budMat3& axis, int bits = 15 );		// Default to 15 bits per component, which has almost unnoticeable quantization
#endif
	
	void	Serialize( budMat3& axis );			// Raw 3x3 matrix serialize
	void	SerializeC( budMat3& axis );			// Uses compressed quaternion
	
	template< typename _type_ >
	void	SerializeListElement( const budList<_type_* >& list, const _type_*& element );
	...
	//void	SerializeString( budAtomicString & s )		{ SanityCheck(); if ( writing ) { msg->WriteString(s); } else { budStr temp; msg->ReadString( temp ); s.Set( temp ); } }
	void	SerializeString( budStr& s )
	{
		SanityCheck();
		if( writing )
		{
	...
	//void	SerializeString( budStrId & s )				{ SanityCheck(); if ( writing ) { msg->WriteString(s.GetKey()); } else { budStr key; msg->ReadString( key ); s.Set( key );} }
	...
	void	SerializeAngle( float& value )
	{
		...
		if( writing )
		{
			float nAngle = budMath::AngleNormalize360( value );
	...
	//			SanityCheck();
	//			// convert to degrees
	//			degrees_t d( value.Get() * budMath::M_RAD2DEG );
	//			Serialize( d );
	//			if ( !writing ) {
	//				// if reading, get the value we read in degrees and convert back to radians
	//				value.Set( d.Get() * budMath::M_DEG2RAD );
	//			}
	//		}
	...
	void	SanityCheck()
	{
		...
		else
		{
			...
			if( cccc != 0xCCCC )
			{
				libBud::Error( "budSerializer::SanityCheck - cccc != 0xCCCC" );
			}
			if( m != magic )
			{
				libBud::Error( "budSerializer::SanityCheck - m != magic" );
			}
		}
		magic++;
#endif
	...
	void SerializeCheckpoint( const char* file, int line )
	{
		...
		if( tag != tagValue )
		{
			libBud::Error( "SERIALIZE_CHECKPOINT: tag != tagValue (file: %s - line: %i)", file, line );
		}
#endif
	}
...
class budSerializerScopedBlock
{
public:
	budSerializerScopedBlock( budSerializer& ser_, int maxSizeBytes_ )
	{
		ser = &ser_;
	...
	~budSerializerScopedBlock()
	{
		...
		{
			libBud::Warning( "budSerializerScopedBlock went over maxSize (%d > %d)", sizeBytes, maxSizeBytes );
			return;
		}
...
/*
========================
budSerializer::SerializeQ
========================
*/
#ifndef SERIALIZE_NO_QUANT
BUD_INLINE void budSerializer::SerializeQ( budMat3& axis, int bits )
{
	SanityCheck();
	...
	if( IsWriting() )
	{
		...
		{
			if( budMath::Fabs( quat[i] ) > budMath::Fabs( quat[maxIndex] ) )
			{
				maxIndex = i;
			}
		}
		...
		msg->WriteBits( budMath::Ftoi( out.x * scale ), -bits );
		msg->WriteBits( budMath::Ftoi( out.y * scale ), -bits );
		msg->WriteBits( budMath::Ftoi( out.z * scale ), -bits );
	...
	else if( IsReading() )
	{
		...
		quat[maxIndex] = budMath::Sqrt( budMath::Fabs( 1.0f - in.x * in.x - in.y * in.y - in.z * in.z ) );
...
/*
========================
budSerializer::Serialize
========================
*/
BUD_INLINE void budSerializer::Serialize( budMat3& axis )
{
	SanityCheck();
...
/*
========================
budSerializer::SerializeC
========================
*/
BUD_INLINE void budSerializer::SerializeC( budMat3& axis )
{
	SanityCheck();
...
/*
========================
budSerializer::SerializeListElement
========================
*/
template< typename _type_ >
BUD_INLINE void budSerializer::SerializeListElement( const budList<_type_* >& list, const _type_*& element )
{
	SanityCheck();
...
========================
*/
BUD_INLINE void budSerializer::SerializePacked( int& original )
{
	SanityCheck();
...
/*
========================
budSerializer::SerializeSPacked
Writes out 7 bits at a time, using every 8th bit to signify more bits exist
...
========================
*/
BUD_INLINE void budSerializer::SerializeSPacked( int& value )
{
	SanityCheck();
	...
	if( IsWriting() )
	{
		...
		uint32 uvalue = budMath::Abs( value );
