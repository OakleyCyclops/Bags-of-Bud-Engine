...
budCVar net_ucmdRate( "net_ucmdRate", "40", CVAR_SYSTEM | CVAR_INTEGER, "How many milliseconds between sending usercmds" );

budCVar net_debug_snapShotTime( "net_debug_snapShotTime", "0", CVAR_BOOL | CVAR_ARCHIVE, "" );
budCVar com_forceLatestSnap( "com_forceLatestSnap", "0", CVAR_BOOL, "" );
...
budCVar net_effectiveSnapRateEnable( "net_effectiveSnapRateEnable", "1", CVAR_BOOL, "Dynamically adjust client snaprate" );
budCVar net_effectiveSnapRateDebug( "net_effectiveSnapRateDebug", "0", CVAR_BOOL, "Debug" );
...
budCVar net_minBufferedSnapPCT_Static( "net_minBufferedSnapPCT_Static", "1.0", CVAR_FLOAT, "Min amount of snapshot buffer time we want need to buffer" );
budCVar net_maxBufferedSnapMS( "net_maxBufferedSnapMS", "336", CVAR_INTEGER, "Max time to allow for interpolation cushion" );
budCVar net_minBufferedSnapWinPCT_Static( "net_minBufferedSnapWinPCT_Static", "1.0", CVAR_FLOAT, "Min amount of snapshot buffer time we want need to buffer" );
...
budCVar net_interpolationCatchupRate( "net_interpolationCatchupRate", "1.3", CVAR_FLOAT, "Scale interpolationg rate when we fall behind" );
budCVar net_interpolationFallbackRate( "net_interpolationFallbackRate", "0.95", CVAR_FLOAT, "Scale interpolationg rate when we fall behind" );
budCVar net_interpolationBaseRate( "net_interpolationBaseRate", "1.0", CVAR_FLOAT, "Scale interpolationg rate when we fall behind" );

// Enabled a dynamic ideal snap buffer window: we will scale the distance and size
budCVar net_optimalDynamic( "net_optimalDynamic", "1", CVAR_BOOL, "How fast to add to our optimal time buffer when we are playing snapshots faster than server is feeding them to us" );

// These values are used instead if net_optimalDynamic is 0 (don't scale by actual snap rate/interval)
budCVar net_optimalSnapWindow( "net_optimalSnapWindow", "112", CVAR_FLOAT, "" );
budCVar net_optimalSnapTime( "net_optimalSnapTime", "112", CVAR_FLOAT, "How fast to add to our optimal time buffer when we are playing snapshots faster than server is feeding them to us" );
...
budCVar net_interpolationSlowdownStart( "net_interpolationSlowdownStart", "0.5", CVAR_FLOAT, "Scale interpolation rate when we fall behind" );


// Extrapolation is now disabled
budCVar net_maxExtrapolationInMS( "net_maxExtrapolationInMS", "0", CVAR_INTEGER, "Max time in MS that extrapolation is allowed to occur." );

static const int SNAP_USERCMDS = 8192;

/*
===============
budCommonLocal::IsMultiplayer
===============
*/
bool budCommonLocal::IsMultiplayer()
{
	idLobbyBase& lobby = session->GetPartyLobbyBase();
	return ( ( ( lobby.GetMatchParms().matchFlags & MATCH_ONLINE ) != 0 ) && ( session->GetState() > budSession::IDLE ) );
}

/*
===============
budCommonLocal::IsServer
===============
*/
bool budCommonLocal::IsServer()
{
...
/*
===============
budCommonLocal::IsClient
===============
*/
bool budCommonLocal::IsClient()
{
...
/*
===============
budCommonLocal::SendSnapshots
===============
*/
int budCommonLocal::GetSnapRate()
{
...
===============
*/
void budCommonLocal::SendSnapshots()
{
	if( !mapSpawned )
	{
		return;
...
/*
===============
budCommonLocal::NetReceiveSnapshot
===============
*/
void budCommonLocal::NetReceiveSnapshot( class budSnapShot& ss )
{
	...
	{
		libBud::Printf( "Overwritting oldest snapshot %d with new snapshot %d\n", readSnapshotIndex, writeSnapshotIndex );
...
/*
===============
budCommonLocal::SendUsercmd
===============
*/
void budCommonLocal::SendUsercmds( int localClientNum )
{
	if( !mapSpawned )
	{
		return;
...
/*
========================
budCommonLocal::ProcessSnapshot
========================
*/
void budCommonLocal::ProcessSnapshot( budSnapShot& ss )
{
	...
	{
		libBud::Printf( "^2ProcessSnapshot. delta serverTime: %d  delta localTime: %d \n", ( snapCurrent.serverTime - snapPrevious.serverTime ), timeSinceLastSnap );
	}
	...
	for ( int i = 0; i < MAX_PLAYERS; i++ ) {
		budBitMsg msg;
		if ( ss.GetObjectMsgByID( budSession::SS_PLAYER + i, msg ) ) {
			if ( msg.GetSize() == 0 ) {
			...
			extern budCVar com_drawSnapshots;
			if ( com_drawSnapshots.GetInteger() == 3 ) {
				console->AddSnapObject( "players", msg.GetSize(), ss.CompareObject( &oldss, budSession::SS_PLAYER + i ) );
			}
		}
	}
	*/
...
/*
========================
budCommonLocal::NetReadUsercmds
========================
*/
void budCommonLocal::NetReadUsercmds( int clientNum, budBitMsg& msg )
{
	if( clientNum == -1 )
	{
		libBud::Warning( "NetReadUsercmds: Trying to read commands from invalid clientNum %d", clientNum );
		return;
	}
	...
	budStaticList< usercmd_t, NUM_USERCMD_RELAY >	newCmdBuffer;
...
/*
========================
budCommonLocal::ProcessNextSnapshot
========================
*/
void budCommonLocal::ProcessNextSnapshot()
{
	if( readSnapshotIndex == writeSnapshotIndex )
	{
		libBud::Printf( "No snapshots to process.\n" );
...
/*
========================
budCommonLocal::CalcSnapTimeBuffered
...
========================
*/
int budCommonLocal::CalcSnapTimeBuffered( int& totalBufferedTime, int& totalRecvTime )
{
	...
	//libBud::Printf( "CalcSnapTimeBuffered. timeLeft: %d totalRecvTime: %d, totalTimeBuffered: %d\n", timeLeft, totalRecvTime, totalBufferedTime );
	return timeLeft;
}

/*
========================
budCommonLocal::InterpolateSnapshot
========================
*/
void budCommonLocal::InterpolateSnapshot( netTimes_t& prev, netTimes_t& next, float fraction, bool predict )
{
...
/*
========================
budCommonLocal::RunNetworkSnapshotFrame
========================
*/
void budCommonLocal::RunNetworkSnapshotFrame()
{
	...
	if( snapPrevious.serverTime >= 0 )
	{
		...
		int	msec_interval = 1 + budMath::Ftoi( ( float )initialBaseTicksPerSec );
		
		static int clientTimeResidual = 0;
		static int lastTime = Sys_Milliseconds();
		int currentTime = Sys_Milliseconds();
		int deltaFrameTime = budMath::ClampInt( 1, 33, currentTime - lastTime );
		
		clientTimeResidual += budMath::ClampInt( 0, 50, currentTime - lastTime );
		lastTime = currentTime;
		
		extern budCVar com_fixedTic;
		...
		do
		{
			...
			snapCurrentTime = budMath::ClampInt( 0, snapRate + Min( ( int )snapRate, ( int )net_maxExtrapolationInMS.GetInteger() ), snapCurrentTime );
			
			if( snapRate <= 0 )
			{
				libBud::Warning( "snapRate <= 0. Resetting to 100" );
				snapRate = 100;
			}
			...
			if( !IsValid( fraction ) )
			{
				libBud::Warning( "Interpolation Fraction invalid: snapCurrentTime %d / snapRate %d", ( int )snapCurrentTime, ( int )snapRate );
				fraction = 0.0f;
			}
			...
			{
				optimalTimeBuffered = budMath::ClampInt( 0, net_maxBufferedSnapMS.GetInteger(), snapRate * optimalPCTBuffer );
			...
			{
				if( snapTimeBuffered <= budMath::FLT_SMALLEST_NON_DENORMAL )
				{
					snapRateScale = 0;
				}
				else
				{
					...
					{
						float frac = budMath::ClampFloat( 0.0f, 1.0f, snapTimeBuffered / startSlowdown );
						if( !IsValid( frac ) )
						{
							frac = 0.0f;
						}
			...
			if( net_effectiveSnapRateEnable.GetBool() )
			{
				...
				snapCurrentResidual = budMath::Frac( delta_interpolate ); // fixme: snapCurrentTime should just be a float, but would require changes in d4 too
...
/*
========================
budCommonLocal::ExecuteReliableMessages
========================
*/
void budCommonLocal::ExecuteReliableMessages()
{
...
/*
========================
budCommonLocal::ResetNetworkingState
========================
*/
void budCommonLocal::ResetNetworkingState()
{
	snapTime		= 0;
	snapTimeWrite	= 0;
	snapCurrentTime	= 0;
