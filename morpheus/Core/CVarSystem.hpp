...
public:
	...
	budCVar()
	{
		assert( typeid( this ) != typeid( budCVar ) );
	}
	
	// Always use one of the following constructors.
	budCVar( const char* name, const char* value, int flags, const char* description,
	...
	void					SetInternalVar( budCVar* cvar )
	{
		internalVar = cvar;
	}
...
protected:
	...
	budCVar* 				internalVar;			// internal cvar
	budCVar* 				next;					// next statically declared cvar
...
private:
	...
	static budCVar* 			staticVars;
};

BUD_INLINE budCVar::budCVar( const char* name, const char* value, int flags, const char* description,
...
public:
	...
	virtual bool			Command( const budCmdArgs& args ) = 0;
	...
	virtual void			WriteFlaggedVariables( int flags, const char* setCmd, budFile* f ) const = 0;
...
extern budCVarSystem* 		cvarSystem;
...
===============================================================================
	...
	budCVar::staticVars like this: budCVar * budCVar::staticVars = NULL;
	Furthermore budCVar::RegisterStaticVars() has to be called after the
	cvarSystem pointer is set when the DLL is first initialized.

===============================================================================
*/

BUD_INLINE void budCVar::Init( const char* name, const char* value, int flags, const char* description,
							 float valueMin, float valueMax, const char** valueStrings, argCompletion_t valueCompletion )
{
	this->name = name;
	this->value = value;
	...
	if( staticVars != ( budCVar* )UINTPTR_MAX )
	{
...
BUD_INLINE void budCVar::RegisterStaticVars()
{
	if( staticVars != ( budCVar* )UINTPTR_MAX )
	{
		for( budCVar* cvar = staticVars; cvar; cvar = cvar->next )
		{
			cvarSystem->Register( cvar );
		}
		staticVars = ( budCVar* )UINTPTR_MAX;
	}
}
