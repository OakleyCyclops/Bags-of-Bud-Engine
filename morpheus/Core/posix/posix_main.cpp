...
static budStr			basepath;
static budStr			savepath;
...
static budStr			history[ COMMAND_HISTORY ];	// cycle buffer
...
// terminal support
budCVar in_tty( "in_tty", "1", CVAR_BOOL | CVAR_INIT | CVAR_SYSTEM, "terminal tab-completion and history" );

static bool				tty_enabled = false;
static struct termios	tty_tc;

// pid - useful when you attach to gdb..
budCVar com_pid( "com_pid", "0", CVAR_INTEGER | CVAR_INIT | CVAR_SYSTEM, "process id" );
...
{
	budStr::Copynz( exit_spawn, exeName, 1024 );
}

/*
==================
budSysLocal::StartProcess
...
==================
*/
void budSysLocal::StartProcess( const char* exeName, bool quit )
{
	if( quit )
	{
...
================
*/
int Sys_ListFiles( const char* directory, const char* extension, budStrList& list )
{
	struct dirent* d;
	DIR* fdir;
	bool dironly = false;
	...
	{
		// DG end
		budStr::snPrintf( search, sizeof( search ), "%s/%s", directory, d->d_name );
		if( stat( search, &st ) == -1 )
			continue;
		if( !dironly )
		{
		...
		while( ( ret = read( STDIN_FILENO, &key, 1 ) ) > 0 )
		{
			...
			switch( key )
			{
				...
				case 8:
					...
					break;
				case '\n':
					budStr::Copynz( input_ret, input_field.GetBuffer(), sizeof( input_ret ) );
					assert( hidden );
					tty_Show();
					write( STDOUT_FILENO, &key, 1 );
					input_field.Clear();
...
=================
*/
void budSysLocal::OpenURL( const char* url, bool quit )
{
	...
	budStr::snPrintf( cmdline, 1024, "%s '%s' &",  script_path, url );
