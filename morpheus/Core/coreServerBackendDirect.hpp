...
public:
	...
	virtual void				StartHosting( const budMatchParameters& p, float skillLevel, lobbyBackendType_t type );
	virtual void				StartFinding( const budMatchParameters& p, int numPartyUsers, float skillLevel );
	virtual void				JoinFromConnectInfo( const lobbyConnectInfo_t& connectInfo );
	virtual void				GetSearchResults( budList< lobbyConnectInfo_t >& searchResults );
	virtual void				FillMsgWithPostConnectInfo( budBitMsg& msg ) {}
	virtual void				PostConnectFromMsg( budBitMsg& msg ) {}
	...
	virtual void				Pump();
	virtual void				UpdateMatchParms( const budMatchParameters& p );
