...
#define NET_VERBOSESNAPSHOT_PRINT_LEVEL( X, Y )  if ( net_verboseSnapshot.GetInteger() >= ( X ) ) libBud::Printf( "%s", Y )
...
public:
	budSnapShot();
	budSnapShot( const budSnapShot& other );
	~budSnapShot();
	
	void operator=( const budSnapShot& other );
	...
	bool ReadDeltaForJob( const char* deltaMem, int deltaSize, int visIndex, budSnapShot* templateStates );
	bool ReadDelta( budFile* file, int visIndex );
	...
	struct objectBuffer_t
	{
		...
		// (not making private because of budSnapShot)
		void _AddRef();
		void _Release();
	private:
		byte* 			data;
	...
	struct submitDeltaJobsInfo_t
	{
		...
		budSnapShot* 		oldSnap;				// snap we are comparing this snap to (to produce a delta)
		int					visIndex;
		int					baseSequence;
		
		budSnapShot* 		templateStates;			// states for new snapObj that arent in old states
	...
	bool WriteDelta( budSnapShot& old, int visIndex, budFile* file, int maxLength, int optimalLength = 0 );
	
	// Adds an object to the state, overwrites any existing object with the same number
	objectState_t* S_AddObject( int objectNum, uint32 visMask, const budBitMsg& msg, const char* tag = NULL )
	{
	...
	bool CopyObject( const budSnapShot& oldss, int objectNum, bool forceStale = false );
	int CompareObject( const budSnapShot* oldss, int objectNum, int start = 0, int end = 0, int oldStart = 0 );
	...
	int GetObjectMsgByIndex( int i, budBitMsg& msg, bool ignoreIfStale = false ) const;
	
	// returns true if the object was found in the snapshot
	bool GetObjectMsgByID( int objectNum, budBitMsg& msg, bool ignoreIfStale = false )
	{
	...
	void			ApplyToExistingState( int objId, budBitMsg& msg );
	objectState_t* 	GetTemplateState( int objNum, budSnapShot* templateStates, objectState_t* newState = NULL );
...
private:
	...
	void WriteObject( budFile* file, int visIndex, objectState_t* newState, objectState_t* oldState, int& lastobjectNum );
