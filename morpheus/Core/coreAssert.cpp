...
budCVar com_assertOutOfDebugger( "com_assertOutOfDebugger", "0", CVAR_BOOL, "by default, do not assert while not running under the debugger" );
...
static budStaticList< skippedAssertion_t, 20 > skippedAssertions;
...
bool AssertFailed( const char* file, int line, const char* expression )
{
	...
	libBud::Warning( "ASSERTION FAILED! %s(%d): '%s'", file, line, expression );
