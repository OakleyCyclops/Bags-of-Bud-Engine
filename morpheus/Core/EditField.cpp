...
static void FindMatches( const char* s )
{
	...
	if( budStr::Icmpn( s, globalAutoComplete.completionString, strlen( globalAutoComplete.completionString ) ) != 0 )
	{
		return;
	}
	...
	{
		budStr::Copynz( globalAutoComplete.currentMatch, s, sizeof( globalAutoComplete.currentMatch ) );
		return;
	}
...
{
	if( budStr::Icmpn( s, globalAutoComplete.currentMatch, strlen( globalAutoComplete.currentMatch ) ) == 0 )
	{
		common->Printf( "    %s\n", s );
	}
}
...
void idEditField::AutoComplete()
{
	...
	if( !autoComplete.valid )
	{
		...
		budStr::Copynz( autoComplete.completionString, args.Argv( 0 ), sizeof( autoComplete.completionString ) );
		budStr::Copynz( completionArgString, args.Args(), sizeof( completionArgString ) );
		...
		if( autoComplete.matchCount == 1 || completionArgString[0] != '\0' )
		{
			...
			budStr::Append( autoComplete.completionString, sizeof( autoComplete.completionString ), " " );
			budStr::Append( autoComplete.completionString, sizeof( autoComplete.completionString ), completionArgString );
			autoComplete.matchCount = 0;
			...
			budStr::snPrintf( buffer, sizeof( buffer ), "%s", autoComplete.currentMatch );
			
			if( autoComplete.matchCount == 0 )
			{
				// no argument matches
				budStr::Append( buffer, sizeof( buffer ), " " );
				budStr::Append( buffer, sizeof( buffer ), completionArgString );
	...
	else if( autoComplete.matchCount != 1 )
	{
		...
		// and print it
		budStr::snPrintf( buffer, sizeof( buffer ), autoComplete.currentMatch );
...
{
	Clear();
	budStr::Copynz( buffer, buf, sizeof( buffer ) );
	SetCursor( strlen( buffer ) );
}
...
// 		// Skip color code
// 		if( budStr::IsColor( buffer + prestep ) )
// 		{
// 			prestep += 2;
// 		}
// 		if( prestep > 0 && budStr::IsColor( buffer + prestep - 1 ) )
// 		{
// 			prestep++;
// 		}
// 	}
