...
#endif
#define OUTPUT_FUNC()				libBud::PrintfIf( saveGame_verbose.GetBool(), "[%s] Enter\n", __FUNCTION__ )
#define OUTPUT_FUNC_EXIT()			libBud::PrintfIf( saveGame_verbose.GetBool(), "[%s] Exit\n", __FUNCTION__ )
...
extern budCVar saveGame_enable;

class budGameSpawnInfo;
class budSession;
class budSessionLocal;
...
public:
	...
	{
		return ( budStr::Icmp( slotName, other.slotName ) == 0 );
	}
...
public:
	...
	budStrStatic< MAX_FOLDER_NAME_LENGTH >	slotName;		// [out] folder/slot name, e.g. AUTOSAVE
};

typedef budStaticList< idSaveGameDetails, MAX_SAVEGAMES > saveGameDetailsList_t;

// Making a auto_ptr to handle lifetime issues better
typedef budList< budFile_SaveGame*, TAG_SAVEGAMES > saveFileEntryList_t;
...
public:
	budStrStatic< MAX_FOLDER_NAME_LENGTH >		directory;			// [in] real directory of the savegame package
	budStrStatic< MAX_FILENAME_LENGTH_PATTERN >	pattern;			// [in] pattern to use while enumerating/deleting files within a savegame folder
	budStrStatic< MAX_FILENAME_LENGTH_PATTERN >	postPattern;		// [in] pattern at the end of the file to use while enumerating/deleting files within a savegame folder
	...
	// Internal vars
	// ----------------------
	budSysSignal							callbackSignal;				// [internal] used to signal savegame manager that the Process() call is completed (we still might have more Process() calls to make though...)
...
================================================
*/
class idSaveGameThread : public budSysThread
{
public:
...
private:
	...
	budStaticList< idCallback*, MAX_COMPLETED_CALLBACKS >	completedCallbacks;
};
...
private:
	...
	budStaticList< idSaveGameProcessor*, 4 >	processorQueue;
	bool										cancel;
