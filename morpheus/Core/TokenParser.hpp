...
public:
	...
	budBinaryToken( const budToken& tok )
	{
		token = tok.c_str();
		tokenType = tok.type;
		tokenSubType = tok.subtype;
	}
	bool operator==( const budBinaryToken& b ) const
	{
...
public:
	...
	void LoadFromParser( budParser& parser, const char* guiName );
...
private:
	budList< budBinaryToken > tokens;
	budList< budTokenIndexes > guiTokenIndexes;
	int currentToken;
