...
public:
	...
	static budFile_Memory* CombineFiles( const budList< budFile_Memory* >& srcFiles );
	// extracts multiple in-memory files from a single memory file
	static bool			ExtractFiles( budFile_Memory*& srcFile, budList< budFile_Memory* >& destFiles );
	
	void				CleanSourceFolder();
	
	bool				CreateZipFileFromFileList( const char* name, const budList< budFile_Memory* >& srcFiles );
	
	zipFile				CreateZipFile( const char* name );
	bool				AddFile( zipFile zf, budFile_Memory* fm, bool deleteFile );
	void				CloseZipFile( zipFile zf );
private:
	bool				CreateZipFile( bool appendFiles );
	bool				CreateZipFileFromFiles( const budList< budFile_Memory* >& srcFiles );
	bool				GetFileTime( const budStr& filename, unsigned long* dostime ) const;
	bool				IsFiltered( const budStr& filename ) const;
	bool				IsUncompressed( const budStr& filename ) const;
	
private:
	budStr				zipFileName;				// os path to the zip file
	budStr				sourceFolderName;			// source folder of files to zip or add
	budStrList			filterExts;					// file extensions we want to compressed
	budStrList			uncompressedFilterExts;		// file extensions we don't want to compress
};
