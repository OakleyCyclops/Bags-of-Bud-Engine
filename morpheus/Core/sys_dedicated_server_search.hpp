...
public:
	...
	void			HandleQueryAck( lobbyAddress_t& addr, budBitMsg& msg );
	
	
	bool			GetAddrAtIndex( netadr_t& addr, int i );
	const serverInfo_t* 	DescribeServerAtIndex( int i ) const;
	const budList< budStr >* 	GetServerPlayersAtIndex( int i ) const;
...
private:
	struct serverInfoDedicated_t
	{
		...
		budList< budStr >				connectedPlayers;
	};
	
	budList< serverInfoDedicated_t >	list;
	idCallback* 		callback;
};
