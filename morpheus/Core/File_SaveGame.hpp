...
================================================
*/
class budFile_SaveGame : public budFile_Memory
{
public:
	budFile_SaveGame() : type( SAVEGAMEFILE_NONE ), error( false ) {}
	budFile_SaveGame( const char* _name ) : budFile_Memory( _name ), type( SAVEGAMEFILE_NONE ), error( false ) {}
	budFile_SaveGame( const char* _name, int type_ ) : budFile_Memory( _name ), type( type_ ), error( false ) {}
	
	virtual ~budFile_SaveGame() { }
	
	bool operator==( const budFile_SaveGame& other ) const
	{
		return budStr::Icmp( GetName(), other.GetName() ) == 0;
	}
	bool operator==( const char* _name ) const
	{
		return budStr::Icmp( GetName(), _name ) == 0;
	}
...
/*
================================================
budFile_SaveGamePipelined uses threads to pipeline overlap compression and IO
================================================
*/
...
class budFile_SaveGamePipelined : public budFile
{
public:
	...
	budFile_SaveGamePipelined();
	virtual					~budFile_SaveGamePipelined();
	...
	bool					OpenForReading( budFile* file );
	bool					OpenForWriting( budFile* file );
...
private:
	...
	budSysSignal				blockRequested;
	budSysSignal				blockAvailable;
	budSysSignal				blockFinished;
	
	budStrStatic< 32 >		buildVersion;		// build version this file was saved with
