...
========================
*/
void idLobbyBackendDirect::StartHosting( const budMatchParameters& p, float skillLevel, lobbyBackendType_t type )
{
...
========================
*/
void idLobbyBackendDirect::StartFinding( const budMatchParameters& p, int numPartyUsers, float skillLevel )
{
	isLocal = MatchTypeIsLocal( p.matchFlags );
	isHost	= false;
...
========================
*/
void idLobbyBackendDirect::GetSearchResults( budList< lobbyConnectInfo_t >& searchResults )
{
...
lobbyConnectInfo_t idLobbyBackendDirect::GetConnectInfo()
{
	...
	if( IsHost() )
	{
		...
		if( ip == NULL || budStr::Length( ip ) == 0 || budStr::Icmp( ip, "localhost" ) == 0 )
			ip = "0.0.0.0";
		// DG end
...
========================
*/
void idLobbyBackendDirect::UpdateMatchParms( const budMatchParameters& p )
{
}
