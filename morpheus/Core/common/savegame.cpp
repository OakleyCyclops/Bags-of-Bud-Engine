...
extern budCVar saveGame_checksum;
extern budCVar savegame_error;
...
idLocalUserWin* GetLocalUserFromSaveParms( const saveGameThreadArgs_t& data )
{
	if( ( data.saveLoadParms != NULL ) && ( data.saveLoadParms->inputDeviceId >= 0 ) )
	{
		...
		if( user != NULL )
		{
			...
			if( userWin != NULL && data.saveLoadParms->userId == budStr::Hash( userWin->GetGamerTag() ) )
			{
				return userWin;
			}
		}
...
int idSaveGameThread::Save()
{
	...
	int64 requiredSizeBytes = 0;
	{
		...
		{
			budFile_SaveGame* 	file = callback->files[i];
	...
	if( ( callback->mode & SAVEGAME_MBF_DELETE_FILES ) && !callback->cancelled )
	{
		...
		{
			budFileList* files = fileSystem->ListFilesTree( saveFolder.c_str(), "*.*" );
	...
	*/
	...
	for( int i = 0; i < callback->files.Num() && ret == ERROR_SUCCESS && !callback->cancelled; i++ )
	{
		...
		budStr fileName = saveFolder;
		fileName.AppendPath( file->GetName() );
		budStr tempFileName = va( "%s.temp", fileName.c_str() );
		
		budFile* outputFile = fileSystem->OpenFileWrite( tempFileName, "fs_savePath" );
		if( outputFile == NULL )
		{
#ifdef _WIN32 // DG: unify windows and posix savegames => replace GetLastError with strerror(errno)
			libBud::Warning( "[%s]: Couldn't open file for writing, %s. Error = %08x", __FUNCTION__, tempFileName.c_str(), GetLastError() );
#else
			libBud::Warning( "[%s]: Couldn't open file for writing, %s. Error = %s", __FUNCTION__, tempFileName.c_str(), strerror( errno ) );
#endif // DG end
			file->error = true;
		...
		if( ( file->type & SAVEGAMEFILE_PIPELINED ) != 0 )
		{
			...
			budFile_SaveGamePipelined* inputFile = dynamic_cast< budFile_SaveGamePipelined* >( file );
			...
			while( inputFile->NextWriteBlock( & block ) )
			{
				...
				{
#ifdef _WIN32 // DG: unify windows and posix savegames => replace GetLastError with strerror(errno)
					libBud::Warning( "[%s]: Write failed. Error = %08x", __FUNCTION__, GetLastError() );
#else
					libBud::Warning( "[%s]: Write failed. Error = %s", __FUNCTION__, strerror( errno ) );
#endif // DG end
					file->error = true;
					callback->errorCode = SAVEGAME_E_INSUFFICIENT_ROOM;
					ret = -1;
		...
		else
		{
			...
			{
				libBud::PrintfIf( saveGame_verbose.GetBool(), "Saved %s (%s)\n", fileName.c_str(), outputFile->GetFullPath() );
			}
		}
		...
		if( ret == ERROR_SUCCESS )
		{
			...
			{
				libBud::Warning( "Could not start to rename temporary file %s to %s.", tempFileName.c_str(), fileName.c_str() );
			}
		}
		else
		{
			fileSystem->RemoveFile( tempFileName );
			libBud::Warning( "Invalid write to temporary file %s.", tempFileName.c_str() );
		}
	}
	...
	if( callback->errorCode != SAVEGAME_E_NONE )
	{
		...
		{
			budFileList* files = fileSystem->ListFilesTree( saveFolder, "/|*" );
			for( int i = 0; i < files->GetNumFiles(); i++ )
			{
...
int idSaveGameThread::Load()
{
	...
	for( int i = 0; i < callback->files.Num() && ret == ERROR_SUCCESS && !callback->cancelled; i++ )
	{
		...
		budFile* inputFile = fileSystem->OpenFileRead( filename.c_str() );
		if( inputFile == NULL )
		{
			file->error = true;
		...
		if( ( file->type & SAVEGAMEFILE_PIPELINED ) != 0 )
		{
			...
			budFile_SaveGamePipelined* outputFile = dynamic_cast< budFile_SaveGamePipelined* >( file );
...
int idSaveGameThread::Enumerate()
{
	...
	if( fileSystem->IsFolder( saveFolder, "fs_savePath" ) == FOLDER_YES )
	{
		budFileList* files = fileSystem->ListFilesTree( saveFolder, SAVEGAME_DETAILS_FILENAME );
		...
		for( int i = 0; i < fileList.Num() && !callback->cancelled; i++ )
		{
			...
			budStr directory = fileList[i];
			
			budFile* file = fileSystem->OpenFileRead( directory.c_str() );
...
#else
				// DG: just use the budFile object's timestamp - the windows code gets file attributes and
...
int idSaveGameThread::EnumerateFiles()
{
	...
	if( fileSystem->IsFolder( folder, "fs_savePath" ) == FOLDER_YES )
	{
		...
		budFileList* files = fileSystem->ListFilesTree( folder, "*.*" );
		
		// look for the instance pattern
		for( int i = 0; i < files->GetNumFiles() && ret == 0 && !callback->cancelled; i++ )
		{
			budStr fullFilename = files->GetFile( i );
			budStr filename = fullFilename;
			filename.StripPath();
			...
			if( callback->mode & SAVEGAME_MBF_READ_DETAILS )
			{
				...
				budFile* uncompressed = fileSystem->OpenFileRead( fullFilename.c_str() );
			...
			budFile_SaveGame* file = new( TAG_SAVEGAMES ) budFile_SaveGame( filename, SAVEGAMEFILE_AUTO_DELETE );
...
int idSaveGameThread::DeleteFiles()
{
	...
	{
		budFile_SaveGame* file = callback->files[j];
		budStr fullpath = folder;
...
void Sys_SaveGameCheck( bool& exists, bool& autosaveExists )
{
	...
	const budStr autosaveFolderStr = AddSaveFolderPrefix( SAVEGAME_AUTOSAVE_FOLDER, idSaveGameManager::PACKAGE_GAME );
	const char* autosaveFolder = autosaveFolderStr.c_str();
	const char* saveFolder = "savegame";
	
	if( fileSystem->IsFolder( saveFolder, "fs_savePath" ) == FOLDER_YES )
	{
		budFileList* files = fileSystem->ListFiles( saveFolder, "/" );
		const budStrList& fileList = files->GetList();
		
		libBud::PrintfIf( saveGame_verbose.GetBool(), "found %d savegames\n", fileList.Num() );
		...
		for( int i = 0; i < fileList.Num(); i++ )
		{
			...
			if( fileSystem->IsFolder( directory, "fs_savePath" ) == FOLDER_YES )
			{
				...
				libBud::PrintfIf( saveGame_verbose.GetBool(), "found savegame: %s\n", fileList[i].c_str() );
				
				if( budStr::Icmp( fileList[i].c_str(), autosaveFolder ) == 0 )
				{
					autosaveExists = true;
					break;
				}
