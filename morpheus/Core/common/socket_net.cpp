...
#include <iptypes.h>
#include <iphlpapi.h>
// force these libs to be included, so users of libBud don't need to add them to every project
...
budCVar net_socksServer( "net_socksServer", "", CVAR_ARCHIVE, "" );
budCVar net_socksPort( "net_socksPort", "1080", CVAR_ARCHIVE | CVAR_INTEGER, "" );
budCVar net_socksUsername( "net_socksUsername", "", CVAR_ARCHIVE, "" );
budCVar net_socksPassword( "net_socksPassword", "", CVAR_ARCHIVE, "" );

budCVar net_ip( "net_ip", "localhost", CVAR_NOCHEAT, "local IP address" );
...
static bool Net_ExtractPort( const char* src, char* buf, int bufsize, int* port )
{
	...
	p = buf;
	p += Min( bufsize - 1, budStr::Length( src ) );
	*p = '\0';
	p = strchr( buf, ':' );
	if( !p )
	{
...
int NET_IPSocket( const char* bind_ip, int port, netadr_t* bound_to )
{
	...
	if( port != PORT_ANY )
	{
		if( bind_ip )
		{
			libBud::Printf( "Opening IP socket: %s:%i\n", bind_ip, port );
		}
		else
		{
			libBud::Printf( "Opening IP socket: localhost:%i\n", port );
		}
	}
	
	if( ( newsocket = socket( AF_INET, SOCK_DGRAM, IPPROTO_UDP ) ) == INVALID_SOCKET )
	{
		libBud::Printf( "WARNING: UDP_OpenSocket: socket: %s\n", NET_ErrorString() );
		return 0;
	}
	...
	{
		libBud::Printf( "WARNING: UDP_OpenSocket: ioctl FIONBIO: %s\n", NET_ErrorString() );
	...
	if( flags < 0 )
	{
		libBud::Printf( "WARNING: UDP_OpenSocket: fcntl F_GETFL: %s\n", NET_ErrorString() );
		closesocket( newsocket );
		return 0;
	}
	flags |= O_NONBLOCK;
	...
	{
		libBud::Printf( "WARNING: UDP_OpenSocket: fcntl F_SETFL with O_NONBLOCK: %s\n", NET_ErrorString() );
		closesocket( newsocket );
		return 0;
	}
#endif
	...
	{
		libBud::Printf( "WARNING: UDP_OpenSocket: setsockopt SO_BROADCAST: %s\n", NET_ErrorString() );
		closesocket( newsocket );
		return 0;
	}
	
	if( !bind_ip || !bind_ip[0] || !budStr::Icmp( bind_ip, "localhost" ) )
	{
	...
	{
		libBud::Printf( "WARNING: UDP_OpenSocket: bind: %s\n", NET_ErrorString() );
		closesocket( newsocket );
		return 0;
	}
...
void NET_OpenSocks( int port )
{
	...
	libBud::Printf( "Opening connection to SOCKS server.\n" );
	
	if( ( socks_socket = socket( AF_INET, SOCK_STREAM, IPPROTO_TCP ) ) == INVALID_SOCKET )
	{
		libBud::Printf( "WARNING: NET_OpenSocks: socket: %s\n", NET_ErrorString() );
		return;
	}
	...
	if( h == NULL )
	{
		libBud::Printf( "WARNING: NET_OpenSocks: gethostbyname: %s\n", NET_ErrorString() );
		return;
	}
	if( h->h_addrtype != AF_INET )
	{
		libBud::Printf( "WARNING: NET_OpenSocks: gethostbyname: address type was not AF_INET\n" );
		return;
	}
	address.sin_family = AF_INET;
	...
	{
		libBud::Printf( "NET_OpenSocks: connect: %s\n", NET_ErrorString() );
		return;
	}
	...
	{
		libBud::Printf( "NET_OpenSocks: send: %s\n", NET_ErrorString() );
		return;
	}
	...
	if( len == SOCKET_ERROR )
	{
		libBud::Printf( "NET_OpenSocks: recv: %s\n", NET_ErrorString() );
		return;
	}
	if( len != 2 || buf[0] != 5 )
	{
		libBud::Printf( "NET_OpenSocks: bad response\n" );
		return;
	}
	switch( buf[1] )
	{
		...
		case 2: // username/password authentication
			break;
		default:
			libBud::Printf( "NET_OpenSocks: request denied\n" );
			return;
	}
	...
	if( buf[1] == 2 )
	{
		...
		// build the request
		ulen = budStr::Length( net_socksUsername.GetString() );
		plen = budStr::Length( net_socksPassword.GetString() );
		...
		if( buf[1] != 0 )
		{
			libBud::Printf( "NET_OpenSocks: authentication failed\n" );
			return;
		}
	}
	...
	*( short* )&buf[8] = htons( ( short )port );		// port
	...
	if( buf[1] != 0 )
	{
		libBud::Printf( "NET_OpenSocks: request denied: %i\n", buf[1] );
		return;
	}
	if( buf[3] != 1 )
	{
		libBud::Printf( "NET_OpenSocks: relay address is not IPV4: %i\n", buf[3] );
		return;
	}
...
bool Net_WaitForData( int netSocket, int timeout )
{
	...
	if( ret == -1 )
	{
		libBud::Printf( "Net_WaitForData select(): %s\n", NET_ErrorString() );
		return false;
	}
...
bool Net_GetUDPPacket( int netSocket, netadr_t& net_from, char* data, int& size, int maxSize )
{
	...
	if( ret == SOCKET_ERROR )
	{
		...
		libBud::Printf( "Net_GetUDPPacket: %s\n", NET_ErrorString() );
		return false;
	}
#if 0
	...
	if( ret > maxSize )
	{
		libBud::Printf( "Net_GetUDPPacket: oversize packet from %s\n", Sys_NetAdrToString( net_from ) );
		return false;
	}
...
void Net_SendUDPPacket( int netSocket, int length, const void* data, const netadr_t to )
{
	...
	if( ret == SOCKET_ERROR )
	{
		...
		libBud::Printf( "UDP sendto error - packet dropped: %s\n", NET_ErrorString() );
	}
}

static void ip_to_addr( const char ip[4], char* addr )
{
	budStr::snPrintf( addr, 16, "%d.%d.%d.%d", ( unsigned char )ip[0], ( unsigned char )ip[1],
...
void Sys_InitNetworking()
{
	...
	if( r )
	{
		libBud::Printf( "WARNING: Winsock initialization failed, returned %d\n", r );
		return;
	}
	
	winsockInitialized = true;
	libBud::Printf( "Winsock Initialized\n" );
	...
	if( !pAdapterInfo )
	{
		libBud::FatalError( "Sys_InitNetworking: Couldn't malloc( %d )", sizeof( IP_ADAPTER_INFO ) );
	}
	...
	if( GetAdaptersInfo( pAdapterInfo, &ulOutBufLen ) == ERROR_BUFFER_OVERFLOW )
	{
		...
		if( !pAdapterInfo )
		{
			libBud::FatalError( "Sys_InitNetworking: Couldn't malloc( %ld )", ulOutBufLen );
		}
	}
	...
	if( ( dwRetVal = GetAdaptersInfo( pAdapterInfo, &ulOutBufLen ) ) != NO_ERROR )
	{
		...
		libBud::Printf( "Sys_InitNetworking: GetAdaptersInfo failed (%ld).\n", dwRetVal );
	}
	else
	{
		...
		while( pAdapter )
		{
			libBud::Printf( "Found interface: %s %s - ", pAdapter->AdapterName, pAdapter->Description );
			pIPAddrString = &pAdapter->IpAddressList;
			while( pIPAddrString )
			{
				unsigned long ip_a, ip_m;
				if( !budStr::Icmp( "127.0.0.1", pIPAddrString->IpAddress.String ) )
				{
					foundloopback = true;
				}
				...
				if( !ip_m )
				{
					libBud::Printf( "%s NULL netmask - skipped\n", pIPAddrString->IpAddress.String );
				...
				budStr::Copynz( netint[num_interfaces].addr, pIPAddrString->IpAddress.String, sizeof( netint[num_interfaces].addr ) );
				num_interfaces++;
				if( num_interfaces >= MAX_INTERFACES )
				{
					libBud::Printf( "Sys_InitNetworking: MAX_INTERFACES(%d) hit.\n", MAX_INTERFACES );
					free( pAdapterInfo );
					return;
				}
				pIPAddrString = pIPAddrString->Next;
	...
	{
		libBud::Printf( "Sys_InitNetworking: adding loopback interface\n" );
...
const char* Sys_NetAdrToString( const netadr_t a )
{
	...
	//            or maybe return an budStr and change calling code accordingly
	static int index = 0;
	...
	if( a.type == NA_IP || a.type == NA_LOOPBACK )
		budStr::snPrintf( s, 64, "%i.%i.%i.%i:%i", a.ip[0], a.ip[1], a.ip[2], a.ip[3], a.port );
	else if( a.type == NA_BROADCAST )
		budStr::snPrintf( s, 64, "BROADCAST" );
	else if( a.type == NA_BAD )
		budStr::snPrintf( s, 64, "BAD_IP" );
	else
	{
		budStr::snPrintf( s, 64, "WTF_UNKNOWN_IP_TYPE_%i", a.type );
	}
	return s;
}
...
bool Sys_CompareNetAdrBase( const netadr_t a, const netadr_t b )
{
	...
	libBud::Printf( "Sys_CompareNetAdrBase: bad address type\n" );
	return false;
}
...
void idUDP::SendPacket( const netadr_t to, const void* data, int size )
{
	if( to.type == NA_BAD )
	{
		libBud::Warning( "idUDP::SendPacket: bad address type NA_BAD - ignored" );
		return;
	}
