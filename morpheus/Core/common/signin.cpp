...
#endif
#ifdef _DEBUG
budCVar win_userPersistent( "win_userPersistent", "1", CVAR_BOOL, "debugging cvar for profile persistence status" );
budCVar win_userOnline( "win_userOnline", "1", CVAR_BOOL, "debugging cvar for profile online status" );
budCVar win_isInParty( "win_isInParty", "0", CVAR_BOOL, "debugging cvar for platform party status" );
budCVar win_partyCount( "win_partyCount", "0", CVAR_INTEGER, "debugginc var for platform party count" );
#endif

// DG: D3BFG got the username from steam, in the GPL release it just uses the hostname.
//     this adds a name to set a player name
budCVar ui_name( "ui_name", "", CVAR_ARCHIVE, "player name - leave empty for default name (system's hostname)" );
// DG end
...
void idSignInManagerWin::RegisterLocalUser( int inputDevice )
{
	...
	budStr name( nameSource );
	int nameLength = name.Length();
	if( budStr::IsValidUTF8( nameSource, nameLength ) )
	{
		int nameIndex = 0;
		int numChars = 0;
		name.Empty();
		...
		{
			uint32 c = budStr::UTF8Char( nameSource, nameIndex );
			name.AppendUTF8Char( c );
		}
	}
	
	libBud::Printf( "Added local user: %s\n", name.c_str() );
...
{
	//budScopedGlobalHeap	everythingHereGoesInTheGlobalHeap;	// users obviously persist across maps
