...
public:
	...
	void Reset()
	{
		...
		reliable = budDataQueue< MAX_RELIABLE_QUEUE, MAX_MSG_SIZE >();
...
private:
	...
	int FinalizeRead( budBitMsg& inMsg, budBitMsg& outMsg, int& userValue );
...
public:
	...
	bool ProcessOutgoing( const int time, const budBitMsg& msg, bool isOOB, int userData );
	// Used to get each fragment for sending through the actual net connection
	bool GetSendFragment( const int time, sessionId_t sessionID, budBitMsg& outMsg );
	// Used to process a fragment received.  Returns true when msg was reconstructed.
	int ProcessIncoming( int time, sessionId_t expectedSessionID, budBitMsg& msg, budBitMsg& out, int& userData, const int peerNum );
	...
	static bool ProcessConnectionlessOutgoing( budBitMsg& msg, budBitMsg& out, int lobbyType, int userData );
	static bool ProcessConnectionlessIncoming( budBitMsg& msg, budBitMsg& out, int& userData );
	
	// Used to "peek" at a session id of a message fragment
	static sessionId_t GetSessionID( budBitMsg& msg );
...
private:
	...
	budDataQueue< MAX_RELIABLE_QUEUE, MAX_MSG_SIZE >	reliable;	// list of unacknowledged reliable messages
