...
/*
================================================
budSnapShotProcessor
================================================
*/
class budSnapShotProcessor
{
public:
	...
	bool TrySetPendingSnapshot( budSnapShot& ss );
	...
	bool ApplyDeltaToSnapshot( budSnapShot& snap, const char* deltaMem, int deltaSize, int visIndex );
	...
	bool ReceiveSnapshotDelta( const byte* deltaData, int deltaLength, int visIndex, int& outSeq, int& outBaseSeq, budSnapShot& outSnap, bool& fullSnap );
	...
	budSnapShot* GetBaseState()
	{
		return &baseState;
	}
	budSnapShot* GetPendingSnap()
	{
		return &pendingSnap;
	}
	...
	void AddSnapObjTemplate( int objID, budBitMsg& msg );
...
private:
	...
	// on the jobbed snapshot compression path, the sequence is incremented in NewLZWStream and pulled into this in budSnapShotProcessor::GetPendingSnapDelta
	int				snapSequence;
	int				baseSequence;
	int				lastFullSnapBaseSequence;		// Latest base sequence number that is a full snap
	
	budSnapShot		baseState;			// known snapshot base on the client
	budDataQueue< MAX_SNAPSHOT_QUEUE, MAX_SNAPSHOT_QUEUE_MEM >	deltas;		// list of unacknowledged snapshot deltas
	
	budSnapShot		pendingSnap;		// Current snap waiting to be fully sent
	...
	struct jobMemory_t
	{
		...
		// Parm memory to jobs
		budArray<objParms_t, MAX_OBJ_PARMS>		objParms;
		budArray<objHeader_t, MAX_OBJ_HEADERS>	headers;
		budArray<lzwParm_t, MAX_LZW_PARMS>		lzwParms;
		
		// Output memory from jobs
		budArray<lzwDelta_t, MAX_LZW_DELTAS>	lzwDeltas;			// Info about each pending delta output from jobs
		budArray<byte, MAX_LZW_MEM>		lzwMem;				// Memory for output from lzw jobs
	...
	budSnapShot		submittedState;
	
	budSnapShot		templateStates;			// holds default snapshot states for some newly spawned object
	budSnapShot		submittedTemplateStates;
