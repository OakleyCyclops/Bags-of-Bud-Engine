...
budCVar net_verboseSnapshot( "net_verboseSnapshot", "0", CVAR_INTEGER | CVAR_NOCHEAT, "Verbose snapshot code to help debug snapshot problems. Greater the number greater the spam" );
budCVar net_verboseSnapshotCompression( "net_verboseSnapshotCompression", "0", CVAR_INTEGER | CVAR_NOCHEAT, "Verbose snapshot code to help debug snapshot problems. Greater the number greater the spam" );
budCVar net_verboseSnapshotReport( "net_verboseSnapshotReport", "0", CVAR_INTEGER | CVAR_NOCHEAT, "Verbose snapshot code to help debug snapshot problems. Greater the number greater the spam" );

budCVar net_ssTemplateDebug( "net_ssTemplateDebug", "0", CVAR_BOOL, "Debug snapshot template states" );
budCVar net_ssTemplateDebug_len( "net_ssTemplateDebug_len", "32", CVAR_INTEGER, "Offset to start template state debugging" );
budCVar net_ssTemplateDebug_start( "net_ssTemplateDebug_start", "0", CVAR_INTEGER, "length of template state to print in debugging" );
...
{
	libBud::Printf( "%25s: 0x", text );
}

/*
========================
budSnapShot::objectState_t::Print
Helper function for net_ssTemplateDebug debugging
========================
*/
void budSnapShot::objectState_t::Print( const char* name )
{
...
/*
========================
budSnapShot::objectBuffer_t::Alloc
========================
*/
void budSnapShot::objectBuffer_t::Alloc( int s )
{
	//assert( mem.IsMapHeap() );
	if( !verify( s < SIZE_NOT_STALE ) )
	{
		libBud::FatalError( "s >= SIZE_NOT_STALE" );
	}
	_Release();
...
/*
========================
budSnapShot::objectBuffer_t::AddRef
========================
*/
void budSnapShot::objectBuffer_t::_AddRef()
{
	if( data != NULL )
	{
		assert( size > 0 );
...
/*
========================
budSnapShot::objectBuffer_t::Release
========================
*/
void budSnapShot::objectBuffer_t::_Release()
{
	//assert( mem.IsMapHeap() );
	if( data != NULL )
	{
...
/*
========================
budSnapShot::objectBuffer_t::operator=
========================
*/
void budSnapShot::objectBuffer_t::operator=( const budSnapShot::objectBuffer_t& other )
{
	//assert( mem.IsMapHeap() );
	if( this != &other )
	{
...
========================
*/
budSnapShot::budSnapShot( const budSnapShot& other ) : time( 0 ), recvTime( 0 )
{
	*this = other;
}
...
/*
========================
budSnapShot::operator=
========================
*/
void budSnapShot::operator=( const budSnapShot& other )
{
	//assert( mem.IsMapHeap() );
...
/*
========================
budSnapShot::PeekDeltaSequence
========================
*/
void budSnapShot::PeekDeltaSequence( const char* deltaMem, int deltaSize, int& sequence, int& baseSequence )
{
...
/*
========================
budSnapShot::ReadDeltaForJob
========================
*/
bool budSnapShot::ReadDeltaForJob( const char* deltaMem, int deltaSize, int visIndex, budSnapShot* templateStates )
{
	...
	while( lzwCompressor.ReadAgnostic( delta, true ) == sizeof( delta ) )
	{
		...
		if( objectNum >= 0xFFFF )
		{
			...
			if( net_verboseSnapshotCompression.GetBool() )
			{
				...
				libBud::Printf( "Snapshot (%d/%d). ReadSize: %d DeltaSize: %d Ratio: %.3f\n", sequence, baseSequence, bytesRead, deltaSize, compRatio );
			}
			return true;
		}
		...
		if( newsize == 0 )
		{
			...
			{
				libBud::PrintfIf( net_ssTemplateDebug.GetBool(), "Clearing old template state[%d] [%d<%d]\n", objectNum, objTemplateState->expectedSequence, baseSequence );
		...
		if( state.buffer.Size() > 0 )
		{
			...
			{
				libBud::Error( " Invalid snapshot checksum" );
			}
		}
#endif
	}
...
/*
========================
budSnapShot::SubmitObjectJob
========================
*/

void budSnapShot::SubmitObjectJob(	const submitDeltaJobsInfo_t& 	submitDeltaJobsInfo,
								...
								)
{
	...
	{
		libBud::Error( "Out of parms for snapshot jobs.\n" );
	}
...
)
{
	...
	{
		libBud::Error( "SubmitLZWJob: Not enough lzwParams.\n" );
...
========================
*/
budSnapShot::objectState_t* budSnapShot::GetTemplateState( int objNum, budSnapShot* templateStates, budSnapShot::objectState_t* newState /*=NULL*/ )
{
...
/*
========================
budSnapShot::SubmitWriteDeltaToJobs
========================
*/
void budSnapShot::SubmitWriteDeltaToJobs( const submitDeltaJobsInfo_t& submitDeltaJobInfo )
{
	...
	for( int i = 0; i < objectStates.Num(); i++ )
	{
		...
		if( !verify( newState.buffer.Size() > 0 ) )
		{
			...
			libBud::Warning( "Snap obj [%d] state.size <= 0... skipping ", newState.objectNum );
			continue;
		}
...
/*
========================
budSnapShot::ReadDelta
========================
*/
bool budSnapShot::ReadDelta( budFile* file, int visIndex )
{
...
/*
========================
budSnapShot::WriteObject
========================
*/
void budSnapShot::WriteObject( budFile* file, int visIndex, objectState_t* newState, objectState_t* oldState, int& lastobjectNum )
{
	assert( newState != NULL || oldState != NULL );
...
/*
========================
budSnapShot::PrintReport
========================
*/
void budSnapShot::PrintReport()
{
}

/*
========================
budSnapShot::WriteDelta
========================
*/
bool budSnapShot::WriteDelta( budSnapShot& old, int visIndex, budFile* file, int maxLength, int optimalLength )
{
	file->WriteBig( time );
	...
	for( int i = 0; i < objectStates.Num(); i++ )
	{
		...
		if( !verify( newState.buffer.Size() < maxLength ) )
		{
			...
			libBud::Warning( "Snap obj [%d] state.size > max packet length. Skipping... ", newState.objectNum );
			continue;
		}
...
/*
========================
budSnapShot::AddObject
========================
*/
budSnapShot::objectState_t* budSnapShot::S_AddObject( int objectNum, uint32 visMask, const char* data, int _size, const char* tag )
{
...
/*
========================
budSnapShot::CopyObject
========================
*/

bool budSnapShot::CopyObject( const budSnapShot& oldss, int objectNum, bool forceStale )
{
...
========================
*/
int budSnapShot::CompareObject( const budSnapShot* oldss, int objectNum, int start, int end, int oldStart )
{
	if( oldss == NULL )
	{
		return 0;
...
/*
========================
budSnapShot::GetObjectMsgByIndex
========================
*/
int budSnapShot::GetObjectMsgByIndex( int i, budBitMsg& msg, bool ignoreIfStale ) const
{
...
/*
========================
budSnapShot::ObjectIsStaleByIndex
========================
*/
bool budSnapShot::ObjectIsStaleByIndex( int i ) const
{
	if( i < 0 || i >= objectStates.Num() )
	{
		return false;
...
/*
========================
budSnapShot::ObjectChangedCountByIndex
========================
*/
int budSnapShot::ObjectChangedCountByIndex( int i ) const
{
	if( i < 0 || i >= objectStates.Num() )
	{
		return false;
...
/*
========================
budSnapShot::FindObjectIndexByID
========================
*/
int budSnapShot::FindObjectIndexByID( int objectNum ) const
{
...
/*
========================
budSnapShot::BinarySearch
========================
*/
int budSnapShot::BinarySearch( int objectNum ) const
{
	int lo = 0;
...
/*
========================
budSnapShot::FindOrCreateObjectByID
========================
*/
budSnapShot::objectState_t& budSnapShot::FindOrCreateObjectByID( int objectNum )
{
	//assert( mem.IsMapHeap() );
...
/*
========================
budSnapShot::FindObjectByID
========================
*/
budSnapShot::objectState_t* budSnapShot::FindObjectByID( int objectNum ) const
{
...
/*
========================
budSnapShot::CleanupEmptyStates
========================
*/
void budSnapShot::CleanupEmptyStates()
{
...
/*
========================
budSnapShot::UpdateExpectedSeq
========================
*/
void budSnapShot::UpdateExpectedSeq( int newSeq )
{
	for( int i = 0; i < objectStates.Num(); i++ )
	{
...
/*
========================
budSnapShot::FreeObjectState
========================
*/
void budSnapShot::FreeObjectState( int index )
{
...
/*
========================
budSnapShot::ApplyToExistingState
Take uncompressed state in msg and add it to existing state
========================
*/
void budSnapShot::ApplyToExistingState( int objId, budBitMsg& msg )
{
	...
	if( !objectState->createdFromTemplate )
	{
		...
		if( net_ssTemplateDebug.GetBool() )
		{
			libBud::Printf( "NOT ApplyToExistingState[%d] because object was created from existing base state. %d\n", objId, objectState->expectedSequence );
	...
	if( net_ssTemplateDebug.GetBool() )
	{
		...
		for( int i = 0; i < msg.GetSize(); i++ )
		{
			if( InDebugRange( i ) )
			{
				libBud::Printf( "%02X", msg.GetReadData()[i] );
			}
		}
		libBud::Printf( "\n" );
	}
...
CONSOLE_COMMAND( serializeQTest, "Serialization Sanity Test", 0 )
{
	...
	{
		budBitMsg writeBitMsg;
		writeBitMsg.InitWrite( buffer, sizeof( buffer ) );
		budSerializer writeSerializer( writeBitMsg, true );
	...
	{
		budBitMsg readBitMsg;
		readBitMsg.InitRead( buffer, sizeof( buffer ) );
		budSerializer readSerializer( readBitMsg, false );
		...
		for( int i = 0; i < num; i++ )
		{
			...
			float errorUQ = budMath::Fabs( ( resultUQ - values[i] ) ) / values[i];
			float errorQ  = budMath::Fabs( ( resultQ - values[i] ) ) / values[i];
			
			libBud::Printf( "%s%f SerializeUQ: %f. Error: %f \n", errorUQ > 0.1f ? "^1" : "", values[i], resultUQ, errorUQ );
			libBud::Printf( "%s%f SerializeQ: %f. Error: %f \n",   errorQ > 0.1f ? "^1" : "", values[i], resultQ, errorQ );
		}
	}
}
#endif
