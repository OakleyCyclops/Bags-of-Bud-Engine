...
budCVar saveGame_checksum( "saveGame_checksum", "1", CVAR_BOOL, "data integrity check" );
budCVar saveGame_enable( "saveGame_enable", "1", CVAR_BOOL, "are savegames enabled" );
...
void Sys_ExecuteSavegameCommandAsync( idSaveLoadParms* savegameParms )
{
	...
	{
		libBud::Error( "Programming Error with [%s]", __FUNCTION__ );
		return;
	}
	
	if( !saveGame_enable.GetBool() )
	{
		libBud::Warning( "Savegames are disabled (saveGame_enable = 0). Skipping physical save to media." );
...
========================
*/
budStr GetSaveGameErrorString( int errorMask )
{
	budStr errorString;
...
========================
*/
const budStr& GetSaveFolder( idSaveGameManager::packageType_t type )
{
	static bool initialized = false;
	static budStrStatic<MAX_FOLDER_NAME_LENGTH>	saveFolder[idSaveGameManager::PACKAGE_NUM];
...
/*
========================
budStr AddSaveFolderPrefix

	input	= RAGE_0
	output	= GAMES-RAGE_0
========================
*/
budStr AddSaveFolderPrefix( const char* folder, idSaveGameManager::packageType_t type )
{
	budStr dir = GetSaveFolder( type );
	dir.Append( folder );
...
========================
*/
budStr RemoveSaveFolderPrefix( const char* folder, idSaveGameManager::packageType_t type )
{
	budStr dir = folder;
	budStr prefix = GetSaveFolder( type );
...
========================
*/
bool SavegameReadDetailsFromFile( budFile* file, idSaveGameDetails& details )
{
...
void idSaveLoadParms::SetDefaults( int newInputDevice )
{
	...
	if( user != NULL )
	{
		...
		userId = budStr::Hash( userWin->GetGamerTag() );
		libBud::PrintfIf( saveGame_verbose.GetBool(), "profile userId/gamertag: %s (%d)\n", userWin->GetGamerTag(), userId );
...
void idSaveLoadParms::CancelSaveGameFilePipelines()
{
	for( int i = 0; i < files.Num(); i++ )
	{
		...
		{
			budFile_SaveGamePipelined* file = dynamic_cast< budFile_SaveGamePipelined* >( files[i] );
			assert( file != NULL );
			
			if( file->GetMode() == budFile_SaveGamePipelined::WRITE )
			{
			...
			else if( file->GetMode() == budFile_SaveGamePipelined::READ )
			{
...
bool idSaveGameProcessor::Init()
{
	if( !verify( !IsWorking() ) )
	{
		libBud::Warning( "[%s] Someone is trying to execute this processor twice, this is really bad!", this->Name() );
		return false;
	}
...
{
	libBud::PrintfIf( saveGame_verbose.GetBool(), "[%s] : %s\n", __FUNCTION__, processor->Name() );
	...
	if( this->processor != NULL )
	{
		...
		{
			libBud::Warning( "[idSaveGameManager::ExecuteProcessor]:1 Someone is trying to execute this processor twice, this is really bad, learn patience padawan!" );
		...
		else
		{
			...
			{
				libBud::Warning( "[idSaveGameManager::ExecuteProcessor]:2 Someone is trying to execute this processor twice, this is really bad, learn patience padawan!" );
...
bool idSaveGameManager::DeviceSelectorWaitingOnSaveRetry()
{
	...
	return ( budStr::Icmp( retryFolder, "GAME-autosave" ) == 0 );
}
...
void idSaveGameManager::Pump()
{
	...
	if( processor->IsThreadFinished() )
	{
		...
		if( !cancel && continueProcessing )
		{
			...
			{
				libBud::PrintfIf( saveGame_verbose.GetBool(), "%s calling %s::Process(), error = 0x%08X, %s\n", __FUNCTION__, processor->Name(), processor->GetError(), GetSaveGameErrorString( processor->GetError() ).c_str() );
		...
		if( !continueProcessing )
		{
			...
			libBud::PrintfIf( saveGame_verbose.GetBool(), "%s calling %s::CompletedCallback()\n", __FUNCTION__, processor->Name() );
	...
	else if( processor->ShouldTimeout() )
	{
		...
		libBud::PrintfIf( saveGame_verbose.GetBool(), "----- PROCESSOR TIMEOUT ----- (%s)\n", processor->Name() );
