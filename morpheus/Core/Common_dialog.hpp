...
public:
	...
	dialogType_t			type;
	budSWFScriptFunction* 	acceptCB;
	budSWFScriptFunction* 	cancelCB;
	budSWFScriptFunction* 	altCBOne;
	budSWFScriptFunction* 	altCBTwo;
	bool					showing;
	bool					clear;
	bool					waitClear;
	bool					pause;
...
public:
	...
	void	AddDialog( gameDialogMessages_t msg, dialogType_t type, budSWFScriptFunction* acceptCallback, budSWFScriptFunction* cancelCallback, bool pause, const char* location = NULL, int lineNumber = 0, bool leaveOnMapHeapReset = false, bool waitOnAtlas = false, bool renderDuringLoad = false );
	void	AddDynamicDialog( gameDialogMessages_t msg, const budStaticList< budSWFScriptFunction*, 4 >& callbacks, const budStaticList< budStrId, 4 >& optionText, bool pause, budStrStatic< 256 > overrideMsg, bool leaveOnMapHeapReset = false, bool waitOnAtlas = false, bool renderDuringLoad = false );
	...
	budStr	GetDialogMsg( gameDialogMessages_t msg, budStr& message, budStr& title );
...
private:
	bool	dialogPause;
	budSWF* 	dialog;
	budSWF* 	saveIndicator;
	bool	dialogShowingSaveIndicatorRequested;
	int		dialogShowingSaveIndicatorTimeRemaining;
	
	budStaticList< idDialogInfo, MAX_DIALOGS > messageList;
	budStaticList< idLoadScreenInfo, 16 > loadScreenInfo;
