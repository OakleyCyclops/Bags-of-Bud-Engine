...
extern budCVar net_connectTimeoutInSeconds;
extern budCVar net_headlessServer;

budCVar net_checkVersion( "net_checkVersion", "0", CVAR_INTEGER, "Check for matching version when clients connect. 0: normal rules, 1: force check, otherwise no check (pass always)" );
budCVar net_peerTimeoutInSeconds( "net_peerTimeoutInSeconds", "30", CVAR_INTEGER, "If the host hasn't received a response from a peer in this amount of time (in seconds), the peer will be disconnected." );
budCVar net_peerTimeoutInSeconds_Lobby( "net_peerTimeoutInSeconds_Lobby", "20", CVAR_INTEGER, "If the host hasn't received a response from a peer in this amount of time (in seconds), the peer will be disconnected." );

// NOTE - The snapshot exchange does the bandwidth challenge
budCVar net_bw_challenge_enable( "net_bw_challenge_enable", "0", CVAR_BOOL, "Enable pre game bandwidth challenge for throttling snap rate" );

budCVar net_bw_test_interval( "net_bw_test_interval", "33", CVAR_INTEGER, "MS - how often to send packets in bandwidth test" );
budCVar net_bw_test_numPackets( "net_bw_test_numPackets", "30", CVAR_INTEGER, "Number of bandwidth challenge packets to send" );
budCVar net_bw_test_packetSizeBytes( "net_bw_test_packetSizeBytes", "1024", CVAR_INTEGER, "Size of each packet to send out" );
budCVar net_bw_test_timeout( "net_bw_test_timeout", "500", CVAR_INTEGER, "MS after receiving a bw test packet that client will time out" );
budCVar net_bw_test_host_timeout( "net_bw_test_host_timeout", "3000", CVAR_INTEGER, "How long host will wait in MS to hear bw results from peers" );

budCVar net_bw_test_throttle_rate_pct( "net_bw_test_throttle_rate_pct", "0.80", CVAR_FLOAT, "Min rate % a peer must match in bandwidth challenge before being throttled. 1.0=perfect, 0.0=received nothing" );
budCVar net_bw_test_throttle_byte_pct( "net_bw_test_throttle_byte_pct", "0.80", CVAR_FLOAT, "Min byte % a peer must match in bandwidth challenge before being throttled. 1.0=perfect (received everything) 0.0=Received nothing" );
budCVar net_bw_test_throttle_seq_pct( "net_bw_test_throttle_seq_pct", "0.80", CVAR_FLOAT, "Min sequence % a peer must match in bandwidth test before being throttled. 1.0=perfect. This score will be more adversely affected by packet loss than byte %" );

budCVar net_ignoreConnects( "net_ignoreConnects", "0", CVAR_INTEGER, "Test as if no one can connect to me. 0 = off, 1 = ignore with no reply, 2 = send goodbye" );

budCVar net_skipGoodbye( "net_skipGoodbye", "0", CVAR_BOOL, "" );
...
========================
*/
void idLobby::Initialize( lobbyType_t sessionType_, budSessionCallbacks* callbacks )
{
...
========================
*/
void idLobby::StartHosting( const budMatchParameters& parms_ )
{
	parms = parms_;
...
========================
*/
void idLobby::StartFinding( const budMatchParameters& parms_ )
{
	parms = parms_;
...
void idLobby::Pump()
{
	...
	switch( state )
	{
		...
		case STATE_FAILED:
			break;
		default:
			libBud::Error( "idLobby::Pump:  Unknown state." );
	}
}
...
void idLobby::Shutdown( bool retainMigrationInfo, bool skipGoodbye )
{
	...
	{
		libBud::Printf( "Cancelling host migration on %s.\n", GetLobbyName() );
		EndMigration();
	}
...
void idLobby::HandlePacket( lobbyAddress_t& remoteAddress, budBitMsg fragMsg, idPacketProcessor::sessionId_t sessionID )
{
	...
	if( peerNum >= 0 )
	{
		...
		{
			libBud::Printf( "NET: Received in-band packet from peer %s with no active connection.\n", remoteAddress.ToString() );
			return;
		}
	...
	else
	{
		...
		{
			libBud::Printf( "ProcessConnectionlessIncoming FAILED from %s.\n", remoteAddress.ToString() );
	...
	if( type == idPacketProcessor::RETURN_TYPE_OOB )
	{
		...
		else if( userData == OOB_MIGRATE_INVITE )
		{
			...
			{
				libBud::Printf( "NET: Already migrated to %s.\n", remoteAddress.ToString() );
				return;
			}
			...
			if( migrationInfo.state == MIGRATE_NONE )
			{
				...
				{
					libBud::Printf( "NET: Ignoring migration invite because we are not migrating %s\n", remoteAddress.ToString() );
			...
			if( migrationInfo.state == MIGRATE_BECOMING_HOST )
			{
				...
				if( inviteIndex != -1 )
				{
					...
					{
						libBud::Printf( "NET: Ignoring migration invite from %s, since our ping is better (%i / %i).\n", remoteAddress.ToString(), ping1, ping2 );
						return;
					}
				}
			}
	...
	else if( type == idPacketProcessor::RETURN_TYPE_INBAND )
	{
		...
		if( peerNum < 0 )
		{
			libBud::Printf( "NET: In-band message from unknown peer: %s\n", remoteAddress.ToString() );
			return;
		}
		
		if( !verify( peers[ peerNum ].address.Compare( remoteAddress ) ) )
		{
			libBud::Printf( "NET: Peer with wrong address: %i, %s\n", peerNum, remoteAddress.ToString() );
			return;
		}
		...
		for( int r = 0; r < numReliable; r++ )
		{
			...
			if( peerNum == -1 )
			{
				libBud::Printf( "NET: Dropped peer while processing reliable msg's: %i, %s\n", peerNum, remoteAddress.ToString() );
				break;
			}
			...
			budBitMsg reliableMsg( reliableData, reliableSize );
		...
		if( peerNum == -1 || !peers[ peerNum ].IsConnected() )
		{
			...
			libBud::Printf( "NET: In-band message from host with no active connection: %i, %s\n", peerNum, remoteAddress.ToString() );
			return;
		}
		...
		if( msg.GetRemainingData() > 0 && loaded )
		{
			if( !verify( lobbyType == GetActingGameStateLobbyType() ) )
			{
				libBud::Printf( "NET: Snapshot msg for non game session lobby %s\n", remoteAddress.ToString() );
				return;
			}
			
			if( peerNum == host )
			{
				budSnapShot	localSnap;
				int			sequence = -1;
				int			baseseq = -1;
				bool		fullSnap = false;
				...
				if( peers[ peerNum ].snapProc->ReceiveSnapshotDelta( deltaData, deltaLength, 0, sequence, baseseq, localSnap, fullSnap ) )
				{
					...
					if( sessionCB->GetState() != budSession::INGAME && sequence != -1 )
					{
						...
						byte ackbuffer[32];
						budBitMsg ackmsg( ackbuffer, sizeof( ackbuffer ) );
						...
						{
							incomingBPS = budMath::ClampFloat( 0.0f, static_cast<float>( idLobby::BANDWIDTH_REPORTING_MAX ), peers[host].packetProc->GetIncomingRateBytes() );
			...
			else
			{
				...
				budBitMsg usercmdMsg( ( const byte* )usercmdBuffer, usercmdSize );
...
void idLobby::State_Idle()
{
	...
	if( migrationInfo.persistUntilGameEndsData.hasGameData && sessionCB->GetState() <= budSession::IDLE )
	{
...
void idLobby::State_Searching()
{
	...
	extern budCVar net_maxSearchResultsToTry;
		...
		if( peers[p].address.Compare( remoteAddress ) )
		{
			...
			if( IsPeer() )
			{
				...
				while( numTries-- > 0 && searchStart != sessionID )
				{
					...
					{
						libBud::Printf( "NET: Rolling session ID check found new ID: %i\n", searchStart );
...
void idLobby::SendGoodbye( const lobbyAddress_t& remoteAddress, bool wasFull )
{
	...
	else if( lobbyType == TYPE_GAME && ( sessionCB->GetSessionOptions() & budSession::OPTION_LEAVE_WITH_PARTY ) && !( parms.matchFlags & MATCH_PARTY_INVITE_PLACEHOLDER ) )
	{
...
void idLobby::SetPeerConnectionState( int p, connectionState_t newState, bool skipGoodbye )
{
	...
	{
		libBud::Printf( "NET: SetPeerConnectionState invalid peer index %i\n", p );
		return;
	}
	...
	{
		libBud::Printf( "NET: SetPeerConnectionState: Peer already in state %i\n", newState );
	...
	if( newState == CONNECTION_CONNECTING )
	{
		...
		if( lobbyType == actingGameStateLobbyType )
		{
			...
			peer.snapProc = new( TAG_NETWORKING )budSnapShotProcessor();
		}
	...
	else if( newState == CONNECTION_FREE )
	{
		...
		{
			libBud::Printf( "SetPeerConnectionState: Sending goodbye to peer %s from session %s\n", peer.address.ToString(), GetLobbyName() );
...
void idLobby::QueueReliableMessage( int p, byte type, const byte* data, int dataLen )
{
	...
	{
		libBud::PrintfIf( false, "NET: peer.packetProc->NumQueuedReliables() > 2: %i (%i / %s)\n", peer.packetProc->NumQueuedReliables(), p, peer.address.ToString() );
	}
...
========================
*/
void idLobby::HandleReliablePlayerToPlayerMsg( int peerNum, budBitMsg& msg, int type )
{
	...
	{
		libBud::Warning( "NET: Ignoring invalid reliable player to player message" );
	...
	{
		libBud::Warning( "NET: Can't forward reliable message for remote player: I'm not the host" );
	}
}
...
========================
*/
void idLobby::HandleReliablePlayerToPlayerMsg( const reliablePlayerToPlayerHeader_t& info, budBitMsg& msg, int reliableType )
{
#if 0
	...
	switch( type )
	{
		...
		default:
		{
			libBud::Warning( "NET: Ignored unknown player to player reliable type %i", ( int ) type );
		}
	};
#endif
}
...
{
	budBitMsg msg( data, dataLen );
	msg.SetSize( dataLen );
	
	byte buffer[ idPacketProcessor::MAX_OOB_MSG_SIZE ];
	budBitMsg processedMsg( buffer, sizeof( buffer ) );
...
{
	NET_VERBOSE_PRINT( "NET: budSessionLocal::ConnectTo: fromInvite = %i\n", fromInvite );
...
========================
*/
bool idLobby::CheckVersion( budBitMsg& msg, lobbyAddress_t peerAddress )
{
...
========================
*/
bool idLobby::VerifyNumConnectingUsers( budBitMsg& msg )
{
	int c, b;
	...
	else if( lobbyType == TYPE_PARTY && sessionCB->GetState() >= budSession::GAME_LOBBY && sessionCB->GetGameLobby().IsLobbyActive() && !IsMigrating() )
	{
...
========================
*/
bool idLobby::VerifyLobbyUserIDs( budBitMsg& msg )
{
	int c, b;
	msg.SaveReadState( c, b );
	const int numUsers = msg.ReadByte();
...
========================
*/
int idLobby::HandleInitialPeerConnection( budBitMsg& msg, const lobbyAddress_t& peerAddress, int peerNum )
{
	...
	if( MatchTypeIsJoinInProgress( parms.matchFlags ) )
	{
		...
		if( ( lobbyType == TYPE_GAME || lobbyType == TYPE_GAME_STATE ) && sessionCB->GetState() < budSession::GAME_LOBBY )
		{
	...
	else
	{
		...
		if( lobbyType == TYPE_GAME && sessionCB->GetState() != budSession::GAME_LOBBY )
		{
		...
		if( !migrationInvite && lobbyType == TYPE_PARTY && ( sessionCB->GetState() == budSession::INGAME || sessionCB->GetState() == budSession::LOADING ) )
		{
	...
	{
		libBud::Printf( "NET: Denying user %s with wrong version number\n", peerAddress.ToString() );
		SendGoodbye( peerAddress );
		return -1;
	}
	...
	{
		libBud::Printf( "NET: Denying user %s because they were not invited to an invite only match\n", peerAddress.ToString() );
		SendGoodbye( peerAddress );
		return -1;
	}
	...
	byte buffer[ idPacketProcessor::MAX_PACKET_SIZE ];
	budBitMsg outmsg( buffer, sizeof( buffer ) );
	...
	if( MatchTypeIsJoinInProgress( parms.matchFlags ) )
	{
		...
		if( lobbyType == TYPE_PARTY && sessionCB->GetState() >= budSession::GAME_LOBBY )
		{
			SendPeerMembersToLobby( peerNum, TYPE_GAME, false );
		}
		
		// We are are ingame, then start the client loading immediately
		if( ( lobbyType == TYPE_GAME || lobbyType == TYPE_GAME_STATE ) && sessionCB->GetState() >= budSession::LOADING )
		{
			libBud::Printf( "******* JOIN IN PROGRESS ********\n" );
			if( sessionCB->GetState() == budSession::INGAME )
			{
...
void idLobby::SendPeerMembersToLobby( int peerIndex, lobbyType_t destLobbyType, const lobbyConnectInfo_t& connectInfo, bool waitForOtherMembers )
{
	...
	if( !peer.IsConnected() )
	{
		libBud::Warning( "NET: Can't send peer %i to game lobby: peer isn't in party", peerIndex );
		return;
	}
...
void idLobby::NotifyPartyOfLeavingGameLobby()
{
	...
	if( !( sessionCB->GetSessionOptions() & budSession::OPTION_LEAVE_WITH_PARTY ) )
	{
...
uint32 idLobby::GetPartyTokenAsHost()
{
	...
	if( partyToken == 0 )
	{
		...
		if( masterUser != NULL )
		{
			seed += budStr::Hash( masterUser->GetGamerTag() );
		}
		partyToken = idRandom( seed ).RandomInt();
		libBud::Printf( "NET: PartyToken is %u (seed = %u)\n", partyToken, seed );
	}
	return partyToken;
}
...
#define VERIFY_CONNECTED_PEER( p, sessionType_, msgType )				\
	if ( !verify( lobbyType == sessionType_ ) ) {						\
		libBud::Printf( "NET: " #msgType ", peer:%s invalid session type for " #sessionType_ " %i.\n", peer.address.ToString(), sessionType_ );	\
		return;															\
	}																	\
	if ( peers[p].GetConnectionState() != CONNECTION_ESTABLISHED ) {	\
		libBud::Printf( "NET: " #msgType ", peer:%s not connected for " #sessionType_ " %i.\n", peer.address.ToString(), sessionType_ );	\
		return;															\
	}
...
#define VERIFY_CONNECTING_PEER( p, sessionType_, msgType )				\
	...
	if ( peers[p].GetConnectionState() != CONNECTION_CONNECTING ) {		\
		libBud::Printf( "NET: " #msgType ", peer:%s not connecting for " #sessionType_ " %i.\n", peer.address.ToString(), sessionType_ );	\
		return;															\
	}
...
#define VERIFY_FROM_HOST( p, sessionType_, msgType )					\
	...
	if ( p != host ) {													\
		libBud::Printf( "NET: "#msgType", not from "#sessionType_" host: %s\n", peer.address.ToString() );	\
		return;															\
	}																	\
...
========================
*/
void idLobby::HandleHelloAck( int p, budBitMsg& msg )
{
	peer_t& peer = peers[p];
	
	if( state != STATE_CONNECT_HELLO_WAIT )
	{
		libBud::Printf( "NET: Hello ack for session type %s while not waiting for hello.\n", GetLobbyName() );
	...
	if( p != host )
	{
		...
		libBud::Printf( "NET: Hello ack for session type %s, not from correct host.\n", GetLobbyName() );
		SendGoodbye( peer.address );		// We send a customary goodbye to make sure we are not in their list anymore
		return;
	}
...
========================
*/
void idLobby::HandleReliableMsg( int p, budBitMsg& msg, const lobbyAddress_t* remoteAddress /* = NULL */ )
{
	peer_t& peer = peers[p];
	...
	else if( reliableType == RELIABLE_USER_CONNECT_REQUEST )
	{
		...
		budSession::sessionState_t expectedState = ( lobbyType == TYPE_PARTY ) ? budSession::PARTY_LOBBY : budSession::GAME_LOBBY;
	...
	else if( reliableType == RELIABLE_KICK_PLAYER )
	{
		...
		{
			session->SetSessionOption( budSession::OPTION_LEAVE_WITH_PARTY );
		}
		session->Cancel();
	}
	...
	else if( reliableType == RELIABLE_SNAPSHOT_ACK )
	{
		...
		//libBud::Printf( "NET: Peer %d Ack'd snapshot %d\n", p, snapNum );
	...
	else if( reliableType == RELIABLE_CONNECT_AND_MOVE_TO_LOBBY )
	{
		...
		if( IsHost() )
		{
			libBud::Printf( "RELIABLE_CONNECT_AND_MOVE_TO_LOBBY: We are the host.\n" );
			return;
		}
	...
	else if( reliableType == RELIABLE_PARTY_CONNECT_OK )
	{
		...
		{
			libBud::Printf( "RELIABLE_PARTY_CONNECT_OK: Wasn't waiting for ok.\n" );
		}
	...
	else if( reliableType == RELIABLE_PARTY_LEAVE_GAME_LOBBY )
	{
		...
		if( sessionCB->GetState() != budSession::GAME_LOBBY )
		{
			libBud::Printf( "RELIABLE_PARTY_LEAVE_GAME_LOBBY: Not in a game lobby, ignoring.\n" );
			return;
		}
		
		if( IsHost() )
		{
			libBud::Printf( "RELIABLE_PARTY_LEAVE_GAME_LOBBY: Host of party, ignoring.\n" );
			return;
		}
...
void idLobby::DrawDebugNetworkHUD() const
{
	...
	renderSystem->DrawFilled( budVec4( 0.0f, 0.0f, 0.0f, 0.7f ), X_OFFSET - 10.0f, curY - 10.0f, 1550, ( peers.Num() + numLines ) * Y_SPACING + 20.0f );
	
	renderSystem->DrawSmallStringExt( budMath::Ftoi( X_OFFSET ), budMath::Ftoi( curY ), "# Peer                   | Sent kB/s | Recv kB/s | Sent MB | Recv MB | Ping   | L |  %  | R.NM | R.SZ | R.AK | T", colorGreen, false );
	curY += Y_SPACING;
	
	renderSystem->DrawSmallStringExt( budMath::Ftoi( X_OFFSET ), budMath::Ftoi( curY ), "------------------------------------------------------------------------------------------------------------------------------------", colorGreen, false );
	curY += Y_SPACING;
	...
	for( int p = 0; p < peers.Num(); p++ )
	{
		...
		budVec4 color = sentKps > 20.0f ? colorRed : colorGreen;
		
		int resourcePercent = 0;
		budStr name = peer.address.ToString();
	...
	budVec4 color = totalSentKps > 100.0f ? colorRed : colorGreen;
...
void idLobby::DrawDebugNetworkHUD2() const
{
	...
	renderSystem->DrawFilled( budVec4( 0.0f, 0.0f, 0.0f, 0.7f ), X_OFFSET - 10.0f, curY - 10.0f, 550, ( peers.Num() + 4 ) * Y_SPACING + 20.0f );
	
	const char* stateName = session->GetStateString();
	
	renderSystem->DrawFilled( budVec4( 1.0f, 1.0f, 1.0f, 0.7f ), X_OFFSET - 10.0f, curY - 10.0f, 550, ( peers.Num() + 5 ) * Y_SPACING + 20.0f );
	
	renderSystem->DrawSmallStringExt( budMath::Ftoi( X_OFFSET ), budMath::Ftoi( curY ), va( "State: %s. Local time: %d", stateName, Sys_Milliseconds() ), colorGreen, false );
	curY += Y_SPACING;
	
	renderSystem->DrawSmallStringExt( budMath::Ftoi( X_OFFSET ), budMath::Ftoi( curY ), "Peer           | Sent kB/s | Recv kB/s | L | R | Resources", colorGreen, false );
	curY += Y_SPACING;
	...
	for( int p = 0; p < peers.Num(); p++ )
	{
		...
		budVec4 color = proc.TickFragmentAccumulator() ? colorRed : colorGreen;
...
void idLobby::DrawDebugNetworkHUD_ServerSnapshotMetrics( bool draw )
{
	...
	for( int p = 0; p < peers.Num(); p++ )
	{
		...
		budSnapShotProcessor* snapProc = peer.snapProc;
		...
		renderSystem->DrawFilled( budVec4( 0.0f, 0.0f, 0.0f, 0.7f ), X_OFFSET - 10.0f, curY - 10.0f, width, ( Y_SPACING * numLines ) + 20.0f );
		...
		{
			budVec4 bgColor( vec4_zero );
			peer.debugGraphs[GRAPH_OUTGOING]->SetBackgroundColor( bgColor );
			
			budVec4 lineColor = colorLtGrey;
			lineColor.w	 = 0.5f;
			float outgoingRate = peer.sentBpsHistory[ peer.receivedBpsIndex % MAX_BPS_HISTORY ];
			// peer.packetProc->GetOutgoingRateBytes()
			peer.debugGraphs[GRAPH_OUTGOING]->SetValue( -1, budMath::ClampFloat( 0.0f, 1.0f, outgoingRate / net_debughud3_bps_max.GetFloat() ), lineColor );
		}
		
		
		if( peer.debugGraphs[GRAPH_INCOMINGREPORTED] != NULL )
		{
			budVec4 lineColor = colorYellow;
			extern budCVar net_peer_throttle_bps_peer_threshold_pct;
			extern budCVar net_peer_throttle_bps_host_threshold;
			...
			peer.debugGraphs[GRAPH_INCOMINGREPORTED]->SetFillMode( budDebugGraph::GRAPH_LINE );
			peer.debugGraphs[GRAPH_INCOMINGREPORTED]->SetValue( -1, budMath::ClampFloat( 0.0f, 1.0f, peer.receivedBps / net_debughud3_bps_max.GetFloat() ), lineColor );
		}
...
void idLobby::CheckHeartBeats()
{
	...
	if( sessionCB->GetState() < budSession::LOADING && migrationInfo.state == MIGRATE_NONE )
	{
		...
		for( int p = 0; p < peers.Num(); p++ )
		{
			if( peers[p].IsConnected() )
			{
				...
				if( peerTimeout )
				{
					...
					if( IsHost() )
					{
						libBud::Printf( "Peer %i timed out for %s session @ %d (lastHeartBeat %d)\n", p, GetLobbyName(), time, peers[p].lastHeartBeat );
						DisconnectPeerFromSession( p );
					}
					...
					if( IsPeer() )
					{
						...
						{
							libBud::Printf( "Host timed out for %s session\n", GetLobbyName() );
...
bool idLobby::ShouldShowMigratingDialog() const
{
	...
	return IsMigratedStatsGame() && sessionCB->GetState() != budSession::INGAME;
}
...
void idLobby::PingPeers()
{
	...
	budBitMsg msg( packetCopy, sizeof( packetCopy ) );
...
void idLobby::ThrottlePeerSnapRate( int p )
{
	...
	libBud::Printf( "^1Throttling peer %d %s!\n", p, GetPeerName( p ) );
	libBud::Printf( "  New snaprate: %d\n", peers[p].throttledSnapRate / 1000 );
}
...
void idLobby::BeginBandwidthTest()
{
	if( !verify( IsHost() ) )
	{
		libBud::Warning( "Bandwidth test should only be done on host" );
		return;
	}
	
	if( bandwidthChallengeStartTime > 0 )
	{
		libBud::Warning( "Already started bandwidth test" );
		return;
	}
...
void idLobby::ClientUpdateBandwidthTest()
{
	...
	libBud::Printf( "^3Finished Bandwidth test: \n" );
	libBud::Printf( "  Total time: %d\n", totalTime );
	libBud::Printf( "  Num good packets: %d\n", bandwidthChallengeNumGoodSeq );
	libBud::Printf( "  Total received byes: %d\n\n", totalIncomingBytes );
...
========================
*/
void idLobby::HandleBandwidhTestValue( int p, budBitMsg& msg )
{
	if( !IsHost() )
	{
		return;
	...
	libBud::Printf( "Received RELIABLE_BANDWIDTH_CHECK %d\n", Sys_Milliseconds() );
	
	if( bandwidthChallengeStartTime < 0 || bandwidthChallengeFinished )
	{
		libBud::Warning( "Received bandwidth test results too early from peer %d", p );
		return;
	}
	...
	libBud::Printf( "^3Finished Bandwidth test %s: \n", GetPeerName( p ) );
	libBud::Printf( "  Total time: %dms\n", totalTime );
	libBud::Printf( "  %sNum good packets: %d  (%.2f%%)\n", ( failedSeq ? "^1" : "^2" ), totalGoodSeq, pctPackets );
	libBud::Printf( "  %sTotal received bytes: %d  (%.2f%%)\n", ( failedByte ? "^1" : "^2" ), totalReceivedBytes, bytesPct );
	libBud::Printf( "  %sEffective downstream: %.2fkbs (host: %.2fkbs) -> %.2f%%\n\n", ( failedRate ? "^1" : "^2" ), peerKBS, outgoingKBS, pctKBS );
...
========================
*/
void idLobby::HandleReliablePing( int p, budBitMsg& msg )
{
	int c, b;
	msg.SaveReadState( c, b );
...
========================
*/
void idLobby::HandlePingValues( budBitMsg& msg )
{
	pktPingValues_t packet;
	memset( &packet, 0, sizeof( packet ) );
	for( int i = 0; i < peers.Max(); ++i )
...
void idLobby::ProcessOutgoingMsg( int p, const void* data, int size, bool isOOB, int userData )
{
	...
	{
		libBud::Printf( "peer.GetConnectionState() != CONNECTION_ESTABLISHED\n" );
		return;	// Peer not fully connected for this session type, return
	}
	
	if( peer.packetProc->HasMoreFragments() )
	{
		libBud::Error( "FATAL: Attempt to process a packet while fragments still need to be sent.\n" ); // We can't handle this case
	}
	
	int currentTime = Sys_Milliseconds();
	
	// if ( currentTime - peer.lastProcTime < 30 ) {
	//	 libBud::Printf("ProcessOutgoingMsg called within %dms %s\n", (currentTime - peer.lastProcTime), GetLobbyName() );
	// }
...
========================
*/
void idLobby::UpdateMatchParms( const budMatchParameters& p )
{
	if( !IsHost() )
	{
		return;
...
========================
*/
void idLobby::SendReliable( int type, budBitMsg& msg, bool callReceiveReliable /*= true*/, peerMask_t sessionUserMask /*= MAX_UNSIGNED_TYPE( peerMask_t ) */ )
{
...
========================
*/
void idLobby::SendReliableToLobbyUser( lobbyUserID_t lobbyUserID, int type, budBitMsg& msg )
{
	assert( lobbyType == GetActingGameStateLobbyType() );
	assert( type < 256 );			// QueueReliable only accepts a byte for message type
...
========================
*/
void idLobby::SendReliableToHost( int type, budBitMsg& msg )
{
	assert( lobbyType == GetActingGameStateLobbyType() );
...
/*
========================
budSessionLocal::reliablePlayerToPlayerHeader_t::Read
========================
*/
bool idLobby::reliablePlayerToPlayerHeader_t::Read( idLobby* lobby, budBitMsg& msg )
{
	assert( lobby != NULL );
...
========================
*/
bool idLobby::reliablePlayerToPlayerHeader_t::Write( idLobby* lobby, budBitMsg& msg )
{
...
{
	extern budCVar net_useGameStateLobby;
