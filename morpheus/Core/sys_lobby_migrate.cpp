...
budCVar net_migration_forcePeerAsHost( "net_migration_forcePeerAsHost", "-1", CVAR_INTEGER, "When set to >-1, it forces that peer number to be the new host during migration" );
...
void idLobby::PickNewHost( bool forceMe, bool inviteOldHost )
{
	if( IsHost() )
	{
		libBud::Printf( "PickNewHost: Already host of session %s\n", GetLobbyName() );
		return;
	}
...
void idLobby::PickNewHostInternal( bool forceMe, bool inviteOldHost )
{
	...
	libBud::Printf( "PickNewHost: Started picking new host %s.\n", GetLobbyName() );
	...
	migrationInfo.persistUntilGameEndsData.wasMigratedGame = sessionCB->GetState() == budSession::INGAME;
...
void idLobby::BecomeHost()
{
	...
	{
		libBud::Printf( "BecomeHost: Must be called from PickNewHost.\n" );
		EndMigration();
		return;
	}
	...
	if( IsHost() )
	{
		libBud::Printf( "BecomeHost: Already host of session.\n" );
		EndMigration();
		return;
	}
	...
	libBud::Printf( "BecomeHost: Sending %i invites on %s.\n", migrationInfo.invites.Num(), GetLobbyName() );
...
void idLobby::EndMigration()
{
	if( migrationInfo.state == MIGRATE_NONE )
	{
		libBud::Printf( "budSessionLocal::EndMigration: Not migrating.\n" );
		return;
	}
...
This will setup the passed in budBitMsg to either read or write from the global migration game data buffer
========================
*/
bool idLobby::GetMigrationGameData( budBitMsg& msg, bool reading )
{
	if( reading )
	{
...
This will setup the passed in budBitMsg to either read or write from the user's migration game data buffer
========================
*/
bool idLobby::GetMigrationGameDataUser( lobbyUserID_t lobbyUserID, budBitMsg& msg, bool reading )
{
	...
	if( u != NULL )
	{
		if( reading )
		{
			...
			else
			{
				...
				libBud::Warning( "No migration data for user %d in a migrated game (%d)", userNum, u->migrationGameData );
				return false;
			}
		}
		else
...
========================
*/
void idLobby::HandleMigrationGameData( budBitMsg& msg )
{
...
void idLobby::SendMigrationGameData()
{
	...
	budBitMsg msg( packetData, sizeof( packetData ) );
	...
	for( int i = 0; i < peers.Num(); i++ )
	{
		...
		if( peers[ peerToSend ].IsConnected() && peers[ peerToSend ].loaded )
		{
			...
			else
			{
				...
				{
					libBud::Printf( "NET: Sending migration game data to peer %d. size: %d\n", peerToSend, msg.GetSize() );
				}
