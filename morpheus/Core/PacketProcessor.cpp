...
budCVar net_maxRate( "net_maxRate", "50", CVAR_INTEGER, "max send rate in kilobytes per second" );

budCVar net_showReliableCompression( "net_showReliableCompression", "0", CVAR_BOOL, "Show reliable compression ratio." );
...
================================================
*/
int idPacketProcessor::FinalizeRead( budBitMsg& inMsg, budBitMsg& outMsg, int& userValue )
{
	userValue = 0;
	...
	{
		libBud::Printf( "Received invalid fragmented packet.\n" );
	...
	else
	{
		...
		{
			libBud::Printf( "In-band packet expected, received type %i instead.\n", header.Type() );
			return RETURN_TYPE_NONE;
		}
		...
		if( numReliableRecv > 0 )
		{
			...
			for( int r = 0; r < numReliableRecv; r++ )
			{
				...
				if( reliableSequence + r > reliableSequenceRecv )  		// Only accept newer reliable msg's than we've currently already received
				{
					...
					{
						libBud::Printf( "Reliable msg size overflow.\n" );
						return RETURN_TYPE_NONE;
					}
					if( !verify( numReliable < MAX_RELIABLE_QUEUE ) )
					{
						libBud::Printf( "Reliable msg count overflow.\n" );
						return RETURN_TYPE_NONE;
					}
				...
				{
					extern budCVar net_verboseReliable;
					if( net_verboseReliable.GetBool() )
					{
						libBud::Printf( "Ignoring reliable msg %i because %i was already acked\n", ( reliableSequence + r ), reliableSequenceRecv );
					}
				}
				
				if( !verify( lzwCompressor.IsOverflowed() == false ) )
				{
					libBud::Printf( "lzwCompressor.IsOverflowed() == true.\n" );
					return RETURN_TYPE_NONE;
				}
			}
...
================================================
*/
bool idPacketProcessor::ProcessOutgoing( const int time, const budBitMsg& msg, bool isOOB, int userData )
{
	...
	{
		libBud::Warning( "ProcessOutgoing: fragmentedSend == true!" );
		return false;
	}
	
	if( !verify( unsentMsg.GetRemainingData() == 0 ) )
	{
		libBud::Warning( "ProcessOutgoing: unsentMsg.GetRemainingData() > 0!" );
		return false;
	}
	...
	if( isOOB )
	{
		...
		{
			libBud::Printf( "Out-of-band packet too large %i\n", unsentMsg.GetSize() );
			assert( 0 );
			return false;
		}
	...
	else
	{
		...
		if( reliable.Num() > 0 )
		{
			...
			{
				libBud::Error( "reliable msg compressor overflow." );
			}
			...
			if( net_showReliableCompression.GetBool() )
			{
				...
				libBud::Printf( "Uncompressed: %i, Compressed: %i, TotalUncompressed: %i, TotalCompressed: %i, (%2.2f / %2.2f )\n", uncompressedSize, lzwCompressor.Length(), totalUncompressed, totalCompressed, ratio1, ratio2 );
			}
		}
	}
	...
	if( unsentMsg.GetSize() > MAX_PACKET_SIZE )
	{
		if( isOOB )
		{
			libBud::Error( "oob msg's cannot fragment" );
		}
		fragmentedSend = true;
	}
...
================================================
*/
bool idPacketProcessor::GetSendFragment( const int time, sessionId_t sessionID, budBitMsg& outMsg )
{
	lastSendTime = time;
	...
	{
		int currentSize = budMath::ClampInt( 0, MAX_PACKET_SIZE, unsentMsg.GetRemainingData() );
...
================================================
*/
int idPacketProcessor::ProcessIncoming( int time, sessionId_t expectedSessionID, budBitMsg& msg, budBitMsg& out, int& userData, const int peerNum )
{
	...
	{
		libBud::Printf( "Expected non connectionless ID, but got a connectionless one\n" );
		return RETURN_TYPE_NONE;
	}
	
	if( sessionID != expectedSessionID )
	{
		libBud::Printf( "Expected session id: %8x but got %8x instead\n", expectedSessionID, sessionID );
		return RETURN_TYPE_NONE;
	}
	...
	{
		droppedFrags++;
		libBud::Printf( "Dropped Fragments - PeerNum: %i FragmentSeq: %i, ReadSeq: %i, Total: %i\n", peerNum, fragmentSequence, readSequence, droppedFrags );
	...
	{
		libBud::Error( "ProcessIncoming: Fragmented msg buffer overflow." );
	}
	...
	if( header.Value() == FRAGMENT_END )
	{
		...
		budBitMsg msg( msgBuffer, sizeof( msgBuffer ) );
	...
	{
		libBud::Printf( "ProcessIncoming: Invalid packet.\n" );
	}
...
================================================
*/
bool idPacketProcessor::ProcessConnectionlessOutgoing( budBitMsg& msg, budBitMsg& out, int lobbyType, int userData )
{
...
================================================
*/
bool idPacketProcessor::ProcessConnectionlessIncoming( budBitMsg& msg, budBitMsg& out, int& userData )
{
	...
	if( sessionID != SESSION_ID_CONNECTIONLESS_PARTY && sessionID != SESSION_ID_CONNECTIONLESS_GAME && sessionID != SESSION_ID_CONNECTIONLESS_GAME_STATE )
	{
		...
		libBud::Printf( "ProcessConnectionlessIncoming: Invalid session ID - %d\n", sessionID );
		return false;
	}
	...
	if( header.Type() != PACKET_TYPE_OOB )
	{
		libBud::Printf( "ProcessConnectionlessIncoming: header.Type() != PACKET_TYPE_OOB\n" );
...
================================================
*/
idPacketProcessor::sessionId_t idPacketProcessor::GetSessionID( budBitMsg& msg )
{
...
================================================
*/
budCVar net_verifyReliableQueue( "net_verifyReliableQueue", "2", CVAR_INTEGER, "0: warn only, 1: error, 2: fixup, 3: fixup and verbose, 4: force test" );
#define RELIABLE_VERBOSE if ( net_verifyReliableQueue.GetInteger() >= 3 ) libBud::Printf
void idPacketProcessor::VerifyEmptyReliableQueue( byte keepMsgBelowThis, byte replaceWithThisMsg )
{
	...
	{
		libBud::Error( "reliable queue is not empty: %d messages", reliable.Num() );
		return;
	}
	libBud::Warning( "reliable queue is not empty: %d messages", reliable.Num() );
	if( net_verifyReliableQueue.GetInteger() == 0 )
	{
		return;
	}
	// drop some stuff that is potentially dangerous and should not transmit
	budDataQueue< MAX_RELIABLE_QUEUE, MAX_MSG_SIZE > clean;
