...
budCVar net_optimalSnapDeltaSize( "net_optimalSnapDeltaSize", "1000", CVAR_INTEGER, "Optimal size of snapshot delta msgs." );
budCVar net_debugBaseStates( "net_debugBaseStates", "0", CVAR_BOOL, "Log out base state information" );
budCVar net_skipClientDeltaAppend( "net_skipClientDeltaAppend", "0", CVAR_BOOL, "Simulate delta receive buffer overflowing" );

/*
========================
budSnapShotProcessor::budSnapShotProcessor
========================
*/
budSnapShotProcessor::budSnapShotProcessor()
{
...
/*
========================
budSnapShotProcessor::Reset
========================
*/
void budSnapShotProcessor::Reset( bool cstor )
{
	...
	{
		libBud::Printf( "NET: Reset snapshot base" );
	}
...
/*
========================
budSnapShotProcessor::TrySetPendingSnapshot
========================
*/
bool budSnapShotProcessor::TrySetPendingSnapshot( budSnapShot& ss )
{
...
/*
========================
budSnapShotProcessor::PeekDeltaSequence
========================
*/
void budSnapShotProcessor::PeekDeltaSequence( const char* deltaMem, int deltaSize, int& deltaSequence, int& deltaBaseSequence )
{
	budSnapShot::PeekDeltaSequence( deltaMem, deltaSize, deltaSequence, deltaBaseSequence );
}

/*
========================
budSnapShotProcessor::ApplyDeltaToSnapshot
========================
*/
bool budSnapShotProcessor::ApplyDeltaToSnapshot( budSnapShot& snap, const char* deltaMem, int deltaSize, int visIndex )
{
...
/*
========================
budSnapShotProcessor::SubmitPendingSnap
========================
*/
void budSnapShotProcessor::SubmitPendingSnap( int visIndex, uint8* objMemory, int objMemorySize, lzwCompressionData_t* lzwData )
{
	...
	budSnapShot::submitDeltaJobsInfo_t submitInfo;
...
/*
========================
budSnapShotProcessor::GetPendingSnapDelta
========================
*/
int budSnapShotProcessor::GetPendingSnapDelta( byte* outBuffer, int maxLength )
{
	...
	if( !verify( size != -1 ) )
	{
		...
		libBud::Error( "GetPendingSnapDelta: Delta failed." );
	}
	...
	{
		libBud::Error( "GetPendingSnapDelta: Size overflow." );
	}
	...
	//libBud::Printf( "deltas Num: %i, Size: %i\n", deltas.Num(), deltas.GetDataLength() );
	...
	if( deltaBaseSequence >= baseSequence )
	{
		if( !deltas.Append( snapSequence, deltaData, size ) )
		{
			...
			{
				libBud::Error( "GetPendingSnapDelta: Size overflow for resend." );
			}
...
/*
========================
budSnapShotProcessor::IsBusyConfirmingPartialSnap
========================
*/
bool budSnapShotProcessor::IsBusyConfirmingPartialSnap()
{
...
/*
========================
budSnapShotProcessor::ReceiveSnapshotDelta
...
========================
*/
bool budSnapShotProcessor::ReceiveSnapshotDelta( const byte* deltaData, int deltaLength, int visIndex, int& outSeq, int& outBaseSeq, budSnapShot& outSnap, bool& fullSnap )
{
	...
	//libBud::Printf("Incoming snapshot: %i, %i\n", deltaSequence, deltaBaseSequence );
	...
	if( net_skipClientDeltaAppend.GetBool() || !deltas.Append( deltaSequence, deltaData, deltaLength ) )
	{
		...
		libBud::Printf( "NET: ReceiveSnapshotDelta: No room to append delta %d/%d \n", deltaSequence, deltaBaseSequence );
		return false;
	}
	...
	if( baseSequence != deltaBaseSequence )
	{
		...
		if( !failed )
		{
			libBud::Printf( "NET: incorrect base state? not sure how this can happen... baseSequence: %d  deltaBaseSequence: %d \n", baseSequence, deltaBaseSequence );
		}
		failed = true;
		return false;
	}
	...
	{
		libBud::Printf( "NET: Proper basestate...  baseSequence: %d  deltaBaseSequence: %d \n", baseSequence, deltaBaseSequence );
	}
...
/*
========================
budSnapShotProcessor::ApplySnapshotDelta
...
========================
*/
bool budSnapShotProcessor::ApplySnapshotDelta( int visIndex, int snapshotNumber )
{

	NET_VERBOSESNAPSHOT_PRINT_LEVEL( 6, va( "budSnapShotProcessor::ApplySnapshotDelta snapshotNumber: %d\n", snapshotNumber ) );
...
/*
========================
budSnapShotProcessor::RemoveDeltasForOldBaseSequence
...
========================
*/
void budSnapShotProcessor::RemoveDeltasForOldBaseSequence()
{
...
/*
========================
budSnapShotProcessor::SanityCheckDeltas
Make sure delta sequence and basesequence values are valid, and in order, etc
========================
*/
void budSnapShotProcessor::SanityCheckDeltas()
{
	int deltaSequence			= 0;
	int deltaBaseSequence		= 0;
...
/*
========================
budSnapShotProcessor::AddSnapObjTemplate
========================
*/
void budSnapShotProcessor::AddSnapObjTemplate( int objID, budBitMsg& msg )
{
	extern budCVar net_ssTemplateDebug;
	budSnapShot::objectState_t* state = templateStates.S_AddObject( objID, MAX_UNSIGNED_TYPE( uint32 ), msg );
	if( verify( state != NULL ) )
	{
		if( net_ssTemplateDebug.GetBool() )
		{
			libBud::PrintfIf( net_ssTemplateDebug.GetBool(), "InjectingSnapObjBaseState[%d] size: %d\n", objID, state->buffer.Size() );
