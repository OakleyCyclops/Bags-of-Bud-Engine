...
budCVar sgf_threads( "sgf_threads", "2", CVAR_INTEGER, "0 = all foreground, 1 = background write, 2 = background write + compress" );
budCVar sgf_checksums( "sgf_checksums", "1", CVAR_BOOL, "enable save game file checksums" );
budCVar sgf_testCorruption( "sgf_testCorruption", "-1", CVAR_INTEGER, "test corruption at the 128 kB compressed block" );

// this is supposed to get faster going from -15 to -9, but it gets slower as well as worse compression
budCVar sgf_windowBits( "sgf_windowBits", "-15", CVAR_INTEGER, "zlib window bits" );

bool budFile_SaveGamePipelined::cancelToTerminate = false;

class idSGFcompressThread : public budSysThread
{
public:
	virtual int			Run()
	{
	...
	budFile_SaveGamePipelined* sgf;
};
class idSGFdecompressThread : public budSysThread
{
public:
	virtual int			Run()
	{
...
class idSGFwriteThread : public budSysThread
{
public:
	virtual int			Run()
	{
...
class idSGFreadThread : public budSysThread
{
public:
	virtual int			Run()
	{
...
/*
============================
budFile_SaveGamePipelined::budFile_SaveGamePipelined
============================
*/
budFile_SaveGamePipelined::budFile_SaveGamePipelined() :
	mode( CLOSED ),
...
/*
========================
budFile_SaveGamePipelined::ReadBuildVersion
========================
*/
bool budFile_SaveGamePipelined::ReadBuildVersion()
{
...
/*
========================
budFile_SaveGamePipelined::ReadSaveFormatVersion
========================
*/
bool budFile_SaveGamePipelined::ReadSaveFormatVersion()
{
...
/*
========================
budFile_SaveGamePipelined::GetPointerSize
========================
*/
int budFile_SaveGamePipelined::GetPointerSize() const
{
	if( pointerSize == 0 )
	{
...
/*
============================
budFile_SaveGamePipelined::Finish
============================
*/
void budFile_SaveGamePipelined::Finish()
{
	if( mode == WRITE )
	{
...
/*
============================
budFile_SaveGamePipelined::Abort
============================
*/
void budFile_SaveGamePipelined::Abort()
{
	if( mode == WRITE )
	{
...
/*
============================
budFile_SaveGamePipelined::OpenForWriting
============================
*/
bool budFile_SaveGamePipelined::OpenForWriting( const char* const filename, bool useNativeFile )
{
	assert( mode == CLOSED );
	...
	if( status != Z_OK )
	{
		libBud::FatalError( "budFile_SaveGamePipelined::OpenForWriting: deflateInit2() error %i", status );
	}
...
============================
*/
bool budFile_SaveGamePipelined::OpenForWriting( budFile* file )
{
	assert( mode == CLOSED );
...
/*
============================
budFile_SaveGamePipelined::NextWriteBlock
...
============================
*/
bool budFile_SaveGamePipelined::NextWriteBlock( blockForIO_t* block )
{
	assert( mode == WRITE );
...
/*
============================
budFile_SaveGamePipelined::WriteBlock
...
============================
*/
void budFile_SaveGamePipelined::WriteBlock()
{
...
/*
============================
budFile_SaveGamePipelined::FlushCompressedBlock
...
============================
*/
void budFile_SaveGamePipelined::FlushCompressedBlock()
{
...
/*
============================
budFile_SaveGamePipelined::CompressBlock
...
============================
*/
void budFile_SaveGamePipelined::CompressBlock()
{
	...
	while( zStream.avail_in > 0 || zLibFlushType == Z_FINISH )
	{
		...
		{
			libBud::FatalError( "budFile_SaveGamePipelined::CompressBlock: deflate() returned %i", zstat );
		}
...
/*
============================
budFile_SaveGamePipelined::FlushUncompressedBlock
...
============================
*/
void budFile_SaveGamePipelined::FlushUncompressedBlock()
{
...
/*
============================
budFile_SaveGamePipelined::Write
...
============================
*/
int budFile_SaveGamePipelined::Write( const void* buffer, int length )
{
...
/*
============================
budFile_SaveGamePipelined::OpenForReading
============================
*/
bool budFile_SaveGamePipelined::OpenForReading( const char* const filename, bool useNativeFile )
{
	assert( mode == CLOSED );
	...
	if( status != Z_OK )
	{
		libBud::FatalError( "budFile_SaveGamePipelined::OpenForReading: inflateInit2() error %i", status );
	}
...
============================
*/
bool budFile_SaveGamePipelined::OpenForReading( budFile* file )
{
	assert( mode == CLOSED );
...
/*
============================
budFile_SaveGamePipelined::NextReadBlock
...
============================
*/
bool budFile_SaveGamePipelined::NextReadBlock( blockForIO_t* block, size_t lastReadBytes )
{
	assert( mode == READ );
...
/*
============================
budFile_SaveGamePipelined::ReadBlock
...
============================
*/
void budFile_SaveGamePipelined::ReadBlock()
{
	assert( nativeFile != NULL );
...
/*
============================
budFile_SaveGamePipelined::PumpCompressedBlock
...
============================
*/
void budFile_SaveGamePipelined::PumpCompressedBlock()
{
	// block until the background thread is done with the last block
	if( readThread != NULL )
	{
...
/*
============================
budFile_SaveGamePipelined::DecompressBlock
...
============================
*/
void budFile_SaveGamePipelined::DecompressBlock()
{
	if( zStreamEndHit )
	{
		return;
	...
	while( zStream.avail_out > 0 )
	{
		...
		if( zstat != Z_OK )
		{
			libBud::Warning( "budFile_SaveGamePipelined::DecompressBlock: inflate() returned %i", zstat );
			zStreamEndHit = true;
			return;
		}
	}
...
/*
============================
budFile_SaveGamePipelined::PumpUncompressedBlock
...
============================
*/
void budFile_SaveGamePipelined::PumpUncompressedBlock()
{
	if( decompressThread != NULL )
	{
		// make sure thread has completed the last work
...
/*
============================
budFile_SaveGamePipelined::Read
...
============================
*/
int budFile_SaveGamePipelined::Read( void* buffer, int length )
{
	if( buffer == NULL || length <= 0 )
	{
		return 0;
...
static void TestProcessFile( const char* const filename )
{
	...
	libBud::Printf( "%lld microseconds to compress %i bytes to %i written bytes = %4.1f MB/s\n",
	...
	libBud::Printf( "%lld microseconds to decompress = %4.1f MB/s\n", readMicroseconds, ( float )testDataLength / readMicroseconds );
	...
	{
		libBud::Printf( "Compare failed at %i.\n", comparePoint );
		assert( 0 );
	}
	else
	{
		libBud::Printf( "Compare succeeded.\n" );
	}
	Mem_Free( readData );
	Mem_Free( testData );
}
...
CONSOLE_COMMAND( TestSaveGameFile, "Exercises the pipelined savegame code", 0 )
{
	...
	budFileList* fileList = fileSystem->ListFiles( "", "" );
...
//	budFile *f = fileSystem->OpenFileWrite( "junk/lzwTest.bin" );
	...
	libBud::Printf( "%i microseconds to compress %i bytes to %i written bytes = %4.1f MB/s\n",
				   writeMicroseconds, testDataLength, readDataLength, ( float )readDataLength / writeMicroseconds );
