...
public:
	...
	void								Initialize( lobbyType_t sessionType_, class budSessionCallbacks* callbacks );
	void								StartHosting( const budMatchParameters& parms );
	void								StartFinding( const budMatchParameters& parms_ );
	void								Pump();
	void								ProcessSnapAckQueue();
	void								Shutdown( bool retainMigrationInfo = false, bool skipGoodbye = false );						// Goto idle state
	void								HandlePacket( lobbyAddress_t& remoteAddress, budBitMsg fragMsg, idPacketProcessor::sessionId_t sessionID );
...
public:
	...
	struct peer_t
	{
		...
		void Print()
		{
			libBud::Printf( "   lastResourceTime: %d\n", lastResourceTime );
			libBud::Printf( "   lastSnapTime: %d\n", lastSnapTime );
			libBud::Printf( "   lastProcTime: %d\n", lastProcTime );
			libBud::Printf( "   lastInBandProcTime: %d\n", lastInBandProcTime );
			libBud::Printf( "   lastFragmentSendTime: %d\n", lastFragmentSendTime );
			libBud::Printf( "   needToSubmitPendingSnap: %d\n", needToSubmitPendingSnap );
			libBud::Printf( "   lastSnapJobTime: %d\n", lastSnapJobTime );
		}
		...
		budSnapShotProcessor* 	snapProc;				// Processes snapshots for this peer
		budStaticList< budDebugGraph*, 4 >	debugGraphs; //
	...
	virtual const budMatchParameters& 	GetMatchParms() const
	{
		return parms;
	}
	...
	struct reliablePlayerToPlayerHeader_t
	{
		...
		bool Read( idLobby* lobby, budBitMsg& msg );
		bool Write( idLobby* lobby, budBitMsg& msg );
	};
...
public:		// Turning this on for now, for the sake of getting this up and running to see where things are
	...
	virtual void						SendReliable( int type, budBitMsg& msg, bool callReceiveReliable = true, peerMask_t sessionUserMask = MAX_UNSIGNED_TYPE( peerMask_t ) );
	virtual void						SendReliableToLobbyUser( lobbyUserID_t lobbyUserID, int type, budBitMsg& msg );
	virtual void						SendReliableToHost( int type, budBitMsg& msg );
	...
	void								HandleReliablePlayerToPlayerMsg( int peerNum, budBitMsg& msg, int type );
	void								HandleReliablePlayerToPlayerMsg( const reliablePlayerToPlayerHeader_t& info, budBitMsg& msg, int reliableType );
	...
	bool								CheckVersion( budBitMsg& msg, lobbyAddress_t peerAddress );
	bool								VerifyNumConnectingUsers( budBitMsg& msg );
	bool								VerifyLobbyUserIDs( budBitMsg& msg );
	int									HandleInitialPeerConnection( budBitMsg& msg, const lobbyAddress_t& peerAddress, int peerNum );
	...
	void								SendPingValues();
	void								PumpPings();
	void								HandleReliablePing( int p, budBitMsg& msg );
	void								HandlePingReply( int p, const pktPing_t& ping );
	void								HandlePingValues( budBitMsg& msg );
	void								HandleBandwidhTestValue( int p, budBitMsg& msg );
	void								HandleMigrationGameData( budBitMsg& msg );
	void								HandleHeadsetStateChange( int fromPeer, budBitMsg& msg );
	...
	void								UpdateMatchParms( const budMatchParameters& p );
	...
	void								HandleHelloAck( int p, budBitMsg& msg );
	...
	void								HandleReliableMsg( int p, budBitMsg& msg, const lobbyAddress_t* remoteAddress = NULL );
	...
	void								RemoveSessionUsersBybudList( budList< lobbyUserID_t >& usersToRemoveByID );
	void								SendNewUsersToPeers( int skipPeer, int userStart, int numUsers );
	void								SendPeersMicStatusToNewUsers( int peerNumber );
	void								AddUsersFromMsg( budBitMsg& msg, int fromPeer );
	void								UpdateSessionUserOnPeers( budBitMsg& msg );
	void								HandleUpdateSessionUser( budBitMsg& msg );
	void								CreateUserUpdateMessage( int userIndex, budBitMsg& msg );
	void								UpdateLocalSessionUsers();
	int									PeerIndexForSessionUserIndex( int sessionUserIndex ) const;
	void								HandleUserConnectFailure( int p, budBitMsg& inMsg, int reliableType );
	void								ProcessUserDisconnectMsg( budBitMsg& msg );
	...
	bool								GetMigrationGameData( budBitMsg& msg, bool reading );
	bool								GetMigrationGameDataUser( lobbyUserID_t lobbyUserID, budBitMsg& msg, bool reading );
	...
	void								SendSnapshotToPeer( budSnapShot& ss, int p );
	...
	virtual void						AddSnapObjTemplate( int objID, budBitMsg& msg );
	...
	{
		budArray<short, MAX_PEERS> pings;
	};
	...
	struct migrationInfo_t
	{
		...
		budStaticList< migrationInvite_t, MAX_PEERS > invites;
	...
	budList< lobbyConnectInfo_t >		searchResults;
	
	typedef budStaticList< lobbyUser_t*, MAX_PLAYERS >		idLobbyUserList;
	typedef budStaticList< lobbyUser_t, MAX_PLAYERS >		idLobbyUserPool;
	...
	budList< disconnectedUser_t>			disconnectedUsers;				// List of users which were connected, but aren't anymore, for printing their name on the hud
	
	budStaticList< peer_t, MAX_PEERS >	peers;							// Unique machines connected to this lobby
	
	uint32								partyToken;
	
	budMatchParameters					parms;
	...
	budStaticList< snapDeltaAck_t, 16 >	snapDeltaAckQueue;
};
...
public:
	...
	virtual budSession::sessionState_t GetState() const = 0;
	...
	virtual void					RecvLeaderboardStats( budBitMsg& msg ) = 0;
	...
	virtual void					HandleOobVoiceAudio( const lobbyAddress_t& from, const budBitMsg& msg ) = 0;
	...
	virtual void					HandleServerQueryRequest( lobbyAddress_t& remoteAddr, budBitMsg& msg, int msgType ) = 0;
	virtual void 					HandleServerQueryAck( lobbyAddress_t& remoteAddr, budBitMsg& msg ) = 0;
	
	virtual void					HandlePeerMatchParamUpdate( int peer, int msg ) = 0;
	
	virtual idLobbyBackend* 		CreateLobbyBackend( const budMatchParameters& p, float skillLevel, idLobbyBackend::lobbyBackendType_t lobbyType ) = 0;
	virtual idLobbyBackend* 		FindLobbyBackend( const budMatchParameters& p, int numPartyUsers, float skillLevel, idLobbyBackend::lobbyBackendType_t lobbyType ) = 0;
