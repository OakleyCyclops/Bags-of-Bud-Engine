...
BUD_INLINE bool MatchTypeIsOnline( uint8 matchFlags )
{
	return ( matchFlags & MATCH_ONLINE ) ? true : false;
}
BUD_INLINE bool MatchTypeIsLocal( uint8 matchFlags )
{
	return !MatchTypeIsOnline( matchFlags );
}
BUD_INLINE bool MatchTypeIsPrivate( uint8 matchFlags )
{
	return ( matchFlags & MATCH_PRIVATE ) ? true : false;
}
BUD_INLINE bool MatchTypeIsRanked( uint8 matchFlags )
{
	return ( matchFlags & MATCH_RANKED ) ? true : false;
}
BUD_INLINE bool MatchTypeHasStats( uint8 matchFlags )
{
	return ( matchFlags & MATCH_STATS ) ? true : false;
}
BUD_INLINE bool MatchTypeInviteOnly( uint8 matchFlags )
{
	return ( matchFlags & MATCH_INVITE_ONLY ) ? true : false;
}
BUD_INLINE bool MatchTypeIsSearchable( uint8 matchFlags )
{
	return !MatchTypeIsPrivate( matchFlags );
}
BUD_INLINE bool MatchTypeIsJoinInProgress( uint8 matchFlags )
{
...
public:
	...
	void Write( budBitMsg& msg )
	{
		budSerializer s( msg, true );
		Serialize( s );
	}
	void Read( budBitMsg& msg )
	{
		budSerializer s( msg, false );
		Serialize( s );
	}
	
	void Serialize( budSerializer& serializer )
	{
	...
	budStr	mapName; // This is only used for SP (gameMap == GAME_MAP_SINGLEPLAYER)
	idDict	serverInfo;
};
...
public:
	...
	void WriteToMsg( budBitMsg& msg )
	{
		localUserHandle.WriteToMsg( msg );
		msg.WriteByte( lobbyType );
	}
	
	void ReadFromMsg( const budBitMsg& msg )
	{
...
public:
	...
	virtual void						SendReliable( int type, budBitMsg& msg, bool callReceiveReliable = true, peerMask_t sessionUserMask = MAX_UNSIGNED_TYPE( peerMask_t ) ) = 0;
	virtual void						SendReliableToLobbyUser( lobbyUserID_t lobbyUserID, int type, budBitMsg& msg ) = 0;
	virtual void						SendReliableToHost( int type, budBitMsg& msg ) = 0;
	...
	virtual const budMatchParameters& 	GetMatchParms() const = 0;
	...
	virtual void						AddSnapObjTemplate( int objID, budBitMsg& msg ) = 0;
...
public:
	...
	// Lobby management
	//=====================================================================================================
	virtual void			CreatePartyLobby( const budMatchParameters& parms_ ) = 0;
	virtual void			FindOrCreateMatch( const budMatchParameters& parms_ ) = 0;
	virtual void			CreateMatch( const budMatchParameters& parms_ ) = 0;
	virtual void			CreateGameStateLobby( const budMatchParameters& parms_ ) = 0;
	virtual void			UpdateMatchParms( const budMatchParameters& parms_ ) = 0;
	virtual void			UpdatePartyParms( const budMatchParameters& parms_ ) = 0;
	...
	virtual bool			GetMigrationGameData( budBitMsg& msg, bool reading ) = 0;
	virtual bool			GetMigrationGameDataUser( lobbyUserID_t lobbyUserID, budBitMsg& msg, bool reading ) = 0;
	...
	virtual const budList< budStr >* ServerPlayerList( int i ) = 0;
	...
	// These are on session and not budGame so it can persist across game deallocations
	...
	virtual void				SendUsercmds( budBitMsg& msg ) = 0;
	virtual void				SendSnapshot( class budSnapShot& ss ) = 0;
	...
	budAchievementSystem& 		GetAchievementSystem()
	{
...
protected:
	...
	budAchievementSystem* 		achievementSystem;		// pointer so we can treat dynamically bind platform-specific impl
	idDedicatedServerSearch* 	dedicatedServerSearch;
};

/*
========================
budSession::idGetInputRouting
========================
*/
BUD_INLINE int budSession::GetInputRouting( int inputRouting[ MAX_INPUT_DEVICES ] )
{
