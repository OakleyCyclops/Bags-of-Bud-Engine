...
public:
	void		Serialize( class budSerializer& s, const usercmd_t& base );
...
extern budUsercmdGen*	usercmdGen;
...
public:
	...
	budArray< int, MAX_PLAYERS >			writeFrame;	//"where we write to next"
	budArray< int, MAX_PLAYERS >			readFrame;	//"the last frame we read"
	...
	void PutUserCmdForPlayer( int playerIndex, const usercmd_t& cmd )
	{
		...
		if( writeFrame[ playerIndex ] - readFrame[ playerIndex ] + 1 > USERCMD_BUFFER_SIZE )
		{
			...
			libBud::Printf( "PutUserCmdForPlayer: buffer overflow.\n" );
		}
