...
void idVoiceChatMgr::RegisterTalker( lobbyUser_t* user, int lobbyType, bool isLocal )
{
	...
	if( !verify( i == -1 ) )
	{
		...
		libBud::Printf( "RegisterTalker: Talker already registered.\n" );
		return;
	}
...
void idVoiceChatMgr::UnregisterTalker( lobbyUser_t* user, int lobbyType, bool isLocal )
{
	...
	if( !verify( i != -1 ) )
	{
		libBud::Printf( "UnregisterTalker: Talker not found.\n" );
		return;
	}
...
================================================
*/
void idVoiceChatMgr::GetActiveLocalTalkers( budStaticList< int, MAX_PLAYERS >& localTalkers )
{
...
================================================
*/
void idVoiceChatMgr::GetRecipientsForTalker( int talkerIndex, budStaticList< const lobbyAddress_t*, MAX_PLAYERS >& recipients )
{
...
void idVoiceChatMgr::SetTalkerGroup( const lobbyUser_t* user, int lobbyType, int groupIndex )
{
	...
	if( !verify( i != -1 ) )
	{
		libBud::Printf( "SetTalkerGroup: Talker not found.\n" );
		return;
	}
...
bool idVoiceChatMgr::GetLocalChatData( int talkerIndex, byte* data, int& dataSize )
{
	...
	if( !talker.IsLocal() )
	{
		libBud::Printf( "GetLocalChatData: Talker not local.\n" );
...
void idVoiceChatMgr::SubmitIncomingChatData( const byte* data, int dataSize )
{
	...
	if( i == -1 )
	{
		libBud::Printf( "SubmitIncomingChatData: Talker not found in session.\n" );
