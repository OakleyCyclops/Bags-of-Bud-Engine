...
extern budCVar net_verboseResource;
#define NET_VERBOSERESOURCE_PRINT if ( net_verboseResource.GetBool() ) libBud::Printf

extern budCVar net_verbose;
#define NET_VERBOSE_PRINT if ( net_verbose.GetBool() ) libBud::Printf
...
public:
	...
	void WriteToMsg( budBitMsg& msg ) const;
	void ReadFromMsg( budBitMsg& msg );
...
struct lobbyUser_t
{
	...
	void WriteClientMutableData( budBitMsg& msg )
	{
	...
	void ReadClientMutableData( budBitMsg& msg )
	{
...
public:
	...
	virtual void			StartHosting( const budMatchParameters& p, float skillLevel, lobbyBackendType_t type ) = 0;
	virtual void			StartFinding( const budMatchParameters& p, int numPartyUsers, float skillLevel ) = 0;
	virtual void			JoinFromConnectInfo( const lobbyConnectInfo_t& connectInfo ) = 0;
	virtual void			GetSearchResults( budList< lobbyConnectInfo_t >& searchResults ) = 0;
	virtual lobbyConnectInfo_t GetConnectInfo()	= 0;
	virtual void			FillMsgWithPostConnectInfo( budBitMsg& msg ) = 0;				// Passed itno PostConnectFromMsg
	virtual void			PostConnectFromMsg( budBitMsg& msg ) = 0;						// Uses results from FillMsgWithPostConnectInfo
	...
	virtual void			Pump() = 0;
	virtual void			UpdateMatchParms( const budMatchParameters& p ) = 0;
