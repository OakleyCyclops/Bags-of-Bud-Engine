...
void budTokenParser::LoadFromParser( budParser& parser, const char* guiName )
{
	budToken tok;
	budTokenIndexes tokIdxs;
	tokIdxs.SetName( guiName );
	while( parser.ReadToken( &tok ) )
	{
		tokIdxs.Append( tokens.AddUnique( budBinaryToken( tok ) ) );
	}
	guiTokenIndexes.Append( tokIdxs );
	currentToken = 0;
}

void budTokenParser::LoadFromFile( const char* filename )
{
	Clear();
...
void budTokenParser::WriteToFile( const char* filename )
{
	if( preloaded )
	{
		return;
	}
	budFile* outFile = fileSystem->OpenFileWrite( filename, "fs_basepath" );
	if( outFile != NULL )
	{
...
bool budTokenParser::StartParsing( const char* filename )
{
	currentTokenList = -1;
	for( int i = 0; i < guiTokenIndexes.Num(); i++ )
	{
		if( budStr::Icmp( filename, guiTokenIndexes[ i ].GetName() ) == 0 )
		{
			currentTokenList = i;
			break;
		}
...
bool budTokenParser::ReadToken( budToken* tok )
{
	if( currentToken >= 0 && currentToken < guiTokenIndexes[ currentTokenList ].Num() )
	{
		tok->Clear();
		budBinaryToken& btok = tokens[ guiTokenIndexes[ currentTokenList ][ currentToken ] ];
		*tok = btok.token;
...
int	budTokenParser::ExpectTokenString( const char* string )
{
	budToken token;
	if( !ReadToken( &token ) )
	{
...
int	budTokenParser::ExpectTokenType( int type, int subtype, budToken* token )
{
	budStr str;
	...
	else if( token->type == TT_PUNCTUATION )
	{
		...
		{
			//Error( "expected '%s' but found '%s'", budLexer::GetPunctuationFromId( subtype ), token->c_str() );
			return 0;
		}
	}
	return 1;
...
// expect a token
int budTokenParser::ExpectAnyToken( budToken* token )
{
	if( !ReadToken( token ) )
	{
		Error( "couldn't read expected token" );
...
void budTokenParser::UnreadToken( const budToken* token )
{
	if( currentToken == 0 || currentToken >=  guiTokenIndexes[ currentTokenList ].Num() )
	{
		libBud::common->FatalError( "budTokenParser::unreadToken, unread token twice\n" );
	}
	currentToken--;
}
void budTokenParser::Error( VERIFY_FORMAT_STRING const char* str, ... )
{
	...
	libBud::common->Warning( text );
}
void budTokenParser::Warning( VERIFY_FORMAT_STRING const char* str, ... )
{
	char text[MAX_STRING_CHARS];
	va_list ap;
...
float budTokenParser::ParseFloat( bool* errorFlag )
{
	budToken token;
	if( errorFlag )
	{
