...
BUD_INLINE int MSEC_TO_FRAME_CEIL( int msec )
{
...
BUD_INLINE int MSEC_ALIGN_TO_FRAME( int msec )
{
...
BUD_INLINE void BeginProfileNamedEventColor( uint32 color, VERIFY_FORMAT_STRING const char* szName )
{
}
BUD_INLINE void EndProfileNamedEvent()
{
}

BUD_INLINE void BeginProfileNamedEvent( VERIFY_FORMAT_STRING const char* szName )
{
	BeginProfileNamedEventColor( ( uint32 ) 0xFF00FF00, szName );
}

class budScopedProfileEvent
{
public:
	budScopedProfileEvent( const char* name )
	{
		BeginProfileNamedEvent( name );
	}
	~budScopedProfileEvent()
	{
		EndProfileNamedEvent();
	}
};

#define SCOPED_PROFILE_EVENT( x ) budScopedProfileEvent scopedProfileEvent_##__LINE__( x )

BUD_INLINE bool BeginTraceRecording( const char* szName )
{
	return false;
}

BUD_INLINE bool EndTraceRecording()
{
	return false;
}
...
extern budCVar		com_version;
extern budCVar		com_developer;
extern budCVar		com_allowConsole;
extern budCVar		com_speeds;
extern budCVar		com_showFPS;
extern budCVar		com_showMemoryUsage;
extern budCVar		com_updateLoadSize;
extern budCVar		com_productionMode;
