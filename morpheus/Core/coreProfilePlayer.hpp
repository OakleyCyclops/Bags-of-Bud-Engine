...
protected:
	budPlayerProfile(); // don't instantiate. we static_cast the child all over the place
public:

	virtual			~budPlayerProfile();
	
	static budPlayerProfile* CreatePlayerProfile( int deviceIndex );
	
	void			SetDefaults();
	bool			Serialize( budSerializer& ser );
...
protected:
	...
	budStaticList< profileStatValue_t, MAX_PLAYER_PROFILE_STATS > stats;
};
