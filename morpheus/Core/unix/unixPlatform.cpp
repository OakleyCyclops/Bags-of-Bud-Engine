...
double Sys_ClockTicksPerSecond()
{
	...
	while( pos < len )
	{
		if( !budStr::Cmpn( buf + pos, "cpu MHz", 7 ) )
		{
...
void Sys_CPUCount( int& numLogicalCPUCores, int& numPhysicalCPUCores, int& numCPUPackages )
{
	...
	if( fd != -1 )
	{
		...
		while( pos < len )
		{
			if( !budStr::Cmpn( buf + pos, "cpu cores", 9 ) )
			{
				pos = strchr( buf + pos, ':' ) - buf + 2;
				end = strchr( buf + pos, '\n' ) - buf;
				if( pos < len && end < len )
				{
					budStr::Copynz( number, buf + pos, sizeof( number ) );
			...
			else if( !budStr::Cmpn( buf + pos, "siblings", 8 ) )
			{
				pos = strchr( buf + pos, ':' ) - buf + 2;
				end = strchr( buf + pos, '\n' ) - buf;
				if( pos < len && end < len )
...
void Sys_ReLaunch()
{
	...
	int ret = fork();
	if( ret < 0 )
		libBud::Error( "Sys_ReLaunch(): Couldn't fork(), reason: %s ", strerror( errno ) );
	...
	if( ret == 0 )
	{
		...
		if( sId == ( pid_t ) - 1 )
		{
			libBud::Error( "Sys_ReLaunch(): setsid() failed! Reason: %s ", strerror( errno ) );
		}
		...
		{
			libBud::Warning( "Sys_ReLaunch(): Couldn't open /dev/fd/ - will leak file descriptors. Reason: %s", strerror( errno ) );
		}
		...
		libBud::Error( "Sys_ReLaunch(): WTF exec() failed! Reason: %s ", strerror( errno ) );
