...
lobbyUser_t* idLobby::AllocUser( const lobbyUser_t& defaults )
{
	...
	{
		libBud::Error( "Out of session users" );		// This shouldn't be possible
	}
...
lobbyUser_t idLobby::CreateLobbyUserFromLocalUser( const idLocalUser* localUser )
{
	...
	budStr::Copynz( lobbyUser.gamertag, localUser->GetGamerTag(), sizeof( lobbyUser.gamertag ) );
...
NOTE - This should only be called from the host.  The host will call RemoveSessionUsersBybudList,
...
void idLobby::RemoveUsersWithDisconnectedPeers()
{
	if( !verify( IsHost() ) )
	{
		...
		// If we are the host, RemoveSessionUsersBybudList will forward the call to peers.
		return;
	}
	
	budList< lobbyUserID_t > removeList;
	for( int u = 0; u < GetNumLobbyUsers(); u++ )
	{
		...
		if( user->peerIndex >= peers.Num() )
		{
			...
			libBud::Warning( "idLobby::RemoveUsersWithDisconnectedPeers: user %d %s is out of range in the peers list (%d elements)", u, user->gamertag, peers.Num() );
			continue;
		}
	...
	RemoveSessionUsersBybudList( removeList );
}

/*
========================
idLobby::RemoveSessionUsersBybudList
...
========================
*/
void idLobby::RemoveSessionUsersBybudList( budList< lobbyUserID_t >& usersToRemoveByID )
{
	...
	for( int i = 0; i < usersToRemoveByID.Num(); i++ )
	{
		for( int u = 0; u < GetNumLobbyUsers(); u++ )
		{
			...
			if( user->lobbyUserID == usersToRemoveByID[i] )
			{
				...
				{
					libBud::Printf( "NET: %s left the game.\n", user->gamertag );
				}
				else if( lobbyType == TYPE_PARTY )
				{
					libBud::Printf( "NET: %s left the party.\n", user->gamertag );
				}
...
void idLobby::SendPeersMicStatusToNewUsers( int peerNumber )
{
	...
	for( int i = 0; i < GetNumLobbyUsers(); ++i )
	{
		...
		libBud::Printf( "Packing headset state %d for user %d %s\n", state, i, user->gamertag );
		user->lobbyUserID.WriteToMsg( outmsg );
		outmsg.WriteBool( state );
	}
	
	
	libBud::Printf( "Sending headset states to new peer %d\n", peerNumber );
...
lobbyUserID_t idLobby::AllocLobbyUserSlotForBot( const char* botName )
{
	...
	budStr::Copynz( botSessionUser.gamertag, botName, sizeof( botSessionUser.gamertag ) );
	...
	if( botUser == NULL )
	{
		if( freeUsers.Num() == 0 )
		{
			libBud::Warning( "NET: Out Of Session Users - Can't Add Bot %s!", botName );
	...
	{
		libBud::Warning( "NET: Can't Find Session Slot For Bot!" );
		return lobbyUserID_t();
	}
	else
	{
...
void idLobby::RemoveBotFromLobbyUserList( lobbyUserID_t lobbyUserID )
{
	...
	{
		assert( false );
		libBud::Warning( "RemoveBotFromLobbyUserList: Invalid User Index!" );
		return;
	}
	
	if( !botUser->isBot )
	{
		libBud::Warning( "RemoveBotFromLobbyUserList: User Index Is Not A Bot!" ); // don't accidentally disconnect a human player.
		return;
	}
...
========================
*/
void idLobby::AddUsersFromMsg( budBitMsg& msg, int fromPeer )
{
	...
	for( int userIndex = 0; userIndex < GetNumLobbyUsers(); ++userIndex )
	{
		...
		libBud::Printf( "party %08x user %s\n", user->partyToken, user->gamertag );
	}
	libBud::Printf( "---------------- %s --------------------\n", GetLobbyName() );
}
...
========================
*/
void idLobby::UpdateSessionUserOnPeers( budBitMsg& msg )
{
	for( int p = 0; p < peers.Num(); p++ )
	{
...
========================
*/
void idLobby::HandleHeadsetStateChange( int fromPeer, budBitMsg& msg )
{
	...
	for( int i = 0; i < userCount; ++i )
	{
		...
		libBud::Printf( "User %d headset status: %d\n", talkerIndex, state );
...
========================
*/
void idLobby::HandleUpdateSessionUser( budBitMsg& msg )
{
...
========================
*/
void idLobby::CreateUserUpdateMessage( int userIndex, budBitMsg& msg )
{
	lobbyUser_t* user = GetLobbyUser( userIndex );
...
========================
*/
void idLobby::HandleUserConnectFailure( int p, budBitMsg& inMsg, int reliableType )
{
...
========================
*/
void idLobby::ProcessUserDisconnectMsg( budBitMsg& msg )
{
...
bool idLobby::LobbyUserHasFirstFullSnap( lobbyUserID_t lobbyUserID ) const
{
	...
	if( peers[user->peerIndex].snapProc->GetFullSnapBaseSequence() < budSnapShotProcessor::INITIAL_SNAP_SEQUENCE )
	{
		return false;
	}
...
void idLobby::QueueReliablePlayerToPlayerMessage( int fromSessionUserIndex, int toSessionUserIndex, reliablePlayerToPlayer_t type, const byte* data, int dataLen )
{
	...
	{
		libBud::Warning( "NET: Can't queue invalid reliable player to player msg" );
		return;
	}
