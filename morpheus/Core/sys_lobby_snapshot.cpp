...
budCVar net_snapshot_send_warntime( "net_snapshot_send_warntime", "500", CVAR_INTEGER, "Print warning messages if we take longer than this to send a client a snapshot." );

budCVar net_queueSnapAcks( "net_queueSnapAcks", "1", CVAR_BOOL, "" );

budCVar net_peer_throttle_mode( "net_peer_throttle_mode", "0", CVAR_INTEGER, "= 0 off, 1 = enable fixed, 2 = absolute, 3 = both" );

budCVar net_peer_throttle_minSnapSeq( "net_peer_throttle_minSnapSeq", "150", CVAR_INTEGER, "Minumum number of snapshot exchanges before throttling can be triggered" );

budCVar net_peer_throttle_bps_peer_threshold_pct( "net_peer_throttle_bps_peer_threshold_pct", "0.60", CVAR_FLOAT, "Min reported incoming bps % of sent from host that a peer must maintain before throttling kicks in" );
budCVar net_peer_throttle_bps_host_threshold( "net_peer_throttle_bps_host_threshold", "1024", CVAR_FLOAT, "Min outgoing bps of host for bps based throttling to be considered" );

budCVar net_peer_throttle_bps_decay( "net_peer_throttle_bps_decay", "0.25f", CVAR_FLOAT, "If peer exceeds this number of queued snap deltas, then throttle his effective snap rate" );
budCVar net_peer_throttle_bps_duration( "net_peer_throttle_bps_duration", "3000", CVAR_INTEGER, "If peer exceeds this number of queued snap deltas, then throttle his effective snap rate" );

budCVar net_peer_throttle_maxSnapRate( "net_peer_throttle_maxSnapRate", "4", CVAR_INTEGER, "Highest factor of server base snapRate that a client can be throttled" );

budCVar net_snap_bw_test_throttle_max_scale( "net_snap_bw_test_throttle_max_scale", "0.80", CVAR_FLOAT, "When clamping bandwidth to reported values, scale reported value by this" );

budCVar net_snap_redundant_resend_in_ms( "net_snap_redundant_resend_in_ms", "800", CVAR_INTEGER, "Delay between redundantly sending snaps during initial snap exchange" );
budCVar net_min_ping_in_ms( "net_min_ping_in_ms", "1500", CVAR_INTEGER, "Ping has to be higher than this before we consider throttling to recover" );
budCVar net_pingIncPercentBeforeRecover( "net_pingIncPercentBeforeRecover", "1.3", CVAR_FLOAT, "Percentage change increase of ping before we try to recover" );
budCVar net_maxFailedPingRecoveries( "net_maxFailedPingRecoveries", "10", CVAR_INTEGER, "Max failed ping recoveries before we stop trying" );
budCVar net_pingRecoveryThrottleTimeInSeconds( "net_pingRecoveryThrottleTimeInSeconds", "3", CVAR_INTEGER, "Throttle snaps for this amount of time in seconds to recover from ping spike" );

budCVar net_peer_timeout_loading( "net_peer_timeout_loading", "90000", CVAR_INTEGER, "time in MS to disconnect clients during loading - production only" );
...
void idLobby::SendCompletedPendingSnap( int p )
{
	...
	{
		libBud::Printf( "NET: Took %d ms to send peer %d snapshot\n", timeFromLastSend, p );
	}
	...
	if( peer.throttleSnapsForXSeconds != 0 )
	{
		...
		if( peer.recoverPing != 0 )
		{
			...
			else
			{
				...
				if( peer.snapProc->GetFullSnapBaseSequence() > budSnapShotProcessor::INITIAL_SNAP_SEQUENCE + peer_throttle_minSnapSeq )
				{
					...
					peer.throttledSnapRate = budMath::ClampInt( common->GetSnapRate(), maxRate, peer.throttledSnapRate + common->GetSnapRate() );
				}
			}
		}
...
void idLobby::CheckPeerThrottle( int p )
{
	...
	{
		int deltaT = budMath::ClampInt( 0, 100, time - peer.receivedThrottleTime );
		if( deltaT > 0 && peer.receivedThrottleTime > 0 && peer.receivedBpsIndex > 0 )
		{
			...
			{
				float pct = peer.receivedBps / budMath::ClampFloat( 0.01f, static_cast<float>( BANDWIDTH_REPORTING_MAX ), sentBps ); // note the receivedBps is implicitly clamped on client end to 10k/sec
...
========================
*/
budCVar net_forceDropSnap( "net_forceDropSnap", "0", CVAR_BOOL, "wait on snaps" );
void idLobby::SendSnapshotToPeer( budSnapShot& ss, int p )
{
	assert( lobbyType == GetActingGameStateLobbyType() );
	...
	if( peer.snapProc->TrySetPendingSnapshot( ss ) )
	{
		...
		budSnapShot* baseState = peers[p].snapProc->GetBaseState();
...
bool idLobby::AllPeersHaveBaseState()
{
	...
	for( int i = 0; i < peers.Num(); ++i )
	{
		...
		if( peers[i].snapProc->GetFullSnapBaseSequence() < budSnapShotProcessor::INITIAL_SNAP_SEQUENCE )
		{
...
void idLobby::ThrottleSnapsForXSeconds( int p, int seconds, bool recoverPing )
{
	...
	libBud::Printf( "Throttling peer %i for %i seconds...\n", p, seconds );
...
bool idLobby::FirstSnapHasBeenSent( int p )
{
	...
	budSnapShot* ss = peer.snapProc->GetPendingSnap();
...
bool idLobby::AllPeersHaveStaleSnapObj( int objId )
{
	...
	for( int i = 0; i < peers.Num(); i++ )
	{
		...
		budSnapShot* baseState = peers[i].snapProc->GetBaseState();
		
		budSnapShot::objectState_t* state = baseState->FindObjectByID( objId );
...
========================
*/
void idLobby::AddSnapObjTemplate( int objID, budBitMsg& msg )
{
	assert( lobbyType == GetActingGameStateLobbyType() );
