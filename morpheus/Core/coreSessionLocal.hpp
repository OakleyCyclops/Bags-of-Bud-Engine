...
protected:
	...
	{
		bool						isMounted;
		budStrStatic<128>			displayName;
		budStrStatic< MAX_OSPATH >	packageFileName;
		budStrStatic< MAX_OSPATH >	rootPath;
		int							dlcID;
	};
	
public:
	budSessionLocal();
	virtual					~budSessionLocal();
	...
	// Lobby management
	virtual void			CreatePartyLobby( const budMatchParameters& parms_ );
	virtual void			FindOrCreateMatch( const budMatchParameters& parms );
	virtual void			CreateMatch( const budMatchParameters& parms_ );
	virtual void			CreateGameStateLobby( const budMatchParameters& parms_ );
	
	virtual void			UpdatePartyParms( const budMatchParameters& parms_ );
	virtual void			UpdateMatchParms( const budMatchParameters& parms_ );
	...
	virtual bool			GetMigrationGameData( budBitMsg& msg, bool reading );
	virtual bool			GetMigrationGameDataUser( lobbyUserID_t lobbyUserID, budBitMsg& msg, bool reading );
	...
	virtual void			SendUsercmds( budBitMsg& msg );
	virtual void			SendSnapshot( budSnapShot& ss );
	...
	virtual const budList< budStr >* ServerPlayerList( int i );
	...
	// Leaderboards
	virtual void			LeaderboardUpload( lobbyUserID_t lobbyUserID, const leaderboardDefinition_t* leaderboard, const column_t* stats, const budFile_Memory* attachment = NULL ) = 0;
...
protected:
	...
	budStaticList<contentData_t, MAX_CONTENT_PACKAGES>	downloadedContent;
	...
	void	WriteLeaderboardToMsg( budBitMsg& msg, const leaderboardDefinition_t* leaderboard, const column_t* stats );
	...
	void	RecvLeaderboardStatsForPlayer( budBitMsg& msg );
	
	const leaderboardDefinition_t* ReadLeaderboardFromMsg( budBitMsg& msg, column_t* stats );
	...
	void 	HandleDedicatedServerQueryRequest( lobbyAddress_t& remoteAddr, budBitMsg& msg, int msgType );
	void 	HandleDedicatedServerQueryAck( lobbyAddress_t& remoteAddr, budBitMsg& msg );
	...
	// Voice chat
	void	SendVoiceAudio();
	void	HandleOobVoiceAudio( const lobbyAddress_t& from, const budBitMsg& msg );
	...
	budSessionCallbacks*	 sessionCallbacks;
...
========================
*/
class budSessionLocalCallbacks : public budSessionCallbacks
{
public:
	budSessionLocalCallbacks( budSessionLocal* sessionLocal_ )
	{
	...
	virtual budSession::sessionState_t GetState() const
	{
	...
	virtual void					RecvLeaderboardStats( budBitMsg& msg );
	...
	virtual void					HandleOobVoiceAudio( const lobbyAddress_t& from, const budBitMsg& msg )
	{
