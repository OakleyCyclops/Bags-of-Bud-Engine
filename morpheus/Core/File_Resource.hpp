...
public:
	...
	static void WriteResourceFile( const char* fileName, const budStrList& manifest, const bool& _writeManifest );
	static void WriteManifestFile( const char* name, const budStrList& list );
	static int ReadManifestFile( const char* filename, budStrList& list );
	static void ExtractResourceFile( const char* fileName, const char* outPath, bool copyWavs );
	static void UpdateResourceFile( const char* filename, const budStrList& filesToAdd );
	budFile* OpenFile( const char* fileName );
...
private:
	budStrStatic< 256 > fileName;
	budFile* 	resourceFile;			// open file handle
	...
	budList< idResourceCacheEntry, TAG_RESOURCE>	cacheTable;
