...
void budSysLocal::DebugPrintf( const char* fmt, ... )
{
	va_list argptr;
...
void budSysLocal::DebugVPrintf( const char* fmt, va_list arg )
{
	Sys_DebugVPrintf( fmt, arg );
}

double budSysLocal::GetClockTicks()
{
	return Sys_GetClockTicks();
}

double budSysLocal::ClockTicksPerSecond()
{
	return Sys_ClockTicksPerSecond();
}

cpuid_t budSysLocal::GetProcessorId()
{
	return Sys_GetProcessorId();
}

const char* budSysLocal::GetProcessorString()
{
	return Sys_GetProcessorString();
}

const char* budSysLocal::FPU_GetState()
{
	return Sys_FPU_GetState();
}

bool budSysLocal::FPU_StackIsEmpty()
{
	return Sys_FPU_StackIsEmpty();
}

void budSysLocal::FPU_SetFTZ( bool enable )
{
	Sys_FPU_SetFTZ( enable );
}

void budSysLocal::FPU_SetDAZ( bool enable )
{
	Sys_FPU_SetDAZ( enable );
}

bool budSysLocal::LockMemory( void* ptr, int bytes )
{
	return Sys_LockMemory( ptr, bytes );
}

bool budSysLocal::UnlockMemory( void* ptr, int bytes )
{
	return Sys_UnlockMemory( ptr, bytes );
}

int budSysLocal::DLL_Load( const char* dllName )
{
	return Sys_DLL_Load( dllName );
}

void* budSysLocal::DLL_GetProcAddress( int dllHandle, const char* procName )
{
	return Sys_DLL_GetProcAddress( dllHandle, procName );
}

void budSysLocal::DLL_Unload( int dllHandle )
{
	Sys_DLL_Unload( dllHandle );
}

void budSysLocal::DLL_GetFileName( const char* baseName, char* dllName, int maxLength )
{
	budStr::snPrintf( dllName, maxLength, "%s" CPUSTRING ".dll", baseName );
}

sysEvent_t budSysLocal::GenerateMouseButtonEvent( int button, bool down )
{
	sysEvent_t ev;
	ev.evType = SE_KEY;
...
sysEvent_t budSysLocal::GenerateMouseMoveEvent( int deltax, int deltay )
{
	sysEvent_t ev;
	ev.evType = SE_MOUSE;
	ev.evValue = deltax;
...
void budSysLocal::FPU_EnableExceptions( int exceptions )
{
...
const char* Sys_TimeStampToStr( ID_TIME_T timeStamp )
{
	...
	budStr lang = cvarSystem->GetCVarString( "sys_lang" );
	...
	budStr::Copynz( timeString, out, sizeof( timeString ) );
