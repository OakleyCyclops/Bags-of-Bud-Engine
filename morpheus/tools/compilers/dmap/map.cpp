...
===========
*/
int FindFloatPlane( const budPlane& plane, bool* fixedDegeneracies )
{
	budPlane p = plane;
...
================
*/
static void ParseSurface( const idMapPatch* patch, const idSurface* surface, const budMaterial* material )
{
	int				i;
	mapTri_t*		tri;
	primitive_t*		prim;
...
static void CreateMapLight( const idMapEntity* mapEnt )
{
	...
	// RB begin
	budRenderMatrix::GetFrustumPlanes( light->frustumPlanes, light->def.baseLightProject, true, true );
...
==============
*/
static void CreateMapLights( const budMapFile* dmapFile )
{
	int		i;
...
bool LoadDMapFile( const char* filename )
{
	...
	dmapGlobals.dmapFile = new budMapFile();
