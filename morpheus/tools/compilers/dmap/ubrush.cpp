...
==================
*/
int BrushMostlyOnSide( uBrush_t* brush, budPlane& plane )
{
	int			i, j;
	idWinding*	w;
	float		d, max;
...
void SplitBrush( uBrush_t* brush, int planenum, uBrush_t** front, uBrush_t** back )
{
	...
	*front = *back = NULL;
	budPlane& plane = dmapGlobals.mapPlanes[planenum];
	...
	{
		budPlane& plane2 = dmapGlobals.mapPlanes[brush->sides[i].planenum ^ 1];
