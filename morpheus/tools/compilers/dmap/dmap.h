...
typedef struct bspbrush_s
{
	...
	const budMaterial* 	contentShader;	// one face's shader will determine the volume attributes
...
typedef struct node_s
{
	...
	budBounds			bounds;		// valid after portalization
...
typedef struct
{
	...
	budPlane				frustumPlanes[6];		// RB: should be calculated after R_DeriveLightData()
} mapLight_t;
...
typedef struct optimizeGroup_s
{
	...
	budBounds			bounds;			// set in CarveGroupsByLight
...
int FindFloatPlane( const budPlane& plane, bool* fixedDegeneracies = NULL );
...
uBrush_t*	BrushFromBounds( const budBounds& bounds );
...
void OutputWinding( idWinding* w, budFile* glview );
...
void		BoundTriList( const mapTri_t* list, budBounds& b );
...
void		PlaneForTri( const mapTri_t* tri, budPlane& plane );
idWinding*	WindingForTri( const mapTri_t* tri );
mapTri_t*	WindingToTriList( const idWinding* w, const mapTri_t* originalTri );
void		ClipTriList( const mapTri_t* list, const budPlane& plane, float epsilon, mapTri_t** front, mapTri_t** back );
