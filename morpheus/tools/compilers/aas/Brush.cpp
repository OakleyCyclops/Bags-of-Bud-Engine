...
============
*/
idBrushSide::idBrushSide( const budPlane& plane, int planeNum )
{
	this->flags = 0;
	this->plane = plane;
...
============
*/
int idBrushSide::Split( const budPlane& splitPlane, idBrushSide** front, idBrushSide** back ) const
{
...
============
*/
bool idBrush::FromWinding( const idWinding& w, const budPlane& windingPlane )
{
	int i, j, bestAxis;
	budPlane plane;
...
============
*/
bool idBrush::TryMerge( const idBrush* brush, const budPlaneSet& planeList )
{
...
============
*/
int idBrush::Split( const budPlane& plane, int planeNum, idBrush** front, idBrush** back ) const
{
	int res, i, j;
	...
	for( i = 0; i < sides.Num(); i++ )
	{
		...
		else
		{
			...
			else if( maxFrontWinding[i] > -BRUSH_EPSILON )
			{
				...
				side = side->Copy();
				side->winding = side->winding->Clip( budPlane( plane.Normal(), ( plane.Dist() - ( BRUSH_EPSILON + 0.02f ) ) ), 0.01f, true );
				assert( side->winding );
...
============
*/
void idBrush::ExpandForAxialBox( const budBounds& bounds )
{
	int i, j;
	idBrushSide* side;
	Vector3 v;
...
============
*/
budBounds idBrushList::GetBounds() const
{
	budBounds bounds;
	idBrush* b;
...
============
*/
void idBrushList::Split( const budPlane& plane, int planeNum, idBrushList& frontList, idBrushList& backList, bool useBrushSavedPlaneSide )
{
...
============
*/
void idBrushList::SetFlagOnFacingBrushSides( const budPlane& plane, int flag )
{
	int i;
	idBrush* b;
	const idWinding* w;
...
============
*/
void idBrushList::CreatePlaneList( budPlaneSet& planeList ) const
{
	int i;
	idBrush* b;
	idBrushSide* side;
