...
public:
	...
	void					Flip();
	int						Split( const budPlane& splitPlane, idBrushBSPPortal** front, idBrushBSPPortal** back );
...
public:
	...
	int						PlaneSide( const budPlane& plane, float epsilon = ON_EPSILON ) const;
	// split the leaf node with a plane
	bool					Split( const budPlane& splitPlane, int splitPlaneNum );
	
	
private:
	budPlane					plane;			// split plane if this is not a leaf node
...
private:
	...
	budBounds				treeBounds;
	budPlaneSet				portalPlanes;
	int						numGridCells;
	int						numSplits;
...
private:
	...
	int						BrushSplitterStats( const idBrush* brush, int planeNum, const budPlaneSet& planeList, bool* testedPlanes, struct splitterStats_s& stats );
	int						FindSplitter( idBrushBSPNode* node, const budPlaneSet& planeList, bool* testedPlanes, struct splitterStats_s& bestStats );
