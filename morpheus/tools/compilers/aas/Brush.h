...
public:
	idBrushSide();
	idBrushSide( const budPlane& plane, int planeNum );
	~idBrushSide();
	...
	int						Split( const budPlane& splitPlane, idBrushSide** front, idBrushSide** back ) const;
...
public:
	...
	int						Split( const budPlane& plane, int planeNum, idBrush** front, idBrush** back ) const;
	// expand the brush for an axial bounding box
	void					ExpandForAxialBox( const budBounds& bounds );
	// next brush in list
	idBrush* 				Next() const
	{
		return next;
...
public:
	...
	void					Split( const budPlane& plane, int planeNum, idBrushList& frontList, idBrushList& backList, bool useBrushSavedPlaneSide = false );
	...
	void					SetFlagOnFacingBrushSides( const budPlane& plane, int flag );
