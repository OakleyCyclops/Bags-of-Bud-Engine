...
static budFile * try_to_open(char *name, int decompress, int noise_mode)
{	
	budFile * fp;
...
budFile * open_file(const char *name, int decompress, int noise_mode)
{
	budFile * fp;
...
void close_file(budFile * fp)
{
	delete fp;
}

/* This is meant for skipping a few bytes in a file or fifo. */
void skip(budFile * fp, size_t len)
{
	size_t c;
	char tmp[1024];
	while (len>0)
