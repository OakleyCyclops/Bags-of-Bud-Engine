...
/*
========================
budSWFTextInstance::budSWFTextInstance
========================
*/
budSWFTextInstance::budSWFTextInstance()
{
...
/*
========================
budSWFTextInstance::Init
========================
*/
void budSWFTextInstance::Init( budSWFEditText* _editText, budSWF* _swf )
{
	editText = _editText;
	swf = _swf;
	
	text = budLocalization::GetString( editText->initialText );
...
/*
========================
budSWFTextInstance::GetTextLength
========================
*/
float budSWFTextInstance::GetTextLength()
{
	...
	if( verify( swf != NULL ) )
	{
		...
		if( !variable.IsEmpty() )
		{
			budSWFScriptVar var = swf->GetGlobal( variable );
			if( var.IsUndefined() )
			{
				txtLengthCheck = text;
			}
			...
			txtLengthCheck = budLocalization::GetString( txtLengthCheck );
		}
		else
		{
			txtLengthCheck = budLocalization::GetString( text );
		}
		
		const budSWFEditText* shape = editText;
		budSWFDictionaryEntry* fontEntry = swf->FindDictionaryEntry( shape->fontID, SWF_DICT_FONT );
		budSWFFont* swfFont = fontEntry->font;
		float width = fabs( shape->bounds.br.x - shape->bounds.tl.x );
		float postTrans = SWFTWIP( shape->fontHeight );
		const budFont* fontInfo = swfFont->fontID;
...
/*
========================
budSWFTextInstance::StartParagraphText
========================
*/
void budSWFTextInstance::StartParagraphText( int time )
{
	generatingText = true;
	textSpotsVisible = 0;
	randomtext = "";
	...
	text = budLocalization::GetString( text );
	lengthCalculated = false;
...
/*
========================
budSWFTextInstance::StartRandomText
========================
*/
bool budSWFTextInstance::NeedsSoundPlayed()
{
...
========================
*/
void budSWFTextInstance::StartRandomText( int time )
{
	generatingText = true;
	textSpotsVisible = 0;
	randomtext = "";
...
/*
==============================================
budSWFTextInstance::SwitchSubtitleText
==============================================
*/
void budSWFTextInstance::SwitchSubtitleText( int time )
{
	subNeedsSwitch = false;
}

void budSWFTextInstance::SetSubNextStartIndex( int value )
{
...
/*
==============================================
budSWFTextInstance::UpdateSubtitle
==============================================
*/
bool budSWFTextInstance::UpdateSubtitle( int time )
{
...
/*
==============================================
budSWFTextInstance::SubtitleComplete
==============================================
*/
void budSWFTextInstance::SetSubEndIndex( int endChar, int time )
{
...
==============================================
*/
void budSWFTextInstance::SubtitleComplete()
{
	subInitialLine = true;
	subUpdating = false;
	isSubtitle = false;
...
/*
==============================================
budSWFTextInstance::LastWordChanged
==============================================
*/
void budSWFTextInstance::LastWordChanged( int wordCount, int time )
{
	...
	//libBud::Printf( "switchtime set 1 %d last word %d\n", subSwitchTime, subLastWordIndex );
}

/*
==============================================
budSWFTextInstance::GetSubtitleBreak
==============================================
*/
int budSWFTextInstance::GetApporoximateSubtitleBreak( int time )
{
	...
	if( time >= subSwitchTime )
	{
		...
		for( int i = wordIndex; i < subtitleTimingInfo.Num(); ++i )
		{
			if( subtitleTimingInfo[i].forceBreak )
			{
				if( i + 1 < subtitleTimingInfo.Num() )
				{
					...
					//libBud::Printf( "switchtime set 2 %d\n", subSwitchTime );
					subLastWordIndex = i;
					setSwitchTime = true;
					break;
				}
				else
				{
					...
					//libBud::Printf( "switchtime set 3 %d\n", subSwitchTime );
					subLastWordIndex = i;
					setSwitchTime = true;
					break;
				}
			...
			else
			{
				...
				if( timeSpan > swf_subtitleLengthGuess.GetInteger() )
				{
					...
					{
						subSwitchTime = subStartTime + subtitleTimingInfo[i].startTime;// - swf_subtitleEarlyTrans.GetInteger();
						//libBud::Printf( "switchtime set 4 %d\n", subSwitchTime );
						subLastWordIndex = i - 1;
					}
					else
					{
						subSwitchTime = subStartTime + subtitleTimingInfo[i].startTime;// - swf_subtitleEarlyTrans.GetInteger();
						//libBud::Printf( "switchtime set 5 %d\n", subSwitchTime );
						subLastWordIndex = i;
					}
					setSwitchTime = true;
					break;
		...
		if( !setSwitchTime && subtitleTimingInfo.Num() > 0 )
		{
			...
			//libBud::Printf( "switchtime set 6 %d\n", subSwitchTime );
...
==============================================
*/
void budSWFTextInstance::SubtitleCleanup()
{
	subSourceID = -1;
	subAlign = -1;
	text = "";
...
/*
==============================================
budSWFTextInstance::SetStrokeInfo
==============================================
*/
void budSWFTextInstance::SetStrokeInfo( bool use, float strength, float weight )
{
	useStroke = use;
	if( use )
	{
...
/*
==============================================
budSWFTextInstance::CalcMaxScroll
==============================================
*/
int budSWFTextInstance::CalcMaxScroll( int numLines )
{
	...
	{
		textCheck = budLocalization::GetString( variable );
	}
...
int budSWFTextInstance::CalcNumLines()
{
...
/*
========================
budSWFScriptObject_TextInstancePrototype
========================
*/

#define SWF_TEXT_FUNCTION_DEFINE( x ) budSWFScriptVar budSWFScriptObject_TextInstancePrototype::budSWFScriptFunction_##x::Call( budSWFScriptObject * thisObject, const budSWFParmList & parms )
#define SWF_TEXT_NATIVE_VAR_DEFINE_GET( x ) budSWFScriptVar budSWFScriptObject_TextInstancePrototype::budSWFScriptNativeVar_##x::Get( class budSWFScriptObject * object )
#define SWF_TEXT_NATIVE_VAR_DEFINE_SET( x ) void  budSWFScriptObject_TextInstancePrototype::budSWFScriptNativeVar_##x::Set( class budSWFScriptObject * object, const budSWFScriptVar & value )

#define SWF_TEXT_PTHIS_FUNC( x ) budSWFTextInstance * pThis = thisObject ? thisObject->GetText() : NULL; if ( !verify( pThis != NULL ) ) { libBud::Warning( "SWF: tried to call " x " on NULL edittext" ); return budSWFScriptVar(); }
#define SWF_TEXT_PTHIS_GET( x ) budSWFTextInstance * pThis = object ? object->GetText() : NULL; if ( pThis == NULL ) { return budSWFScriptVar(); }
#define SWF_TEXT_PTHIS_SET( x ) budSWFTextInstance * pThis = object ? object->GetText() : NULL; if ( pThis == NULL ) { return; }
...
budSWFScriptObject_TextInstancePrototype::budSWFScriptObject_TextInstancePrototype()
{
...
SWF_TEXT_NATIVE_VAR_DEFINE_SET( text )
{
	...
	pThis->text = budLocalization::GetString( value.ToString() );
