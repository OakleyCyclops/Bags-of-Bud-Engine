...
class budSWFBitStream
{
public:
	budSWFBitStream();
	budSWFBitStream( const byte* data, uint32 len, bool copy )
	{
		free = false;
		Load( data, len, copy );
	}
	...
	budSWFBitStream& operator=( budSWFBitStream& other );
...
/*
========================
budSWFBitStream::ResetBits
========================
*/
BUD_INLINE void budSWFBitStream::ResetBits()
{
	currentBit = 0;
	currentByte = 0;
}
...
/*
========================
budSWFBitStream::ReadLittle
========================
*/
template< typename T >
void budSWFBitStream::ReadLittle( T& val )
{
...
========================
*/
BUD_INLINE bool   budSWFBitStream::ReadBool()
{
	return ( ReadU( 1 ) != 0 );
}
BUD_INLINE uint8  budSWFBitStream::ReadU8()
{
	ResetBits();
	return *readp++;
}
BUD_INLINE uint16 budSWFBitStream::ReadU16()
{
	ResetBits();
	readp += 2;
	return ( readp[-2] | ( readp[-1] << 8 ) );
}
BUD_INLINE uint32 budSWFBitStream::ReadU32()
{
	ResetBits();
	readp += 4;
	return ( readp[-4] | ( readp[-3] << 8 ) | ( readp[-2] << 16 ) | ( readp[-1] << 24 ) );
}
BUD_INLINE int16  budSWFBitStream::ReadS16()
{
	ResetBits();
	readp += 2;
	return ( readp[-2] | ( readp[-1] << 8 ) );
}
BUD_INLINE int32  budSWFBitStream::ReadS32()
{
	ResetBits();
	readp += 4;
	return ( readp[-4] | ( readp[-3] << 8 ) | ( readp[-2] << 16 ) | ( readp[-1] << 24 ) );
}
BUD_INLINE float  budSWFBitStream::ReadFixed8()
{
	ResetBits();
	readp += 2;
	return SWFFIXED8( ( readp[-2] | ( readp[-1] << 8 ) ) );
}
BUD_INLINE float  budSWFBitStream::ReadFixed16()
{
	ResetBits();
	readp += 4;
	return SWFFIXED16( ( readp[-4] | ( readp[-3] << 8 ) | ( readp[-2] << 16 ) | ( readp[-1] << 24 ) ) );
}
BUD_INLINE float  budSWFBitStream::ReadFloat()
{
	ResetBits();
	readp += 4;
...
BUD_INLINE double budSWFBitStream::ReadDouble()
{
