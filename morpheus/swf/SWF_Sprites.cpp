...
/*
========================
budSWFSprite::budSWFSprite
========================
*/
budSWFSprite::budSWFSprite( budSWF* _swf ) :
	swf( _swf ),
	frameCount( 0 ),
	commandBuffer( NULL )
{
...
========================
*/
void budSWF::DefineSprite( budSWFBitStream& bitstream )
{
	uint16 characterID = bitstream.ReadU16();
	budSWFDictionaryEntry* entry = AddDictionaryEntry( characterID, SWF_DICT_SPRITE );
	if( entry == NULL )
	{
		return;
	}
...
========================
*/
void budSWFSprite::Load( budSWFBitStream& bitstream, bool parseDictionary )
{
	...
	while( true )
	{
		...
		budSWFBitStream tagStream( bitstream.ReadData( recordLength ), recordLength, false );
...
#undef HANDLE_SWF_TAG
		...
		switch( tag )
		{
			...
			case Tag_DoInitAction:
			{
				...
				budSWFBitStream& initaction = doInitActions.Alloc();
			...
			default:
				...
				//libBud::Printf( "Load Sprite: Unhandled tag %s\n", budSWF::GetTagName( tag ) );
				break;
		}
	}
}
...
========================
*/
void budSWFSprite::Read( budFile* f )
{
	int num = 0;
	...
	for( int i = 0; i < num; i++ )
	{
		...
		budSWFBitStream& initaction = doInitActions[i];
...
========================
*/
void budSWFSprite::Write( budFile* f )
{
...
// RB begin
void budSWFSprite::ReadJSON( rapidjson::Value& entry )
{
	...
	for( int i = 0; i < commands.Num(); i++ )
	{
		...
		if( type == "Tag_PlaceObject2" || type == "Tag_PlaceObject3" )
		{
			...
			budFile_SWF file( new budFile_Memory() );
			...
			commands[i].stream.Load( ( byte* ) static_cast<budFile_Memory*>( ( budFile* )file )->GetDataPtr(), streamLength, true );
		}
		...
		else if( type == "Tag_DoAction" )
		{
			...
			budBase64 base64( command["stream"].GetString() );
			base64.Decode( file );
...
void budSWFSprite::WriteJSON( budFile* f, int characterID )
{
	...
	for( int i = 0; i < commands.Num(); i++ )
	{
		budSWFSprite::swfSpriteCommand_t& command = commands[i];
		
		//base64.Encode( command.stream.Ptr(), command.stream.Length() );
		//base64.Decode( src );
		
		//f->WriteFloatString( "%s\t<Command tag=\"%s\" streamLength=\"%i\">%s</Command>\n", indentPrefix, budSWF::GetTagName( commands[i].tag ), src.Length(), src.c_str() );
		//f->WriteFloatString( "%s\t<Command tag=\"%s\" streamLength=\"%i\">%s</Command>\n", indentPrefix, budSWF::GetTagName( commands[i].tag ), commands[i].stream.Length(), base64.c_str() );
		
		//f->WriteFloatString( "%s\t<Command tag=\"%s\" streamLength=\"%i\">\n", indentPrefix, budSWF::GetTagName( command.tag ), command.stream.Length(), base64.c_str() );
...
#undef HANDLE_SWF_TAG
			default:
				break;
				//libBud::Printf( "Export Sprite: Unhandled tag %s\n", budSWF::GetTagName( command.tag ) );
		}
	}
	f->WriteFloatString( "\n\t\t\t]\n" );
...
void budSWFSprite::WriteJSON_PlaceObject2( budFile* file, budSWFBitStream& bitstream, int sourceCharacterID, int commandID, const char* indentPrefix )
{
...
void budSWFSprite::WriteJSON_PlaceObject3( budFile* file, budSWFBitStream& bitstream, int sourceCharacterID, int commandID, const char* indentPrefix )
{
	uint8 flags1 = bitstream.ReadU8();
	...
	if( ( flags2 & PlaceFlagHasFilterList ) != 0 )
	{
		...
		//libBud::Warning( "PlaceObject3: has filters" );
		file->WriteFloatString( ",\n\t\t\t\t\t\"hasFilterList\": true" );
		return;
	}
...
void budSWFSprite::WriteJSON_RemoveObject2( budFile* file, budSWFBitStream& bitstream, int sourceCharacterID, int commandID, const char* indentPrefix )
{
	int depth = bitstream.ReadU16();
...
void budSWFSprite::WriteJSON_DoAction( budFile* file, budSWFBitStream& bitstream, int characterID, int commandID, const char* indentPrefix )
{
	budBase64 base64;
...
void budSWFSprite::WriteSWF( budFile_SWF& f, int characterID )
{
	// TODO frameLabels
	...
	for( int i = 0; i < doInitActions.Num(); i++ )
	{
		tagLength += budFile_SWF::GetTagHeaderSize( Tag_DoInitAction, doInitActions[i].Length() );
	...
	for( int i = 0; i < commands.Num(); i++ )
	{
		...
		switch( command.tag )
		{
			...
			case Tag_RemoveObject2:
			case Tag_DoAction:
				tagLength += budFile_SWF::GetTagHeaderSize( command.tag, command.stream.Length() );
