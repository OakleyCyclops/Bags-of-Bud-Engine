...
/*
========================
budSWFConstantPool::Clear
========================
*/
void budSWFConstantPool::Clear()
{
...
/*
========================
budSWFConstantPool::Copy
========================
*/
void budSWFConstantPool::Copy( const budSWFConstantPool& other )
{
	Clear();
...
/*
========================
budSWFScriptFunction_Script::~budSWFScriptFunction_Script
========================
*/
budSWFScriptFunction_Script::~budSWFScriptFunction_Script()
{
...
========================
*/
budSWFScriptVar budSWFScriptFunction_Script::Call( budSWFScriptObject* thisObject, const budSWFParmList& parms )
{
	...
	budSWFScriptObject* locals = budSWFScriptObject::Alloc();
	...
	for( int i = parms.Num(); i < parameters.Num(); i++ )
	{
		...
		locals->Set( parameters[i].name, budSWFScriptVar() );
	}
	...
	{
		// create "this"
		locals->Set( "this", budSWFScriptVar( thisObject ) );
	}
	if( flags & BIT( 2 ) )
	{
		budSWFScriptObject* arguments = budSWFScriptObject::Alloc();
	...
	if( ( flags & BIT( 3 ) ) == 0 )
	{
		...
		locals->Set( "arguments", budSWFScriptVar( arguments ) );
	...
	{
		// create "super"
		locals->Set( "super", budSWFScriptVar( thisObject->GetPrototype() ) );
	}
	if( flags & BIT( 6 ) )
	{
		// preload _root
	...
	budSWFScriptVar retVal = Run( thisObject, stack, bitstream );
...
/*
========================
budSWFScriptFunction_Script::Run
========================
*/
budSWFScriptVar budSWFScriptFunction_Script::Run( budSWFScriptObject* thisObject, budSWFStack& stack, budSWFBitStream& bitstream )
{
	static int callstackLevel = -1;
	budSWFSpriteInstance* thisSprite = thisObject->GetSprite();
	budSWFSpriteInstance* currentTarget = thisSprite;
	...
	while( bitstream.Tell() < bitstream.Length() )
	{
		...
		switch( code )
		{
			...
			case Action_NextFrame:
				...
				{
					libBud::Printf( "SWF: no target movie clip for nextFrame\n" );
				}
				break;
			case Action_PrevFrame:
				if( verify( currentTarget != NULL ) )
				...
				{
					libBud::Printf( "SWF: no target movie clip for prevFrame\n" );
				}
				break;
			case Action_Play:
				if( verify( currentTarget != NULL ) )
				...
				{
					libBud::Printf( "SWF: no target movie clip for play\n" );
				}
				break;
			case Action_Stop:
				if( verify( currentTarget != NULL ) )
				...
				{
					libBud::Printf( "SWF: no target movie clip for stop\n" );
				}
				break;
			case Action_ToggleQuality:
				break;
			...
			case Action_GotoFrame:
			{
				...
				{
					libBud::Printf( "SWF: no target movie clip for runTo %d\n", frameNum );
				}
				break;
			}
			case Action_SetTarget:
			{
				...
				{
					libBud::Printf( "SWF: no target movie clip for setTarget %s\n", targetName );
				}
				break;
			}
			case Action_GoToLabel:
			{
				...
				{
					libBud::Printf( "SWF: no target movie clip for runTo %s\n", targetName );
				}
				break;
			}
			case Action_Push:
			{
				budSWFBitStream pushstream( bitstream.ReadData( recordLength ), recordLength, false );
			...
			case Action_GetVariable:
			{
				...
				{
					libBud::Printf( "SWF: unknown variable %s\n", variableName.c_str() );
				}
				break;
			}
			case Action_SetVariable:
			...
			case Action_GotoFrame2:
			{
				...
				else if( swf_debug.GetInteger() > 0 )
				{
					if( ( flags & 1 ) != 0 )
					{
						libBud::Printf( "SWF: no target movie clip for gotoAndPlay\n" );
					}
					else
					{
						libBud::Printf( "SWF: no target movie clip for gotoAndStop\n" );
					}
				}
				stack.Pop( 1 );
				break;
			...
			case Action_GetProperty:
			{
				if( verify( thisSprite != NULL ) )
				{
					budSWFSpriteInstance* target = thisSprite->ResolveTarget( stack.B().ToString() );
					stack.B() = target->scriptObject->Get( GetPropertyName( stack.A().ToInteger() ) );
				}
				else if( swf_debug.GetInteger() > 0 )
				{
					libBud::Printf( "SWF: no target movie clip for getProperty\n" );
				}
				stack.Pop( 1 );
				break;
			}
			case Action_SetProperty:
			{
				if( verify( thisSprite != NULL ) )
				{
					budSWFSpriteInstance* target = thisSprite->ResolveTarget( stack.C().ToString() );
					target->scriptObject->Set( GetPropertyName( stack.B().ToInteger() ), stack.A() );
				}
				else if( swf_debug.GetInteger() > 0 )
				{
					libBud::Printf( "SWF: no target movie clip for setProperty\n" );
				}
				stack.Pop( 3 );
				break;
			}
			case Action_Trace:
				libBud::PrintfIf( swf_debug.GetInteger() > 0, "SWF Trace: %s\n", stack.A().ToString().c_str() );
				stack.Pop( 1 );
				break;
			case Action_GetTime:
			...
			case Action_CallFunction:
			{
				...
				{
					libBud::PrintfIf( swf_debug.GetInteger() > 0, "SWF: unknown function %s\n", functionName.c_str() );
					stack.Alloc().SetUndefined();
				}
			...
			case Action_CallMethod:
			{
				...
				if( stack.B().IsObject() )
				{
					...
					{
						libBud::PrintfIf( swf_debug.GetInteger() > 1, "SWF: unknown method %s on %s\n", functionName.c_str(), object->DefaultValue( true ).ToString().c_str() );
					}
				}
				else
				{
			...
			case Action_ConstantPool:
			{
				...
				{
					constants.Append( budSWFScriptString::Alloc( bitstream.ReadString() ) );
				}
				break;
			}
			case Action_DefineFunction:
			{
				...
				{
					thisObject->Set( functionName, budSWFScriptVar( newFunction ) );
				}
				newFunction->Release();
				break;
			}
			case Action_DefineFunction2:
			{
				...
				for( int i = 0; i < numParms; i++ )
				{
					...
					if( reg >= numRegs )
					{
						libBud::Warning( "SWF: Parameter %s in function %s bound to out of range register %d", name, functionName.c_str(), reg );
						reg = 0;
					}
			...
			case Action_Enumerate:
			{
				...
				{
					budSWFScriptObject* object = stack.A().GetObject();
					object->AddRef();
					stack.A().SetNULL();
			...
			case Action_GetMember:
			{
				...
				{
					libBud::Printf( "SWF: tried to get member %s on an invalid object in sprite '%s'\n", stack.A().ToString().c_str(), thisSprite != NULL ? thisSprite->GetName() : "" );
				}
				if( stack.B().IsObject() )
				{
					budSWFScriptObject* object = stack.B().GetObject();
					...
					{
						libBud::Printf( "SWF: unknown member %s\n", stack.A().ToString().c_str() );
					}
				}
				...
				else if( stack.B().IsFunction() )
				{
					...
					if( propertyName.Cmp( "prototype" ) == 0 )
					{
						...
						budSWFScriptFunction* sfs = stack.B().GetFunction();
						budSWFScriptObject* object = sfs->GetPrototype();
						
						if( object == NULL )
						{
							object = budSWFScriptObject::Alloc();
							// Set the __proto__ to the main Object prototype
							budSWFScriptVar baseObjConstructor = scope[0]->Get( "Object" );
							budSWFScriptFunction* baseObj = baseObjConstructor.GetFunction();
							object->Set( "__proto__", baseObj->GetPrototype() );
							sfs->SetPrototype( object );
						}
						
						stack.B() = budSWFScriptVar( object );
					}
					else
					{
						stack.B().SetUndefined();
			...
			case Action_SetMember:
			{
				if( stack.C().IsObject() )
				{
					budSWFScriptObject* object = stack.C().GetObject();
			...
			case Action_NewObject:
			{
				...
				else
				{
					...
					budSWFScriptVar objdef = scope[0]->Get( functionName );
					if( objdef.IsFunction() )
					{
						budSWFScriptFunction* constructorFunction = objdef.GetFunction();
					...
					{
						libBud::Warning( "SWF: Unknown class definition %s", functionName.c_str() );
					}
				}
			...
			case Action_Extends:
			{
				budSWFScriptFunction* superclassConstructorFunction = stack.A().GetFunction();
				budSWFScriptFunction* subclassConstructorFunction = stack.B().GetFunction();
				stack.Pop( 2 );
				
				budSWFScriptObject* scriptObject = budSWFScriptObject::Alloc();
				scriptObject->SetPrototype( superclassConstructorFunction->GetPrototype() );
				scriptObject->Set( "__proto__", budSWFScriptVar( superclassConstructorFunction->GetPrototype() ) );
				scriptObject->Set( "__constructor__", budSWFScriptVar( superclassConstructorFunction ) );
			...
			case Action_With:
			{
				...
				budSWFBitStream bitstream2( bitstream.ReadData( withSize ), withSize, false );
				if( stack.A().IsObject() )
				{
					budSWFScriptObject* withObject = stack.A().GetObject();
					withObject->AddRef();
					stack.Pop( 1 );
				...
				else
				{
					...
					{
						libBud::Printf( "SWF: with() invalid object specified\n" );
					}
					stack.Pop( 1 );
				}
				break;
			...
			case Action_PushDuplicate:
			{
				budSWFScriptVar dup = stack.A();
				stack.Alloc() = dup;
				break;
			}
			case Action_StackSwap:
			{
				budSWFScriptVar temp = stack.A();
				stack.A() = stack.B();
				stack.A() = temp;
				break;
			}
			...
			case Action_DefineLocal2:
			{
				scope[scope.Num() - 1]->Set( stack.A().ToString(), budSWFScriptVar() );
				stack.Pop( 1 );
				break;
			}
			case Action_Delete:
			{
				if( swf_debug.GetInteger() > 0 )
				{
					libBud::Printf( "SWF: Delete ignored\n" );
				}
			...
			case Action_Delete2:
			{
				if( swf_debug.GetInteger() > 0 )
				{
					libBud::Printf( "SWF: Delete2 ignored\n" );
				}
				// We no longer support deleting variables because the performance cost of updating the hash tables is not worth it
				stack.Pop( 1 );
				break;
			...
			case Action_NewMethod:
			default:
				libBud::Warning( "SWF: Unhandled Action %s", budSWF::GetActionName( code ) );
...
void budSWFScriptFunction_Script::QuitCurrentBlock()
{
...
========================
*/
void budSWF::Invoke( const char* functionName, const budSWFParmList& parms )
{
	budSWFScriptObject* obj = mainspriteInstance->GetScriptObject();
	budSWFScriptVar scriptVar = obj->Get( functionName );
	
	if( swf_debugInvoke.GetBool() )
	{
		libBud::Printf( "SWF: Invoke %s with %d parms (%s)\n", functionName, parms.Num(), GetName() );
	}
...
========================
*/
void budSWF::Invoke( const char* functionName, const budSWFParmList& parms, budSWFScriptVar& scriptVar )
{
...
========================
*/
void budSWF::Invoke( const char*   functionName, const budSWFParmList& parms, bool& functionExists )
{
	budSWFScriptObject* obj = mainspriteInstance->GetScriptObject();
	budSWFScriptVar scriptVar = obj->Get( functionName );
