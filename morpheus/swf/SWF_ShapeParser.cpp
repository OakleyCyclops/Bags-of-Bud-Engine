...
/*
========================
budSWFShapeParser::ParseShape
========================
*/
void budSWFShapeParser::Parse( budSWFBitStream& bitstream, budSWFShape& shape, int recordType )
{
	...
	{
		budSWFShapeDrawLine& ld = shape.lineDraws[i];
...
/*
========================
budSWFShapeParser::ParseMorph
========================
*/
void budSWFShapeParser::ParseMorph( budSWFBitStream& bitstream, budSWFShape& shape )
{
	extendedCount = true;
	lineStyle2 = false;
	rgba = true;
...
/*
========================
budSWFShapeParser::ParseFont
========================
*/
void budSWFShapeParser::ParseFont( budSWFBitStream& bitstream, budSWFFontGlyph& shape )
{
	extendedCount = false;
	lineStyle2 = false;
	rgba = false;
...
/*
========================
budSWFShapeParser::ParseShapes
========================
*/
void budSWFShapeParser::ParseShapes( budSWFBitStream& bitstream1, budSWFBitStream* bitstream2, bool swap )
{
	int32 pen1X = 0;
	int32 pen1Y = 0;
	int32 pen2X = 0;
	...
	while( true )
	{
		if( !bitstream1.ReadBool() )
		{
			...
			if( ( stateNewStyles || stateLineStyle || stateFillStyle1 || stateFillStyle0 || stateMoveTo ) == false )
			{
				...
				if( bitstream2 )
				{
					...
					if( flags != 0 )
					{
						libBud::Warning( "budSWFShapeParser: morph stream 1 ends before 2" );
						break;
					}
				}
				break;
			...
			if( bitstream2 )
			{
				...
				if( isEdge )
				{
					libBud::Warning( "budSWFShapeParser: morph stream 1 defines style change, but stream 2 does not" );
					break;
				}
				...
				if( stateFillStyle0 )
				{
					...
					{
						libBud::Warning( "budSWFShapeParser: morph stream 2 defined a different fillStyle0 from stream 1" );
						break;
					}
				}
				if( stateFillStyle1 )
				{
					...
					{
						libBud::Warning( "budSWFShapeParser: morph stream 2 defined a different fillStyle1 from stream 1" );
						break;
					}
				}
				if( stateLineStyle )
				{
					...
					{
						libBud::Warning( "budSWFShapeParser: morph stream 2 defined a different lineStyle from stream 1" );
						break;
					}
				}
				if( stateNewStyles )
				{
					libBud::Warning( "budSWFShapeParser: morph stream 2 defines new styles" );
					break;
				}
			}
		}
		else
		{
			...
			if( bitstream2 )
			{
				...
				if( !isEdge )
				{
					libBud::Warning( "budSWFShapeParser: morph stream 1 defines an edge, but stream 2 does not" );
					break;
				}
...
/*
========================
budSWFShapeParser::ParseEdge
========================
*/
void budSWFShapeParser::ParseEdge( budSWFBitStream& bitstream, int32& penX, int32& penY, swfSPEdge_t& edge )
{
...
/*
========================
budSWFShapeParser::MakeLoops
========================
*/
void budSWFShapeParser::MakeLoops()
{
	...
	for( int i = 0; i < fillDraws.Num(); i++ )
	{
		...
		while( unusedEdges.Num() > 0 )
		{
			...
			{
				libBud::Warning( "budSWFShapeParser: loop with < 3 verts" );
		...
		while( true )
		{
			...
			if( shape == -1 )
			{
				libBud::Warning( "budSWFShapeParser: Hole not in a shape" );
...
/*
========================
budSWFShapeParser::TriangulateSoup
========================
*/
void budSWFShapeParser::TriangulateSoup( budSWFShape& shape )
{
	...
	for( int i = 0; i < fillDraws.Num(); i++ )
	{
		...
		budSWFShapeDrawFill& drawFill = shape.fillDraws[i];
		...
		for( int j = 0; j < spDrawFill.loops.Num(); j++ )
		{
			...
			for( int k = 0; k < numVerts - 2; k++ )
			{
				...
				if( v1 == -1 )
				{
					libBud::Warning( "budSWFShapeParser: could not find an ear vert" );
					break;
				}
...
========================
*/
void budSWFShapeParser::TriangulateSoup( budSWFFontGlyph& shape )
{
...
/*
========================
budSWFShapeParser::FindEarVert
========================
*/
int budSWFShapeParser::FindEarVert( const swfSPLineLoop_t& loop )
{
...
/*
========================
budSWFShapeParser::ReadFillStyle
========================
*/
void budSWFShapeParser::ReadFillStyle( budSWFBitStream& bitstream )
{
	...
	for( int i = 0; i < lineStyleCount; i++ )
	{
		...
		if( lineStyle2 )
		{
			...
			if( hasFillFlag )
			{
				...
				libBud::Warning( "budSWFShapeParser: Ignoring hasFillFlag" );
			}
			else
			{
