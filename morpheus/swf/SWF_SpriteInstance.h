...
{
public:
	budSWFSpriteInstance();
	~budSWFSpriteInstance();
	
	void	Init( budSWFSprite* sprite, budSWFSpriteInstance* parent, int depth );
	...
	budSWFScriptObject* GetScriptObject()
	{
		return scriptObject;
	}
	...
	void SetMaterial( const budMaterial* material, int width = -1, int height = -1 );
	void SetVisible( bool visible );
	bool IsVisible()
	{
		return isVisible;
	}
	void PlayFrame( const budSWFParmList& parms );
	...
	void StopFrame( const budSWFParmList& parms );
	...
	// In C++03, nested members defined in a friend class do NOT have access to private/protected members of the class granting friendship	friend class budSWF;
	...
	const budMaterial* materialOverride;
	...
	budSWFScriptVar onEnterFrame;
	//budSWFScriptVar onLoad;
	...
	// SWF_PlaceObject.cpp
	//----------------------------------
	void			PlaceObject2( budSWFBitStream& bitstream );
	void			PlaceObject3( budSWFBitStream& bitstream );
	void			RemoveObject2( budSWFBitStream& bitstream );
	
	//----------------------------------
	// SWF_Sounds.cpp
	//----------------------------------
	void			StartSound( budSWFBitStream& bitstream );
	...
	void					DoAction( budSWFBitStream& bitstream );
	
	budSWFSpriteInstance* 	FindChildSprite( const char* childName );
	budSWFSpriteInstance* 	ResolveTarget( const char* targetName );
...
================================================
*/
class budSWFScriptObject_SpriteInstancePrototype : public budSWFScriptObject
{
public:
	budSWFScriptObject_SpriteInstancePrototype();
	
#define SWF_SPRITE_FUNCTION_DECLARE( x ) \
	class budSWFScriptFunction_##x : public budSWFScriptFunction { \
	public: \
		void			AddRef() {} \
		void			Release() {} \
...
extern budSWFScriptObject_SpriteInstancePrototype spriteInstanceScriptObjectPrototype;
