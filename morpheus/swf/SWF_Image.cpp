...
/*
========================
budSWF::idDecompressJPEG::idDecompressJPEG
========================
*/
budSWF::idDecompressJPEG::idDecompressJPEG()
{
...
/*
========================
budSWF::idDecompressJPEG::Load
========================
*/
byte* budSWF::idDecompressJPEG::Load( const byte* input, int inputSize, int& width, int& height )
{
	jpeg_decompress_struct* cinfo = ( jpeg_decompress_struct* )vinfo;
	...
	try
	{
		...
		if( cinfo->output_components != 4 )
		{
			...
			libBud::Warning( "JPEG output is not 4 components" );
...
void budSWF::WriteSwfImageAtlas( const char* filename )
{
	...
	libBud::Printf( "%5.2f packing fraction in %ix%i image\n", frac, totalSize.x * 4, totalSize.y * 4 );
...
//		libBud::Printf( "Color normalize: %3i:%3i  %3i:%3i  %3i:%3i  %3i:%3i\n",
//			minV[0], maxV[0], minV[1], maxV[1], minV[2], maxV[2], minV[3], maxV[3] );
		...
		//
		// If we create an budMaterial for each atlas element, we could add
		...
		budSWFDictionaryEntry* entry = FindDictionaryEntry( pack.characterID );
...
========================
*/
void budSWF::LoadImage( int characterID, const byte* imageData, int width, int height )
{
	budSWFDictionaryEntry* entry = AddDictionaryEntry( characterID, SWF_DICT_IMAGE );
	if( entry == NULL )
	{
		return;
	}
...
========================
*/
void budSWF::JPEGTables( budSWFBitStream& bitstream )
{
...
========================
*/
void budSWF::DefineBits( budSWFBitStream& bitstream )
{
...
void budSWF::DefineBitsJPEG3( budSWFBitStream& bitstream )
{
	...
	
	{
		...
		{
			libBud::Warning( "DefineBitsJPEG3: Failed to inflate alpha data" );
			Mem_Free( imageData );
			return;
		}
...
/*
========================
budSWF::DefineBitsLossless
========================
*/
void budSWF::DefineBitsLossless( budSWFBitStream& bitstream )
{
	uint16 characterID = bitstream.ReadU16();
	...
	if( format == 3 )
	{
		...
		{
			libBud::Warning( "DefineBitsLossless: Failed to inflate color map data" );
			return;
		}
	...
	else if( format == 4 )
	{
		...
		{
			libBud::Warning( "DefineBitsLossless: Failed to inflate bitmap data" );
			return;
		}
		for( int h = 0; h < height; h++ )
		{
	...
	{
		libBud::Warning( "DefineBitsLossless: Unknown image format %d", format );
...
/*
========================
budSWF::DefineBitsLossless2
========================
*/
void budSWF::DefineBitsLossless2( budSWFBitStream& bitstream )
{
	uint16 characterID = bitstream.ReadU16();
	uint8 format = bitstream.ReadU8();
	uint16 width = bitstream.ReadU16();
	...
	if( format == 3 )
	{
		...
		if( !Inflate( bitstream.ReadData( colorDataSize ), colorDataSize, colorMapData.Ptr(), ( int )colorMapData.Size() ) )
		{
			libBud::Warning( "DefineBitsLossless2: Failed to inflate color map data" );
			return;
		}
		byte* indices = colorMapData.Ptr() + colorTableSize;
		for( int h = 0; h < height; h++ )
	...
	else if( format == 5 )
	{
		...
		if( !Inflate( bitstream.ReadData( colorDataSize ), colorDataSize, ( byte* )bitmapData.Ptr(), ( int )bitmapData.Size() ) )
		{
			libBud::Warning( "DefineBitsLossless2: Failed to inflate bitmap data" );
			return;
		}
		for( int h = 0; h < height; h++ )
		{
	...
	{
		libBud::Warning( "DefineBitsLossless2: Unknown image format %d", format );
		memset( imageData, 0xFF, width * height * 4 );
	}
