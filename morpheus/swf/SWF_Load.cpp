...
===================
*/
bool budSWF::LoadSWF( const char* fullpath )
{

	budFile* rawfile = fileSystem->OpenFileRead( fullpath );
	if( rawfile == NULL )
	{
		libBud::Printf( "SWF File not found %s\n", fullpath );
		return false;
	}
	...
	{
		libBud::Warning( "Wrong signature bytes" );
		delete rawfile;
		return false;
	}
	...
	if( header.version > 9 )
	{
		libBud::Warning( "Unsupported version %d", header.version );
		delete rawfile;
		return false;
	}
	...
	{
		libBud::Warning( "Unsupported compression type %c", header.compression );
		delete rawfile;
		return false;
	}
	...
	// slurp the raw file into a giant array, which is somewhat atrocious when loading from the preload since it's already an budFile_Memory
	...
	budSWFBitStream bitstream( fileData, fileLength2, false );
	...
	{
		libBud::Warning( "Invalid frameSize top left" );
		Mem_Free( fileData );
		return false;
	}
...
void budSWF::WriteSWF( const char* swfFilename, const byte* atlasImageRGBA, int atlasImageWidth, int atlasImageHeight )
{
	budFile_SWF file( fileSystem->OpenFileWrite( swfFilename, "fs_basepath" ) );
	if( file == NULL )
	{
		return;
	}
	...
	{
		const budSWFDictionaryEntry& entry = dictionary[i];
		...
		switch( dictionary[i].type )
		{
			case SWF_DICT_IMAGE:
			{
				...
				{
					libBud::Warning( "DefineBitsLossless: Failed to deflate bitmap data" );
					//return;
				}
			...
			case SWF_DICT_SHAPE:
			{
				...
				{
					budSWFShapeDrawFill& fillDraw = shape->fillDraws[d];
				...
				budFile_Memory* tagMem = new budFile_Memory( "shapeTag" );
				budFile_SWF tag( tagMem );
				...
				{
					budSWFShapeDrawLine& lineDraw = shape->lineDraws[d];
...
===================
*/
bool budSWF::LoadBinary( const char* bfilename, ID_TIME_T sourceTimeStamp )
{
	budFile* f = fileSystem->OpenFileReadMemory( bfilename );
	...
	for( int i = 0; i < dictionary.Num(); i++ )
	{
		...
		switch( dictionary[i].type )
		{
			...
			case SWF_DICT_SHAPE:
			{
				dictionary[i].shape = new( TAG_SWF ) budSWFShape;
				budSWFShape* shape = dictionary[i].shape;
			...
			case SWF_DICT_SPRITE:
			{
				dictionary[i].sprite = new( TAG_SWF ) budSWFSprite( this );
			...
			case SWF_DICT_TEXT:
			{
				dictionary[i].text = new( TAG_SWF ) budSWFText;
				budSWFText* text = dictionary[i].text;
				...
				{
					budSWFTextRecord& textRecord = text->textRecords[t];
			...
			{
				dictionary[i].edittext = new( TAG_SWF ) budSWFEditText;
				budSWFEditText* edittext = dictionary[i].edittext;
...
===================
*/
void budSWF::WriteBinary( const char* bfilename )
{
	budFileLocal file( fileSystem->OpenFileWrite( bfilename, "fs_basepath" ) );
	if( file == NULL )
	{
		return;
	}
...
/*
===================
budSWF::FileAttributes
Extra data that won't fit in a SWF header
===================
*/
void budSWF::FileAttributes( budSWFBitStream& bitstream )
{
...
===================
*/
void budSWF::Metadata( budSWFBitStream& bitstream )
{
...
/*
===================
budSWF::SetBackgroundColor
===================
*/
void budSWF::SetBackgroundColor( budSWFBitStream& bitstream )
{
...
===================
*/
bool budSWF::LoadJSON( const char* filename )
{
	budFile* f = fileSystem->OpenFileReadMemory( filename );
	if( f == NULL || f->Length() <= 0 )
	{
		return false;
	}
	...
	if( d.HasMember( "version" ) )
	{
		...
		//libBud::Printf( "version = %i", version );
	}
...
===================
*/
void budSWF::WriteJSON( const char* jsonFilename )
{
	const bool exportBitmapShapesOnly = false;
	
	budFileLocal file( fileSystem->OpenFileWrite( jsonFilename, "fs_basepath" ) );
	if( file == NULL )
	{
		return;
	}
	...
	for( int i = 0; i < dictionary.Num(); i++ )
	{
		...
		//if( dictionary[i].type != SWF_DICT_NULL )
		{
			...
			file->WriteFloatString( "\t\t\t\"type\": \"%s\",\n", budSWF::GetDictTypeName( dictionary[i].type ) );
		...
		switch( dictionary[i].type )
		{
			...
			case SWF_DICT_SHAPE:
			{
				...
				if( shape->fillDraws.Num() > 0 )
				{
					...
					{
						libBud::Printf( S_COLOR_YELLOW "WARNING: " S_COLOR_RED "%s.Shape%i has %i fill draws\n", filename.c_str(), i, shape->fillDraws.Num() );
					}
					...
					for( int d = 0; d < shape->fillDraws.Num(); d++ )
					{
						...
						{
							libBud::Printf( "fillDraw %i of characterID %i has no startVerts\n", d, i );
						}
				...
				if( shape->lineDraws.Num() > 0 )
				{
					...
					for( int d = 0; d < shape->lineDraws.Num(); d++ )
					{
						const budSWFShapeDrawLine& lineDraw = shape->lineDraws[d];
			...
			case SWF_DICT_FONT:
			{
				const budSWFFont* font = dictionary[i].font;
			...
			case SWF_DICT_TEXT:
			{
				...
				const budSWFText* text = dictionary[i].text;
				...
				for( int t = 0; t < text->textRecords.Num(); t++ )
				{
					const budSWFTextRecord& textRecord = text->textRecords[t];
