...
========================
*/
void budSWF::DefineFont2( budSWFBitStream& bitstream )
{
	uint16 characterID = bitstream.ReadU16();
	budSWFDictionaryEntry* entry = AddDictionaryEntry( characterID, SWF_DICT_FONT );
	if( entry == NULL )
	{
		return;
	}
	...
	if( flags & BIT( 3 ) )
	{
		...
		budSWFBitStream offsetStream( bitstream.ReadData( offsetTableSize ), offsetTableSize, false );
		if( offsetStream.ReadU32() != offsetTableSize )
		{
			libBud::Warning( "budSWF::DefineFont2: first glyph offset != offsetTableSize" );
			return;
		}
		...
		for( int i = 0; i < numGlyphs; i++ )
		{
			...
			budSWFBitStream shapeStream( bitstream.ReadData( shapeSize ), shapeSize, false );
...
========================
*/
void budSWF::DefineFont3( budSWFBitStream& bitstream )
{
...
========================
*/
void budSWF::DefineTextX( budSWFBitStream& bitstream, bool rgba )
{
	uint16 characterID = bitstream.ReadU16();
	budSWFDictionaryEntry* entry = AddDictionaryEntry( characterID, SWF_DICT_TEXT );
	if( entry == NULL )
	{
		return;
	}
	budSWFText* text = entry->text;
	...
	while( true )
	{
		...
		budSWFTextRecord& textRecord = text->textRecords.Alloc();
...
========================
*/
void budSWF::DefineText( budSWFBitStream& bitstream )
{
...
========================
*/
void budSWF::DefineText2( budSWFBitStream& bitstream )
{
...
/*
========================
budSWF::DefineEditText
========================
*/
void budSWF::DefineEditText( budSWFBitStream& bitstream )
{
	uint16 characterID = bitstream.ReadU16();
	budSWFDictionaryEntry* entry = AddDictionaryEntry( characterID, SWF_DICT_EDITTEXT );
	if( entry == NULL )
	{
		return;
	}
	budSWFEditText* edittext = entry->edittext;
