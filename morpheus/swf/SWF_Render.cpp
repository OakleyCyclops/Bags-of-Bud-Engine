...
========================
*/
void budSWF::DrawStretchPic( float x, float y, float w, float h, float s1, float t1, float s2, float t2, const budMaterial* material )
{
...
========================
*/
void budSWF::Render( budRenderSystem* gui, int time, bool isSplitscreen )
{
	if( !IsLoaded() )
	{
		return;
...
========================
*/
void budSWF::RenderMask( budRenderSystem* gui, const swfDisplayEntry_t* mask, const swfRenderState_t& renderState, const int stencilMode )
{
	...
	budSWFDictionaryEntry& entry = dictionary[ mask->characterID ];
...
========================
*/
void budSWF::RenderSprite( budRenderSystem* gui, budSWFSpriteInstance* spriteInstance, const swfRenderState_t& renderState, int time, bool isSplitscreen )
{

	if( spriteInstance == NULL )
	{
		libBud::Warning( "%s: RenderSprite: spriteInstance == NULL", filename.c_str() );
		return;
	}
	...
	budStaticList<const swfDisplayEntry_t*, 256> activeMasks;
	...
	for( int i = 0; i < spriteInstance->displayList.Num(); i++ )
	{
		...
		{
			//libBud::Warning( "%s: Tried to render an unrenderable character %d", filename.c_str(), entry->type );
		}
	}
...
/*
========================
budSWF::GLStateForBlendMode
========================
*/
uint64 budSWF::GLStateForRenderState( const swfRenderState_t& renderState )
{
...
/*
========================
budSWF::RenderMorphShape
========================
*/
void budSWF::RenderMorphShape( budRenderSystem* gui, const budSWFShape* shape, const swfRenderState_t& renderState )
{
	if( shape == NULL )
	{
		libBud::Warning( "%s: RenderMorphShape: shape == NULL", filename.c_str() );
		return;
	}
	
	for( int i = 0; i < shape->fillDraws.Num(); i++ )
	{
		const budSWFShapeDrawFill& fill = shape->fillDraws[i];
		...
		budDrawVert* verts = gui->AllocTris( fill.startVerts.Num(), fill.indices.Ptr(), fill.indices.Num(), material, renderState.stereoDepth );
		if( verts == NULL )
		{
			continue;
		}
...
========================
*/
void budSWF::RenderShape( budRenderSystem* gui, const budSWFShape* shape, const swfRenderState_t& renderState )
{
	if( shape == NULL )
	{
		libBud::Warning( "%s: RenderShape: shape == NULL", filename.c_str() );
		return;
	}
	...
	if( !swf_skipLineDraws.GetBool() )
	{
		...
		{
			const budSWFShapeDrawLine& line = shape->lineDraws[i];
			swfColorXform_t color;
			...
			budDrawVert* verts = gui->AllocTris( line.startVerts.Num(), line.indices.Ptr(), line.indices.Num(), white, renderState.stereoDepth );
			if( verts == NULL )
			{
				continue;
			}
...
/*
========================
budSWF::RenderEditText
========================
*/
void budSWF::RenderEditText( budRenderSystem* gui, budSWFTextInstance* textInstance, const swfRenderState_t& renderState, int time, bool isSplitscreen )
{
	if( textInstance == NULL )
	{
		libBud::Warning( "%s: RenderEditText: textInstance == NULL", filename.c_str() );
		return;
	}
	...
	if( textInstance->variable.IsEmpty() )
	{
		...
		{
			text = budLocalization::GetString( textInstance->text );
		}
	}
	else
	{
		budSWFScriptVar var = globals->Get( textInstance->variable );
		if( var.IsUndefined() )
		{
			text = budLocalization::GetString( textInstance->text );
		}
		...
		{
			text = budLocalization::GetString( var.ToString() );
		}
	}
	...
	if( inputField && ( ( libBud::frameNumber >> 4 ) & 1 ) == 0 )
	{
		cursorPos = selEnd;
		drawCursor = true;
	}
	...
	budSWFDictionaryEntry* fontEntry = FindDictionaryEntry( shape->fontID, SWF_DICT_FONT );
	if( fontEntry == NULL )
	{
		libBud::Warning( "budSWF::RenderEditText: NULL Font" );
		return;
	}
...
// RB begin
swfRect_t budSWF::CalcRect( const budSWFSpriteInstance* spriteInstance, const swfRenderState_t& renderState )
{
	swfRect_t bounds;
	...
	if( spriteInstance == NULL )
	{
		libBud::Warning( "%s: CalcRect: spriteInstance == NULL", filename.c_str() );
		return bounds;
	}
	...
	for( int i = 0; i < spriteInstance->displayList.Num(); i++ )
	{
		...
		{
			const budSWFShape* shape = entry->shape;
	...
	*/
	...
	while( charIndex < drawText.Length() )
	{
		...
		//if( !ClippedCoords( &drawX, &drawY, &w, &h, &s, &t, &s2, &t2 ) )
		{
			...
			budDrawVert* verts = gui->AllocTris( 4, quadPicIndexes, 6, glyphInfo.material, STEREO_DEPTH_TYPE_NONE );
			if( verts != NULL )
			{
				verts[0].xyz[0] = x1;
				verts[0].xyz[1] = y1;
